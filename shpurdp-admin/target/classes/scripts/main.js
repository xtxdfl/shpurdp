...
angular.module('shpurdpAdminConsole', [
  'ngRoute',
  'ngAnimate',
  'ui.bootstrap',
  'restangular',
...
.config(['RestangularProvider', '$httpProvider', '$provide', 'Settings', function(RestangularProvider, $httpProvider, $provide, Settings) {
  ...
  RestangularProvider.setDefaultHeaders({'X-Requested-By':'shpurdp'});

  $httpProvider.defaults.headers.post['Content-Type'] = 'plain/text';
  $httpProvider.defaults.headers.put['Content-Type'] = 'plain/text';

  $httpProvider.defaults.headers.post['X-Requested-By'] = 'shpurdp';
  $httpProvider.defaults.headers.put['X-Requested-By'] = 'shpurdp';
  $httpProvider.defaults.headers.common['X-Requested-By'] = 'shpurdp';
...
.constant('ROUTES', {
  ...
  views: {
    list: {
      url: '/views',
      templateUrl: 'views/shpurdpViews/viewsList.html',
      controller: 'ViewsListCtrl',
      label: 'Views'
    },
    edit: {
      url: '/views/:viewId/versions/:version/instances/:instanceId/edit',
      templateUrl: 'views/shpurdpViews/edit.html',
...
.run(['$rootScope', 'ROUTES', 'Settings', function ($rootScope, ROUTES, Settings) {
   ...
   * along with all 'href' attribute or methods which invoke redirect to Shpurdp Web.
   * This method is useful to determine actual site root when shpurdp run under proxy server.
   *
   * @param {string} url
   * @returns {string}
   */
...
.config(['$translateProvider', function($translateProvider) {
  $translateProvider.translations('en',{
    ...
    'common.shpurdp': 'Shpurdp',
    'common.apacheShpurdp': 'Apache Shpurdp',
    'common.about': 'About',
    ...
    'common.loginActivities.homeDirectory.alert': 'Many Shpurdp Views store user preferences in the logged in user\'s / user directory in HDFS. Optionally, Shpurdp can auto-create these directories for users on login.',
    ...
    'main.title': 'Welcome to Apache Shpurdp',
    'main.noClusterDescription': 'Provision a cluster, manage who can access the cluster, and customize views for Shpurdp users.',
    ...
    'views.alerts.deployError': 'Error deploying. Check Shpurdp Server log.',
    ...
    'users.admin': 'Admin',
    'users.shpurdpAdmin': 'Shpurdp Admin',
    'users.shpurdpClusterURL': 'Shpurdp Cluster URL',
    ...
    'users.isShpurdpAdmin': 'Is this user an Shpurdp Admin?',
    'users.isActive': 'User Status',
    'users.userIsAdmin': 'This user is an Shpurdp Admin and has all privileges.',
    ...
    'users.adminTip': 'An Shpurdp Admin can create new clusters and other Shpurdp Admin Users.',
    'users.deactivateTip': 'Active Users can log in to Shpurdp. Inactive Users cannot.',
    ...
    'users.roles.shpurdpAdmin': 'Shpurdp Administrator',
    ...
    'versions.networkIssues.publicRepoDisabledMsg': 'Shpurdp does not have access to the Internet and cannot use the Public Repository for installing the software. Your Options:',
    'versions.networkIssues.publicRepoDisabledMsg1': 'Configure your hosts for access to the Internet.',
    'versions.networkIssues.publicRepoDisabledMsg2': 'If you are using an Internet Proxy, refer to the Shpurdp Documentation on how to configure Shpurdp to use the Internet Proxy.',
    ...
    'versions.introduction': 'To register a new version in Shpurdp, provide a Version Definition File, confirm the software repository information and save the version.',
    ...
    'versions.useRedhatSatellite.warning': "In order for Shpurdp to install packages from the right repositories, " +
    ...
    'authentication.description': 'Shpurdp supports authenticating against local Shpurdp users created and stored in the Shpurdp Database, or authenticating against a LDAP server:',
    ...
    'authentication.attributes.test.description': 'To quickly test the chosen attributes click the button below. During this process you can specify a test user name and password and Shpurdp will attempt to authenticate and retrieve group membership information',
    ...
    'remoteClusters.shpurdpClusterName': 'Shpurdp Cluster Name',
    'remoteClusters.clusterURLPlaceholder': 'http://shpurdp.server:8080/api/v1/clusters/c1',
...
.controller('AppCtrl',['$scope','$rootScope', '$route', '$window','Auth', 'Alert', '$modal', 'Cluster', '$translate', '$http', 'Settings', 'Utility', '$q', function($scope, $rootScope, $route, $window, Auth, Alert, $modal, Cluster, $translate, $http, Settings, Utility, $q) {
  ...
  $http.get(Settings.baseUrl + '/users/' + Auth.getCurrentUser() + '/authorizations?fields=*')
    .then(function (data) {
      ...
      if (auth.indexOf('SHPURDP.RENAME_CLUSTER') == -1) {
        $window.location = $rootScope.fromSiteRoot("/#/main/dashboard");
      }
    });

  $scope.about = function () {
    var shpurdpVersion = $scope.shpurdpVersion;
    var modalInstance = $modal.open({
      ...
      controller: ['$scope', function ($scope) {
        ...
        $scope.shpurdpVersion = shpurdpVersion;
      }]
    });
  };
  ...
  function loadShpurdpVersion() {
    Cluster.getShpurdpVersion().then(function (version) {
      $scope.shpurdpVersion = version;
    });
  }
  ...
  $scope.startNoopPolling = function () {
    $rootScope.noopPollingInterval = setInterval(Cluster.getShpurdpTimeout, 10000);
  };

  if (!$rootScope.userActivityTimeoutInterval) {
    Cluster.getShpurdpTimeout().then(function (timeout) {
    ...
    $scope.testAttributes = function () {
      ...
      setTimeout(function (prevValue) {
        ...
        if ($scope.isTestAttributesSuccessful) {
          $scope.attributes.availableGroups = ['HadoopOps', 'HadoopOpsDFW', 'ShpurdpAdmins', 'ExchangeAdmins', 'ShpurdpUsers', 'ExchangeUsers'];
        }
...
function($scope, View, $modal, Alert, ConfirmationModal, $translate, Settings, Pagination, Filters) {
  ...
  $scope.createInstance = function (instanceClone) {
    var modalInstance = $modal.open({
      templateUrl: 'views/shpurdpViews/modals/create.html',
...
function($scope, View, RemoteCluster, Alert, Cluster, $routeParams, $location, UnsavedDialog, $translate, $modalInstance, views, instanceClone, $q) {
  ...
  $scope.nameValidationPattern = /^\s*\w*\s*$/;
  $scope.isLoading = false;
  $scope.clusterType = 'LOCAL_SHPURDP'; // LOCAL_SHPURDP, REMOTE_SHPURDP, NONE
  $scope.views = views;
  ...
  $scope.switchClusterType = function(clusterType) {
    ...
    if (clusterType === 'LOCAL_SHPURDP') {
      $scope.clusterOptions = $scope.localClusters;
      resetErrors();
    } else if (clusterType === 'REMOTE_SHPURDP') {
  ...
  function loadFormData () {
    ...
    $q.all(loadClusters(), loadRemoteClusters()).then(function() {
      $scope.isLoading = false;
      $scope.switchClusterType('LOCAL_SHPURDP');
...
.controller('StackVersionsCreateCtrl', ['$scope', '$rootScope', 'Stack', 'Utility', '$routeParams', '$location', '$timeout' ,'Alert', '$translate', 'Cluster', 'AddRepositoryModal', 'AddVersionModal', 'ConfirmationModal',
  ...
  $scope.save = function () {
    ...
    angular.forEach($scope.osList, function (os) {
      os.OperatingSystems.shpurdp_managed_repositories = !$scope.useRedhatSatellite;
      if (os.selected) {
...
.controller('StackVersionsEditCtrl', ['$scope', '$rootScope', '$location', 'Cluster', 'Stack', '$routeParams', 'ConfirmationModal', 'Alert', '$translate', 'AddRepositoryModal', function($scope, $rootScope, $location, Cluster, Stack, $routeParams, ConfirmationModal, Alert, $translate, AddRepositoryModal) {
  ...
  $scope.loadStackVersionInfo = function () {
    return Stack.getRepo($routeParams.versionId, $routeParams.stackName).then(function (response) {
      ...
      $scope.useRedhatSatellite = !response.shpurdp_managed_repositories;
      $scope.updateObj = response.updateObj;
      $scope.upgradeStack = {
        stack_name: response.stackName,
        stack_version: response.stackVersion,
...
angular.module('shpurdpAdminConsole').constant('UserConstants', {
  /**
  ...
  return {
    ...
    makeUser: function(user) {
      ...
      user.Users.roles = Cluster.sortRoles(user.privileges.filter(function(item) {
        return item.PrivilegeInfo.type === 'CLUSTER' || item.PrivilegeInfo.type === 'SHPURDP';
      }).map(function(item) {
  ...
  .factory('RemoteCluster', ['$http', '$q', 'Settings', function($http, $q, Settings) {
    ...
    RemoteCluster.affectedViews = function(clustername) {
      ...
      $http.get(Settings.baseUrl + '/views?'
          + 'fields=versions%2Finstances/ViewInstanceInfo/cluster_handle,versions%2Finstances/ViewInstanceInfo/cluster_type&versions%2FViewVersionInfo%2Fsystem=false&versions%2Finstances/ViewInstanceInfo/cluster_type=REMOTE_SHPURDP&versions%2Finstances/ViewInstanceInfo/cluster_handle=' + clustername
...
.factory('Cluster', ['$http', '$q', 'Settings', '$translate', function($http, $q, Settings, $translate) {
  ...
  return {
    ...
    orderedLevels: ['SERVICE', 'HOST', 'CLUSTER', 'SHPURDP'],

    ineditableRoles : ['VIEW.USER', 'SHPURDP.ADMINISTRATOR'],

    sortRoles: function(roles) {
      var orderedRoles = ['SHPURDP.ADMINISTRATOR'].concat(this.orderedRoles);
    ...
    getShpurdpVersion: function() {
      var deferred = $q.defer();

      $http.get(Settings.baseUrl + '/services/SHPURDP/components/SHPURDP_SERVER?fields=RootServiceComponents/component_version,RootServiceComponents/properties/server.os_family&minimal_response=true', {mock: '2.1'})
      .then(function(data) {
    ...
    getClusterOS: function() {
      ...
      $http.get(Settings.baseUrl + '/services/SHPURDP/components/SHPURDP_SERVER?fields=RootServiceComponents/properties/server.os_family&minimal_response=true', {mock: 'redhat6'})
      .then(function(data) {
    ...
    getShpurdpTimeout: function() {
      var deferred = $q.defer();
      var url = '/services/SHPURDP/components/SHPURDP_SERVER?fields=RootServiceComponents/properties/user.inactivity.timeout.default';
    ...
    getRolesWithAuthorizations: function() {
      ...
      if (rolesWithAuthorizations) {
        ...
        $http({
          method: 'GET',
          url: Settings.baseUrl + '/permissions?PermissionInfo/resource_name.in(CLUSTER,SHPURDP)',
          mock: 'permission/permissions.json',
          params: {
...
.factory('Alert', [function() {
  ...
  function createAlertBox(innerHTML, moreInfo, type){
    ...
    var elem = angular.element('<div><div class="icon-box"></div></div>').addClass('shpurdpAlert').addClass(type).addClass('invisible');
    ...
    $('<button type="button" class="close"><span aria-hidden="true">&times;</span><span class="sr-only">{{"common.controls.close" | translate}}</span></button>')
      .appendTo(elem)
      .on('click', function() {
        var $box = $(this).closest('.shpurdpAlert');
        $box.remove();
      });
...
.factory('Auth',['$http', 'Settings', function($http, Settings) {
  var shpurdp;
  var currentUserName;
  if (localStorage.shpurdp) {
    shpurdp = JSON.parse(localStorage.shpurdp);
    if (shpurdp && shpurdp.app && shpurdp.app.loginName) {
      currentUserName = shpurdp.app.loginName;
    }
  }
  return {
    signout: function() {
      var data = JSON.parse(localStorage.shpurdp);
      delete data.app.authenticated;
      delete data.app.loginName;
      delete data.app.user;
      localStorage.shpurdp = JSON.stringify(data);
...
.factory('Stack', ['$http', '$q', 'Settings', '$translate', function ($http, $q, Settings,$translate) {
  ...
  return {
    ...
    getGPLLicenseAccepted: function() {
      ...
      $http.get(Settings.baseUrl + '/services/SHPURDP/components/SHPURDP_SERVER?fields=RootServiceComponents/properties/gpl.license.accepted&minimal_response=true', {mock: 'true'})
        .then(function(data) {
    ...
    getRepo: function (repoVersion, stack_name, stack_version) {
      ...
      .then(function (data) {
        ...
        var response = {
          ...
          shpurdp_managed_repositories: data.repository_versions[0].operating_systems[0].OperatingSystems.shpurdp_managed_repositories !== false,
          ...
          $scope.readVersionInfo = function(){
            ...
            return Stack.postVersionDefinitionFile(isXMLdata, data, true).then(function (versionInfo) {
              ...
              var response = {
                ...
                shpurdp_managed_repositories: repo.operating_systems[0].OperatingSystems.shpurdp_managed_repositories !== false,
    ...
    show: function(roles) {
      ...
      var modalInstance = $modal.open({
        ...
        controller: function($scope, $modalInstance) {
          ...
          $scope.title = '';
          $scope.orderedRoles = ['SHPURDP.ADMINISTRATOR'].concat(Cluster.orderedRoles).reverse();
