...
describe('App.Router', function () {
      ...
      var params = {
        loginData: {
          privileges: [{
            PrivilegeInfo: {
              cluster_name: 'c1',
              permission_name: 'SHPURDP.ADMINISTRATOR'
            }
          }]
        }
      };
  ...
  describe("#transitionToAdminView()", function () {
    ...
    it("valid request is sent", function () {
      ...
      var args = testHelpers.findAjaxRequest('name', 'shpurdp.service.load_server_version');
  ...
  describe("#transitionToViews()", function () {
    var mock = {loadShpurdpViews: Em.K};
    beforeEach(function () {
      sinon.stub(App.router, 'get').returns(mock);
      sinon.stub(router, 'transitionTo');
      sinon.spy(mock, 'loadShpurdpViews');
    });
    afterEach(function () {
      App.router.get.restore();
      router.transitionTo.restore();
      mock.loadShpurdpViews.restore();
    });
    it("transitionTo called with corrent route", function () {
      router.transitionToViews();
      expect(mock.loadShpurdpViews.calledOnce).to.be.true;
