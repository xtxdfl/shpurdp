...
describe('data configs site_properties', function () {
  ...
  var stackProperties = siteProperties.filter(function(item){
    return !(item.isRequiredByAgent === false || item.category === 'Shpurdp Principals')
  });

  stackProperties.forEach(function(siteProperty){
    /**
     * Following config attributes are stack driven and should be defined in the stack metainfo instead of shpurdp-web site-properties file
     * isVisible
     * isOverridable
     * value
     * recommendedValue
