...
describe('MainViewsController', function () {
  ...
  describe('#loadShpurdpViews()', function () {
    beforeEach(function () {
    ...
    it('should load views if the user is logged in', function () {
      ...
      mainViewsController.loadShpurdpViews();
  ...
  describe('#loadShpurdpViewsSuccess()', function () {

    it('data has items', function () {
      mainViewsController.loadShpurdpViewsSuccess({items: [{}]});
    ...
    it('data is empty', function () {
      ...
      var args = testHelpers.findAjaxRequest('name', 'views.instances');
      expect(args).to.not.exists;
      expect(mainViewsController.get('shpurdpViews')).to.be.empty;
      expect(mainViewsController.get('isDataLoaded')).to.be.true;
    });
  });

  describe('#loadShpurdpViewsError()', function () {

    it('shpurdpViews should be empty', function () {
      mainViewsController.loadShpurdpViewsError();
      expect(mainViewsController.get('shpurdpViews')).to.be.empty;
      expect(mainViewsController.get('isDataLoaded')).to.be.true;
    });
  });
  ...
  describe("#loadViewInstancesSuccess()", function () {
    ...
    it('one view instance is parsed', function () {
      expect(mainViewsController.get('shpurdpViews.length')).to.be.equal(1);
    });


    Object.keys(viewInstanceFields).forEach(function (fieldName) {
      it(JSON.stringify(fieldName) + ' is set correctly', function () {
        expect(mainViewsController.get('shpurdpViews.firstObject.' + fieldName)).to.be.equal(viewInstanceFields[fieldName]);
      });
    });
