...
describe('App.clusterController', function () {
  ...
  describe('#loadShpurdpProperties()', function() {

    it('App.ajax.send should be called', function() {
      controller.loadShpurdpProperties();
  ...
  describe('#loadShpurdpPropertiesSuccess()', function() {
    var data = {
      RootServiceComponents: {
        properties: {
          p1: '1'
    ...
    beforeEach(function() {
      ...
      sinon.stub(App.router.get('mainController'), 'setShpurdpServerVersion');
      sinon.stub(controller, 'setServerClock');
      controller.loadShpurdpPropertiesSuccess(data);
    });

    afterEach(function() {
      App.router.get('mainController').monitorInactivity.restore();
      App.router.get('mainController').setShpurdpServerVersion.restore();
      controller.setServerClock.restore();
    });

    it('should set shpurdpProperties', function() {
      expect(controller.get('shpurdpProperties')).to.be.eql({p1: '1'});
    });
    ...
    it('setShpurdpServerVersion should be called', function() {
      expect(App.router.get('mainController').setShpurdpServerVersion.calledWith(data)).to.be.true;
    });
