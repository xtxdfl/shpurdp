...
describe('App.WizardStep2Controller', function () {
  ...
  App.TestAliases.testAsComputedIfThenElse(getController(), 'installType', 'manualInstall', 'manualDriven', 'shpurdpDriven');

  describe('#updateHostNameArr()', function () {

    var controller = App.WizardStep2Controller.create({
      hostNames: 'apache.shpurdp'
    });
  ...
  describe('#isAllHostNamesValid()', function () {
    ...
    it('should return true if all host names are valid', function(){
      controller.set('hostNames', 'amache.org shpurdp.com');
  ...
  describe('#checkHostError()', function () {
    ...
    it('should set hostsError to null if hostNames is valid', function () {
      controller.set('content', {'installOptions': {'hostNames': 'shpurdp'}});
      controller.checkHostError();
  ...
  describe('#agentUserError', function () {
    ...
    userErrorTests.forEach(function(test) {
      it('Shpurdp Agent user account customize enabled', function() {
    ...
    userErrorTests.forEach(function(test) {
      it('Shpurdp Agent user account customize disabled', function() {
  ...
  describe('#getHostInfo()', function () {
    ...
    it('should return object with bootStatus, installType and name for every element in hostNameArr', function () {
      ...
      expect(test).to.eql({
        ...
        'shpurdp':{'name':'shpurdp', 'installType': 'manualDriven', 'bootStatus': 'PENDING', isInstalled: false}
      });
    })
  });
  ...
  describe('#setShpurdpJavaHome', function() {

    it('should do ajax-request', function() {
      var controller = App.WizardStep2Controller.create({onGetShpurdpJavaHomeSuccess: Em.K, onGetShpurdpJavaHomeError: Em.K});
      controller.setShpurdpJavaHome();
      var args = testHelpers.findAjaxRequest('name', 'shpurdp.service');
      expect(args).exists;
    });
  });

  describe('#onGetShpurdpJavaHomeSuccess', function() {
    it('should set java.home value receiced from server', function() {
      ...
      controller.onGetShpurdpJavaHomeSuccess(test);
      expect(controller.content.installOptions.javaHome).to.equal('/root');
    });
  });

  describe('#onGetShpurdpJavaHomeError', function() {
    it('should set default java.home value', function() {
      var controller = App.WizardStep2Controller.create({content: {installOptions: {}}});
      controller.onGetShpurdpJavaHomeError();
  ...
  describe('#saveHosts', function() {
    beforeEach(function() {
      sinon.stub(c, 'setShpurdpJavaHome', Em.K);
      c.reopen({
        hostNameArr: ['h1'],
        content: {
          hosts: null
    ...
    afterEach(function() {
      c.setShpurdpJavaHome.restore();
    });
    it('should call setShpurdpJavaHome', function() {
      c.saveHosts();
      expect(c.setShpurdpJavaHome.calledOnce).to.equal(true);
    });
