...
App.Router = Em.Router.extend({
   ...
   * redirect to auth server different from shpurdp-server. Used in some types of
  ...
  loginGetClustersSuccessCallback: function (clustersData, opt, params) {
    ...
    var router = this;
    var isAdmin = privileges.mapProperty('PrivilegeInfo.permission_name').contains('SHPURDP.ADMINISTRATOR');
  ...
  transitionToAdminView: function() {
    return App.ajax.send({
      name: 'shpurdp.service.load_server_version',
      sender: this,
  ...
  transitionToViews: function() {
    App.router.get('mainViewsController').loadShpurdpViews();
  ...
  root: Em.Route.extend({
    ...
    experimental: Em.Route.extend({
      route: '/experimental',
      enter: function (router, context) {
        if (!App.isAuthorized('SHPURDP.MANAGE_SETTINGS')) {
