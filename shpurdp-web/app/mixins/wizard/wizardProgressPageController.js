...
App.wizardProgressPageControllerMixin = Em.Mixin.create(App.InstallComponent, {
  ...
  startServices: function (runSmokeTest, services, startListedServicesFlag,dataType="json") {
    ...
    var skipServiceCheck = App.router.get('clusterController.shpurdpProperties')['skip.service.checks'] === "true";
    var data = {
      'ServiceInfo': {
        'state': 'STARTED'
      }
  ...
  onCreateComponentError: function (error) {
    if (error.responseText.indexOf('org.apache.shpurdp.server.controller.spi.ResourceAlreadyExistsException') !== -1) {
