...
var checkboxConfigView = Ember.Checkbox.extend(App.ServiceConfigPopoverSupport, App.ServiceConfigCalculateId, App.SupportsDependentConfigs, {
  ...
  toggleValue: function() {
    if (this.isNotAppropriateValue()){
      ...
      //if the checkbox being toggled is the 'Have Shpurdp manage UIDs' in Misc Tab, show/hide uid/gid column accordingly
...
App.CheckDBConnectionView = Ember.View.extend({
  ...
  /** @property {Object} shpurdpRequiredProperties - properties that need for custom action request **/
  shpurdpRequiredProperties: null,
  ...
  requiredProperties: function() {
    ...
    var propertiesMap = {
      ...
      HIVE: ['shpurdp.hive.db.schema.name', 'javax.jdo.option.ConnectionUserName', 'javax.jdo.option.ConnectionPassword', 'javax.jdo.option.ConnectionDriverName', 'javax.jdo.option.ConnectionURL'],
      KERBEROS: ['kdc_hosts'],
  ...
  /** Check validation and load shpurdp properties **/
  didInsertElement: function() {
  ...
  /**
   * Set up shpurdp properties required for custom action request
   *
   * @method getShpurdpProperties
   **/
  getShpurdpProperties: function() {
    var clusterController = App.router.get('clusterController');
    var _this = this;
    if (!App.isEmptyObject(App.db.get('tmp', 'shpurdpProperties')) && !this.get('shpurdpProperties')) {
      this.set('shpurdpProperties', App.db.get('tmp', 'shpurdpProperties'));
      return;
    }
    if (App.isEmptyObject(clusterController.get('shpurdpProperties'))) {
      clusterController.loadShpurdpProperties().done(function(data) {
        _this.formatShpurdpProperties(data.RootServiceComponents.properties);
      });
    } else {
      this.formatShpurdpProperties(clusterController.get('shpurdpProperties'));
    }
  },

  formatShpurdpProperties: function(properties) {
    var defaults = {
      threshold: "60",
      shpurdp_server_host: location.hostname,
    ...
    App.db.set('tmp', 'shpurdpProperties', properties);
    this.set('shpurdpProperties', properties);
  },
  /**
  ...
  createCustomAction: function() {
    ...
    var params = $.extend(true, {}, { db_name: this.get('databaseName').toLowerCase() }, this.get('connectionProperties'), this.get('shpurdpProperties'));
    App.ajax.send({
