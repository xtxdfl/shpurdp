...
App.db.cleanUp = function () {
  ...
  localStorage.setObject('shpurdp', App.db.data);
};
...
App.db.updateStorage = function () {
  App.db.data = localStorage.getObject('shpurdp');
...
App.db.mergeStorage = function () {
  if (localStorage.getObject('shpurdp') == null) {
    App.db.cleanUp();
  } else {
    localStorage.setObject('shpurdp', $.extend(true, {}, InitialData, App.db.data));
  }
};
