...
App.InstallerController = App.WizardController.extend(App.Persist, {
  ...
  getServerVersionSuccessCallback: function (data) {
    ...
    this.set('shpurdpServerVersion', serverVersion);
    if (clientVersion) {
  ...
  prepareRepoForSaving: function(repo) {
    ...
    var shpurdpManagedRepositories = !repo.get('useRedhatSatellite');
    var k = 0;
      ...
      if (os.get('isSelected')) {
        repoVersion.operating_systems.push({
          "OperatingSystems": {
            ...
            "shpurdp_managed_repositories": shpurdpManagedRepositories
          },
          "repositories": []
        });
    ...
    '1': [
      {
        ...
        callback: function () {
          ...
          this.loadStacks().done(function(stacksLoaded) {
            App.router.get('clusterController').loadShpurdpProperties().always(function() {
  ...
  /**
   * Compare jdk versions used for shpurdp and selected stack.
  ...
  validateJDKVersion: function (successCallback, failCallback) {
    var selectedStack = App.Stack.find().findProperty('isSelected', true),
        currentJDKVersion = App.router.get('clusterController.shpurdpProperties')['java.version'],
