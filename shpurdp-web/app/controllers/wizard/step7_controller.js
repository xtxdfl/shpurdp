...
App.WizardStep7Controller = Em.Controller.extend(App.ServerValidatorMixin, App.EnhancedConfigsMixin, App.ToggleIsRequiredMixin, App.GroupsMappingMixin, App.AddSecurityConfigs, App.KDCCredentialsControllerMixin, {
  ...
  _updateIsEditableFlagForConfig: function (serviceConfigProperty, defaultGroupSelected) {
    if (App.isAuthorized('SHPURDP.ADD_DELETE_CLUSTERS')) {
  ...
  applyServicesConfigs: function (configs, isRestoring) {
    ...
    var infraSolrService = App.StackService.find().findProperty('serviceName', 'SHPURDP_INFRA_SOLR');
  ...
  */
  ...
  /**
   * Check whether hive New MySQL database is on the same host as Shpurdp server MySQL server
   * @return {$.ajax|null}
   * @method checkMySQLHost
   */
  checkMySQLHost: function () {
    // get shpurdp database type and hostname
    return App.ajax.send({
      name: 'shpurdp.service',
      data: {
      ...
      sender: this,
      success: 'getShpurdpDatabaseSuccess'
    });
  },

  /**
   * Success callback for shpurdp database, get Shpurdp DB type and DB server hostname, then
   * Check whether hive New MySQL database is on the same host as Shpurdp server MySQL server
   * @param {object} data
   * @method getShpurdpDatabaseSuccess
   */
  getShpurdpDatabaseSuccess: function (data) {
    var shpurdpServerDBType = Em.getWithDefault(data.hostComponents, '0.RootServiceHostComponents.properties', {})['server.jdbc.database'],
        shpurdpServerHostName = Em.getWithDefault(data.hostComponents, '0.RootServiceHostComponents.host_name', false),
    ...
    if (shpurdpServerHostName) {
      this.set('mySQLServerConflict', shpurdpServerDBType.contains('mysql') && hiveConnectionURL.contains(shpurdpServerHostName));
    } else {
   ...
   * and it is not located on the same host as Shpurdp server
