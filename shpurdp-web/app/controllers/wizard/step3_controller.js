...
App.WizardStep3Controller = Em.Controller.extend(App.ReloadPopupMixin, App.CheckHostMixin, {
  ...
  loadStep: function () {
    ...
    if (!isHostsLoaded || isPrevStepSmallerThenCurrent ||
      ...
      this.clearStep();
      App.router.get('clusterController').loadShpurdpProperties();
      this.loadHosts();
    }
  },
  ...
  /**
   * Get JDK name from server to determine if user had setup a customized JDK path when doing 'shpurdp-server setup'.
   * The Shpurdp properties are different from default shpurdp-server setup, property 'jdk.name' will be missing if a customized jdk path is applied.
   * @return {$.ajax}
   * @method getJDKName
   */
  getJDKName: function () {
