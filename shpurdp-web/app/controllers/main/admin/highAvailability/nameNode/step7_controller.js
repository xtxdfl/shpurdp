...
App.HighAvailabilityWizardStep7Controller = App.HighAvailabilityProgressPageController.extend({
  ...
  commands: ['startZooKeeperServers', 'startShpurdpInfra', 'startMysqlServer', 'startRanger', 'startNameNode'],

  initializeTasks: function () {
    this._super();
    const tasksToRemove = [];

    if (!App.Service.find('SHPURDP_INFRA_SOLR').get('isLoaded')) {
      tasksToRemove.push('startShpurdpInfra');
    }
  ...
  startShpurdpInfra: function () {
    this.startServices(false, ['SHPURDP_INFRA_SOLR'], true);
  },
