...
App.KerberosWizardStep4Controller = App.WizardStep7Controller.extend(App.AddSecurityConfigs, App.ToggleIsRequiredMixin, App.KDCCredentialsControllerMixin, {
  ...
  createServiceConfig: function(configs) {
    ...
    // storm user principal is not required for shpurdp operation
    ...
    var categoryForGeneralConfigs = [
      ...
      App.ServiceConfigCategory.create({ name: 'Shpurdp Principals', displayName: 'Shpurdp Principals'})
    ];
  ...
  prepareConfigProperties: function(configs) {
    ...
    var installedServiceNames = ['Cluster', 'SHPURDP'].concat(App.Service.find().mapProperty('serviceName'));
    ...
    configProperties.forEach(function(property, item, allConfigs) {
      ...
      // All user identity except storm should be grouped under "Shpurdp Principals" category
      if (property.get('identityType') == 'user') property.set('category', 'Shpurdp Principals');
