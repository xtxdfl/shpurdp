  ...
  getQueryParameters: function (skipNonFilterProperties) {
    ...
    savedFilterConditions.forEach(function (filter) {
      ...
      if (property && filter.value.length > 0 && !filter.skipFilter) {
        ...
        if (filter.type === 'number' || filter.type === 'shpurdp-bandwidth') {
        ...
        if (filter.type === 'shpurdp-bandwidth' && result.type == 'EQUAL' && result.value) {
        ...
        } else if (filter.type === 'shpurdp-bandwidth' && result.type != 'EQUAL' && result.value){
