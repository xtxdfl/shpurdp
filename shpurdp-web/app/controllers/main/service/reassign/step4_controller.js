...
App.ReassignMasterWizardStep4Controller = App.HighAvailabilityProgressPageController.extend(App.WizardEnableDone, {
  ...
  onDeleteHostComponentsError: function (error) {
    if (error.responseText.indexOf('org.apache.shpurdp.server.controller.spi.NoSuchResourceException') !== -1) {
      this.onComponentsTasksSuccess();
    } else {
      this.onTaskError();
    }
  ...
  requiredProperties: function() {
    var propertiesMap = {
      ...
      HIVE: {
        type: 'hive-site',
        names: ['shpurdp.hive.db.schema.name', 'javax.jdo.option.ConnectionUserName', 'javax.jdo.option.ConnectionPassword', 'javax.jdo.option.ConnectionDriverName', 'javax.jdo.option.ConnectionURL']
      }
    };
  ...
  prepareDBCheckAction: function() {
    ...
    var shpurdpProperties = App.router.get('clusterController.shpurdpProperties');

    params['db_name'] = this.get('content.databaseType');
    params['jdk_location'] = shpurdpProperties['jdk_location'];
    params['jdk_name'] = shpurdpProperties['jdk.name'];
    params['java_home'] = shpurdpProperties['java.home'];

    params['threshold'] = 60;
    params['shpurdp_server_host'] = location.hostname;
