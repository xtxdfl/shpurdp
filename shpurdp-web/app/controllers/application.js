...
App.ApplicationController = Em.Controller.extend(App.Persist, {
  ...
  /**
   * set shpurdp server version from installerController or mainController, making sure version shown up all the time
   */
  shpurdpVersion: function () {
    return App.router.get('installerController.shpurdpServerVersion') || App.router.get('mainController.shpurdpServerVersion') || Em.I18n.t('common.notAvailable');
  }.property('App.router.installerController.shpurdpServerVersion', 'App.router.mainController.shpurdpServerVersion'),
  ...
  /**
   * Determines if "Manage Shpurdp" menu-item should be shown
   *
   * @type {boolean}
   */
  showManageShpurdp: function () {
    if (App.router.get('clusterInstallCompleted')) {
      return this.get('isClusterDataLoaded');
    }
  ...
  showAboutPopup: function() {
    App.ModalPopup.show({
      header: Em.I18n.t('common.aboutShpurdp'),
      secondary: false,
      bodyClass: Em.View.extend({
        templateName: require('templates/common/about'),
        shpurdpVersion: this.get('shpurdpVersion')
      })
    });    
  }
