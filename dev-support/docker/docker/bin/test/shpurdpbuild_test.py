...
from bin import shpurdpbuild


# TODO move this to a proper location
def unittest():
  # parse
  result = shpurdpbuild.parse(["test"])
  ...
  result = shpurdpbuild.parse(["server"])
  assert result.is_deep_clean == False
  ...
  result = shpurdpbuild.parse(["agent"])
  assert result.is_deep_clean == False
  assert result.is_test == False
  assert result.is_rebuild == False
  assert result.stack_distribution == None
  ...
  result = shpurdpbuild.parse(["agent", "-b"])
  assert result.is_deep_clean == False
  assert result.is_test == False
  ...
  result = shpurdpbuild.parse(["deploy", "-d"])
  assert result.is_deep_clean == False
  assert result.is_test == False
  assert result.is_rebuild == False
  assert result.stack_distribution == None
  ...
  result = shpurdpbuild.parse(["deploy", "-b", "-s", "BIGTOP", "-d", "-c"])
