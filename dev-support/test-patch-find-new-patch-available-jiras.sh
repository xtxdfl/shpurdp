...
JIRAAVAILPATCHQUERY="https://issues.apache.org/jira/sr/jira.issueviews:searchrequest-xml/temp/SearchRequest.xml?jqlQuery=project+in+%28SHPURDP%29+AND+status+%3D+%22Patch+Available%22+ORDER+BY+updated+DESC&tempMax=1000"
TESTPATCHJOBURL="https://builds.apache.org/job/Shpurdp-trunk-test-patch"
# TESTPATCHJOBURL="http://10.103.219.57:8080/job/Shpurdp-trunk-test-patch"
TOKEN=""
SUBMIT="false"
...
printUsage() {
  ...
  echo "          --submit --token=<SHPURDP PRECOMMIT JOB TOKEN>"
...
### to yield lines with pairs (issueNumber,largestAttachmentId). Example: SHPURDP-123,456984
cat ${TEMPDIR}/patch-attachments.element | awk '{ if ( $1 ~ /^SHPURDP\-/) {JIRA=$1 }; if ($1 ~ /id=/) { print JIRA","$1} }' | sed 's/id\="//' | sed 's/"//' > ${TEMPDIR}/patch-availables.pair
