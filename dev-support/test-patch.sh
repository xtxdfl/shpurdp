...
printUsage() {
  ...
  echo "  defect-number is a JIRA defect number (e.g. 'SHPURDP-1234') to test (Jenkins only)"
  echo
  echo "Options:"
...
findModules () {
  ...
  for module in $(cat $TMP_MODULES | sort | uniq); do
    ...
    if [ "$?" != 0 ] || [ "$module" == "shpurdp-web" ]; then # shpurdp-web is packing = pom but has unit tests
