...
class Host:
  """
  Abstraction of the elements unique to each Shpurdp Agent running on this VM.
  """
...
class Multiplier:
  """
  In order to perform scale testing, this class bootstraps multiple Shpurdp Agents to run on the same host.
  Each Shpurdp Agent has its own home directory with subdirectories for configs, logs, etc.
  ...
  CONFIG_FILE = "/etc/shpurdp-agent/conf/agent-multiplier.conf"
  ...
  def __init__(self, args):
    ...
    self.log_dir = "/var/log/shpurdp-agent"
    self.config_dir = "/etc/shpurdp-agent/conf"
    self.pid_file = "/var/run/shpurdp-agent/shpurdp-agent.pid"
    self.prefix_dir = "/var/lib/shpurdp-agent/data"
    self.cache_dir = "/var/lib/shpurdp-agent/cache"

    # Shpurdp Agent config file to use as a template
    # Will change hostname and port after copying
    self.source_config_file = "/etc/shpurdp-agent/conf/shpurdp-agent.ini"
    self.source_version_file = "/var/lib/shpurdp-agent/data/version"
    ...
    # All hostnames that will be managed by Shpurdp Agents on this host
    self.hosts = []
  ...
  def validate(self):
    ...
    if not os.path.isfile(self.source_config_file):
      errors.append(
        f"Shpurdp Agent config file does not exist at {self.source_config_file}"
      )
  ...
  def bootstrap(self):
    """
    Bootstrap each Shpurdp Agent that will run on this host with the directories it needs and configuration file.
    """
    for host in self.hosts:
      ...
      # Copy config file
      host_config_file = os.path.join(host_config_dir, "shpurdp-agent.ini")
  ...
  def modify_etc_hosts_file(self):
    ...
    fake host names that will be generated for the Shpurdp Agents.
    """
    etc_hosts = "/etc/hosts"
  ...
  def cmd_start(self):
    ...
    for host in self.hosts:
      cmd = f"shpurdp-agent start --home {host.home_dir}"
      os.environ["SHPURDP_AGENT_CONF_DIR"] = os.path.join(
        host.home_dir, "etc/shpurdp-agent/conf"
      )
  ...
  def cmd_stop(self):
    ...
    for host in self.hosts:
      cmd = f"shpurdp-agent stop --home {host.home_dir}"
      os.environ["SHPURDP_AGENT_CONF_DIR"] = os.path.join(
        host.home_dir, "etc/shpurdp-agent/conf"
      )
      subprocess.call(cmd, shell=True, env=os.environ)
  ...
  def cmd_restart(self):
    ...
    for host in self.hosts:
      cmd = f"shpurdp-agent restart --home {host.home_dir}"
      os.environ["SHPURDP_AGENT_CONF_DIR"] = os.path.join(
        host.home_dir, "etc/shpurdp-agent/conf"
      )
      subprocess.call(cmd, shell=True, env=os.environ)
