...
def setup_solr_znode_env():
  ...
  security_json_file_location = (
    custom_security_json_location
    if params.infra_solr_security_json_content
    and str(params.infra_solr_security_json_content).strip()
    else format("{infra_solr_conf}/security.json")
  )  # security.json file to upload

  create_shpurdp_solr_znode(java_opts, jaas_file)
...
def create_shpurdp_solr_znode(java_opts, jaas_file):
  import params
