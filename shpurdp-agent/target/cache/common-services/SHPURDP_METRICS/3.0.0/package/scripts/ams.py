...
def ams(name=None):
  ...
  if name == "collector":
    if not check_windows_service_exists(params.ams_collector_win_service_name):
      Execute(
        format(
          "cmd /C cd {ams_collector_home_dir} & shpurdp-metrics-collector.cmd setup"
        )
      )
    ...
    if not params.is_local_fs_rootdir:
      ...
      links_pairs = [
        (
          "%COLLECTOR_HOME%\\hbase\\lib\\shpurdp-metrics-hadoop-sink-with-common.jar",
          "%SINK_HOME%\\hadoop-sink\\shpurdp-metrics-hadoop-sink-with-common-*.jar",
        ),
      ]
    ...
    if not check_windows_service_exists(params.ams_monitor_win_service_name):
      Execute(
        format("cmd /C cd {ams_monitor_home_dir} & shpurdp-metrics-monitor.cmd setup")
      )

    # creating symbolic links on ams jars to make them available to services
    links_pairs = [
      (
        "%HADOOP_HOME%\\share\\hadoop\\common\\lib\\shpurdp-metrics-hadoop-sink-with-common.jar",
        "%SINK_HOME%\\hadoop-sink\\shpurdp-metrics-hadoop-sink-with-common-*.jar",
      ),
      (
        "%HBASE_HOME%\\lib\\shpurdp-metrics-hadoop-sink-with-common.jar",
        "%SINK_HOME%\\hadoop-sink\\shpurdp-metrics-hadoop-sink-with-common-*.jar",
      ),
    ]
    for link_pair in links_pairs:
  ...
  if name == "collector":
    ...
    if params.clusterHostInfoDict:
      ...
      if master_components:
        if "shpurdp_server" not in master_components:
          master_components.insert(0, "shpurdp_server")
