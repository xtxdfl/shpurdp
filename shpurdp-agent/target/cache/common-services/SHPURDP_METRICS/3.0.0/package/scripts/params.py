...
from shpurdp_commons.shpurdp_metrics_helper import (
...
# find shpurdp version for grafana dashboards
def get_shpurdp_version():
  ...
  if os.path.exists(SHPURDP_AGENT_CONF):
    try:
      ...
      Logger.info("Unable to determine shpurdp version from version file.")
      Logger.debug(f"Exception: {str(e)}")
      # No hostname script identified in the shpurdp agent conf
      pass
    pass
...
ams_collector_conf_dir = "/etc/shpurdp-metrics-collector/conf"
...
ams_monitor_conf_dir = "/etc/shpurdp-metrics-monitor/conf"
ams_monitor_pid_dir = status_params.ams_monitor_pid_dir
ams_monitor_script = "/usr/sbin/shpurdp-metrics-monitor"

ams_grafana_script = "/usr/sbin/shpurdp-metrics-grafana"
ams_grafana_home_dir = "/usr/lib/shpurdp-metrics-grafana"
ams_grafana_log_dir = default(
  "/configurations/ams-grafana-env/metrics_grafana_log_dir",
  "/var/log/shpurdp-metrics-grafana",
)
ams_grafana_pid_dir = status_params.ams_grafana_pid_dir
ams_grafana_conf_dir = "/etc/shpurdp-metrics-grafana/conf"
ams_grafana_data_dir = default(
  "/configurations/ams-grafana-env/metrics_grafana_data_dir",
  "/var/lib/shpurdp-metrics-grafana",
)
...
ams_grafana_cert_file = default(
  ...
  "/etc/shpurdp-metrics/conf/ams-grafana.crt",
)
ams_grafana_cert_key = default(
  "/configurations/ams-grafana-ini/cert_key", "/etc/shpurdp-metrics/conf/ams-grafana.key"
)
...
shpurdp_java_home = default("/shpurdpLevelParams/shpurdp_java_home", None)
# not supporting 32 bit jdk.
java64_home = shpurdp_java_home if shpurdp_java_home is not None else java_home
shpurdp_java_version = default("/shpurdpLevelParams/shpurdp_java_version", None)
if shpurdp_java_version:
  java_version = expect("/shpurdpLevelParams/shpurdp_java_version", int)
else:
  java_version = expect("/shpurdpLevelParams/java_version", int)
...
# Shpurdp metrics log4j settings
...
min_hadoop_sink_version = default(
  "/configurations/ams-env/min_shpurdp_metrics_hadoop_sink_version", "2.7.0.0"
)
