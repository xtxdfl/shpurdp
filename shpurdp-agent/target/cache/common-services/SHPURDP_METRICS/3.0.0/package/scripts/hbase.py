...
def hbase(
  ...
  if os.path.exists(params.ams_hbase_real_conf_dir) and not os.path.islink(
    params.ams_hbase_real_conf_dir
  ):
    Directory(params.ams_hbase_real_conf_dir, action="delete")

  Link(params.ams_hbase_real_conf_dir, to=params.hbase_conf_dir)
  ...
  if not params.is_hbase_distributed:
    File(
      format("{hbase_conf_dir}/core-site.xml"), action="delete", owner=params.hbase_user
    )

    File(
      format("{hbase_conf_dir}/hdfs-site.xml"), action="delete", owner=params.hbase_user
    )
