...
# Local imports
from shpurdp_configuration import ShpurdpConfiguration
...
class StackAdvisor(object):
  ...
  @param hosts: Dictionary containing all information about hosts in this cluster
    Example: {
      "items": [
        {
          Hosts: {
            "host_name": "c6401.shpurdp.apache.org",
            "public_host_name" : "c6401.shpurdp.apache.org",
            "ip": "192.168.1.101",
            "cpu_count" : 1,
            "disk_info" : [
              {
    ...
    Recommendations are made in a Shpurdp Blueprints friendly format.
  ...
  def recommendComponentLayout(self, services, hosts):
    ...
    @param hosts: Dictionary containing all information about hosts in this cluster
    @rtype: dictionary
    @return: Layout recommendation of service components on cluster hosts in Shpurdp Blueprints friendly format.
        Example: {
          "resources" : [
            {
              "hosts" : [
              ...
              "recommendations" : {
                ...
                "blueprint_cluster_binding" : {
                  "host_groups" : [
                    {
                      "name" : "host-group-1",
                      "hosts" : [ { "fqdn" : "c6401.shpurdp.apache.org" } ]
                    },
                    {
                      "name" : "host-group-2",
                      "hosts" : [ { "fqdn" : "c6402.shpurdp.apache.org" } ]
                    }
                  ]
                }
              }
...
class DefaultStackAdvisor(StackAdvisor):
  ...
  def getServiceComponentLayoutValidations(self, services, hosts):
    ...
    :param services: Dictionary of the form:
    {
      ...
      'shpurdp-server-properties': {'key': 'value', ...},
      'services': [
        {'StackServices': {
          'advisor_path': '/var/lib/shpurdp-server/resources/common-services/MYSERVICE/1.2.3/service_advisor.py',
        ...
        'components': [
          {'StackServiceComponents': {
            ...
            'cardinality': '0+|1|1+',
            'hostnames': ['c6401.shpurdp.apache.org'],
  ...
  def get_shpurdp_configuration(self, services):
    """
    Gets the ShpurdpConfiguration object that can be used to request details about
    the Shpurdp configuration. For example LDAP and SSO configurations

    :param services: the services structure containing the "shpurdp-server-configurations" block
    :return: an ShpurdpConfiguration
    """
    return ShpurdpConfiguration(services)
  ...
  def recommendHadoopProxyUsers(self, configurations, services, hosts):
    ...
    self.recommendShpurdpProxyUsersForHDFS(
      services,
      configurations,
      servicesList,
      putCoreSiteProperty,
  ...
  def recommendShpurdpProxyUsersForHDFS(
    self,
    services,
    configurations,
    servicesList,
    ...
    if "HDFS" in servicesList:
      ...
      if old_shpurdp_user is not None:
        putCoreSitePropertyAttribute(
          f"hadoop.proxyuser.{old_shpurdp_user}.hosts", "delete", "true"
        )
        putCoreSitePropertyAttribute(
          f"hadoop.proxyuser.{old_shpurdp_user}.groups", "delete", "true"
        )
  ...
  def getShpurdpUser(self, services):
    ...
    if (
      ...
      shpurdp_user = services["configurations"]["cluster-env"]["properties"][
        "shpurdp_principal_name"
      ]
  ...
  def validatorEnoughDiskSpace(
    ...
    if mountPoints[mountPoint] < reqiuredDiskSpace:
      msg = (
        "Shpurdp Metrics disk space requirements not met. \n"
