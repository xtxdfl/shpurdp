...
def setup_unlimited_key_jce_policy():
  """
  Sets up the unlimited key JCE policy if needed. (sets up shpurdp JCE as well if shpurdp and the  stack use different JDK)
  """
  import params
  ...
  if params.shpurdp_jce_name and params.shpurdp_jce_name != params.jce_policy_zip:
    __setup_unlimited_key_jce_policy(
      custom_java_home=params.shpurdp_java_home,
      custom_jdk_name=params.shpurdp_jdk_name,
      custom_jce_name=params.shpurdp_jce_name,
    )
...
def __setup_unlimited_key_jce_policy(
  ...
  The following criteria must be met:
    ...
    * Shpurdp is managing the host's JVM - /shpurdpLevelParams/jdk_name is set
  ...
  If the conditions are met, the following steps are taken to install the unlimited key JCE policy JARs
    ...
    1. The unlimited key JCE policy ZIP file is downloaded from the Shpurdp server and stored in the
        Shpurdp agent's temporary directory
  ...
  if params.sysprep_skip_setup_jce:
    ...
    Logger.info(
      "Skipping unlimited key JCE policy check and setup since the Java VM is not managed by Shpurdp"
    )
    ...
    if jcePolicyInfo.is_unlimited_key_jce_policy():
      ...
      jce_zip_source = format("{shpurdp_server_resources_url}/{custom_jce_name}")
