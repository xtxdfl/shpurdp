...
if not security_enabled:
  ...
  if dfs_http_policy == "HTTPS_ONLY":
    secure_dn_ports_are_in_use = is_secure_port(dfs_dn_port) or is_secure_port(
      dfs_dn_https_port
    )
  elif dfs_http_policy == "HTTP_AND_HTTPS":
    secure_dn_ports_are_in_use = (
      is_secure_port(dfs_dn_port)
      or is_secure_port(dfs_dn_http_port)
      or is_secure_port(dfs_dn_https_port)
    )
...
dfs_ha_namenode_ids_all_ns = (
  get_properties_for_all_nameservices(hdfs_site, "dfs.ha.namenodes")
