...
from shpurdp_commons.repo_manager import ManagerFactory
from shpurdp_commons.os_check import OSCheck, OSConst
from shpurdp_commons.inet_utils import download_file
from resource_management import Script, Execute, format
from shpurdp_agent.HostInfo import HostInfo
from shpurdp_agent.HostCheckReportFileHandler import HostCheckReportFileHandler
...
class CheckHost(Script):
  ...
  ADDITIONAL_PACKAGES = ["shpurdp-log4j"]

  # ignore repos from the list of repos to be cleaned
  IGNORE_REPOS = ["HDP-UTILS", "SHPURDP", "BASE", "EXTRAS"]
  ...
  def execute_db_connection_check(self, config, tmp_dir):
    ...
    shpurdp_server_hostname = config["commandParams"]["shpurdp_server_host"]
    ...
    if db_name == DB_MYSQL:
      jdbc_driver_mysql_name = default(
        "/shpurdpLevelParams/custom_mysql_jdbc_name", None
      )
      if not jdbc_driver_mysql_name:
        no_jdbc_error_message = "The MySQL JDBC driver has not been set. Please ensure that you have executed 'shpurdp-server setup --jdbc-db=mysql --jdbc-driver=/path/to/jdbc_driver'."
      else:
      ...
      jdbc_driver_oracle_name = default(
        "/shpurdpLevelParams/custom_oracle_jdbc_name", None
      )
      if not jdbc_driver_oracle_name:
        no_jdbc_error_message = "The Oracle JDBC driver has not been set. Please ensure that you have executed 'shpurdp-server setup --jdbc-db=oracle --jdbc-driver=/path/to/jdbc_driver'."
      else:
      ...
      jdbc_driver_postgres_name = default(
        "/shpurdpLevelParams/custom_postgres_jdbc_name", None
      )
      if not jdbc_driver_postgres_name:
        no_jdbc_error_message = "The Postgres JDBC driver has not been set. Please ensure that you have executed 'shpurdp-server setup --jdbc-db=postgres --jdbc-driver=/path/to/jdbc_driver'."
      else:
      ...
      jdbc_driver_mssql_name = default(
        "/shpurdpLevelParams/custom_mssql_jdbc_name", None
      )
      if not jdbc_driver_mssql_name:
        no_jdbc_error_message = "The MSSQL JDBC driver has not been set. Please ensure that you have executed 'shpurdp-server setup --jdbc-db=mssql --jdbc-driver=/path/to/jdbc_driver'."
      else:
      ...
      jdbc_driver_sqla_name = default(
        "/shpurdpLevelParams/custom_sqlanywhere_jdbc_name", None
      )
      if not jdbc_driver_sqla_name:
        no_jdbc_error_message = "The SQLAnywhere JDBC driver has not been set. Please ensure that you have executed 'shpurdp-server setup --jdbc-db=sqlanywhere --jdbc-driver=/path/to/jdbc_driver'."
      else:
    ...
    environment = {"no_proxy": format("{shpurdp_server_hostname}")}
    ...
    if not os.path.isfile(java_exec):
      ...
      try:
        ...
        message = (
          "Error downloading JDK from Shpurdp Server resources. Check network access to "
          "Shpurdp Server.\n" + str(e)
        )
      ...
      if jdk_name.endswith(".exe"):
        install_cmd = "{0} /s INSTALLDIR={1} STATIC=1 WEB_JAVA=0 /L \\var\\log\\shpurdp-agent".format(
    ...
    # download DBConnectionVerification.jar from shpurdp-server resources
    try:
      download_file(check_db_connection_url, check_db_connection_path)

    except Exception as e:
      message = (
        "Error downloading DBConnectionVerification.jar from Shpurdp Server resources. Check network access to "
        "Shpurdp Server.\n" + str(e)
      )
      Logger.exception(message)
      db_connection_check_structured_output = {"exit_code": 1, "message": message}
    ...
    # download jdbc driver from shpurdp-server resources
    try:
      ...
      message = format(
        "Error: Shpurdp Server cannot download the database JDBC driver and is unable to test the "
        "database connection. You must run shpurdp-server setup --jdbc-db={db_name} "
        "--jdbc-driver=/path/to/your/{db_name}/driver.jar on the Shpurdp Server host to make the JDBC "
    ...
    db_connection_check_command = format(
      ...
      '{jdbc_jar_path} -Djava.library.path={java_library_path} org.apache.shpurdp.server.DBConnectionVerification "{db_connection_url}" '
