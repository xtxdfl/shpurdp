...
PROPERTIES_TO_CHECK = {
  ...
  # ZOOKEEPER
  "ZOOKEEPER_SERVER": {"zookeeper-env": ["zk_data_dir", "zk_log_dir", "zk_pid_dir"]},
  ...
  # HIVE
  "HIVE_METASTORE": {"hive-env": ["hive_log_dir", "hive_pid_dir"]},
  "HIVE_SERVER": {"hive-env": ["hive_log_dir", "hive_pid_dir"]},
  "MYSQL_SERVER": {"hive-env": ["hive_log_dir", "hive_pid_dir"]},
  "HIVE_CLIENT": {"hive-env": ["hive_log_dir", "hive_pid_dir"]},
  "HCAT": {"hive-env": ["hcat_log_dir", "hcat_pid_dir"]},
  # OOZIE
  "OOZIE_SERVER": {"oozie-env": ["oozie_data_dir", "oozie_log_dir", "oozie_pid_dir"]},
...
USERS_TO_GROUP_MAPPING = {
  ...
  # MAPREDUCE
  "JOBTRACKER": {"mapred-env": {"mapred_user": "mapred_user"}},
  "TASKTRACKER": {"mapred-env": {"mapred_user": "mapred_user"}},
  "MAPREDUCE_CLIENT": {"mapred-env": {"mapred_user": "mapred_user"}},
  "HISTORYSERVER": {"mapred-env": {"mapred_user": "mapred_user"}},
  # MAPREDUCE2
  "MAPREDUCE2_CLIENT": {"mapred-env": {"mapred_user": "mapred_user"}},
  # YARN
  "RESOURCEMANAGER": {"yarn-env": {"yarn_user": "yarn_user"}},
  "NODEMANAGER": {"yarn-env": {"yarn_user": "yarn_user"}},
  "YARN_CLIENT": {"yarn-env": {"yarn_user": "yarn_user"}},
  # ZOOKEEPER
  "ZOOKEEPER_SERVER": {"zookeeper-env": {"zk_user": "zk_user"}},
  ...
  "HBASE_CLIENT": {"hbase-env": {"hbase_user": "hbase_user"}},
  # HIVE
  "HIVE_METASTORE": {"hive-env": {"hive_user": "hive_user"}},
  "HIVE_SERVER": {"hive-env": {"hive_user": "hive_user"}},
  "MYSQL_SERVER": {"hive-env": {"hive_user": "hive_user"}},
  "HIVE_CLIENT": {"hive-env": {"hive_user": "hive_user"}},
  "HCAT": {"hive-env": {"hive_user": "hive_user"}},
  # OOZIE
  "OOZIE_SERVER": {"oozie-env": {"oozie_user": "oozie_user"}},
  # PIG - no users to check
  # SQOOP - no users to check
  # WEBHCAT - no users to check
  # FALCON
  "FALCON_CLIENT": {"falcon-env": {"falcon_user": "falcon_user"}},
  "FALCON_SERVER": {"falcon-env": {"falcon_user": "falcon_user"}},
  # STORM
  "NIMBUS": {"storm-env": {"storm_user": "storm_user"}},
  "STORM_REST_API": {"storm-env": {"storm_user": "storm_user"}},
  "SUPERVISOR": {"storm-env": {"storm_user": "storm_user"}},
  "STORM_UI_SERVER": {"storm-env": {"storm_user": "storm_user"}},
  "DRPC_SERVER": {"storm-env": {"storm_user": "storm_user"}},
  # TEZ
  "TEZ_CLIENT": {"tez-env": {"tez_user": "tez_user"}},
}
...
class ValidateConfigs(Script):
  ...
  def check_directories(self, params):
    ...
    for property, value in params.items():
      if property in properties_to_check:
        if self.dir_exists_or_not_empty_or_cant_be_created(
          self.get_value(property, params)
        ):
  ...
  def flatten_dict(
    self, dic, prefix=CONFIG_PARAM_PREFIX, separator=CONFIG_PARAM_SEPARATOR
  ):
    result = []
  ...
  def dir_exists_or_not_empty_or_cant_be_created(self, filepath):
    if (
      os.path.isdir(filepath) and os.access(filepath, os.W_OK) and os.listdir(filepath)
    ):
      msg = (
        "Validation failed. Directory "
        + filepath
