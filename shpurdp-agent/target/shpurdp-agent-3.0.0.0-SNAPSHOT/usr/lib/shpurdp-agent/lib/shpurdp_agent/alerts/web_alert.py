...
from shpurdp_commons.inet_utils import (
  resolve_address,
...
ensure_ssl_using_protocol(
  ShpurdpConfig.get_resolved_config().get_force_https_protocol_name(),
  ShpurdpConfig.get_resolved_config().get_ca_cert_file_path(),
)
...
class WebAlert(BaseAlert):
  ...
  def _collect(self):
    ...
    logger.debug(
      "[Alert][{0}] Calculated web URI to be {1} (ssl={2})".format(
    ...
    if (
      ...
      and status_code in self.uri_property_keys.acceptable_codes
    ):
  ...
  def _make_web_request(self, url):
    ...
    try:
      ...
      if (
        ...
        and security_enabled is not None
        and security_enabled.lower() == "true"
      ):
      ...
      return WebResponse(
        status_code=response_code, time_millis=time_millis, error_msg=error_msg
      )
    ...
    try:
      ...
      if response is not None:
        try:
          ...
          if logger.isEnabledFor(logging.DEBUG):
            logger.exception(
              f"[Alert][{self.get_name()}] Unable to close socket connection"
            )
