...
from shpurdp_commons.buffered_queue import BufferedQueue
...
def launch_subprocess(command, term_geometry=(42, 255), env=None):
  ...
  if not is_under_root() and not isinstance(command, str):
    command = f"{SHPURDP_SUDO_BINARY} -H -E {string_cmd_from_args_list(command)}"  # core.shell.as_sudo
...
def __handle_retries(
  ...
  """
  :type cmd list
  :type repo_properties shpurdp_commons.repo_manager.generic_manager.GenericManagerProperties
...
def repository_manager_executor(
  ...
  :type cmd list
  :type repo_properties shpurdp_commons.repo_manager.generic_manager.GenericManagerProperties | YumManagerProperties | ZypperManagerProperties | AptManagerProperties
  :type context RepoCallContext
  :type env dict
