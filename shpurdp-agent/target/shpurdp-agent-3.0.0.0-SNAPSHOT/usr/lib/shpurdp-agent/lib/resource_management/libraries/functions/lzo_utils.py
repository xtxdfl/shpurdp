...
INSTALLING_LZO_WITHOUT_GPL = "Cannot install LZO.  The GPL license must be explicitly enabled using 'shpurdp-server setup' on the Shpurdp host, then restart the server and try again."
...
def is_gpl_license_accepted():
  return default("/shpurdpLevelParams/gpl_license_accepted", False)
...
def skip_package_operations():
  ...
  """
  return default("/shpurdpLevelParams/host_sys_prepped", False) and default(
