  ...
  def inner(command, **kwargs):
    ...
    if "logoutput" in function.__code__.co_varnames:
      kwargs["logoutput"] = (
        (
          "logoutput" in kwargs
          and kwargs["logoutput"]
          and Logger.isEnabledFor(logging.INFO)
        )
        or (
          "logoutput" in kwargs
          and kwargs["logoutput"] == None
          and Logger.isEnabledFor(logging.DEBUG)
        )
        or (
          not "logoutput" in kwargs
          and not is_internal_call
          and Logger.isEnabledFor(logging.DEBUG)
        )
      )
...
def _call_wrapper(command, **kwargs):
  ...
  for i in range(0, tries):
    ...
    if not is_last_try:
      ...
      kwargs_copy["throw_on_failure"] = (
        True  # we need to know when non-last try fails, to handle retries
      )
    else:
      kwargs_copy = kwargs
...
def _call(
  ...
  command_alias = (
    string_cmd_from_args_list(command_alias)
    if isinstance(command_alias, (list, tuple))
    else command_alias
  )
  ...
  for placeholder, replacement in PLACEHOLDERS_TO_STR.items():
    subprocess_command = [
      cmd.replace(placeholder, replacement.format(env_str=env_str))
      for cmd in subprocess_command
    ]

  if shell:
    # --noprofile is used to preserve PATH set for shpurdp-agent
    subprocess_command = [
      "/bin/bash",
      "--login",
      "--noprofile",
  ...
  try:
    ...
    if timeout:
      ...
      timer = threading.Timer(
        timeout, _on_timeout, [proc, timeout_event, timeout_kill_strategy]
      )
      timer.start()
    ...
    logoutput = (
      logoutput is True
      and Logger.logger.isEnabledFor(logging.INFO)
      or logoutput is None
      and Logger.logger.isEnabledFor(logging.DEBUG)
    )
    read_set = []
  ...
  if timeout:
    ...
    # timeout occurred
      ...
      err_msg = (
        f"Execution of '{command}' was killed due timeout after {timeout} seconds"
      )
  ...
  if throw_on_failure and not code in returns:
    err_msg = Logger.filter_text(
      f"Execution of '{command_alias}' returned {code}. {all_output}"
    )
...
def as_sudo(command, env=None, auto_escape=True):
  ...
  if isinstance(command, (list, tuple)):
    ...
    # Since shpurdp user sudoer privileges may be restricted,
    ...
    err_msg = Logger.filter_text(
      f"String command '{command}' cannot be run as sudo. Please supply the command as a tuple of arguments"
    )
    raise Fail(err_msg)
...
def as_user(command, user, env=None, auto_escape=True):
  ...
  export_env = (
    f"export {_get_environment_str(_add_current_path_to_env(env))} ; "
    if env
    else EXPORT_PLACEHOLDER
  )
...
def _add_current_path_to_env(env):
  ...
  if not set(os.environ["PATH"].split(os.pathsep)).issubset(
