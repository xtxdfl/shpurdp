...
from shpurdp_commons.os_windows import SystemWideLock
...
INSTALL_MSI_CMD = (
  "cmd /C start /wait msiexec /qn /i  {msi_path} /lv {log_path} MSIUSEREALADMINDETECTION=1 "
  "HDP_LAYOUT={layout_path} DESTROY_DATA=yes HDP_USER={hadoop_user} HDP_USER_PASSWORD={hadoop_password_arg} HDP=yes "
  'KNOX=yes KNOX_MASTER_SECRET="ShpurdpHDP2Windows" FALCON=yes STORM=yes HBase=yes STORM=yes FLUME=yes PHOENIX=no RANGER=no'
)
CREATE_SERVICE_SCRIPT = os.path.abspath("sbin\createservice.ps1")
CREATE_SERVICE_CMD = (
  'cmd /C powershell -ExecutionPolicy Bypass -File "{script}" -username {username} -password "{password}" -servicename '
...
def _ensure_services_created(hadoop_user, hadoop_password):
  ...
  if not check_windows_service_exists("journalnode"):
    Execute(
      CREATE_SERVICE_CMD.format(
        script=CREATE_SERVICE_SCRIPT,
        username=hadoop_user,
        password=hadoop_password,
        servicename="journalnode",
        resourcedir=resource_dir_hdfs,
        servicecmd=service_cmd_hdfs,
      ),
      logoutput=True,
    )
...
def _create_symlinks(stack_version):
  ...
  if (
    ...
    and compare_versions(stack_version_formatted, "2.2") >= 0
  ):
...
def _is_msi_installed():
  return os.path.exists(
    os.path.join(_working_dir, INSTALL_MARKER_OK)
  ) and check_windows_service_exists("namenode")


# check if msi was installed correctly and raise Fail in case of broken install
def _validate_msi_install():
  if not _is_msi_installed() and os.path.exists(
    os.path.join(_working_dir, INSTALL_MARKER_FAILED)
  ):
...
def install_windows_msi(
  url_base, save_dir, save_files, hadoop_user, hadoop_password, stack_version
):
  global _working_dir
  ...
  try:
    ...
    File(
      os.path.join(msi_save_dir, "properties.txt"),
