...
-----------------------------------------------------------------------
 ...
 setup(name='simplejson', version='3.16.1',
       ext_modules=[
         Extension("shpurdp_commons._speedups", ["_speedups.c"])
       ])

 - Run python setup.py build
 - Check build folder for compiled library
 - Place resulting file to "shpurdp_simplejson/_speedups/<your platform>/" folder and create empty __init__.py at the same path
 - Add newly added library path to c extension loader at shpurdp_simplejson/_import_paths.py#get -> _import_paths
