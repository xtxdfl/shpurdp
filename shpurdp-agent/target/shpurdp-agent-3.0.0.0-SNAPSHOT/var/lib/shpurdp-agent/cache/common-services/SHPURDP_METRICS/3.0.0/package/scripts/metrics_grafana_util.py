...
METRICS_GRAFANA_DATASOURCE_NAME = "SHPURDP_METRICS"
  ...
  for i in range(0, params.grafana_connect_attempts):
    try:
      ...
      if i < params.grafana_connect_attempts - 1:
        ...
        raise Fail(f"Shpurdp Metrics Grafana update failed due to: {str(ex)}")
      pass
...
def create_grafana_admin_pwd():
  ...
  if response and response.status != 200:
    ...
    if response.status == 200:
      Logger.info("Shpurdp Metrics Grafana password updated.")

    elif response.status == 500:
      Logger.info("Shpurdp Metrics Grafana password update failed. Not retrying.")
      raise Fail(
        "Shpurdp Metrics Grafana password update failed. PUT request status: %s %s \n%s"
        % (response.status, response.reason, data)
      )
    else:
      raise Fail(
        "Shpurdp Metrics Grafana password creation failed. "
...
def create_ams_datasource():
  ...
  if response and response.status == 200:
    ...
    for i in range(0, len(datasources_json)):
      ...
      if datasource_name == METRICS_GRAFANA_DATASOURCE_NAME:
        ...
        Logger.info(
          "Shpurdp Metrics Grafana datasource already present. Checking Metrics Collector URL"
        )
        ...
        if update_datasource:  # Metrics datasource present, but collector host is wrong or the datasource type is outdated.
          ...
          if response.status == 200:
            Logger.info("Shpurdp Metrics Grafana data source updated.")

          elif response.status == 500:
            Logger.info(
              "Shpurdp Metrics Grafana data source update failed. Not retrying."
            )
            raise Fail(
              "Shpurdp Metrics Grafana data source update failed. PUT request status: %s %s \n%s"
              % (response.status, response.reason, data)
            )
          else:
            raise Fail(
              "Shpurdp Metrics Grafana data source creation failed. "
              "PUT request status: %s %s \n%s"
              % (response.status, response.reason, data)
            )
        pass
    ...
    Logger.info(
      "Error checking for Shpurdp Metrics Grafana datasource. Will attempt to create."
    )
  ...
  if not create_datasource:
    ...
    if response.status == 200:
      Logger.info("Shpurdp Metrics Grafana data source created.")
    elif response.status == 500:
      Logger.info("Shpurdp Metrics Grafana data source creation failed. Not retrying.")
      raise Fail(
        "Shpurdp Metrics Grafana data source creation failed. POST request status: %s %s \n%s"
        % (response.status, response.reason, data)
      )
    else:
      Logger.info("Shpurdp Metrics Grafana data source creation failed.")
      raise Fail(
        "Shpurdp Metrics Grafana data source creation failed. POST request status: %s %s \n%s"
        % (response.status, response.reason, data)
      )
...
def create_ams_dashboards():
  ...
  version = params.get_shpurdp_version()
  Logger.info(f"Checking dashboards to update for Shpurdp version : {version}")
        ...
        for dashboard in existing_dashboards:
          if dashboard.title == dashboard_def["title"]:
            if version not in dashboard.tags:
              # Found existing dashboard with wrong shpurdp version - update dashboard
              update_def = True
