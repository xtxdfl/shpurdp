...
from shpurdp_agent.alerts.collector import AlertCollector
from shpurdp_agent.alerts.base_alert import BaseAlert
from shpurdp_agent.alerts.metric_alert import MetricAlert
from shpurdp_agent.alerts.port_alert import PortAlert
from shpurdp_agent.alerts.script_alert import ScriptAlert
from shpurdp_agent.alerts.web_alert import WebAlert
from shpurdp_agent.alerts.recovery_alert import RecoveryAlert
from shpurdp_agent.apscheduler.scheduler import Scheduler
from shpurdp_agent.ClusterConfigurationCache import ClusterConfigurationCache
from shpurdp_commons.urllib_handlers import RefreshHeaderProcessor
...
class TestAlerts(TestCase):
  ...
  def test_start(self, aps_add_interval_job_mock, aps_start_mock):
    test_file_path = os.path.join("shpurdp_agent", "dummy_files")
    test_stack_path = os.path.join("shpurdp_agent", "dummy_files")
    test_common_services_path = os.path.join("shpurdp_agent", "dummy_files")
    test_extensions_path = os.path.join("shpurdp_agent", "dummy_files")
    test_host_scripts_path = os.path.join("shpurdp_agent", "dummy_files")
  ...
  def test_recovery_alert(self, rm_get_actions_mock, is_stale_mock):
    ...
    alert.set_cluster("c1", "0", "c6401.shpurdp.apache.org")
  ...
  def test_port_alert_complex_uri(self, socket_connect_mock, get_configuration_mock):
    ...
    configuration = {
      "hdfs-site": {
        "my-key": "c6401.shpurdp.apache.org:2181,c6402.shpurdp.apache.org:2181,c6403.shpurdp.apache.org:2181"
      }
    }
    ...
    alert.set_cluster("c1", "0", "c6402.shpurdp.apache.org")
  ...
  def test_port_alert_no_sub(self):
    definition_json = {
      ...
      "source": {
        "type": "PORT",
        "uri": "http://c6401.shpurdp.apache.org",
        "default_port": 50070,
        "reporting": {
          "ok": {"text": "(Unit Tests) TCP OK - {0:.4f} response time on port {1}"},
          "critical": {"text": "(Unit Tests) Could not load process info: {0}"},
        },
      },
    }
    ...
    self.assertEqual("http://c6401.shpurdp.apache.org", alert.uri)
  ...
  def test_script_alert(self, get_configuration_mock):
    ...
    definition_json["source"]["stacks_directory"] = os.path.join(
      "shpurdp_agent", "dummy_files"
    )
    definition_json["source"]["common_services_directory"] = os.path.join(
      "shpurdp_agent", "common-services"
    )
    definition_json["source"]["extensions_directory"] = os.path.join(
      "shpurdp_agent", "extensions"
    )
    definition_json["source"]["host_scripts_directory"] = os.path.join(
      "shpurdp_agent", "host_scripts"
    )
    ...
    self.assertEqual(
      definition_json["source"]["extensions_directory"], alert.extensions_dir
    )
    self.assertEqual(
      definition_json["source"]["common_services_directory"], alert.common_services_dir
    )
    self.assertEqual(
      definition_json["source"]["host_scripts_directory"], alert.host_scripts_dir
    )
  ...
  def test_metric_alert(self, ma_load_jmx_mock, get_configuration_mock):
    ...
    configuration = {
      "hdfs-site": {"dfs.datanode.http.address": "c6401.shpurdp.apache.org:80"}
    }
  ...
  def test_alert_uri_structure(self, ma_load_jmx_mock, get_configuration_mock):
    ...
    configuration = {
      "hdfs-site": {
        ...
        "dfs.datanode.http.address": "c6401.shpurdp.apache.org:80",
      }
    }
    ...
    configuration = {
      "hdfs-site": {
        "dfs.http.policy": "HTTP_ONLY",
        "dfs.datanode.https.address": "c6401.shpurdp.apache.org:443",
      }
    }
  ...
  def test_web_alert(self, wa_make_web_request_mock, get_configuration_mock):
    ...
    wa_make_web_request_mock.return_value = WebResponse(
      500, 1.234, "Internal Server Error"
    )
    collector = AlertCollector()
    alert = WebAlert(definition_json, definition_json["source"], self.config)
    alert.set_helpers(
    ...
    self.assertEqual("WARNING", alerts[0]["state"])
    self.assertEqual(
      "(Unit Tests) warning: 500 (Internal Server Error)", alerts[0]["text"]
    )
    ...
    self.assertEqual("CRITICAL", alerts[0]["state"])
    self.assertEqual(
      "(Unit Tests) critical: http://c6401.shpurdp.apache.org:80. error message",
      alerts[0]["text"],
    )
    ...
    configuration = {
      "hdfs-site": {
        ...
        "dfs.datanode.http.address": "c6401.shpurdp.apache.org:80",
        "dfs.datanode.https.address": "c6401.shpurdp.apache.org:443/test/path",
      }
    }
    ...
    self.assertEqual("CRITICAL", alerts[0]["state"])
    self.assertEqual(
      "(Unit Tests) critical: https://c6401.shpurdp.apache.org:443/test/path. error message",
      alerts[0]["text"],
    )
  ...
  def test_immediate_alert(self):
    ...
    execution_commands = [
      {
        "clusterName": "c1",
        "hostName": "c6401.shpurdp.apache.org",
  ...
  def test_default_reporting_text(self):
    ...
    alert = PortAlert(definition_json, definition_json["source"], self.config)
    self.assertEqual(
      alert._get_reporting_text(alert.RESULT_OK),
      "TCP OK - {0:.4f} response on port {1}",
    )
    self.assertEqual(
      alert._get_reporting_text(alert.RESULT_WARNING),
      "TCP OK - {0:.4f} response on port {1}",
    )
    self.assertEqual(
      alert._get_reporting_text(alert.RESULT_CRITICAL),
    ...
    alert = WebAlert(definition_json, definition_json["source"], self.config)
    self.assertEqual(
      alert._get_reporting_text(alert.RESULT_OK), "HTTP {0} response in {2:.4f} seconds"
    )
    self.assertEqual(
      alert._get_reporting_text(alert.RESULT_WARNING),
    ...
    self.assertEqual(
      alert._get_reporting_text(alert.RESULT_CRITICAL), "Connection failed to {1}"
    )
    ...
    self.assertEqual(
      alert._get_reporting_text(alert.RESULT_OK),
      "No recovery operations executed for {2}{0}.",
    )
    self.assertEqual(
      alert._get_reporting_text(alert.RESULT_WARNING),
      "{1} recovery operations executed for {2}{0}.",
    )
    self.assertEqual(
      alert._get_reporting_text(alert.RESULT_CRITICAL),
      "{1} recovery operations executed for {2}{0}.",
  ...
  def test_uri_structure_parsing(self, get_configuration_mock):
    ...
    configuration = {
      "hdfs-site": {
        "dfs.namenode.http.address": "c6401.shpurdp.apache.org:80",
        "dfs.namenode.https.address": "c6401.shpurdp.apache.org:443",
      }
    }
    ...
    self.assertFalse(
      alert._check_uri_ssl_property(uri_keys, {"configurations": configuration})
    )

    uri = alert._get_uri_from_structure(uri_keys)
    self.assertEqual("c6401.shpurdp.apache.org:80", uri.uri)
    ...
    uri = alert._get_uri_from_structure(uri_keys)
    self.assertEqual("c6401.shpurdp.apache.org:443", uri.uri)
    ...
    configuration = {
      "hdfs-site": {
        ...
        "dfs.namenode.http-address.c1ha.nn1": "c6401.shpurdp.apache.org:8080",
        "dfs.namenode.http-address.c1ha.nn2": "c6402.shpurdp.apache.org:8080",
      }
    }
    ...
    uri = alert._get_uri_from_structure(uri_keys)
    self.assertEqual("c6401.shpurdp.apache.org:8080", uri.uri)
    self.assertEqual(False, uri.is_ssl_enabled)
    ...
    configuration = {
      "hdfs-site": {
        ...
        "dfs.namenode.http-address.c1ha.nn1": "c6401.shpurdp.apache.org:8080",
        "dfs.namenode.http-address.c1ha.nn2": "c6402.shpurdp.apache.org:8080",
        "dfs.namenode.https-address.c1ha.nn1": "c6401.shpurdp.apache.org:8443",
        "dfs.namenode.https-address.c1ha.nn2": "c6402.shpurdp.apache.org:8443",
      }
    }
    ...
    uri = alert._get_uri_from_structure(uri_keys)
    self.assertEqual("c6401.shpurdp.apache.org:8443", uri.uri)
    self.assertEqual(True, uri.is_ssl_enabled)
  ...
  def test_uri_structure_parsing_without_namespace(self, get_configuration_mock):
    ...
    configuration = {
      "yarn-site": {
        ...
        "yarn.resourcemanager.webapp.address": "c6401.shpurdp.apache.org:80",
        "yarn.resourcemanager.webapp.http.address": "c6401.shpurdp.apache.org:443",
        "yarn.resourcemanager.webapp.address.rm1": "c6401.shpurdp.apache.org:8080",
        "yarn.resourcemanager.webapp.https.address.rm1": "c6401.shpurdp.apache.org:8443",
        "yarn.resourcemanager.webapp.address.rm2": "c6402.shpurdp.apache.org:8080",
        "yarn.resourcemanager.webapp.https.address.rm2": "c6402.shpurdp.apache.org:8443",
    ...
    uri = alert._get_uri_from_structure(uri_keys)
    self.assertEqual("c6402.shpurdp.apache.org:8443", uri.uri)
    self.assertEqual(True, uri.is_ssl_enabled)

  @patch.object(ConfigurationBuilder, "get_configuration")
  @patch("http.client.HTTPConnection")
  @patch.object(RefreshHeaderProcessor, "http_response")
  def test_metric_alert_uses_refresh_processor(
    self, http_response_mock, http_connection_mock, get_configuration_mock
  ):
    """
  ...
  def test_get_configuration_values(self, get_configuration_mock):
    ...
    self.assertEqual(
      "constant", alert._get_configuration_value(configuration_full, "constant")
    )
    self.assertEqual(
      "value1",
      alert._get_configuration_value(configuration_full, "{{foo-site/foo-key1}}"),
    )
    self.assertEqual(
      "value2",
      alert._get_configuration_value(configuration_full, "{{foo-site/foo-key2}}"),
    )
    self.assertEqual(
      "asterisk",
      alert._get_configuration_value(
        configuration_full, "{{foo-site/special-character-*}}"
      ),
    )
    self.assertEqual(
      "dollar sign",
    ...
    self.assertEqual(
      "http://value1/servlet",
      alert._get_configuration_value(
        configuration_full, "http://{{foo-site/foo-key1}}/servlet"
      ),
    )
    self.assertEqual(
      "http://value1/servlet/value2",
      alert._get_configuration_value(
        configuration_full, "http://{{foo-site/foo-key1}}/servlet/{{foo-site/foo-key2}}"
      ),
    )
    ...
    self.assertEqual(
      ...
      alert._get_configuration_value(configuration_full, "{{foo-site}}"),
    )

  @patch.object(ConfigurationBuilder, "get_configuration")
  @patch.object(MetricAlert, "_load_jmx")
  def test_metric_alert_floating_division(
    self, ma_load_jmx_mock, get_configuration_mock
  ):
  ...
  @patch.object(ConfigurationBuilder, "get_configuration")
  @patch.object(socket.socket, "connect")
  def test_alert_definition_value_error_conversion(
    self, socket_connect_mock, get_configuration_mock
  ):
    """
    ...
    configuration = {"hdfs-site": {"my-key": "c6401.shpurdp.apache.org:2181"}}
  ...
  @patch("os.open")
  @patch("os.fdopen")
  def __update_cluster_configuration(
  ...
  def _get_script_alert_definition_with_parameters(self):
    return {
      ...
      "source": {
        "type": "SCRIPT",
        "path": "test_script.py",
        "parameters": [{"name": "script.parameter.foo", "value": "foo bar baz"}],
      },
    }
  ...
  def _get_metric_alert_definition(self):
    return {
      ...
      "source": {
        ...
        "jmx": {
          "property_list": ["someJmxObject/value", "someOtherJmxObject/value"],
          "value": "({0} * 100) + {1}",
        },
        "reporting": {
          "ok": {
  ...
  def _get_alert_definition_with_value_error_text(self):
    return {
      ...
      "source": {
        ...
        "default_port": 50070,
        "reporting": {
          "ok": {"text": "(Unit Tests) TCP OK {0:.4d}"},
          "warning": {"text": "(Unit Tests) TCP Warning {0:.4d}", "value": 1.5},
          "critical": {"text": "(Unit Tests) TCP Critical {0:.4d}", "value": 5.0},
        },
      },
    }
  ...
  def _get_alert_definition_with_too_many_positional_arguments(self):
    return {
      ...
      "source": {
        ...
        "default_port": 50070,
        "reporting": {
          "ok": {"text": "Bad Syntax Going To Mess You Up {0:.4d} {1} {2} {3} {4}"},
          "warning": {
...
class MockAlert(BaseAlert):
  ...
  def __init__(self):
    super(MockAlert, self).__init__(None, None, ShpurdpConfig())
