...
from shpurdp_agent.Grep import Grep
import socket
import os, sys
import logging
...
class TestGrep(TestCase):
  ...
  def setUp(self):
    self.string_good = (
      open("shpurdp_agent" + os.sep + "dummy_puppet_output_good.txt", "r")
      .read()
      .replace("\n", os.linesep)
    )
    self.string_bad = (
      open("shpurdp_agent" + os.sep + "dummy_puppet_output_error.txt", "r")
      .read()
      .replace("\n", os.linesep)
    )
    pass
...
debug: Received report to process from shpurdp-dmi
debug: Processing report from shpurdp-dmi with processor Puppet::Reports::Store
""".replace("\n", os.linesep).strip()
    self.assertEqual(
