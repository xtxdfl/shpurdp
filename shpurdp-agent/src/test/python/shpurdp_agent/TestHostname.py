...
class TestHostname(TestCase):
  ...
  def test_server_hostnames(self):
    ...
    config.set("server", "hostname", "shpurdp-host")
    server_hostnames = hostname.server_hostnames(config)
    self.assertEqual(
      ["shpurdp-host"],
      server_hostnames,
      f"expected host name ['shpurdp-host']; got {server_hostnames}",
    )
  ...
  def test_server_hostnames_multiple(self):
    ...
    config = ShpurdpConfig()
    default_server_hostname = config.get("server", "hostname")
    config.set("server", "hostname", "shpurdp-host, shpurdp-host2, shpurdp-host3")
    server_hostnames = hostname.server_hostnames(config)
    self.assertEqual(len(server_hostnames), 3)
    self.assertEqual(
      ["shpurdp-host", "shpurdp-host2", "shpurdp-host3"],
      server_hostnames,
      f"expected host name ['shpurdp-host']; got {server_hostnames}",
    )
    config.set("server", "hostname", default_server_hostname)
