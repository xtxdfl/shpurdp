...
class TestClusterConfigurationCache:  # (TestCase):
  ...
  def test_cluster_configuration_cache_initialization(self, json_load_mock):
    ...
    cluster_configuration = ClusterConfigurationCache(
      os.path.join(os.sep, "tmp", "bar", "baz")
    )
  ...
  def test_cluster_configuration_update(self, json_dump_mock):
    ...
    osopen_mock, osfdopen_mock = self.__update_cluster_configuration(
      cluster_configuration, configuration
    )

    json_dump_mock.assert_called_with(
      {"0": {"foo-site": {"baz": "rendered-baz", "bar": "rendered-bar"}}}, ANY, indent=2
    )
    pass
