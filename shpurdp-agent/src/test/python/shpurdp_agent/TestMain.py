...
import tempfile
import configparser
import shpurdp_agent.hostname as hostname
import resource
...
with patch.object(
  ...
  from shpurdp_agent import NetUtil, security
  ...
  import shpurdp_agent.HeartbeatHandlers as HeartbeatHandlers
  from shpurdp_commons.os_check import OSConst, OSCheck
...
class TestMain:  # (unittest.TestCase):
  ...
  @patch("shpurdp_agent.HeartbeatHandlers.HeartbeatStopHandlersLinux")
  @patch("sys.exit")
  @patch("os.getpid")
  ...
  def test_setup_logging(
    ...
    # Testing silent mode
    main.setup_logging(
      logging.getLogger(), "/var/log/shpurdp-agent/shpurdp-agent.log", 20
    )
    ...
    main.setup_logging(
      logging.getLogger(), "/var/log/shpurdp-agent/shpurdp-agent.log", 10
    )
    self.assertTrue(addHandler_mock.called)
  ...
  def test_resolve_shpurdp_config(self, read_mock, exists_mock, platform_mock):
    platform_mock.return_value = "Linux"
    # Trying case if conf file exists
    exists_mock.return_value = True
    main.resolve_shpurdp_config()
  ...
  @patch.object(OSCheck, "os_distribution", new=MagicMock(return_value=os_distro_value))
  @patch("shpurdp_commons.shell.shellRunnerLinux.run")
  @patch("sys.exit")
  ...
  def test_perform_prestart_checks(
    ...
    main.config = ShpurdpConfig().getConfig()
  ...
  def test_daemonize_and_stop(self, exists_mock, sleep_mock):
    from shpurdp_commons.shell import shellRunnerLinux
    ...
    with patch("shpurdp_commons.shell.shellRunnerLinux.run") as kill_mock:
      ...
      kill_mock.assert_has_calls(
        [
          call(["shpurdp-sudo.sh", "kill", "-15", pid]),
          call(["shpurdp-sudo.sh", "kill", "-0", pid]),
        ]
      )
      ...
      kill_mock.assert_has_calls(
        [
          call(["shpurdp-sudo.sh", "kill", "-15", pid]),
          call(["shpurdp-sudo.sh", "kill", "-0", pid]),
          call(["shpurdp-sudo.sh", "kill", "-9", pid]),
        ]
      )
  ...
  def init_shpurdp_config_mock(self):
  ...
  @patch.object(ShpurdpConfig, "getConfigFile")
  ...
  def test_main(
    ...
    shpurdp_config_mock.return_value = self.init_shpurdp_config_mock()
