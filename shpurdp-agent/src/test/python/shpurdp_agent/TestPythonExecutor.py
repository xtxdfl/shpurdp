...
class TestPythonExecutor(TestCase):
  ...
  def test_watchdog_1(self, kill_process_with_children_mock):
    ...
    executor = PythonExecutor("/tmp", ShpurdpConfig())
  ...
  def test_back_up_log_file_if_exists(self, rename_mock, isfile_mock):
    ...
    log_file = "/var/lib/shpurdp-agent/data/output-13.txt"
    executor = PythonExecutor("/tmp", ShpurdpConfig())
    ...
    self.assertEqual(isfile_mock.called, True)
    self.assertEqual(
      rename_mock.call_args_list[0][0][0], "/var/lib/shpurdp-agent/data/output-13.txt"
    )
    self.assertEqual(
      rename_mock.call_args_list[0][0][1], "/var/lib/shpurdp-agent/data/output-13.txt.2"
    )
    pass
