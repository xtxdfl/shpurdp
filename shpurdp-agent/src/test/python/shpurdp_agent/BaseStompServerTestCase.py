...
class BaseStompServerTestCase(unittest.TestCase):
  ...
  def init_stdout_logger(self):
    ...
    logging.getLogger("shpurdp_agent.apscheduler").setLevel(logging.WARN)
    logging.getLogger("apscheduler").setLevel(logging.WARN)
    logging.getLogger("shpurdp_agent.alerts").setLevel(logging.WARN)
    logging.getLogger("alerts").setLevel(logging.WARN)
    logging.getLogger("shpurdp_agent.AlertSchedulerHandler").setLevel(logging.WARN)
...
class TestCaseTcpConnection(shpurdp_stomp.Connection):
  def __init__(self, url):
    self.lock = threading.RLock()
    self.correlation_id = -1
    shpurdp_stomp.Connection.__init__(self, host_and_ports=[("127.0.0.1", 21613)])
  ...
  def send(
    ...
    logger.info(
      f"Event to server at {destination} (correlation_id={self.correlation_id}): {message}"
    )

    body = json.dumps(message)
    shpurdp_stomp.Connection.send(
      self,
      destination,
      body,
...
security.ShpurdpStompConnection = TestCaseTcpConnection
