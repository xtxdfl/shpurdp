...
from shpurdp_agent.FileCache import FileCache, CachingException
...
class TestFileCache(TestCase):
  def setUp(self):
    ...
    self.config.set("agent", "cache_dir", "/var/lib/shpurdp-agent/cache")
  ...
  def test_get_service_base_dir(self, provide_directory_mock):
    ...
    command = {
      ...
      "shpurdpLevelParams": {"jdk_location": "server_url_pref"},
    }
  ...
  def test_get_hook_base_dir(self, provide_directory_mock):
    ...
    self.assertEqual(
      pprint.pformat(provide_directory_mock.call_args_list[0][0]),
      "('/var/lib/shpurdp-agent/cache', " "{0}, " "'server_url_pref')".format(
  ...
  def test_get_custom_actions_base_dir(self, provide_directory_mock):
    ...
    self.assertEqual(
      pprint.pformat(provide_directory_mock.call_args_list[0][0]),
      "('/var/lib/shpurdp-agent/cache', 'custom_actions', 'server_url_pref')",
    )
    self.assertEqual(res, "dummy value")
  ...
  def test_get_custom_resources_subdir(self, provide_directory_mock):
    ...
    self.assertEqual(
      pprint.pformat(provide_directory_mock.call_args_list[0][0]),
      "('/var/lib/shpurdp-agent/cache', 'dashboards', 'server_url_pref')",
    )
    self.assertEqual(res, "dummy value")
  ...
  def test_unpack_archive(self):
    ...
    dummy_archive_name = os.path.join(
      "shpurdp_agent", "dummy_files", "dummy_archive.zip"
    )
