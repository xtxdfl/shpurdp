...
class TestFileResource(TestCase):
  ...
  def test_action_create_dir_exist(self, isdir_mock, dirname_mock):
    ...
    try:
      with Environment("/") as env:
        File("/existent_directory", action="create", mode=0o777, content="file-content")
  ...
  @patch("resource_management.core.sudo.path_isdir")
  def test_action_create_non_existent_file(
    self, isdir_mock, exists_mock, create_file_mock, read_file_mock, ensure_mock
  ):
    """
  ...
  @patch("resource_management.core.sudo.path_exists")
  @patch("resource_management.core.sudo.path_isdir")
  def test_action_create_replace(
    self, isdir_mock, exists_mock, create_file_mock, read_file_mock, ensure_mock
  ):
    """
    ...
    create_file_mock.assert_called_with(
      "/directory/file", "new-content", encoding=None, on_file_created=ANY
    )
  ...
  def test_ensure_metadata(
    self, stat_mock, chmod_mock, chown_mock, getgrnam_mock, getpwnam_mock
  ):
    """
  ...
  @patch("resource_management.core.sudo.path_exists")
  @patch("resource_management.core.sudo.path_isdir")
  def test_action_create_encoding(
    self,
    isdir_mock,
    exists_mock,
    create_file_mock,
