...
class TestRepositoryResource(TestCase):
  ...
  def test_create_repo_redhat(
    ...
    with Environment("/") as env:
      with patch.object(
        repository, "__file__", new="/shpurdp/test/repo/dummy/path/file"
      ):
        Repository(
          "hadoop",
  ...
  @patch(
    "shpurdp_commons.os_utils.current_user", new=MagicMock(return_value="shpurdp-agent")
  )
  def test_create_repo_ubuntu_repo_exists(
    ...
    self.assertEqual(
      copy_item0,
      "call('/tmp/1.txt', content=StaticFile('/etc/apt/sources.list.d/HDP.list'), owner='shpurdp-agent')",
    )
    self.assertEqual(
      copy_item1,
