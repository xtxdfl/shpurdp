...
class TestDirectoryResource(TestCase):
  ...
  def test_create_directory_recursive(
    ...
    os_chown_mock.assert_any_call(
      "/a/b/c/d", getpwnam_mock.return_value, getgrnam_mock.return_value
    )
  ...
  @patch("pwd.getpwnam")
  @patch("grp.getgrnam")
  def test_create_directory_not_recursive(
    self,
    getgrnam_mock,
    getpwnam_mock,
    os_chown_mock,
    ...
    with Environment("/") as env:
      Directory("/a/b/c/d", action="create", mode=0o777, owner="hdfs", group="hadoop")
  ...
  @patch.object(os.path, "dirname")
  @patch("resource_management.core.sudo.path_isdir")
  def test_create_directory_failed_no_parent(
    self, isdir_mock, os_dirname_mock, os_path_exists_mock
  ):
    os_path_exists_mock.return_value = False
    os_dirname_mock.return_value = "/a/b/c"
    ...
    try:
      ...
      self.assertEqual(
        "Applying Directory['/a/b/c/d'] failed, parent directory /a/b/c doesn't exist",
        str(e),
      )
  ...
  def test_create_directory_path_is_file_or_line(self, isdir_mock, os_path_exists_mock):
    ...
    try:
      ...
      self.assertEqual(
        "Applying Directory['/a/b/c/d'] failed, file /a/b/c/d already exists", str(e)
      )
  ...
  @patch("resource_management.core.sudo.path_exists")
  @patch("resource_management.core.sudo.path_isdir")
  def test_delete_directory_with_path_to_file(
    self, isdir_mock, os_path_exists_mock, rmtree_mock
  ):
    os_path_exists_mock.return_value = True
    isdir_mock.return_value = False
    ...
    try:
      ...
      self.assertEqual(
        "Applying Directory['/a/b/c/d'] failed, /a/b/c/d is not a directory", str(e)
      )
