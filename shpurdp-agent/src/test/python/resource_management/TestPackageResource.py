...
from shpurdp_commons import shell as ac_shell
from shpurdp_commons.repo_manager.apt_manager import replace_underscores
...
class TestPackageResource(TestCase):
  ...
  def test_action_install_pattern_installed_rhel(self, shell_mock):
    ...
    sys.modules["yum"].YumBase.return_value.rpmdb.simplePkgList.return_value = [
      (b"some_package_1_2_3",)
    ]
    with Environment("/") as env:
      Package("some_package*", logoutput=False)
  ...
  def test_action_install_pattern_suse(self, shell_mock, call_mock):
    ...
    call_mock.assert_has_calls(
      [
        call("installed_pkgs=`rpm -qa 'some_package*'` ; [ ! -z \"$installed_pkgs\" ]"),
        call(
