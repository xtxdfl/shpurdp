...
from shpurdp_agent import Hardware
...
class HostInfoLinux(HostInfo):
  ...
  DEFAULT_PROJECT_NAMES = [
    ...
    "ranger",
    "accumulo",
    "shpurdp-metrics-collector",
    "shpurdp-metrics-monitor",
    "atlas",
    "zeppelin",
  ]
  ...
  def checkUnlimitedJce(self):
    if not self.config or not self.config.has_option(
      ShpurdpConfig.SHPURDP_PROPERTIES_CATEGORY, "java.home"
    ):
      return None
    try:
      jcePolicyInfo = JcePolicyInfo(
        self.config.get(ShpurdpConfig.SHPURDP_PROPERTIES_CATEGORY, "java.home")
      )
...
class HostInfoWindows(HostInfo):
  ...
  def createAlerts(self, alerts):
    # TODO SHPURDP-7849 Implement createAlerts for Windows
    return alerts

  def javaProcs(self, list):
    try:
      from shpurdp_commons.os_windows import run_powershell_script
