...
class DataCleaner(threading.Thread):
  ...
  def __init__(self, config):
    ...
    self.cleanup_interval = (
      int(self.cleanup_interval) if self.cleanup_interval else None
    )
    if (
    ...
    self.cleanup_max_size_MB = (
      int(self.cleanup_max_size_MB) if self.cleanup_max_size_MB else None
    )
    if (
  ...
  def cleanup(self):
    ...
    if len(file_path_to_timestamp) and total_size_bytes > target_size_bytes:
      logger.info(
        "DataCleaner values need to be more aggressive. Current size in bytes for all log files is %d, "
        "and will try to clean to reach %d bytes."
        % (total_size_bytes, target_size_bytes)
      )
      # Prune oldest files first
      count = 0
      file_path_oldest_first_list = sorted(
        file_path_to_timestamp, key=file_path_to_timestamp.get, reverse=True
      )
...
def main():
  data_cleaner = DataCleaner(ShpurdpConfig.config)
  data_cleaner.start()
  data_cleaner.join()
