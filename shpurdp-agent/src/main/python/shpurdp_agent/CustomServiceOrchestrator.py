...
from shpurdp_commons import shell
...
from shpurdp_agent.models.commands import AgentCommand
from shpurdp_agent.Utils import Utils

from shpurdp_agent.AgentException import AgentException
from shpurdp_agent.PythonExecutor import PythonExecutor
import subprocess
...
class CustomServiceOrchestrator(object):
  ...
  SHPURDP_SERVER_HOST = "shpurdp_server_host"
  SHPURDP_SERVER_PORT = "shpurdp_server_port"
  SHPURDP_SERVER_USE_SSL = "shpurdp_server_use_ssl"
  ...
  DEFAULT_CREDENTIAL_SHELL_LIB_PATH = "/var/lib/shpurdp-agent/cred/lib"
  DEFAULT_CREDENTIAL_CONF_DIR = "/var/lib/shpurdp-agent/cred/conf"
  ...
  def generateJceks(self, commandJson):
    ...
    java_home = commandJson["shpurdpLevelParams"]["java_home"]
  ...
  def runCommand(
    ...
    try:
      ...
      """:type hooks shpurdp_agent.CommandHooksOrchestrator.ResolvedHooks"""
  ...
  def decompress_cluster_host_info(self, cluster_host_info):
    ...
    shpurdp_server_host = info.pop(self.SHPURDP_SERVER_HOST)
    shpurdp_server_port = info.pop(self.SHPURDP_SERVER_PORT)
    shpurdp_server_use_ssl = info.pop(self.SHPURDP_SERVER_USE_SSL)
    ...
    decompressed_map[self.SHPURDP_SERVER_HOST] = shpurdp_server_host
    decompressed_map[self.SHPURDP_SERVER_PORT] = shpurdp_server_port
    decompressed_map[self.SHPURDP_SERVER_USE_SSL] = shpurdp_server_use_ssl
