...
from shpurdp_agent.Facter import Facter
...
class Hardware:
  ...
  def _check_remote_mounts(self):
    ...
    if (
      self.config
      and self.config.has_option(
        ShpurdpConfig.SHPURDP_PROPERTIES_CATEGORY, Hardware.CHECK_REMOTE_MOUNTS_KEY
      )
      and self.config.get(
        ShpurdpConfig.SHPURDP_PROPERTIES_CATEGORY, Hardware.CHECK_REMOTE_MOUNTS_KEY
      ).lower()
