...
import threading
from shpurdp_stomp.adapter.websocket import WsConnection
...
class VerifiedHTTPSConnection:
  ...
  def connect(self):
    ...
    if not self.two_way_ssl_required:
      conn = ShpurdpStompConnection(self.connection_url)
      ...
      conn = ShpurdpStompConnection(self.connection_url, ssl_options=ssl_options)
      ...
      try:
        ...
        logger.error(
          ...
          "server configuration(shpurdp.properties) "
          "\nExiting.."
        )
        raise
      return conn
...
class ShpurdpStompConnection(WsConnection):
  ...
  def reqSignCrt(self):
    ...
    if result == "OK":
      ...
      logger.error(
        ...
        "authentication in server configuration(shpurdp.properties) "
        "\nExiting.."
      )
      raise ssl.SSLError
