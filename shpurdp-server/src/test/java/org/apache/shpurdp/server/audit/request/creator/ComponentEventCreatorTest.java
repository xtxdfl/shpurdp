...
import org.apache.shpurdp.server.audit.event.request.StartOperationRequestAuditEvent;
import org.apache.shpurdp.server.audit.request.eventcreator.ComponentEventCreator;
...
public class ComponentEventCreatorTest extends AuditEventCreatorTestBase {
  ...
  public void deleteTest() {
    ...
    properties.put(HostComponentResourceProvider.HOST_NAME, "shpurdp1.example.com");
    ...
    String expected = "User(" + userName + "), RemoteIp(1.2.3.4), Operation(Delete component MyComponent), Host name(shpurdp1.example.com), RequestId(1), Status(Successfully queued)";
  ...
  private void allHostsTest(Request.Type type) {
    ...
    String actual = event.getAuditMessage();
    String expected = "User(" + userName + "), RemoteIp(1.2.3.4), Operation(STARTED: all services on all hosts that matches hostname.in(a,b,c) (mycluster)), Host name(shpurdp1.example.com), RequestId(1), Status(Successfully queued)";
  ...
  public void hostTest() {
    ...
    request.getBody().addRequestInfoProperty(RequestOperationLevel.OPERATION_HOST_NAME, "shpurdp1.example.com");
    request.getBody().addRequestInfoProperty(RequestOperationLevel.OPERATION_CLUSTER_ID, "mycluster");
    ...
    String actual = event.getAuditMessage();
    String expected = "User(" + userName + "), RemoteIp(1.2.3.4), Operation(STARTED: MYCOMPONENT,MYCOMPONENT2 on shpurdp1.example.com (mycluster)), Host name(shpurdp1.example.com), RequestId(1), Status(Successfully queued)";
  ...
  public void hostComponentTest() {
    ...
    String actual = event.getAuditMessage();
    String expected = "User(" + userName + "), RemoteIp(1.2.3.4), Operation(STARTED: MYCOMPONENT/MYSERVICE on shpurdp1.example.com (mycluster)), Host name(shpurdp1.example.com), RequestId(1), Status(Successfully queued)";
  ...
  public void maintenanceModeTest() {
    ...
    String actual = event.getAuditMessage();
    String expected = "User(" + userName + "), RemoteIp(1.2.3.4), Operation(Turn ON Maintenance Mode for MYCOMPONENT), Host name(shpurdp1.example.com), RequestId(1), Status(Successfully queued)";
  ...
  public void failureTest() {
    ...
    String actual = event.getAuditMessage();
    String expected = "User(" + userName + "), RemoteIp(1.2.3.4), Operation(Turn ON Maintenance Mode for MYCOMPONENT), Host name(shpurdp1.example.com), RequestId(1), Status(Failed to queue), Reason(Failed for testing)";
