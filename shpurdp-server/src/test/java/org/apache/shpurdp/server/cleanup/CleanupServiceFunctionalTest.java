...
package org.apache.shpurdp.server.cleanup;
...
public class CleanupServiceFunctionalTest {
  ...
  public static void beforeClass() throws Exception {
    ...
    injector.getInstance(ShpurdpJpaPersistService.class).start();

  }

  private static Module getTestPersistModule() {
    ShpurdpJpaPersistModule persistModule = new ShpurdpJpaPersistModule("shpurdp-server");
    ...
    //persistenceProperties.setProperty(PersistenceUnitProperties.SCHEMA_GENERATION_CREATE_SCRIPT_SOURCE, "Shpurdp-DDL-Postgres-CREATE.sql");
    ...
    persistenceProperties.setProperty(PersistenceUnitProperties.JDBC_USER, "shpurdp");
  ...
  public static void afterClass() {
    injector.getInstance(ShpurdpJpaPersistService.class).stop();
  }

  /**
   * Override JPA config values read from the shpurdp.properties
   *
   * @return
   */
  private static Properties getTestProperties() {
    ...
    properties.put("server.jdbc.user.name", "shpurdp");
    //properties.put("server.jdbc.user.passwd", "bigdata");
    properties.put("server.jdbc.url", "jdbc:postgresql://192.168.59.103:5432/shpurdp");
    properties.put(Configuration.SHARED_RESOURCES_DIR.getKey(), "/Users/lpuskas/prj/shpurdp/shpurdp-server/src/test/resources");
