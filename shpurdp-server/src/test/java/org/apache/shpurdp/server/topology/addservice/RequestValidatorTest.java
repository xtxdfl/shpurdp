 ...
 */
package org.apache.shpurdp.server.topology.addservice;
...
import org.apache.shpurdp.server.topology.SecurityConfiguration;
import org.apache.shpurdp.server.topology.SecurityConfigurationFactory;
import org.apache.shpurdp.server.topology.StackFactory;
...
public class RequestValidatorTest extends EasyMockSupport {
  ...
  private final ShpurdpManagementController controller = createNiceMock(ShpurdpManagementController.class);
  ...
  public void reportsUnknownStackFromRequest() throws Exception {
    ...
    expect(stackFactory.createStack(requestStackId.getStackName(), requestStackId.getStackVersion(), controller)).andThrow(new ShpurdpException("Stack not found"));
    replayAll();
  ...
  public void reportsUnknownStackFromCluster() throws Exception {
    ...
    expect(stackFactory.createStack(clusterStackId.getStackName(), clusterStackId.getStackVersion(), controller)).andThrow(new ShpurdpException("Stack not found"));
    replayAll();
  ...
  public void acceptsKnownComponents() {
    ...
    Map<String, Map<String, Set<String>>> expectedNewServices = ImmutableMap.of(
      "KAFKA", ImmutableMap.of("KAFKA_BROKER", ImmutableSet.of("c7401.shpurdp.apache.org"))
    );
    assertEquals(expectedNewServices, validator.getState().getNewServices());
  }
  ...
  public void acceptsKnownHosts() {
    Set<String> requestHosts = ImmutableSet.of("c7401.shpurdp.apache.org", "c7402.shpurdp.apache.org");
    Set<String> otherHosts = ImmutableSet.of("c7403.shpurdp.apache.org", "c7404.shpurdp.apache.org");
  ...
  public void rejectsUnknownHosts() {
    Set<String> clusterHosts = ImmutableSet.of("c7401.shpurdp.apache.org", "c7402.shpurdp.apache.org");
    Set<String> otherHosts = ImmutableSet.of("c7403.shpurdp.apache.org", "c7404.shpurdp.apache.org");
  ...
  @Test
  public void combinesRequestConfigWithClusterAndStack() throws ShpurdpException {
  ...
  private void requestServices(boolean validated, String... services) {
    expect(request.getServices()).andReturn(
      Arrays.stream(services)
        .map(org.apache.shpurdp.server.topology.addservice.Service::of)
        .collect(toSet())
    ).anyTimes();
    if (validated) {
  ...
  private void requestComponents(String... components) {
    expect(request.getComponents()).andReturn(
      Arrays.stream(components)
        .map(componentName -> Component.of(componentName, "c7401.shpurdp.apache.org"))
        .collect(toSet())
    );
  }
