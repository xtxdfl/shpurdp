...
public class AlertScriptDispatcherTest {
  ...
  public void testGetScriptLocation() throws Exception {
    ...
    //1.shpurdp.dispatch-property.script.filename is not set in notification
    Assert.assertEquals(dispatcher.getScriptLocation(notification),null);

    //2.shpurdp.dispatch-property.script.filename is set in notification,but notification.dispatch.alert.script.directory not in shpurdp.properties
    ...
    Assert.assertEquals(dispatcher.getScriptLocation(notification),"/var/lib/shpurdp-server/resources/scripts/foo.py");

    //3.both properties are set
    final String scriptDirectory = "/var/lib/shpurdp-server/resources/scripts/foo";
    m_configuration.setProperty(DISPATCH_PROPERTY_SCRIPT_DIRECTORY_KEY,scriptDirectory);
    Assert.assertEquals(dispatcher.getScriptLocation(notification),"/var/lib/shpurdp-server/resources/scripts/foo/foo.py");
  }
