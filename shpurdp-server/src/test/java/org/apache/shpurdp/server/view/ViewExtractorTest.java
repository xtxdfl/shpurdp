...
public class ViewExtractorTest extends EasyMockSupport {
  ...
  public void testExtractViewArchive() throws Exception {
    ...
    if (System.getProperty("os.name").contains("Windows")) {
      ...
      expect(metaInfManifest.getAbsolutePath()).andReturn("\\var\\lib\\shpurdp-server\\resources\\views\\work\\MY_VIEW{1.0.0}\\META-INF\\MANIFEST.MF").anyTimes();
  ...
  public void testEnsureExtractedArchiveDirectory() throws Exception {
    ...
    if (System.getProperty("os.name").contains("Windows")) {
      Assert.assertTrue(viewExtractor.ensureExtractedArchiveDirectory("\\var\\lib\\shpurdp-server\\resources\\views\\work"));
    }
    else {
      Assert.assertTrue(viewExtractor.ensureExtractedArchiveDirectory("/var/lib/shpurdp-server/resources/views/work"));
    }
    ...
    if (System.getProperty("os.name").contains("Windows")) {
      Assert.assertFalse(viewExtractor.ensureExtractedArchiveDirectory("\\var\\lib\\shpurdp-server\\resources\\views\\work"));
    }
    else {
      Assert.assertFalse(viewExtractor.ensureExtractedArchiveDirectory("/var/lib/shpurdp-server/resources/views/work"));
    }
  ...
  private ViewExtractor getViewExtractor(ViewEntity viewDefinition) throws Exception {
    ...
    if (System.getProperty("os.name").contains("Windows")) {
      ...
      files.put("\\var\\lib\\shpurdp-server\\resources\\views\\work\\MY_VIEW{1.0.0}\\META-INF\\MANIFEST.MF", metaInfManifest);
