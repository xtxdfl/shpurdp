 ...
 */
package org.apache.shpurdp.server.stack.upgrade.orchestrate;
...
import org.apache.shpurdp.server.controller.KerberosDetails;
...
import org.apache.shpurdp.server.stack.HostsType;
import org.apache.shpurdp.server.stack.MasterHostResolver;
import org.apache.shpurdp.server.stack.StackManagerMock;
import org.apache.shpurdp.server.stack.upgrade.ConfigUpgradeChangeDefinition;
import org.apache.shpurdp.server.stack.upgrade.ConfigUpgradeChangeDefinition.ConfigurationKeyValue;
import org.apache.shpurdp.server.stack.upgrade.ConfigUpgradePack;
import org.apache.shpurdp.server.stack.upgrade.ConfigureTask;
import org.apache.shpurdp.server.stack.upgrade.Direction;
import org.apache.shpurdp.server.stack.upgrade.ExecuteTask;
import org.apache.shpurdp.server.stack.upgrade.Grouping;
import org.apache.shpurdp.server.stack.upgrade.HostOrderGrouping;
import org.apache.shpurdp.server.stack.upgrade.HostOrderItem;
import org.apache.shpurdp.server.stack.upgrade.HostOrderItem.HostOrderActionType;
import org.apache.shpurdp.server.stack.upgrade.ManualTask;
import org.apache.shpurdp.server.stack.upgrade.SecurityCondition;
import org.apache.shpurdp.server.stack.upgrade.ServiceCheckGrouping;
import org.apache.shpurdp.server.stack.upgrade.StopGrouping;
import org.apache.shpurdp.server.stack.upgrade.Task;
import org.apache.shpurdp.server.stack.upgrade.UpgradePack;
import org.apache.shpurdp.server.stack.upgrade.UpgradeScope;
...
import org.apache.shpurdp.server.state.UpgradeState;
...
import org.apache.shpurdp.spi.upgrade.OrchestrationOptions;
...
public class UpgradeHelperTest extends EasyMockSupport {
  ...
  private ShpurdpManagementController m_managementController;
  ...
  public void before() throws Exception {
    ...
    m_managementController = injector.getInstance(ShpurdpManagementController.class);
  ...
  public void testUpgradeOrchestration() throws Exception {
    Map<String, UpgradePack> upgrades = shpurdpMetaInfo.getUpgradePacks("foo", "bar");
    assertTrue(upgrades.isEmpty());

    upgrades = shpurdpMetaInfo.getUpgradePacks("HDP", "2.1.1");
    ...
    // upgrade
    ServiceInfo si = shpurdpMetaInfo.getService("HDP", "2.2.0", "ZOOKEEPER");
  ...
  public void testUpgradeServerActionOrchestration() throws Exception {
    Map<String, UpgradePack> upgrades = shpurdpMetaInfo.getUpgradePacks("HDP", "2.1.1");

    ServiceInfo si = shpurdpMetaInfo.getService("HDP", "2.1.1", "ZOOKEEPER");
    si.setDisplayName("Zk");
    ComponentInfo ci = si.getComponentByName("ZOOKEEPER_SERVER");
    ci.setDisplayName("ZooKeeper1 Server2");
  ...
  public void testConditionalDeleteTask() throws Exception {
    ...
    assertTrue(upgrades.containsKey("upgrade_test"));
    UpgradePack upgrade = upgrades.get("upgrade_test");
    ConfigUpgradePack cup = shpurdpMetaInfo.getConfigUpgradePack("HDP", "2.1.1");
    assertNotNull(upgrade);
  ...
  public void testServiceCheckUpgradeStages() throws Exception {
    Map<String, UpgradePack> upgrades = shpurdpMetaInfo.getUpgradePacks("HDP", "2.2.0");
    ...
    Set<String> additionalServices = new HashSet<String>() {{ add("HBASE"); add("PIG"); add("TEZ"); add("SHPURDP_METRICS"); }};
    ...
    for(Service service : services) {
      ServiceInfo si = shpurdpMetaInfo.getService(c.getCurrentStackVersion().getStackName(),
      ...
      if (service.getName().equalsIgnoreCase("SHPURDP_METRICS")) {
        continue;
      }
  ...
  private Cluster makeCluster() throws ShpurdpException, AuthorizationException {
   ...
   */
  private Cluster makeCluster(boolean clean) throws ShpurdpException, AuthorizationException {
  ...
  public void testUpgradeOrchestrationWithRack() throws Exception {
    ...
    assertTrue(upgrades.isEmpty());
    upgrades = shpurdpMetaInfo.getUpgradePacks("HDP", "2.1.1");
    ServiceInfo serviceInfo = shpurdpMetaInfo.getService("HDP", "2.2.0", "ZOOKEEPER");
  ...
  public void testMergeConfigurations() throws Exception {
    ...
    expect(configHelper.createConfigTypes(EasyMock.anyObject(Cluster.class),
        EasyMock.anyObject(StackId.class), EasyMock.anyObject(ShpurdpManagementController.class),
    ...
    Map<String, UpgradePack> upgradePacks = shpurdpMetaInfo.getUpgradePacks("HDP", "2.1.1");
    ...
    expect(context.getShpurdpMetaInfo()).andReturn(shpurdpMetaInfo).anyTimes();
  ...
  public void testMergeConfigurationsWithClusterEnv() throws Exception {
    ...
    expect(m_configHelper.createConfigTypes(
            ...
            EasyMock.anyObject(ShpurdpManagementController.class),
  ...
  /**
   * Extend {@link org.apache.shpurdp.server.stack.MasterHostResolver} in order
