...
public class ShpurdpConfigurationDAOTest extends EasyMockSupport {
  ...
  public static void beforeKDCKerberosOperationHandlerTest() throws Exception {
    methodMerge = ShpurdpConfigurationDAO.class.getMethod("merge", ShpurdpConfigurationEntity.class);
    methodRemove = CrudDAO.class.getMethod("remove", Object.class);
    methodCreate = ShpurdpConfigurationDAO.class.getMethod("create", ShpurdpConfigurationEntity.class);
    methodFindByCategory = ShpurdpConfigurationDAO.class.getMethod("findByCategory", String.class);
  ...
  public void testReconcileCategoryNewCategory() throws Exception {
    Capture<ShpurdpConfigurationEntity> capturedEntities = newCapture(CaptureType.ALL);

    ShpurdpConfigurationDAO dao = createDao();
  ...
  public void testReconcileCategoryReplaceCategory() throws Exception {
    ...
    Capture<ShpurdpConfigurationEntity> capturedCreatedEntities = newCapture(CaptureType.ALL);
    Capture<ShpurdpConfigurationEntity> capturedRemovedEntities = newCapture(CaptureType.ALL);
  ...
  public void testReconcileCategoryUpdateCategoryKeepNotSpecified() throws Exception {
    ...
    Capture<ShpurdpConfigurationEntity> capturedCreatedEntities = newCapture(CaptureType.ALL);
    Capture<ShpurdpConfigurationEntity> capturedMergedEntities = newCapture(CaptureType.ALL);

    ShpurdpConfigurationDAO dao = createDao();

    expect(dao.findByCategory(CATEGORY_NAME)).andReturn(toEntities(CATEGORY_NAME, existingProperties)).once();

    expect(dao.merge(capture(capturedMergedEntities))).andReturn(createNiceMock(ShpurdpConfigurationEntity.class)).anyTimes();
  ...
  private ShpurdpConfigurationDAO createDao() throws IllegalAccessException {
    ShpurdpConfigurationDAO dao = createMockBuilder(ShpurdpConfigurationDAO.class)
  ...
  private List<ShpurdpConfigurationEntity> toEntities(String categoryName, Map<String, String> properties) {
  ...
  private void validateCapturedEntities(String expectedCategoryName, Map<String, String> expectedProperties, Capture<ShpurdpConfigurationEntity> capturedEntities) {
    Assert.assertTrue(capturedEntities.hasCaptured());

    List<ShpurdpConfigurationEntity> entities = capturedEntities.getValues();
