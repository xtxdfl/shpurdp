...
public class ShpurdpViewErrorHandlerProxyTest {

  final ShpurdpErrorHandler shpurdpErrorHandler = createNiceMock(ShpurdpErrorHandler.class);
  ...
  public void testHandleInternalServerError() throws Throwable {
    ...
    shpurdpErrorHandler.handle(target, request, httpServletRequest, httpServletResponse);
    expectLastCall();

    replay(shpurdpErrorHandler, errorHandler, httpServletRequest, httpServletResponse, th);

    //when
    ShpurdpViewErrorHandlerProxy proxy = new ShpurdpViewErrorHandlerProxy(errorHandler, shpurdpErrorHandler);
    proxy.handle(target, request, httpServletRequest, httpServletResponse);

    //then
    verify(shpurdpErrorHandler, errorHandler, httpServletRequest, httpServletResponse, th);
  }
  ...
  public void testShowStacks() {
    ...
    Capture<Boolean> captureShowStacksShpurdpErrorHandler = EasyMock.newCapture();
    shpurdpErrorHandler.setShowStacks(captureBoolean(captureShowStacksShpurdpErrorHandler));
    expectLastCall();

    replay(errorHandler, shpurdpErrorHandler);
    ...
    assertTrue(captureShowStacksShpurdpErrorHandler.getValue());

    verify(errorHandler, shpurdpErrorHandler);
