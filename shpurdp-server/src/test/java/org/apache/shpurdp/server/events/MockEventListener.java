...
public class MockEventListener {
   ...
   * added to the list.
   */
  private final Map<Class<?>, List<ShpurdpEvent>> m_receivedShpurdpEvents = new HashMap<>();
   ...
   * @return
   */
  public boolean isShpurdpEventReceived(Class<? extends ShpurdpEvent> clazz) {
    if (!m_receivedShpurdpEvents.containsKey(clazz)) {
      return false;
    }

    return m_receivedShpurdpEvents.get(clazz).size() > 0;
  }
   ...
   * @return
   */
  public int getShpurdpEventReceivedCount(Class<? extends ShpurdpEvent> clazz) {
    if (!m_receivedShpurdpEvents.containsKey(clazz)) {
      return 0;
    }

    return m_receivedShpurdpEvents.get(clazz).size();
  }
   ...
   * @return
   */
  public List<ShpurdpEvent> getShpurdpEventInstances(Class<? extends ShpurdpEvent> clazz){
    if (!m_receivedShpurdpEvents.containsKey(clazz)) {
      return Collections.emptyList();
    }

    return m_receivedShpurdpEvents.get(clazz);
  }
   ...
   */
  @Subscribe
  public void onShpurdpEvent(ShpurdpEvent event) {
    List<ShpurdpEvent> events = m_receivedShpurdpEvents.get(event.getClass());
    if (null == events) {
      events = new ArrayList<>();
      m_receivedShpurdpEvents.put(event.getClass(), events);
    }
