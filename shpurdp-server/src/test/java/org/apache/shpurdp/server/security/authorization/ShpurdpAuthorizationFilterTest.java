...
public class ShpurdpAuthorizationFilterTest {
  @After
  ...
  public void testDoFilter_adminAccess() throws Exception {
    ...
    urlTests.put("/api/v1/clusters/cluster/", "GET",  true);  // This should probably be an invalid URL, but Shpurdp seems to allow it.
    urlTests.put("/api/v1/clusters/cluster/", "POST",  true); // This should probably be an invalid URL, but Shpurdp seems to allow it.
    ...
    urlTests.put("/api/v1/clusters/c1/credentials/shpurdp.credential", "POST", true);
    urlTests.put("/api/v1/clusters/c1/credentials/shpurdp.credential", "PUT", true);
    urlTests.put("/api/v1/clusters/c1/credentials/shpurdp.credential", "GET", true);
    urlTests.put("/api/v1/clusters/c1/credentials/shpurdp.credential", "DELETE", true);
  ...
  public void testDoFilter_NotLoggedIn_UseDefaultUser() throws Exception {
    ...
    expect(users.getUserAuthorities("user1")).andReturn(Collections.<ShpurdpGrantedAuthority>emptyList()).once();
    ...
    Injector injector = Guice.createInjector(new AbstractModule() {
      ...
      protected void configure() {
        ...
        bind(ShpurdpLdapConfigurationProvider.class).toInstance(EasyMock.createMock(ShpurdpLdapConfigurationProvider.class));
      }
    });

    ShpurdpAuthorizationFilter filter = new ShpurdpAuthorizationFilter(createNiceMock(ShpurdpEntryPoint.class), injector.getInstance(Configuration.class),
  ...
  private void performGeneralDoFilterTest(Authentication authentication, Table<String, String, Boolean> urlTests, boolean expectRedirect) throws Exception {
    ...
    final ShpurdpAuthorizationFilter filter = createMockBuilder(ShpurdpAuthorizationFilter.class)
        .addMockedMethod("getSecurityContext")
        .addMockedMethod("getViewRegistry")
        .withConstructor(createNiceMock(ShpurdpEntryPoint.class),
            configuration,
    ...
    for (final Cell<String, String, Boolean> urlTest: urlTests.cellSet()) {
      ...
      if (expectRedirect) {
        String redirectURL = ShpurdpAuthorizationFilter.LOGIN_REDIRECT_BASE + urlTest.getRowKey();
