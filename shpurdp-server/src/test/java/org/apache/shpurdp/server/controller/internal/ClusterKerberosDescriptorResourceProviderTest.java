...
import org.apache.shpurdp.server.state.kerberos.KerberosPrincipalDescriptorTest;
import org.apache.shpurdp.server.state.kerberos.KerberosServiceDescriptorTest;
...
public class ClusterKerberosDescriptorResourceProviderTest extends EasyMockSupport {
  ...
  private void testGetResourcesWithInvalidKerberosDescriptorType(Authentication authentication) throws Exception {
    ...
    ShpurdpManagementController managementController = createMock(ShpurdpManagementController.class);
    expect(managementController.getClusters()).andReturn(clusters).atLeastOnce();
    expect(managementController.getShpurdpMetaInfo()).andReturn(metaInfo).atLeastOnce();
