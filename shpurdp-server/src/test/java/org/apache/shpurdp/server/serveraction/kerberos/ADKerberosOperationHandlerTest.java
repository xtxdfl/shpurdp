...
import org.apache.shpurdp.server.security.InternalSSLSocketFactoryNonTrusting;
import org.apache.shpurdp.server.security.InternalSSLSocketFactoryTrusting;
...
public class ADKerberosOperationHandlerTest extends KerberosOperationHandlerTest {
  ...
  public void testProcessCreateTemplateDefault() throws Exception {
    ...
    handler.open(kc, DEFAULT_REALM, getKerberosEnv());
    handler.createPrincipal("nn/c6501.shpurdp.apache.org", "secret", true);
    ...
    Assert.assertEquals("nn/c6501.shpurdp.apache.org", attributes.get("cn").get());

    Assert.assertNotNull(attributes.get("servicePrincipalName"));
    Assert.assertEquals("nn/c6501.shpurdp.apache.org", attributes.get("servicePrincipalName").get());

    Assert.assertNotNull(attributes.get("userPrincipalName"));
    Assert.assertEquals("nn/c6501.shpurdp.apache.org@HDP01.LOCAL", attributes.get("userPrincipalName").get());
  ...
  public void testLive() throws Throwable {
    ...
    System.out.println("Principal exists: " + handler.principalExists("nn/c1508.shpurdp.apache.org", true));
    ...
    evaluatedPrincipal = "nn/c6501.shpurdp.apache.org@" + DEFAULT_REALM;
    ...
    handler.removePrincipal("abcdefg/c1509.shpurdp.apache.org@" + DEFAULT_REALM, true);

    handler.createPrincipal("abcdefg/c1509.shpurdp.apache.org@" + DEFAULT_REALM, "some password", true);
    handler.createPrincipal("abcdefg@" + DEFAULT_REALM, "some password", false);

    //update the password
    handler.setPrincipalPassword("abcdefg/c1509.shpurdp.apache.org@" + DEFAULT_REALM, "some password", true);
