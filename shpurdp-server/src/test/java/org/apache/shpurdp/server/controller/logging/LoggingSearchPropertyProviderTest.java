...
import org.apache.shpurdp.server.state.LogDefinition;
 ...
 * HostComponent resources in Shpurdp.
 *
 */
public class LoggingSearchPropertyProviderTest {
  ...
  private void testBasicCall(Authentication authentication, boolean authorizedForLogSearch) throws Exception {
    ...
    final String expectedShpurdpURL = "http://c6401.shpurdp.apache.org:8080";
    ...
    expect(resourceMock.getPropertyValue(PropertyHelper.getPropertyId("HostRoles", "host_name"))).andReturn("c6401.shpurdp.apache.org").atLeastOnce();
    ...
    ShpurdpManagementController controllerMock =
      mockSupport.createMock(ShpurdpManagementController.class);
    ...
    if(authorizedForLogSearch) {
      ...
      ShpurdpMetaInfo metaInfoMock =
          mockSupport.createMock(ShpurdpMetaInfo.class);
      ...
      expect(controllerMock.getShpurdpServerURI(expectedSearchEnginePath)).
          andReturn(expectedShpurdpURL + expectedSearchEnginePath).atLeastOnce();
      expect(controllerMock.getShpurdpMetaInfo()).andReturn(metaInfoMock).atLeastOnce();
      ...
      expect(dataRetrievalServiceMock.getLogFileNames(expectedLogSearchComponentName, "c6401.shpurdp.apache.org", "clusterone")).andReturn(Collections.singleton(expectedLogFilePath)).atLeastOnce();
      expect(dataRetrievalServiceMock.getLogFileTailURI(expectedShpurdpURL + expectedSearchEnginePath, expectedLogSearchComponentName, "c6401.shpurdp.apache.org", "clusterone")).andReturn("").atLeastOnce();

      expect(loggingRequestHelperFactoryMock.getHelper(anyObject(ShpurdpManagementController.class), anyObject(String.class)))
    ...
    propertyProvider.setShpurdpManagementController(controllerMock);
    ...
    if(authorizedForLogSearch) {
      ...
      assertEquals("Incorrect URL path to searchEngine",
          expectedShpurdpURL + expectedSearchEnginePath, definitionInfo.getSearchEngineURL());
  ...
  private void testBasicCallWithNullTailLogURIReturned(Authentication authentication, boolean authorizedForLogSearch) throws Exception {
    ...
    if(authorizedForLogSearch) {
      ...
      expect(dataRetrievalServiceMock.getLogFileTailURI(expectedShpurdpURL + expectedSearchEnginePath, expectedLogSearchComponentName, "c6401.shpurdp.apache.org", "clusterone")).andReturn(null).atLeastOnce();
  ...
  private void testCheckWhenLogSearchNotAvailable(Authentication authentication, boolean authorizedForLogSearch) throws Exception {
    ...
    if(authorizedForLogSearch) {
      ...
      expect(dataRetrievalServiceMock.getLogFileNames(expectedLogSearchComponentName, "c6401.shpurdp.apache.org", "clusterone")).andReturn(null).atLeastOnce();
