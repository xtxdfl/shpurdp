...
public class PermissionResourceProviderTest {
  ...
  public void testGetResources() throws Exception {
    ...
    expect(permissionEntity.getPermissionName()).andReturn("SHPURDP.ADMINISTRATOR");
    expect(permissionEntity.getPermissionLabel()).andReturn("Shpurdp Administrator");
    expect(permissionEntity.getSortOrder()).andReturn(1);
    expect(permissionEntity.getResourceType()).andReturn(resourceTypeEntity);
    expect(resourceTypeEntity.getName()).andReturn("SHPURDP");
    ...
    Assert.assertEquals("SHPURDP.ADMINISTRATOR", resource.getPropertyValue(PermissionResourceProvider.PERMISSION_NAME_PROPERTY_ID));
    Assert.assertEquals("Shpurdp Administrator", resource.getPropertyValue(PermissionResourceProvider.PERMISSION_LABEL_PROPERTY_ID));
    Assert.assertEquals("SHPURDP", resource.getPropertyValue(PermissionResourceProvider.RESOURCE_NAME_PROPERTY_ID));
