...
public class TestShpurdpLdapAuthoritiesPopulator extends EasyMockSupport {
  ...
  public void testGetGrantedAuthorities() throws Exception {
    ...
    ShpurdpLdapAuthoritiesPopulator populator = createMockBuilder(ShpurdpLdapAuthoritiesPopulator.class)
  ...
  public void testGetGrantedAuthoritiesWithLoginAlias() throws Exception {
    ...
    String shpurdpUserName = "user";

    PowerMock.mockStatic(AuthorizationHelper.class);
    expect(AuthorizationHelper.resolveLoginAliasToUserName(loginAlias)).andReturn(shpurdpUserName);
    ...
    expect(userDAO.findUserByName(shpurdpUserName)).andReturn(userEntity); // user should be looked up by user name instead of login alias
