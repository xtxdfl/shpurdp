...
import org.apache.shpurdp.server.security.authorization.ShpurdpLdapUtils;
...
@PrepareForTest(ShpurdpLdapUtils.class)
@PowerMockIgnore({"org.apache.logging.log4j.*","org.slf4j.*"})
public class ShpurdpLdapDataPopulatorTest {
  public static class ShpurdpLdapDataPopulatorTestInstance extends TestShpurdpLdapDataPopulator {
    public ShpurdpLdapDataPopulatorTestInstance(Provider<ShpurdpLdapConfiguration> configurationProvider, Users users) {
  ...
  public static class TestShpurdpLdapDataPopulator extends ShpurdpLdapDataPopulator {

    protected LdapTemplate ldapTemplate;
    private LdapContextSource ldapContextSource;
    private PagedResultsDirContextProcessor processor;

    public TestShpurdpLdapDataPopulator(Provider<ShpurdpLdapConfiguration> configurationProvider, Users users) {
      super(configurationProvider, users);
    }
  ...
  public void testIsLdapEnabled_badConfiguration() {
    final Provider<ShpurdpLdapConfiguration> configurationProvider = createNiceMock(Provider.class);
    final ShpurdpLdapConfiguration configuration = createNiceMock(ShpurdpLdapConfiguration.class);
    ...
    final ShpurdpLdapDataPopulatorTestInstance populator = new ShpurdpLdapDataPopulatorTestInstance(configurationProvider, users);
  ...
  public void testReferralMethod() {
    ...
    final TestShpurdpLdapDataPopulator populator = new TestShpurdpLdapDataPopulator(configurationProvider, users);
  ...
  public void synchronizeExistingLdapGroups() throws Exception {
    ...
    final Provider<ShpurdpLdapConfiguration> configurationProvider = createNiceMock(Provider.class);
    ShpurdpLdapConfiguration configuration = createNiceMock(ShpurdpLdapConfiguration.class);
    ...
    ShpurdpLdapDataPopulatorTestInstance populator = createMockBuilder(ShpurdpLdapDataPopulatorTestInstance.class)
  ...
  public void testSynchronizeExistingLdapGroups_removeDuringIteration() throws Exception {
    ...
    ShpurdpLdapDataPopulator dataPopulator = createMockBuilder(ShpurdpLdapDataPopulatorTestInstance.class)
        .withConstructor(configurationProvider, users)
        .addMockedMethod("getLdapGroups")
  ...
  public void testCleanUpLdapUsersWithoutGroup() throws ShpurdpException {
    final Provider<ShpurdpLdapConfiguration> configurationProvider = createNiceMock(Provider.class);
    final ShpurdpLdapConfiguration configuration = createNiceMock(ShpurdpLdapConfiguration.class);
    final Users users = createNiceMock(Users.class);
  ...
  public void testGetLdapUserByMemberAttr() throws Exception {
    ...
    expect(ldapServerProperties.getBaseDN()).andReturn("cn=testUser,ou=Shpurdp,dc=SME,dc=support,dc=com").anyTimes();
    expect(ldapServerProperties.getUsernameAttribute()).andReturn("uid").anyTimes();
    expect(processor.getCookie()).andReturn(cookie).anyTimes();
    expect(cookie.getCookie()).andReturn(null).anyTimes();

    expect(ldapTemplate.search(eq(LdapUtils.newLdapName("cn=testUser,ou=Shpurdp,dc=SME,dc=support,dc=com")), eq("(&(objectClass=objectClass)(uid=foo))"), anyObject(SearchControls.class), capture(contextMapperCapture), eq(processor))).andReturn(list);

    replay(ldapTemplate, ldapServerProperties, users, configurationProvider, configuration, processor, cookie);

    ShpurdpLdapDataPopulatorTestInstance populator = new ShpurdpLdapDataPopulatorTestInstance(configurationProvider, users);
  ...
  public void testGetLdapUserByMemberAttrNoPagination() throws Exception {
    ...
    expect(ldapTemplate.search(eq(LdapUtils.newLdapName("cn=testUser,ou=Shpurdp,dc=SME,dc=support,dc=com")), eq("(&(objectClass=objectClass)(uid=foo))"), anyObject(SearchControls.class), capture(contextMapperCapture))).andReturn(list);
  ...
  public void testLdapUserContextMapper_uidIsNull() throws Exception {
    ...
    expect(adapter.getNameInNamespace()).andReturn("cn=testUser,ou=Shpurdp,dc=SME,dc=support,dc=com");

    PowerMock.mockStatic(ShpurdpLdapUtils.class);
    expect(ShpurdpLdapUtils.isLdapObjectOutOfScopeFromBaseDn(adapter, "dc=SME,dc=support,dc=com"))
        .andReturn(false).anyTimes();

    replay(adapter, ldapServerProperties);
    PowerMock.replayAll();

    ShpurdpLdapDataPopulator.LdapUserContextMapper ldapUserContextMapper = new ShpurdpLdapDataPopulator.LdapUserContextMapper(ldapServerProperties);
    LdapUserDto userDto = (LdapUserDto) ldapUserContextMapper.mapFromContext(adapter);

    assertNotNull(userDto);
    assertNull(userDto.getUid());
    assertEquals("testuser", userDto.getUserName());
    assertEquals("cn=testuser,ou=shpurdp,dc=sme,dc=support,dc=com", userDto.getDn());
  }
