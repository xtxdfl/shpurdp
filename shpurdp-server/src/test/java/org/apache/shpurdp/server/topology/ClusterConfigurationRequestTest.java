...
@PrepareForTest({ShpurdpContext.class})
public class ClusterConfigurationRequestTest {
  ...
  private Capture<? extends Set<String>> testProcessWithKerberos(String blueprintPropertyValue, String
    stackPropertyValue,  Map<String, Map<String, String>> kerberosConfig) throws ShpurdpException, KerberosInvalidConfigurationException,
    ...
    expect(shpurdpContext.getClusterName(Long.valueOf(1))).andReturn("testCluster").anyTimes();
    expect(shpurdpContext.createConfigurationRequests(EasyMock.anyObject())).andReturn(Collections
      .emptyList()).anyTimes();
    ...
    shpurdpContext.waitForConfigurationResolution(capture(captureClusterName), capture
      (captureUpdatedConfigTypes));
    expectLastCall();

    PowerMock.replay(stack, blueprint, topology, controller, clusters, kerberosHelper,
        shpurdpContext, ShpurdpContext.class, configHelper);

    ClusterConfigurationRequest clusterConfigurationRequest = new ClusterConfigurationRequest(
      shpurdpContext, topology, false, stackAdvisorBlueprintProcessor, true);
    clusterConfigurationRequest.process();

    verify(blueprint, topology, shpurdpContext, controller, kerberosHelper, configHelper);
  ...
  public void testProcessClusterConfigRequestDontIncludeKererosConfigs() throws Exception {
    ...
    PowerMock.replay(stack, blueprint, topology, controller, clusters, shpurdpContext,
        ShpurdpContext.class, configHelper);

    ClusterConfigurationRequest clusterConfigurationRequest = new ClusterConfigurationRequest(
      shpurdpContext, topology, false, stackAdvisorBlueprintProcessor);
    clusterConfigurationRequest.process();

    verify(blueprint, topology, shpurdpContext, controller, configHelper);
  ...
  public void testProcessClusterConfigRequestRemoveUnusedConfigTypes() throws Exception {
    ...
    EasyMock.replay(stack, blueprint, topology, shpurdpContext, configHelper);
    // WHEN
    new ClusterConfigurationRequest(shpurdpContext, topology, false, stackAdvisorBlueprintProcessor);
    // THEN
    ...
    verify(stack, blueprint, topology, shpurdpContext, configHelper);
  }
