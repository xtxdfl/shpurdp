...
import org.apache.shpurdp.server.events.AggregateAlertRecalculateEvent;
import org.apache.shpurdp.server.events.AlertEvent;
import org.apache.shpurdp.server.events.AlertStateChangeEvent;
import org.apache.shpurdp.server.events.MockEventListener;
...
public class AlertStateChangedEventTest extends EasyMockSupport {
  ...
  public void setup() throws Exception {
    ...
    EventBusSynchronizer.synchronizeShpurdpEventPublisher(injector).register(m_listener);
   ...
   * upgrade as long as they are for the SHPURDP service.
   *
   * @throws Exception
   */
  @Test
  @SuppressWarnings("unchecked")
  public void testShpurdpAlertsSendDuringUpgrade() throws Exception {
    // expect an upgrading cluster
    expectUpgradingCluster();
    ...
    // create the definition for the SHPURDP service
    AlertDefinitionEntity definition = createNiceMock(AlertDefinitionEntity.class);
    EasyMock.expect(definition.getDefinitionId()).andReturn(1L).anyTimes();
    EasyMock.expect(definition.getServiceName()).andReturn(RootService.SHPURDP.name()).anyTimes();
    EasyMock.expect(definition.getLabel()).andReturn("shpurdp-foo-alert").anyTimes();
    EasyMock.expect(definition.getDescription()).andReturn("Shpurdp Foo Alert").anyTimes();
