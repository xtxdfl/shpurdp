...
public class ShpurdpKerberosAuthenticationFilterTest extends EasyMockSupport {
  ...
  private ShpurdpAuthenticationEventHandler eventHandler;

  @Before
  public void setUp() {
    SecurityContextHolder.getContext().setAuthentication(null);

    entryPoint = createMock(ShpurdpEntryPoint.class);
    configuration = createMock(Configuration.class);
    authenticationManager = createMock(AuthenticationManager.class);
    eventHandler = createMock(ShpurdpAuthenticationEventHandler.class);
  }

  @Test (expected = IllegalArgumentException.class)
  public void ensureNonNullEventHandler() {
    new ShpurdpKerberosAuthenticationFilter(authenticationManager, entryPoint, configuration, null);
  }
  ...
  public void shouldApplyTrue() throws Exception {
    ...
    ShpurdpKerberosAuthenticationFilter filter = new ShpurdpKerberosAuthenticationFilter(
  ...
  public void testDoFilterSuccessful() throws IOException, ServletException {
    ...
    replayAll();
    // WHEN
    ShpurdpAuthenticationFilter filter = new ShpurdpKerberosAuthenticationFilter(authenticationManager, entryPoint, configuration, eventHandler);
  ...
  private ShpurdpKerberosAuthenticationProperties createProperties(Boolean enabled) {
