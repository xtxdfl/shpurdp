...
public class ConfigureClusterTaskTest extends EasyMockSupport {
  ...
  public void before() {
    resetAll();
    testSubject = new ConfigureClusterTask(clusterTopology, clusterConfigurationRequest, shpurdpEventPublisher);
  }
  ...
  public void taskShouldBeExecutedIfRequiredHostgroupsAreResolved() throws Exception {
    ...
    expect(clusterTopology.getShpurdpContext()).andReturn(shpurdpContext);
    expect(shpurdpContext.getClusterName(1L)).andReturn("testCluster");
    clusterConfigurationRequest.process();
    shpurdpEventPublisher.publish(anyObject(ShpurdpEvent.class));
    replayAll();
