...
public class ViewContextImplTest {
  ...
  @Test
  public void testGetShpurdpStreamProvider() throws Exception {
    InstanceConfig instanceConfig = InstanceConfigTest.getInstanceConfigs().get(0);
    ViewEntity viewDefinition = ViewEntityTest.getViewEntity();
    ViewInstanceEntity viewInstanceDefinition = new ViewInstanceEntity(viewDefinition, instanceConfig);
    ViewRegistry viewRegistry = createNiceMock(ViewRegistry.class);
    ViewShpurdpStreamProvider shpurdpStreamProvider = createNiceMock(ViewShpurdpStreamProvider.class);
    ...
    expect(viewRegistry.createShpurdpStreamProvider()).andReturn(shpurdpStreamProvider);

    replay(viewRegistry);

    ViewContextImpl viewContext = new ViewContextImpl(viewInstanceDefinition, viewRegistry);

    Assert.assertEquals(shpurdpStreamProvider, viewContext.getShpurdpStreamProvider());
