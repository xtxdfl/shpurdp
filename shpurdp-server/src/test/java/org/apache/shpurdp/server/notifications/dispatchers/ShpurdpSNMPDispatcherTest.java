...
import org.apache.shpurdp.server.notifications.DispatchCallback;
...
import org.apache.shpurdp.server.notifications.Recipient;
import org.apache.shpurdp.server.notifications.TargetConfigurationResult;
...
import org.apache.shpurdp.server.state.AlertState;
...
public class ShpurdpSNMPDispatcherTest {
    ...
    public void testDispatch_nullProperties() throws Exception {
        ShpurdpSNMPDispatcher dispatcher = new ShpurdpSNMPDispatcher(DEFAULT_SNMP_PORT);
    ...
    public void testDispatchUdpTransportMappingCrash() throws Exception {
        ShpurdpSNMPDispatcher dispatcher = spy(new ShpurdpSNMPDispatcher(DEFAULT_SNMP_PORT));
        ShpurdpSNMPDispatcher.SnmpVersion snmpVersion = ShpurdpSNMPDispatcher.SnmpVersion.SNMPv1;
        Notification notification = mock(AlertNotification.class);
        notification.Callback = mock(DispatchCallback.class);
        notification.CallbackIds = mock(List.class);
        Map<String, String> properties = new HashMap<>();
        properties.put(ShpurdpSNMPDispatcher.PORT_PROPERTY, "3");
        properties.put(ShpurdpSNMPDispatcher.COMMUNITY_PROPERTY, "4");
        properties.put(ShpurdpSNMPDispatcher.SNMP_VERSION_PROPERTY, "SNMPv1");
    ...
    public void testDispatch_sendTrapError() throws Exception {
        ...
        notification.DispatchProperties = properties;
        notification.Recipients = Arrays.asList(new Recipient());
        doThrow(new RuntimeException()).when(dispatcher).sendTraps(eq(notification), any(ShpurdpSNMPDispatcher.SnmpVersion.class));
        dispatcher.dispatch(notification);
        verify(notification.Callback).onFailure(notification.CallbackIds);
        verify(notification.Callback, never()).onSuccess(notification.CallbackIds);
    }
    ...
    public void testDispatch_incorrectSnmpVersion() throws Exception {
        ...
        properties.put(ShpurdpSNMPDispatcher.PORT_PROPERTY, "3");
        properties.put(ShpurdpSNMPDispatcher.COMMUNITY_PROPERTY, "4");
        properties.put(ShpurdpSNMPDispatcher.SNMP_VERSION_PROPERTY, "SNMPv11");
        notification.DispatchProperties = properties;
        notification.Recipients = Arrays.asList(new Recipient());
        dispatcher.dispatch(notification);
        verify(notification.Callback).onFailure(notification.CallbackIds);
    ...
    public void testDispatch_successful_v2() throws Exception {
        ShpurdpSNMPDispatcher dispatcher = spy(new ShpurdpSNMPDispatcher(DEFAULT_SNMP_PORT));
        ShpurdpSNMPDispatcher.SnmpVersion snmpVersion = ShpurdpSNMPDispatcher.SnmpVersion.SNMPv2c;
        Notification notification = mock(AlertNotification.class);
        notification.Callback = mock(DispatchCallback.class);
        notification.CallbackIds = mock(List.class);
        Map<String, String> properties = new HashMap<>();
        properties.put(ShpurdpSNMPDispatcher.PORT_PROPERTY, "3");
        properties.put(ShpurdpSNMPDispatcher.COMMUNITY_PROPERTY, "4");
        properties.put(ShpurdpSNMPDispatcher.SNMP_VERSION_PROPERTY, "SNMPv2c");
        notification.DispatchProperties = properties;
        notification.Recipients = Arrays.asList(new Recipient());
        doNothing().when(dispatcher).sendTraps(notification, snmpVersion);
        dispatcher.dispatch(notification);
    ...
    public void testDispatch_successful_v3() throws Exception {
        ...
        notification.CallbackIds = mock(List.class);
        Map<String, String> properties = new HashMap<>();
        properties.put(ShpurdpSNMPDispatcher.PORT_PROPERTY, "162");
        properties.put(ShpurdpSNMPDispatcher.COMMUNITY_PROPERTY, "public");
        properties.put(ShpurdpSNMPDispatcher.SNMP_VERSION_PROPERTY, "SNMPv3");
        properties.put(ShpurdpSNMPDispatcher.SECURITY_USERNAME_PROPERTY, "USER");
        properties.put(ShpurdpSNMPDispatcher.SECURITY_AUTH_PASSPHRASE_PROPERTY, "PASSPHRASE1");
        properties.put(ShpurdpSNMPDispatcher.SECURITY_PRIV_PASSPHRASE_PROPERTY, "PASSPHRASE2");
        properties.put(ShpurdpSNMPDispatcher.SECURITY_LEVEL_PROPERTY, "AUTH_NOPRIV");
        notification.DispatchProperties = properties;
    ...
    assertEquals(11, variableBindings.size());
        assertEquals(ShpurdpSNMPDispatcher.SHPURDP_ALERT_TRAP_OID, variableBindings.get(SnmpConstants.snmpTrapOID.toString()).toValueString());
        assertTrue(variableBindings.get(SnmpConstants.snmpTrapOID.toString()).getVariable() instanceof OID);
        assertEquals(String.valueOf(DEFINITION_ID), variableBindings.get(ShpurdpSNMPDispatcher.SHPURDP_ALERT_DEFINITION_ID_OID).toValueString());
        assertTrue(variableBindings.get(ShpurdpSNMPDispatcher.SHPURDP_ALERT_DEFINITION_ID_OID).getVariable() instanceof Integer32);
        assertEquals(DEFINITION_NAME, variableBindings.get(ShpurdpSNMPDispatcher.SHPURDP_ALERT_DEFINITION_NAME_OID).toValueString());
        assertTrue(variableBindings.get(ShpurdpSNMPDispatcher.SHPURDP_ALERT_DEFINITION_NAME_OID).getVariable() instanceof OctetString);
        assertEquals(ALERT_LABEL, variableBindings.get(ShpurdpSNMPDispatcher.SHPURDP_ALERT_NAME_OID).toValueString());
        assertTrue(variableBindings.get(ShpurdpSNMPDispatcher.SHPURDP_ALERT_NAME_OID).getVariable() instanceof OctetString);
        assertEquals(ALERT_TEXT, variableBindings.get(ShpurdpSNMPDispatcher.SHPURDP_ALERT_TEXT_OID).toValueString());
        assertTrue(variableBindings.get(ShpurdpSNMPDispatcher.SHPURDP_ALERT_TEXT_OID).getVariable() instanceof OctetString);
        assertEquals(String.valueOf(ALERT_STATE.getIntValue()), variableBindings.get(ShpurdpSNMPDispatcher.SHPURDP_ALERT_STATE_OID).toValueString());
        assertTrue(variableBindings.get(ShpurdpSNMPDispatcher.SHPURDP_ALERT_STATE_OID).getVariable() instanceof Integer32);
        assertEquals(ALERT_HOSTNAME, variableBindings.get(ShpurdpSNMPDispatcher.SHPURDP_ALERT_HOST_NAME_OID).toValueString());
        assertTrue(variableBindings.get(ShpurdpSNMPDispatcher.SHPURDP_ALERT_HOST_NAME_OID).getVariable() instanceof OctetString);
        assertEquals(ALERT_SERVICE_NAME, variableBindings.get(ShpurdpSNMPDispatcher.SHPURDP_ALERT_SERVICE_NAME_OID).toValueString());
        assertTrue(variableBindings.get(ShpurdpSNMPDispatcher.SHPURDP_ALERT_SERVICE_NAME_OID).getVariable() instanceof OctetString);
        assertEquals(ALERT_COMPONENT_NAME, variableBindings.get(ShpurdpSNMPDispatcher.SHPURDP_ALERT_COMPONENT_NAME_OID).toValueString());
        assertTrue(variableBindings.get(ShpurdpSNMPDispatcher.SHPURDP_ALERT_COMPONENT_NAME_OID).getVariable() instanceof OctetString);
    }
    ...
    assertEquals(11, variableBindings.size());
        assertEquals("null", variableBindings.get(ShpurdpSNMPDispatcher.SHPURDP_ALERT_COMPONENT_NAME_OID).toValueString());
    }
    ...
    public void testSendTraps_v1() throws Exception {
        ...
        ShpurdpSNMPDispatcher dispatcher = spy(new ShpurdpSNMPDispatcher(snmp));
    ...
    public void testSendTraps_v3() throws Exception {
        ShpurdpSNMPDispatcher.SnmpVersion snmpVersion = ShpurdpSNMPDispatcher.SnmpVersion.SNMPv3;
        Snmp snmp = mock(Snmp.class);
        ShpurdpSNMPDispatcher dispatcher = spy(new ShpurdpSNMPDispatcher(snmp));
        PDU trap = mock(PDU.class);
        Notification notification = new AlertNotification();
    ...
    @Test(expected = ShpurdpSNMPDispatcher.InvalidSnmpConfigurationException.class)
    public void testSendTraps_v3_incorrectSecurityLevelVersion() throws Exception {
        ...
        properties.put(ShpurdpSNMPDispatcher.SECURITY_AUTH_PASSPHRASE_PROPERTY, "PASSPHRASE1");
        properties.put(ShpurdpSNMPDispatcher.SECURITY_PRIV_PASSPHRASE_PROPERTY, "PASSPHRASE2");
        properties.put(ShpurdpSNMPDispatcher.SECURITY_LEVEL_PROPERTY, "INCORRECT");
        notification.DispatchProperties = properties;
        Recipient rec1 = new Recipient();
        rec1.Identifier = "192.168.0.2";
        notification.Recipients = Arrays.asList(rec1);
    ...
    public void testValidateAlertValidation_SNMPv1() throws Exception {
        ...
        properties.put(ShpurdpSNMPDispatcher.SNMP_VERSION_PROPERTY, "SNMPv1");
        properties.put(ShpurdpSNMPDispatcher.COMMUNITY_PROPERTY, "public");
        NotificationDispatcher dispatcher = new ShpurdpSNMPDispatcher(DEFAULT_SNMP_PORT);
    ...
    public void testValidateAlertValidation_incorrectSNMPversion() throws Exception {
        Map<String, Object> properties = new HashMap<>();
        properties.put(ShpurdpSNMPDispatcher.PORT_PROPERTY, "162");
        properties.put(ShpurdpSNMPDispatcher.SNMP_VERSION_PROPERTY, "SNMPv4");
        properties.put(ShpurdpSNMPDispatcher.COMMUNITY_PROPERTY, "public");
        NotificationDispatcher dispatcher = new ShpurdpSNMPDispatcher(DEFAULT_SNMP_PORT);
        TargetConfigurationResult configValidationResult = dispatcher.validateTargetConfig(properties);
    ...
    public void testValidateAlertValidation_SNMPv3_noAuthNoPriv() throws Exception {
        ...
        properties.put(ShpurdpSNMPDispatcher.SNMP_VERSION_PROPERTY, "SNMPv3");
        properties.put(ShpurdpSNMPDispatcher.SECURITY_USERNAME_PROPERTY, "USER");
        properties.put(ShpurdpSNMPDispatcher.SECURITY_LEVEL_PROPERTY, "NOAUTH_NOPRIV");
        NotificationDispatcher dispatcher = new ShpurdpSNMPDispatcher(DEFAULT_SNMP_PORT);
        TargetConfigurationResult configValidationResult = dispatcher.validateTargetConfig(properties);
        assertEquals(TargetConfigurationResult.Status.VALID, configValidationResult.getStatus());
    }
    ...
    public void testValidateAlertValidation_SNMPv3_AuthPriv_valid() throws Exception {
        ...
        properties.put(ShpurdpSNMPDispatcher.SECURITY_AUTH_PASSPHRASE_PROPERTY, "PASSPHRASE1");
        properties.put(ShpurdpSNMPDispatcher.SECURITY_PRIV_PASSPHRASE_PROPERTY, "PASSPHRASE2");
        properties.put(ShpurdpSNMPDispatcher.SECURITY_LEVEL_PROPERTY, "AUTH_PRIV");
        NotificationDispatcher dispatcher = new ShpurdpSNMPDispatcher(DEFAULT_SNMP_PORT);
        TargetConfigurationResult configValidationResult = dispatcher.validateTargetConfig(properties);
        assertEquals(TargetConfigurationResult.Status.VALID, configValidationResult.getStatus());
    }
