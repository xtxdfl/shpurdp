...
import static org.apache.shpurdp.server.upgrade.UpgradeCatalog272.BLUEPRINT_PROVISIONING_STATE_COLUMN;
import static org.apache.shpurdp.server.upgrade.UpgradeCatalog272.CLUSTERS_TABLE;
import static org.apache.shpurdp.server.upgrade.UpgradeCatalog272.HOST_COMPONENT_DESIRED_STATE_TABLE;
import static org.apache.shpurdp.server.upgrade.UpgradeCatalog272.RENAME_COLLISION_BEHAVIOR_PROPERTY_SQL;
...
public class UpgradeCatalog272Test {
  ...
  public void shouldRenameCollisionBehaviorLdapCategoryPropertyNameIfTableWithDataExists() throws Exception {
    ...
    expect(dbAccessor.tableExists(SHPURDP_CONFIGURATION_TABLE)).andReturn(true).once();
  ...
  public void shouldNotRenameCollisionBehaviorLdapCategoryPropertyNameIfTableDoesNotExist() throws Exception {
    ...
    expect(dbAccessor.tableExists(SHPURDP_CONFIGURATION_TABLE)).andReturn(false).once();
    replay(dbAccessor);
    final UpgradeCatalog272 upgradeCatalog272 = new UpgradeCatalog272(injector);
    upgradeCatalog272.dbAccessor = dbAccessor;
    assertEquals(expectedResult, upgradeCatalog272.renameLdapSynchCollisionBehaviorValue());
