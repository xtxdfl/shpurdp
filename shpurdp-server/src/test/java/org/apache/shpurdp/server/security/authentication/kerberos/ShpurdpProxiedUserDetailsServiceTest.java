...
public class ShpurdpProxiedUserDetailsServiceTest extends EasyMockSupport {
  @Test
  public void testValidateHost() throws UnknownHostException {
    ...
    ShpurdpProxiedUserDetailsService service = EasyMock.partialMockBuilder(ShpurdpProxiedUserDetailsService.class)
    ...
    ShpurdpTProxyConfiguration tproxyConfiguration = createStrictMock(ShpurdpTProxyConfiguration.class);
  ...
  public void testValidateUser() {
    ShpurdpProxiedUserDetailsService service = new ShpurdpProxiedUserDetailsService(createNiceMock(Configuration.class), createNiceMock(Users.class));

    ShpurdpTProxyConfiguration tproxyConfigration = createMock(ShpurdpTProxyConfiguration.class);
    ...
    // shpurdp.tproxy.proxyuser.proxyUser.users = "*"
    Assert.assertTrue(service.validateUser(tproxyConfigration, "proxyUser", "validUser"));

    // shpurdp.tproxy.proxyuser.proxyUser.users = "validUser"
    Assert.assertTrue(service.validateUser(tproxyConfigration, "proxyUser", "validUser"));

    // shpurdp.tproxy.proxyuser.proxyUser.users = "validuser"  (different case)
    Assert.assertTrue(service.validateUser(tproxyConfigration, "proxyUser", "validUser"));

    // shpurdp.tproxy.proxyuser.proxyUser.users = "validUser, tom, *"
    Assert.assertTrue(service.validateUser(tproxyConfigration, "proxyUser", "validUser"));

    // shpurdp.tproxy.proxyuser.proxyUser.users = null
    Assert.assertFalse(service.validateUser(tproxyConfigration, "proxyUser", "validUser"));

    // shpurdp.tproxy.proxyuser.proxyUser.users = ""
    Assert.assertFalse(service.validateUser(tproxyConfigration, "proxyUser", "validUser"));

    // shpurdp.tproxy.proxyuser.proxyUser.users = "notValidUser"
    Assert.assertFalse(service.validateUser(tproxyConfigration, "proxyUser", "validUser"));
  ...
  public void testValidateGroup() {
    ...
    // shpurdp.tproxy.proxyuser.proxyUser.groups = "*"
    Assert.assertTrue(service.validateGroup(tproxyConfigration, "proxyUser", userEntity));

    // shpurdp.tproxy.proxyuser.proxyUser.groups = "validGroup"
    Assert.assertTrue(service.validateGroup(tproxyConfigration, "proxyUser", userEntity));

    // shpurdp.tproxy.proxyuser.proxyUser.groups = "validgroup"  (different case)
    Assert.assertTrue(service.validateGroup(tproxyConfigration, "proxyUser", userEntity));

    // shpurdp.tproxy.proxyuser.proxyUser.groups = "validGroup, *"
    Assert.assertTrue(service.validateGroup(tproxyConfigration, "proxyUser", userEntity));

    // shpurdp.tproxy.proxyuser.proxyUser.groups = null
    Assert.assertFalse(service.validateGroup(tproxyConfigration, "proxyUser", userEntity));

    // shpurdp.tproxy.proxyuser.proxyUser.groups = ""
    Assert.assertFalse(service.validateGroup(tproxyConfigration, "proxyUser", userEntity));

    // shpurdp.tproxy.proxyuser.proxyUser.groups = "notValidGroup"
    Assert.assertFalse(service.validateGroup(tproxyConfigration, "proxyUser", userEntity));
