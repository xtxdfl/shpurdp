...
public class ComponentsInstallationCheckTest {
  ...
  public void setup() throws Exception {
    ...
    m_checkHelper.setMetaInfoProvider(new Provider<ShpurdpMetaInfo>() {
      @Override
  ...
  public void testPerform() throws Exception {
    ...
    componentsInstallationCheck.shpurdpMetaInfo = new Provider<ShpurdpMetaInfo>() {
      @Override
      public ShpurdpMetaInfo get() {
    ...
    m_services.put("SHPURDP_METRICS", amsService);

    Mockito.when(hdfsService.getName()).thenReturn("HDFS");
    Mockito.when(tezService.getName()).thenReturn("TEZ");
    Mockito.when(amsService.getName()).thenReturn("SHPURDP_METRICS");
    ...
    Mockito.when(cluster.getService("SHPURDP_METRICS")).thenReturn(amsService);

    Mockito.when(shpurdpMetaInfo.getComponent(Mockito.anyString(), Mockito.anyString(),
    ...
    PowerMockito.when(HostComponentSummary.getHostComponentSummaries("SHPURDP_METRICS", "METRICS_COLLECTOR")).thenReturn(Arrays.asList(hcsMetricsCollector));
    PowerMockito.when(HostComponentSummary.getHostComponentSummaries("SHPURDP_METRICS", "METRICS_MONITOR")).thenReturn(Arrays.asList(hcsMetricsMonitor));
