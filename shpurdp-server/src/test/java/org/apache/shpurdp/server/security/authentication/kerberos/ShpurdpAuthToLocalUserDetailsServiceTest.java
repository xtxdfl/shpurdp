...
public class ShpurdpAuthToLocalUserDetailsServiceTest extends EasyMockSupport {
  @Before
  public void setup() {
  ...
  public void loadUserByUsernameSuccess() throws Exception {
    ShpurdpKerberosAuthenticationProperties properties = new ShpurdpKerberosAuthenticationProperties();
    ...
    Collection<ShpurdpGrantedAuthority> userAuthorities = Collections.singletonList(createNiceMock(ShpurdpGrantedAuthority.class));
    ...
    UserDetailsService userdetailsService = new ShpurdpAuthToLocalUserDetailsService(configuration, users);

    UserDetails userDetails = userdetailsService.loadUserByUsername("user1@EXAMPLE.COM");

    Assert.assertNotNull(userDetails);
    Assert.assertTrue(userDetails instanceof ShpurdpUserDetails);

    ShpurdpUserDetails shpurdpUserDetails = (ShpurdpUserDetails) userDetails;
    Assert.assertEquals("user1", shpurdpUserDetails.getUsername());
    Assert.assertEquals(Integer.valueOf(1), shpurdpUserDetails.getUserId());
    Assert.assertEquals(userAuthorities.size(), shpurdpUserDetails.getAuthorities().size());
