...
import org.apache.shpurdp.server.testing.DeadlockWarningThread;
...
/**
 * Tests SHPURDP-9368 and SHPURDP-9761 which produced a deadlock during read and
...
public class ClusterDeadlockTest {
  ...
  /**
   * Tests SHPURDP-9368 which produced a deadlock during read and writes of some
   * of the impl classes.
   */
    ...
    public void run() {
      try {
        for (int i = 0; i < 10; i++) {
          ...
          cluster.setProvisioningState(org.apache.shpurdp.server.state.State.INIT);
          service.setMaintenanceState(MaintenanceState.OFF);
          nameNodeComponent.setDesiredState(org.apache.shpurdp.server.state.State.STARTED);
          dataNodeComponent.setDesiredState(org.apache.shpurdp.server.state.State.INSTALLED);

          nameNodeSCH.setState(org.apache.shpurdp.server.state.State.STARTED);
          dataNodeSCH.setState(org.apache.shpurdp.server.state.State.INSTALLED);
  ...
  private Service installService(String serviceName) throws ShpurdpException {
