...
/**
 * Tests SHPURDP-12526 which produced a deadlock during concurrent writes of
...
public class ServiceComponentHostConcurrentWriteDeadlockTest {
  ...
  /**
   * Tests SHPURDP-12526 by constantly writing version and state to service
    ...
    public void run() {
      try {
        for (int i = 0; i < 1000; i++) {
          org.apache.shpurdp.server.state.State state = (i % 2 == 0)
              ? org.apache.shpurdp.server.state.State.INSTALLING
              : org.apache.shpurdp.server.state.State.INSTALL_FAILED;
