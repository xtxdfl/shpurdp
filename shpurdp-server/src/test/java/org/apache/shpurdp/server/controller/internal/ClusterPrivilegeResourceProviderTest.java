...
public class ClusterPrivilegeResourceProviderTest extends EasyMockSupport {
  ...
  private void getResourcesTest(Authentication authentication) throws Exception {
    ...
    Assert.assertEquals("CLUSTER.ADMINISTRATOR", resource.getPropertyValue(ShpurdpPrivilegeResourceProvider.PERMISSION_NAME));
    Assert.assertEquals("Cluster Administrator", resource.getPropertyValue(ShpurdpPrivilegeResourceProvider.PERMISSION_LABEL));
