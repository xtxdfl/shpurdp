...
import org.apache.shpurdp.server.controller.internal.TemporalInfoImpl;
...
import org.apache.shpurdp.server.controller.metrics.ganglia.TestStreamProvider;
import org.apache.shpurdp.server.controller.metrics.timeline.cache.TimelineMetricCache;
...
@PrepareForTest({AMSPropertyProvider.class, ShpurdpServer.class})
...
public class AMSPropertyProviderTest {
  ...
  public void testPopulateMetricsForEmbeddedHBase() throws Exception {
    ShpurdpManagementController amc = createNiceMock(ShpurdpManagementController.class);
    PowerMock.mockStatic(ShpurdpServer.class);
    expect(ShpurdpServer.getController()).andReturn(amc).anyTimes();
    ...
    expect(amc.getShpurdpMetaInfo()).andReturn(shpurdpMetaInfo).anyTimes();
    expect(shpurdpMetaInfo.getComponentToService("HDP", "2.2", "METRICS_COLLECTOR")).andReturn("AMS").anyTimes();
    expect(shpurdpMetaInfo.getComponent("HDP", "2.2", "AMS", "METRICS_COLLECTOR"))
      .andReturn(componentInfo).anyTimes();
    expect(componentInfo.getTimelineAppid()).andReturn("AMS-HBASE");
    replay(amc, clusters, cluster, amsService, shpurdpMetaInfo, componentInfo);
    PowerMock.replayAll();
  ...
  public void testAggregateFunctionForComponentMetrics() throws Exception {
    ...
    expect(cluster.getCurrentStackVersion()).andReturn(stackId).anyTimes();
    expect(amc.getShpurdpMetaInfo()).andReturn(shpurdpMetaInfo).anyTimes();
    expect(shpurdpMetaInfo.getComponentToService("HDP", "2.2", "HBASE_REGIONSERVER")).andReturn("HBASE").anyTimes();
    expect(shpurdpMetaInfo.getComponent("HDP", "2.2", "HBASE", "HBASE_REGIONSERVER"))
      .andReturn(componentInfo).anyTimes();
    expect(componentInfo.getTimelineAppid()).andReturn("HBASE");
    replay(amc, clusters, cluster, hbaseService, shpurdpMetaInfo, componentInfo);
    PowerMock.replayAll();
  ...
  private void setUpCommonMocks() throws ShpurdpException {
    ShpurdpManagementController ams = createNiceMock(ShpurdpManagementController.class);
    PowerMock.mockStatic(ShpurdpServer.class);
    expect(ShpurdpServer.getController()).andReturn(ams).anyTimes();
    ShpurdpMetaInfo shpurdpMetaInfo = createNiceMock(ShpurdpMetaInfo.class);
    Clusters clusters = createNiceMock(Clusters.class);
    Cluster cluster = createNiceMock(Cluster.class);
    ComponentInfo componentInfo = createNiceMock(ComponentInfo.class);
    ...
    expect(cluster.getCurrentStackVersion()).andReturn(stackId).anyTimes();
    expect(ams.getShpurdpMetaInfo()).andReturn(shpurdpMetaInfo).anyTimes();
    expect(shpurdpMetaInfo.getComponentToService(anyObject(String.class),
      anyObject(String.class), anyObject(String.class))).andReturn("HDFS").anyTimes();
    expect(shpurdpMetaInfo.getComponent(anyObject(String.class), anyObject(String.class),
      anyObject(String.class), anyObject(String.class)))
      .andReturn(componentInfo).anyTimes();
    expect(componentInfo.getTimelineAppid()).andReturn(null).anyTimes();

    replay(ams, clusters, cluster, shpurdpMetaInfo, componentInfo);
    PowerMock.replayAll();
  }
