...
from bootstrap import SHPURDP_PASSPHRASE_VAR_NAME
...
class TestBootstrap:  # (TestCase):
  ...
  def test_getShpurdpPort(self):
    ...
    self.assertEqual(bootstrap_obj.getShpurdpPort(), "8440")
    shared_state.server_port = None
    bootstrap_obj = Bootstrap("hostname", shared_state)
    self.assertEqual(bootstrap_obj.getShpurdpPort(), "null")
  ...
  def test_getRunSetupWithPasswordCommand(self, environ_mock):
    ...
    expected = (
      "/var/lib/shpurdp-agent/tmp/shpurdp-sudo.sh -S python /var/lib/shpurdp-agent/tmp/setupAgent{0}.py hostname TEST_PASSPHRASE "
      "shpurdpServer root  8440 < /var/lib/shpurdp-agent/tmp/host_pass{0}".format(utime)
    )
  ...
  def test_generateRandomFileName(self):
    ...
    bootstrap_obj = Bootstrap("hostname", shared_state)
    self.assertTrue(
      bootstrap_obj.generateRandomFileName(None) == bootstrap_obj.getUtime()
    )
  ...
  def test_run_setup_agent_command_ends_with_project_version(self):
    os.environ[SHPURDP_PASSPHRASE_VAR_NAME] = ""
    version = "1.1.1"
    shared_state = SharedState(
      "root",
      "123",
  ...
  def test_getRepoFile(self, is_suse_mock):
    ...
    self.assertEqual(rf, "/etc/yum.repos.d/shpurdp.repo")
  ...
  def test_createTargetDir(self, write_mock, run_mock, init_mock):
    ...
    self.assertEqual(
      command,
      'SUDO=$([ "$EUID" -eq 0 ] && echo || echo sudo) ; $SUDO mkdir -p /var/lib/shpurdp-agent/tmp ; '
      "$SUDO chown -R root /var/lib/shpurdp-agent/tmp ; "
      "$SUDO chmod 755 /var/lib/shpurdp-agent ; "
      "$SUDO chmod 755 /var/lib/shpurdp-agent/data ; "
      "$SUDO chmod 1777 /var/lib/shpurdp-agent/tmp",
    )
  ...
  def test_getRepoFile(
    ...
    self.assertEqual(
      rf, "/var/lib/shpurdp-agent/tmp/shpurdp-sudo.sh mv RemoteName target/shpurdp.repo"
    )
    # With password
    ...
    self.assertEqual(
      rf,
      "/var/lib/shpurdp-agent/tmp/shpurdp-sudo.sh -S mv RemoteName target/shpurdp.repo < RemoteName",
    )
  ...
  def test_copyNeededFiles(
    ...
    self.assertEqual(
      command, "/var/lib/shpurdp-agent/tmp/shpurdp-sudo.sh chmod 644 RepoFile"
    )
    # Another order
    ...
    #
    # Shpurdp repo file does not exist
    #
  ...
  @patch.object(SSH, "run")
  @patch.object(HostLog, "write")
  def test_runOsCheckScript(
    self, write_mock, run_mock, init_mock, getOsCheckScriptRemoteLocation_mock
  ):
    shared_state = SharedState(
      "root",
      "123",
    ...
    self.assertEqual(
      ...
      "env PYTHONPATH=$PYTHONPATH:/var/lib/shpurdp-agent/tmp OsCheckScriptRemoteLocation centos6",
    )
  ...
  @patch.object(SSH, "run")
  @patch.object(HostLog, "write")
  def test_runSetupAgent(
    self, write_mock, run_mock, getRunSetupCommand_mock, init_mock
  ):
    shared_state = SharedState(
      "root",
      "123",
  ...
  def test_getRunSetupCommand(
    self,
    getRunSetupWithoutPasswordCommand_mock,
  ...
  @patch.object(SSH, "run")
  @patch.object(HostLog, "write")
  def test_checkSudoPackageUbuntu(
    self,
    write_mock,
    run_mock,
    init_mock,
  ...
  def test_deletePasswordFile(
    self, getPasswordFile_mock, write_mock, run_mock, init_mock
  ):
    shared_state = SharedState(
      "root",
      "123",
  ...
  @patch.object(HostLog, "write")
  @patch.object(BootstrapDefault, "getPasswordFile")
  def test_changePasswordFileModeOnHost(
    self, getPasswordFile_mock, write_mock, run_mock, init_mock
  ):
    shared_state = SharedState(
      "root",
