...
class TestRUExecuteTasks(RMFTestCase):
  def get_custom_actions_dir(self):
    return os.path.normpath(
      os.path.join(self.get_src_folder(), "test", "resources", "custom_actions")
    )
  ...
  def setUp(self, os_type_mock, error_mock, info_mock):
    ...
    self.CACHE_MOCK_DIR = (
      "/var/lib/shpurdp-agent/cache/common-services/HDFS/2.1.0.2.0/package"
    )
    self.AGENT_INI_FILE_PATH = os.path.normpath(
      os.path.join(
        self.get_src_folder(), "../../shpurdp-agent/conf/unix/shpurdp-agent.ini"
      )
    )
  ...
  def test_execution(
    ...
    shpurdp_agent_ini_file_path = self.AGENT_INI_FILE_PATH
    self.assertTrue(os.path.isfile(shpurdp_agent_ini_file_path))
    get_config_file_mock.return_value = shpurdp_agent_ini_file_path
    ...
    call_mock.assert_called_with(
      "source /var/lib/shpurdp-agent/shpurdp-env.sh ; /usr/bin/shpurdp-python-wrap /var/lib/shpurdp-agent/cache/common-services/HDFS/2.1.0.2.0/package"
      + os.sep
      + "scripts/namenode.py prepare_rolling_upgrade /tmp",
      logoutput=True,
      quiet=True,
    )
    pass
  ...
  def test_execution_custom_action(
    self,
    cache_mock,
    get_config_mock,
    get_tmp_dir_mock,
    ...
    cache_mock.return_value = "/var/lib/shpurdp-agent/cache/custom_actions"
    get_config_mock.return_value = config_dict
    get_tmp_dir_mock.return_value = "/tmp"

    shpurdp_agent_ini_file_path = os.path.join(
      self.get_src_folder(), "../../shpurdp-agent/conf/unix/shpurdp-agent.ini"
    )
    self.assertTrue(os.path.isfile(shpurdp_agent_ini_file_path))
    get_config_file_mock.return_value = shpurdp_agent_ini_file_path
    ...
    call_mock.assert_called_with(
      "source /var/lib/shpurdp-agent/shpurdp-env.sh ; /usr/bin/shpurdp-python-wrap /var/lib/shpurdp-agent/cache/custom_actions"
      + os.sep
      + "scripts/namenode.py prepare_rolling_upgrade /tmp",
      logoutput=True,
      quiet=True,
