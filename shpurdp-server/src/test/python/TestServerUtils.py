...
class TestServerUtils(TestCase):
  def test_get_shpurdp_server_api_base(self):
    ...
    properties = FakeProperties(
      {SSL_API: "true", SSL_API_PORT: "8443", CLIENT_API_PORT_PROPERTY: None}
    )
    result = get_shpurdp_server_api_base(properties)
    self.assertEqual(result, f"https://{fqdn}:8443/api/v1/")

  def test_get_shpurdp_admin_credentials_from_cli_options(self):
    user_name = "admin"
    password = "s#perS3cr3tP4ssw0d!"
    options = MagicMock()
    options.shpurdp_admin_username = user_name
    options.shpurdp_admin_password = password
    user, pw = get_shpurdp_admin_username_password_pair(options)
    self.assertEqual(user, user_name)
    self.assertEqual(pw, password)

  @patch("shpurdp_server.serverUtils.get_validated_string_input")
  def test_get_shpurdp_admin_credentials_from_user_input(
    self, get_validated_string_input_mock
  ):
    user_name = "admin"
    password = "s#perS3cr3tP4ssw0d!"
    options = MagicMock()
    ...
    def valid_input_side_effect(*args, **kwargs):
      return user_name if "Shpurdp Admin login" in args[0] else password
  ...
  def test_is_api_ssl_enabled(self):
    ...
    properties = FakeProperties({SSL_API: None})
    self.assertFalse(is_api_ssl_enabled(properties))
