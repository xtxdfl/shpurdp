            ...
            with patch.object(os_utils, "is_service_exist", return_value=True):
              with patch("glob.glob", return_value=["/etc/init.d/postgresql-9.3"]):
                ...
                with patch("builtins.open"):
                  ...
                  from shpurdp_commons.os_check import OSCheck, OSConst
                  from shpurdp_commons.os_family_impl import (
                    OsFamilyImpl,
                    OsFamilyFuncImpl,
                  )
                  ...
                  from shpurdp_commons.os_utils import (
                    run_os_command,
                    search_file,
                  ...
                  from shpurdp_server.dbConfiguration_linux import (
                    PGConfig,
                    LinuxDBMSConfig,
                    OracleConfig,
                  )
                  from shpurdp_server.properties import Properties
                  from shpurdp_server.resourceFilesKeeper import (
                    ResourceFilesKeeper,
                    KeeperException,
                  )
                  ...
                  from shpurdp_server.serverSetup import (
                    check_selinux,
                    check_shpurdp_user,
                    setup_jdbc,
                    SE_STATUS_DISABLED,
                    ...
                    setup_jce_policy,
                    expand_jce_zip_file,
                    check_shpurdp_java_version_is_valid,
                  )
                  from shpurdp_server.serverUpgrade import (
                    upgrade,
                    run_schema_upgrade,
                    move_user_custom_actions,
                    find_and_copy_custom_services,
                  )
                  from shpurdp_server.setupHttps import (
                    is_valid_https_port,
                    setup_https,
                  ...
                  from shpurdp_server.userInput import (
                    get_YN_input,
                    get_choice_string_input,
                    get_validated_string_input,
                    read_password,
                  )
                  from shpurdp_server_main import (
                  ...
                  from shpurdp_server.hostUpdate import update_host_names
                  from shpurdp_server.checkDatabase import check_database

CURR_SHPURDP_VERSION = "2.0.0"
...
@patch(
  "shpurdp_server.dbConfiguration_linux.get_postgre_hba_dir",
  new=MagicMock(return_value="/var/lib/pgsql/data"),
)
@patch(
  "shpurdp_server.dbConfiguration_linux.get_postgre_running_status",
  new=MagicMock(return_value="running"),
)
class TestShpurdpServer(TestCase):
  def setUp(self):
    out = io.StringIO()
    sys.stdout = out
  ...
  @patch("shpurdp_server.dbConfiguration_linux.run_os_command")
  def test_configure_pg_hba_shpurdpdb_users(self, run_os_command_method):
    # Prepare mocks
    run_os_command_method.return_value = (0, "", "")
    database_username = "ffdf"
    tf1 = tempfile.NamedTemporaryFile()
    # Run test
    PGConfig._configure_pg_hba_shpurdpdb_users(tf1.name, database_username)
    # Check results
    self.assertTrue(run_os_command_method.called)
    string_expected = self.get_file_string(
      self.get_samples_dir("configure_pg_hba_shpurdpdb_users1")
    )
  ...
  def test_servicename_regex(self, input_method):
    ...
    result = OracleConfig._get_validated_service_name("shpurdp", 1)
  ...
  def test_dbname_regex(self, input_method):
    ...
    set_silent(False)
    input_method.return_value = "OT100"
    result = LinuxDBMSConfig._get_validated_db_name("Database", "shpurdp")
    self.assertEqual("OT100", result, "Not accepting digits")
    pass
  ...
  def test_configure_pg_hba_postgres_user(self):
    ...
    expected = self.get_file_string(
      self.get_samples_dir("configure_pg_hba_shpurdpdb_users2")
    )
  ...
  def test_get_pass_file_path(self):
    result = get_pass_file_path("/etc/shpurdp/conf_file", JDBC_PASSWORD_FILENAME)
    self.assertEqual("/etc/shpurdp/password.dat", result)
    pass

  @patch.object(OSCheck, "os_distribution", new=MagicMock(return_value=os_distro_value))
  @patch.object(_shpurdp_server_, "setup_security")
  @patch.object(_shpurdp_server_, "logger")
  @patch("shpurdp_server.serverConfiguration.get_shpurdp_properties")
  @patch.object(_shpurdp_server_, "setup_logging")
  @patch.object(_shpurdp_server_, "init_logging")
  def test_main_test_setup_security(
    ...
    tmp_argv = sys.argv
    try:
      sys.argv = ["shpurdp-server", "setup-security", "--security-option=setup-security"]
      setup_security_method.return_value = None

      _shpurdp_server_.mainBody()
  ...
  @patch.object(OSCheck, "os_distribution", new=MagicMock(return_value=os_distro_value))
  @patch.object(_shpurdp_server_, "setup_shpurdp_krb5_jaas")
  @patch.object(_shpurdp_server_, "setup_sensitive_data_encryption")
  @patch.object(_shpurdp_server_, "setup_truststore")
  @patch.object(_shpurdp_server_, "setup_https")
  @patch.object(_shpurdp_server_, "get_validated_string_input")
  @patch.object(_shpurdp_server_, "logger")
  def test_setup_security(
    ...
    _shpurdp_server_.setup_security(args)
    self.assertTrue(setup_shpurdp_krb5_jaas_mock.called)
  ...
  @patch("shpurdp_server.setupSecurity.get_validated_string_input")
  @patch("shpurdp_server.setupSecurity.search_file")
  @patch("os.path.exists")
  def test_setup_shpurdp_krb5_jaas(
    self,
    exists_mock,
    search_mock,
    get_validated_string_input_mock,
    ...
    # Negative case
    try:
      setup_shpurdp_krb5_jaas(self._create_empty_options_mock())
  ...
  @patch("sys.exit")
  @patch.object(OSCheck, "os_distribution", new=MagicMock(return_value=os_distro_value))
  @patch.object(_shpurdp_server_, "setup")
  @patch.object(_shpurdp_server_, "start")
  @patch.object(_shpurdp_server_, "stop")
  @patch.object(_shpurdp_server_, "reset")
  @patch.object(_shpurdp_server_, "logger")
  @patch("shpurdp_server.serverConfiguration.get_shpurdp_properties")
  @patch.object(_shpurdp_server_, "setup_logging")
  @patch.object(_shpurdp_server_, "init_logging")
  def test_main_test_setup(
    ...
    try:
      ...
      sys.argv = ["shpurdp-server", "setup", "-v"]
  ...
  @patch.object(_shpurdp_server_, "setup_logging")
  @patch.object(_shpurdp_server_, "init_logging")
  @patch("shpurdp_server.serverSetup.check_shpurdp_user")
  @patch("shpurdp_server.serverSetup.download_and_install_jdk")
  @patch("shpurdp_server.serverSetup.configure_os_settings")
  @patch.object(DBMSConfig, "setup_database")
  @patch("shpurdp_server.serverSetup.check_jdbc_drivers")
  @patch("shpurdp_server.serverSetup.extract_views")
  @patch("shpurdp_server.serverSetup.adjust_directory_permissions")
  @patch("shpurdp_server.serverSetup.service_setup")
  @patch("shpurdp_server.serverConfiguration.search_file")
  def test_main_with_preset_dbms(
    ...
    check_shpurdp_user_mock.return_value = (0, False, "user", None)
    configure_os_settings_mock.return_value = 0
    search_file_mock.return_value = "/tmp/shpurdp.properties"
    import sys

    tmp_argv = sys.argv
    try:
      sys.argv = ["shpurdp-server", "setup", "-s"]
  ...
  @patch.object(OSCheck, "os_distribution", new=MagicMock(return_value=os_distro_value))
  @patch.object(_shpurdp_server_, "setup")
  @patch.object(_shpurdp_server_, "fix_database_options")
  @patch.object(_shpurdp_server_, "logger")
  @patch("shpurdp_server.serverConfiguration.get_shpurdp_properties")
  @patch.object(_shpurdp_server_, "setup_logging")
  @patch.object(_shpurdp_server_, "init_logging")
  ...
  def test_main_test_start_debug_short(
    ...
    try:
      sys.argv = ["shpurdp-server", "start", "-g"]
  ...
  def test_main_test_start_debug_short(
    ...
    try:
      sys.argv = ["shpurdp-server", "pstart", "-g"]
  ...
  def test_main_test_start_debug_long(
    ...
    try:
      sys.argv = ["shpurdp-server", "start", "--debug"]
  ...
  def test_main_test_start_debug_long(
    ...
    try:
      sys.argv = ["shpurdp-server", "pstart", "--debug"]
  ...
  @patch.object(_shpurdp_server_, "stop")
  @patch.object(_shpurdp_server_, "reset")
  @patch.object(_shpurdp_server_, "backup")
  @patch.object(_shpurdp_server_, "restore")
  @patch.object(_shpurdp_server_, "logger")
  @patch("shpurdp_server.serverConfiguration.get_shpurdp_properties")
  @patch.object(_shpurdp_server_, "setup_logging")
  @patch.object(_shpurdp_server_, "init_logging")
  def test_main_test_backup(
    ...
    tmp_argv = sys.argv
    try:
      sys.argv = ["shpurdp-server", "backup"]
  ...
  def test_main_test_restore(
    ...
    tmp_argv = sys.argv
    try:
      sys.argv = ["shpurdp-server", "restore"]
      _shpurdp_server_.mainBody()
  ...
  @patch.object(_shpurdp_server_, "setup")
  @patch.object(_shpurdp_server_, "start")
  @patch.object(_shpurdp_server_, "is_server_runing")
  @patch.object(_shpurdp_server_, "reset")
  ...
  @patch("shpurdp_server.dbConfiguration_linux.run_os_command")
  @patch.object(PGConfig, "_get_postgre_status")
  @patch("shpurdp_server.dbConfiguration_linux.print_info_msg")
  ...
  @patch("shpurdp_commons.os_linux.print_info_msg")
  ...
  @patch("shlex.split")
  @patch("subprocess.Popen")
  @patch("shpurdp_commons.os_windows.print_info_msg")
  def test_run_os_command(self, printInfoMsg_mock, popenMock, splitMock):
    p = MagicMock()
    p.communicate.return_value = (None, None)
    p.returncode = 3
  ...
  @not_for_platform(PLATFORM_WINDOWS)
  @patch("shpurdp_server.dbConfiguration.decrypt_password_for_alias")
  @patch("shpurdp_server.dbConfiguration_linux.run_os_command")
  ...
  @not_for_platform(PLATFORM_WINDOWS)
  @patch("shpurdp_server.serverSetup.get_YN_input")
  @patch("shpurdp_server.serverSetup.run_os_command")
  ...
  @not_for_platform(PLATFORM_WINDOWS)
  @patch("shpurdp_server.serverConfiguration.print_info_msg")
  def test_get_shpurdp_jars(self, printInfoMsg_mock):
    env = "/shpurdp/jars"
    os.environ[SHPURDP_SERVER_LIB] = env
    result = get_shpurdp_jars()
    self.assertEqual(env, result)

    del os.environ[SHPURDP_SERVER_LIB]
    result = get_shpurdp_jars()
    self.assertEqual("/usr/lib/shpurdp-server", result)
  ...
  def test_get_conf_dir(self, printInfoMsg_mock):
    env = "/dummy/shpurdp/conf"
    os.environ[SHPURDP_CONF_VAR] = env
    result = get_conf_dir()
    self.assertEqual(env, result)

    del os.environ[SHPURDP_CONF_VAR]
    result = get_conf_dir()
    self.assertEqual("/etc/shpurdp-server/conf", result)
    pass
  ...
  @patch("shpurdp_server.serverConfiguration.get_shpurdp_properties")
  @patch("shpurdp_server.serverConfiguration.Properties")
  def test_read_shpurdp_user(self, properties_mock, get_shpurdp_properties_mock):
    # Testing with defined user
    properties_mock.__getitem__.return_value = "dummy_user"
    get_shpurdp_properties_mock.return_value = properties_mock
  ...
  @patch("shpurdp_server.serverConfiguration.get_shpurdp_properties")
  @patch("shpurdp_server.serverConfiguration.Properties")
  def test_read_active_instance(self, properties_mock, get_shpurdp_properties_mock):
    # Set up the mock
  ...
  @patch("shpurdp_server.setupSecurity.get_file_owner")
  @patch("shpurdp_server.setupSecurity.get_shpurdp_repo_file_full_name")
  @patch("os.path.exists")
  @patch("shpurdp_server.setupSecurity.set_file_permissions")
  @patch("shpurdp_server.setupSecurity.get_shpurdp_properties")
  @patch("shpurdp_server.setupSecurity.get_resources_location")
  @patch("shpurdp_server.setupSecurity.get_value_from_properties")
  @patch("os.mkdir")
  @patch("shutil.rmtree")
  @patch("shpurdp_commons.os_utils.print_info_msg")
  @patch("shpurdp_server.setupSecurity.change_owner")
  def test_adjust_directory_permissions(
    ...
    exists_mock,
    get_shpurdp_repo_file_full_name_mock,
    get_file_owner_mock,
  ):
    ...
    try:
      configDefaults.NR_ADJUST_OWNERSHIP_LIST = [
        ("/etc/shpurdp-server/conf", "755", "{0}", True),
        ("/etc/shpurdp-server/conf/shpurdp.properties", "644", "{0}", False),
      ]

      configDefaults.NR_CHANGE_OWNERSHIP_LIST = [("/etc/shpurdp-server", "{0}", True)]
    ...
    #
    # Test shpurdp repo file permission change call
    #

    # Test the case when shpurdp repo file is available
    ...
    # Set up the mock function for os_utils.get_shpurdp_repo_file_full_name()
    get_shpurdp_repo_file_full_name_mock.return_value = "shpurdp.dummy.repo"

    # Set up the mock function for os_utils.get_file_owner()
    get_file_owner_mock.return_value = "dummy.root"

    # Set os.path.exists to return true when the input file is an shpurdp repo file
    def file_exists_side_effect(*args, **kwargs):
      if args[0] == get_shpurdp_repo_file_full_name_mock():
        return True
      else:
        return False
    ...
    try:
      ...
      # One of the entries in NR_ADJUST_OWNERSHIP_LIST should be the full path to the shpurdp repo file.
      # These are the expected values:

      shpurdp_repo_file_entry = (
        get_shpurdp_repo_file_full_name_mock(),
        "644",
        get_file_owner_mock(),
        False,
      ...
      for args_entry in set_file_permissions_mock.call_args_list:
        if args_entry[0][0] == shpurdp_repo_file_entry[0]:  # File name
          # shpurdp repo file name matched; assert the rest of the entries
          self.assertEqual(args_entry[0][1], shpurdp_repo_file_entry[1])  # Permissions
          self.assertEqual(args_entry[0][2], shpurdp_repo_file_entry[2])  # File owner
          self.assertEqual(args_entry[0][3], shpurdp_repo_file_entry[3])  # Non-recursive
          entry_found = True
          break

      # Ensure that the shpurdp repo file entry was found
    ...
    # Test the case when shpurdp repo file is unavailable
    ...
    try:
      ...
      # Ensure that the shpurdp repo file entry was not found
  ...
  @not_for_platform(PLATFORM_WINDOWS)
  @patch("os.path.exists")
  @patch("shpurdp_commons.os_linux.os_run_os_command")
  @patch("shpurdp_commons.os_linux.print_warning_msg")
  @patch("shpurdp_commons.os_utils.print_info_msg")
  ...
  @not_for_platform(PLATFORM_WINDOWS)
  @patch.object(OSCheck, "os_distribution", new=MagicMock(return_value=os_distro_value))
  @patch("shpurdp_server.serverSetup.get_validated_string_input")
  @patch("shpurdp_server.serverSetup.print_info_msg")
  @patch("shpurdp_server.serverSetup.print_warning_msg")
  @patch("shpurdp_server.serverSetup.run_os_command")
  def test_create_custom_user(
    ...
    userChecks = ShpurdpUserChecks(options)
  ...
  @not_for_platform(PLATFORM_WINDOWS)
  @patch.object(OSCheck, "os_distribution", new=MagicMock(return_value=os_distro_value))
  @patch("shpurdp_server.serverSetup.read_shpurdp_user")
  @patch("shpurdp_server.serverSetup.get_YN_input")
  @patch("shpurdp_server.serverSetup.get_validated_string_input")
  @patch("shpurdp_server.serverSetup.adjust_directory_permissions")
  @patch("shpurdp_server.serverSetup.run_os_command")
  @patch("shpurdp_server.serverSetup.print_error_msg")
  @patch("shpurdp_server.serverSetup.print_warning_msg")
  @patch("shpurdp_server.serverSetup.print_info_msg")
  def test_check_shpurdp_user(
    self,
    print_info_msg_mock,
    print_warning_msg_mock,
    ...
    read_shpurdp_user_mock.return_value = "dummy-user"
    get_YN_input_mock.return_value = False
    result = check_shpurdp_user(options)
    ...
    read_shpurdp_user_mock.return_value = None
    get_YN_input_mock.return_value = True
  ...
  @only_for_platform(PLATFORM_WINDOWS)
  @patch.object(OSCheck, "os_distribution", new=MagicMock(return_value=os_distro_value))
  @patch("shpurdp_commons.os_windows.UserHelper.add_user_privilege")
  @patch("shpurdp_commons.os_windows.UserHelper.create_user")
  @patch("shpurdp_commons.os_windows.UserHelper.find_user")
  @patch("shpurdp_server.serverSetup.read_shpurdp_user")
  @patch("shpurdp_server.serverSetup.get_YN_input")
  @patch("shpurdp_server.serverSetup.get_validated_string_input")
  @patch("shpurdp_server.serverSetup.adjust_directory_permissions")
  ...
  @patch("shpurdp_server.serverConfiguration.search_file")
  @patch("builtins.open")
  @patch("shpurdp_server.serverConfiguration.read_shpurdp_user")
  @patch("shpurdp_server.serverConfiguration.set_file_permissions")
  def test_store_password_file(
    self, set_file_permissions_mock, read_shpurdp_user_mock, open_mock, search_file_mock
  ):
    search_file_mock.return_value = "/etc/shpurdp-server/conf/shpurdp.properties"
  ...
  @patch("shpurdp_server.setupHttps.get_validated_filepath_input")
  @patch("shpurdp_server.setupHttps.get_validated_string_input")
  @patch("shpurdp_server.setupHttps.run_os_command")
  @patch("shpurdp_server.setupHttps.get_and_persist_truststore_type")
  @patch("builtins.open")
  @patch("shpurdp_server.setupHttps.find_properties_file")
  @patch("shpurdp_server.setupHttps.run_component_https_cmd")
  @patch("shpurdp_server.setupHttps.get_delete_cert_command")
  @patch("shpurdp_server.setupHttps.get_and_persist_truststore_password")
  @patch("shpurdp_server.setupHttps.get_and_persist_truststore_path")
  @patch("shpurdp_server.setupHttps.get_YN_input")
  @patch("shpurdp_server.setupHttps.get_shpurdp_properties")
  @patch("shpurdp_server.setupHttps.find_jdk")
  def test_setup_truststore(
    ...
    try:
      ...
      # Expected
      self.assertTrue(
        'No JDK found, please run the "shpurdp-server setup" command to install a'
  ...
  @patch("builtins.open")
  @patch("shpurdp_commons.logging_utils.get_silent")
  @patch("shpurdp_server.setupHttps.find_jdk")
  @patch("shpurdp_server.setupHttps.get_shpurdp_properties")
  @patch("shpurdp_server.setupHttps.get_YN_input")
  @patch("shpurdp_server.setupHttps.get_and_persist_truststore_type")
  ...
  @patch("shpurdp_server.setupHttps.run_os_command")
  @patch("shpurdp_server.setupHttps.get_validated_filepath_input")
  @patch("shpurdp_server.setupHttps.get_import_cert_command")
  @patch("shpurdp_server.setupHttps.run_component_https_cmd")
  def test_reconfigure_truststore(
    ...
    def reset_mocks():
      ...
      get_shpurdp_properties_mock.reset_mock()
      get_YN_input_mock.reset_mock()
      get_and_persist_truststore_type_mock.reset_mock()
      get_and_persist_truststore_path_mock.reset_mock()
      get_and_persist_truststore_password_mock.reset_mock()
  ...
  @patch("shpurdp_server.setupHttps.adjust_directory_permissions")
  @patch("shpurdp_server.setupHttps.read_shpurdp_user")
  @patch("shpurdp_server.setupHttps.get_validated_string_input")
  @patch("shpurdp_server.setupHttps.find_properties_file")
  @patch("shpurdp_server.setupHttps.get_shpurdp_properties")
  @patch("shpurdp_server.setupHttps.import_cert_and_key_action")
  @patch("shpurdp_server.setupHttps.get_YN_input")
  @patch("builtins.open")
  @patch("shpurdp_server.setupHttps.is_root")
  @patch("shpurdp_server.setupHttps.is_valid_cert_host")
  @patch("shpurdp_server.setupHttps.is_valid_cert_exp")
  def test_setup_https(
    ...
    read_shpurdp_user_method.return_value = "user"
  ...
  @patch("shpurdp_server.setupHttps.import_cert_and_key")
  ...
  @patch("shpurdp_server.setupHttps.remove_file")
  @patch("shpurdp_server.setupHttps.copy_file")
  @patch("shpurdp_server.setupHttps.read_shpurdp_user")
  @patch("shpurdp_server.setupHttps.set_file_permissions")
  @patch("shpurdp_server.setupHttps.import_file_to_keystore")
  @patch("builtins.open")
  @patch("shpurdp_server.setupHttps.run_os_command")
  @patch("os.path.join")
  ...
  @patch("shpurdp_server.setupHttps.is_valid_cert_host")
  @patch("shpurdp_server.setupHttps.is_valid_cert_exp")
  def test_shpurdpServerSetupWithCustomDbName(
    self,
    is_valid_cert_exp_mock,
    is_valid_cert_host_mock,
    get_validated_string_input_mock,
  ...
  @patch("shpurdp_server.setupHttps.remove_file")
  @patch("shpurdp_server.setupHttps.copy_file")
  @patch("shpurdp_server.setupHttps.generate_random_string")
  @patch("shpurdp_server.setupHttps.read_shpurdp_user")
  @patch("shpurdp_server.setupHttps.set_file_permissions")
  @patch("shpurdp_server.setupHttps.import_file_to_keystore")
  @patch("builtins.open")
  ...
  @patch("shpurdp_server.setupHttps.get_fqdn")
  ...
  @patch("shpurdp_server.setupHttps.get_shpurdp_properties")
  def test_is_valid_https_port(self, get_shpurdp_properties_mock):
    # No shpurdp.properties
    get_shpurdp_properties_mock.return_value = -1
    ...
    portTwoWay = "2222"
    validPort = "3333"
    get_shpurdp_properties_mock.return_value = {
  ...
  @patch("shpurdp_server.setupHttps.get_shpurdp_properties")
  def test_get_fqdn(self, get_shpurdp_properties_mock, url_open_mock, getfqdn_mock):
    # No shpurdp.properties
    get_shpurdp_properties_mock.return_value = -1
    fqdn = get_fqdn()
    self.assertEqual(fqdn, None)

    # Check mbari_server.GET_FQDN_SERVICE_URL property name (SHPURDP-2612)
    ...
    get_shpurdp_properties_mock.return_value = p
  ...
  @not_for_platform(PLATFORM_WINDOWS)
  @patch.object(OSCheck, "os_distribution", new=MagicMock(return_value=os_distro_value))
  @patch("shpurdp_server.serverUtils.run_os_command")
  @patch("builtins.open")
  @patch("os.path.exists")
  ...
  def test_is_server_runing_bad_file(
    ...
    open_mock.side_effect = IOError(
      "[Errno 13] Permission denied: /var/run/shpurdp-server/shpurdp-server.pid"
    )
  ...
  @patch("os.chdir")
  @patch("os.makedirs")
  @patch("shpurdp_server.serverSetup.JDKSetupLinux.adjust_jce_permissions")
  @patch("shpurdp_server.serverSetup.expand_jce_zip_file")
  @patch("shpurdp_server.serverSetup.force_download_file")
  @patch("shpurdp_server.serverSetup.get_YN_input")
  @patch("shpurdp_server.serverSetup.run_os_command")
  @patch("shpurdp_server.serverSetup.update_properties")
  @patch("shpurdp_server.serverSetup.get_validated_string_input")
  @patch("shpurdp_server.serverSetup.print_info_msg")
  @patch("shpurdp_server.serverSetup.validate_jdk")
  @patch("shpurdp_server.serverSetup.get_JAVA_HOME")
  @patch("shpurdp_server.serverSetup.get_resources_location")
  @patch("shpurdp_server.serverSetup.get_shpurdp_properties")
  @patch("shpurdp_server.serverSetup.check_shpurdp_java_version_is_valid")
  ...
  def test_download_jdk(
    ...
    exit_mock,
    copyfile_mock,
    check_shpurdp_java_version_is_valid_mock,
    get_shpurdp_properties_mock,
    get_resources_location_mock,
    get_JAVA_HOME_mock,
    validate_jdk_mock,
    ...
    read_shpurdp_user_mock.return_value = "shpurdp"
    get_shpurdp_properties_mock.return_value = p
    check_shpurdp_java_version_is_valid_mock.return_value = True
    # Test case: shpurdp.properties not found
    try:
      download_and_install_jdk(args)
      self.fail("Should throw exception because of not found shpurdp.properties")
    except FatalException:
      # Expected
      self.assertTrue(get_shpurdp_properties_mock.called)
      pass
    ...
    with patch(
      "shpurdp_server.serverSetup.JDKSetup._download_jce_policy"
    ...
    # Test case: Update JAVA_HOME location using command: shpurdp-server setup -j %NEW_LOCATION%
    ...
    # Test case: Setup shpurdp-server first time, Custom JDK selected, JDK exists
    args.java_home = None
    args.jdk_location = None
    validate_jdk_mock.return_value = False
    update_properties_mock.reset_mock()
    ...
    # Test case: Setup shpurdp-server first time, Custom JDK selected, JDK not exists
    update_properties_mock.reset_mock()
    validate_jdk_mock.return_value = False
    path_existsMock.reset_mock()
    path_existsMock.side_effect = pem_side_effect1
    ...
    # Test case: Setup shpurdp-server with java home passed. Path to java home doesn't exist
    args.java_home = "somewhere"
    validate_jdk_mock.return_value = False
    path_existsMock.reset_mock()
    path_existsMock.side_effect = pem_side_effect1
  ...
  @patch("shpurdp_commons.logging_utils.print_info_msg")
  @patch("shpurdp_commons.logging_utils.print_error_msg")
  @patch("shpurdp_server.serverSetup.get_shpurdp_properties")
  @patch("shpurdp_server.serverSetup.write_property")
  @patch("shpurdp_server.serverConfiguration.get_conf_dir")
  ...
  @patch.object(OSCheck, "os_distribution", new=MagicMock(return_value=os_distro_value))
  @patch("shpurdp_server.dbConfiguration.get_shpurdp_properties")
  def test_prompt_db_properties_default(self, get_shpurdp_properties_mock):
    args = MagicMock()
  ...
  @not_for_platform(PLATFORM_WINDOWS)
  @patch.object(subprocess, "Popen")
  def test_check_shpurdp_java_version_is_valid(self, popenMock):
    ...
    p.returncode = 0
    popenMock.return_value = p
    result = check_shpurdp_java_version_is_valid(
    ...
    p.returncode = 1
    try:
      check_shpurdp_java_version_is_valid("/usr/jdk64/jdk_1.8.0/", "java", 8, properties)
      self.fail("Should throw exception")
    except FatalException:
      # expected
      pass
  ...
  @patch("shpurdp_server.dbConfiguration_linux.print_info_msg")
  @patch("shpurdp_server.dbConfiguration_linux.read_password")
  @patch("shpurdp_server.dbConfiguration_linux.get_validated_string_input")
  @patch("shpurdp_server.dbConfiguration.get_validated_string_input")
  @patch("shpurdp_server.serverSetup.get_YN_input")
  def test_prompt_db_properties_oracle_sname(
    ...
    list_of_return_values = ["shpurdp-server", "shpurdp", "1", "1521", "localhost", "2"]
    ...
    self.assertEqual(dbmsConfig.database_name, "shpurdp")
    self.assertEqual(dbmsConfig.database_username, "shpurdp")
    ...
    self.assertEqual(dbmsConfig.database_username, "shpurdp-server")
  ...
  def test_prompt_db_properties_postgre_adv(
    ...
    gyni_mock.return_value = True
    list_of_return_values = ["shpurdp-server", "shpurdp", "2", "1521", "localhost", "2"]
    get_os_family_mock.return_value = OSConst.SUSE_FAMILY
    pw = MagicMock()
    pw.setattr("pw_uid", 0)
    pw.setattr("pw_gid", 0)
  ...
  def test_prompt_db_properties_postgre_adv(
    ...
    gyni_mock.return_value = True
    list_of_return_values = ["shpurdp-server", "postgres", "shpurdp", "shpurdp", "1"]
    get_os_family_mock.return_value = OSConst.SUSE_FAMILY
    pw = MagicMock()
    pw.setattr("pw_uid", 0)
    pw.setattr("pw_gid", 0)
  ...
  @not_for_platform(PLATFORM_WINDOWS)
  @patch.object(OSCheck, "os_distribution", new=MagicMock(return_value=os_distro_value))
  @patch("shpurdp_server.dbConfiguration_linux.store_password_file")
  @patch("shpurdp_server.dbConfiguration_linux.read_password")
  @patch("shpurdp_server.dbConfiguration_linux.get_validated_string_input")
  @patch("shpurdp_server.dbConfiguration_linux.get_YN_input")
  def test_prompt_db_properties_for_each_database_type(
    ...
    hostname = "localhost"
    db_name = "db_shpurdp"
    postgres_schema = "sc_shpurdp"
    port = "1234"
    local_admin_user = "postgres"
    oracle_service = "1"
    oracle_service_name = "shpurdp"
    user_name = "shpurdp"
    ...
    if SHPURDP_CONF_VAR in os.environ:
      del os.environ[SHPURDP_CONF_VAR]

    tempdir = tempfile.gettempdir()
    os.environ[SHPURDP_CONF_VAR] = tempdir
    prop_file = os.path.join(tempdir, "shpurdp.properties")

    for i in range(0, 5):
      # Use the expected path of the shpurdp.properties file to delete it if it exists, and then create a new one
      ...
      serverConfiguration.SHPURDP_PROPERTIES_FILE = prop_file
  ...
  @patch("glob.glob")
  @patch("shpurdp_server.serverConfiguration.get_JAVA_HOME")
  @patch("shpurdp_server.serverConfiguration.validate_jdk")
  ...
  @patch("os.path.split")
  @patch("os.listdir")
  @patch("shpurdp_server.serverSetup.copy_files")
  @patch("shutil.rmtree")
  ...
  @patch("shpurdp_server.serverSetup.get_shpurdp_properties")
  @patch("shpurdp_commons.os_utils.search_file")
  @patch("builtins.open")
  @patch("shpurdp_server.serverSetup.logger")
  def test_setup_jce_policy(
    ...
    get_shpurdp_properties_mock.return_value = properties
    conf_file = "etc/shpurdp-server/conf/shpurdp.properties"
  ...
  @patch("os.path.exists")
  @patch("os.path.isfile")
  @patch("shpurdp_commons.os_utils.remove_file")
  @patch(
    "shpurdp_server.dbConfiguration_linux.LinuxDBMSConfig.ensure_jdbc_driver_installed"
  )
  @patch("shpurdp_server.dbConfiguration_linux.get_YN_input")
  @patch("shpurdp_server.serverSetup.update_properties")
  @patch("shpurdp_server.dbConfiguration.get_shpurdp_properties")
  @patch("shpurdp_server.dbConfiguration_linux.get_shpurdp_properties")
  @patch("shpurdp_server.dbConfiguration_linux.store_password_file")
  @patch("shpurdp_server.dbConfiguration_linux.run_os_command")
  @patch("shpurdp_server.dbConfiguration_linux.PGConfig._configure_postgres")
  @patch("shpurdp_server.dbConfiguration_linux.PGConfig._check_postgre_up")
  @patch("shpurdp_server.dbConfiguration_linux.PGConfig._is_jdbc_user_changed")
  @patch("shpurdp_server.serverSetup.verify_setup_allowed")
  @patch("shpurdp_server.dbConfiguration_linux.read_password")
  @patch("shpurdp_server.dbConfiguration_linux.get_validated_string_input")
  @patch("shpurdp_server.dbConfiguration.get_validated_string_input")
  @patch("shpurdp_server.serverSetup.get_YN_input")
  ...
  @patch("shpurdp_server.serverSetup.check_shpurdp_user")
  @patch("shpurdp_server.serverSetup.check_jdbc_drivers")
  @patch("shpurdp_server.serverSetup.disable_security_enhancements")
  @patch("shpurdp_server.serverSetup.is_root")
  @patch("shpurdp_server.serverSetup.setup_jdbc")
  @patch("shpurdp_server.serverSetup.extract_views")
  @patch("shpurdp_server.serverSetup.adjust_directory_permissions")
  @patch("shpurdp_server.serverSetup.service_setup")
  @patch("shpurdp_server.serverSetup.read_shpurdp_user")
  @patch("shpurdp_server.serverSetup.expand_jce_zip_file")
  @patch("shpurdp_server.serverSetup.write_gpl_license_accepted")
  def test_setup_linux(
    ...
    get_shpurdp_properties_1_mock,
    get_shpurdp_properties_2_mock,
    update_properties_mock,
    get_YN_input_1_mock,
    ...
    get_shpurdp_properties_mock.return_value = properties
    get_shpurdp_properties_1_mock.return_value = properties
    get_shpurdp_properties_2_mock.return_value = properties
    ...
    def reset_mocks():
      ...
      check_shpurdp_user_mock.reset_mock()
      run_os_command_mock.reset_mock()
    ...
    try:
      ...
      self.fail("Should not throw exception, only print warning")  # see SHPURDP-15245
    ...
    self.assertEqual(None, result)
    self.assertTrue(check_shpurdp_user_mock.called)
    ...
    self.assertTrue(
      "Shpurdp-DDL-Postgres-CREATE.sql" in run_os_command_1_mock.call_args[0][0][3]
    )
    ...
    # test not run setup if shpurdp-server setup executed with jdbc properties
    args = reset_mocks()
    ...
    self.assertFalse(check_shpurdp_user_mock.called)
  ...
  @patch("shpurdp_commons.os_utils.remove_file")
  @patch(
    "shpurdp_server.dbConfiguration_windows.MSSQLConfig.ensure_jdbc_driver_installed"
  )
  @patch("shpurdp_server.serverSetup.update_properties")
  @patch("shpurdp_server.dbConfiguration_windows.store_password_file")
  @patch("shpurdp_server.dbConfiguration_windows.run_os_command")
  @patch("shpurdp_server.serverSetup.verify_setup_allowed")
  @patch("shpurdp_server.dbConfiguration_windows.get_validated_string_input")
  @patch("shpurdp_server.dbConfiguration.get_validated_string_input")
  @patch("shpurdp_server.serverSetup.get_YN_input")
  @patch("shpurdp_server.serverSetup.get_shpurdp_properties")
  @patch("shpurdp_server.serverSetup.configure_os_settings")
  @patch("shpurdp_server.serverSetup.download_and_install_jdk")
  @patch("shpurdp_server.serverSetup.check_firewall")
  @patch("shpurdp_server.serverSetup.check_shpurdp_user")
  @patch("shpurdp_server.serverSetup.check_jdbc_drivers")
  @patch("shpurdp_server.serverSetup.disable_security_enhancements")
  @patch("shpurdp_server.serverSetup.is_root")
  ...
  @patch("shpurdp_server.serverSetup.is_server_runing")
  @patch("shpurdp_server.dbConfiguration_linux.get_YN_input")
  @patch("shpurdp_server.serverSetup.get_YN_input")
  @patch.object(PGConfig, "_setup_db")
  @patch("shpurdp_server.dbConfiguration_linux.print_warning_msg")
  @patch("shpurdp_server.dbConfiguration_linux.print_info_msg")
  @patch("shpurdp_server.dbConfiguration_linux.run_os_command")
  @patch("shpurdp_server.dbConfiguration.decrypt_password_for_alias")
  @patch("shpurdp_server.serverSetup.get_shpurdp_properties")
  ...
  def test_reset(
    ...
    setup_db_mock.side_effect = [
      (0, None, None),
      (0, None, "ERROR: database 'shpurdp' is being accessed by other users"),
  ...
  @patch("shpurdp_server.serverSetup.is_server_runing")
  @patch("shpurdp_server.serverSetup.get_YN_input")
  @patch("shpurdp_server.dbConfiguration_windows.print_warning_msg")
  @patch("shpurdp_server.dbConfiguration_windows.print_info_msg")
  @patch("shpurdp_server.dbConfiguration_windows.run_os_command")
  @patch("shpurdp_server.dbConfiguration.decrypt_password_for_alias")
  @patch("shpurdp_server.serverSetup.get_shpurdp_properties")
  @patch("shpurdp_server.serverSetup.is_root")
  ...
  @only_for_platform(PLATFORM_WINDOWS)
  @patch.object(OSCheck, "os_distribution", new=MagicMock(return_value=os_distro_value))
  @patch("shpurdp_server.dbConfiguration_windows.MSSQLConfig._execute_db_script")
  @patch("shpurdp_server.serverSetup.get_shpurdp_properties")
  @patch("shpurdp_server.serverSetup.is_root")
  @patch("shpurdp_server.serverSetup.is_server_runing")
  def test_silent_reset(
  ...
  @patch.object(
    ServerClassPath,
    "get_full_shpurdp_classpath_escaped_for_shell",
    new=MagicMock(
      return_value="/etc/conf" + os.pathsep + "test" + os.pathsep + "path12"
    ),
  )
  @patch("shpurdp_server_main.get_is_active_instance")
  @patch.object(OSCheck, "os_distribution", new=MagicMock(return_value=os_distro_value))
  @patch("sys.stdout.flush")
  @patch("sys.stdout.write")
  @patch("shpurdp_server_main.looking_for_pid")
  @patch("shpurdp_server_main.wait_for_ui_start")
  @patch("shpurdp_server_main.save_main_pid_ex")
  @patch("shpurdp_server_main.check_exitcode")
  @patch("os.makedirs")
  @patch("shpurdp_server_main.locate_file")
  @patch.object(_shpurdp_server_, "is_server_runing")
  @patch("os.chown")
  @patch("shpurdp_server.setupSecurity.get_master_key_location")
  @patch("shpurdp_server.setupSecurity.save_master_key")
  @patch("shpurdp_server.setupSecurity.get_is_persisted")
  @patch("shpurdp_server.setupSecurity.get_is_secure")
  @patch("os.chmod", autospec=True)
  @patch("shpurdp_server.serverConfiguration.write_property")
  @patch("shpurdp_server.serverConfiguration.get_validated_string_input")
  @patch("os.environ")
  @patch("shpurdp_server.dbConfiguration.get_shpurdp_properties")
  @patch("shpurdp_server.setupSecurity.get_shpurdp_properties")
  @patch("shpurdp_server.serverSetup.get_shpurdp_properties")
  @patch("shpurdp_server.serverConfiguration.get_shpurdp_properties")
  @patch("shpurdp_server_main.get_shpurdp_properties")
  @patch("os.path.exists")
  @patch("builtins.open")
  @patch.object(subprocess, "Popen")
  @patch("shpurdp_server.serverConfiguration.search_file")
  @patch("shpurdp_server_main.check_database_name_property")
  @patch("shpurdp_server_main.find_jdk")
  @patch("shpurdp_server_main.print_warning_msg")
  @patch("shpurdp_server_main.print_info_msg")
  @patch.object(PGConfig, "_check_postgre_up")
  @patch("shpurdp_server_main.read_shpurdp_user")
  @patch("shpurdp_server.setupSecurity.is_root")
  @patch("shpurdp_server.dbConfiguration_linux.is_root")
  @patch("shpurdp_server_main.is_root")
  ...
  def test_start(
    ...
    get_shpurdp_properties_mock,
    get_shpurdp_properties_2_mock,
    get_shpurdp_properties_3_mock,
    get_shpurdp_properties_4_mock,
    get_shpurdp_properties_5_mock,
    os_environ_mock,
    ...
    p = Properties()
    p.process_pair(PID_DIR_PROPERTY, "/var/run/shpurdp-server")
    ...
    get_shpurdp_properties_5_mock.return_value = (
      get_shpurdp_properties_4_mock.return_value
    ) = get_shpurdp_properties_3_mock.return_value = (
      get_shpurdp_properties_2_mock.return_value
    ) = get_shpurdp_properties_mock.return_value = p
    ...
    try:
      _shpurdp_server_.start(args)
      self.fail("Should fail with 'Server is running'")
    except FatalException as e:
      # Expected
      self.assertTrue("Shpurdp Server is already running." in e.reason)
    ...
    # Checking situation when shpurdp user is not set up
    read_shpurdp_user_mock.return_value = None
    try:
      _shpurdp_server_.start(args)
      self.fail("Should fail with 'Can not detect a system user for Shpurdp'")
    except FatalException as e:
      # Expected
      self.assertTrue("Unable to detect a system user for Shpurdp Server." in e.reason)
    ...
    try:
      _shpurdp_server_.start(args)
      self.fail("Should fail with 'Can not start shpurdp-server as user...'")
    except FatalException as e:
      # Expected
      self.assertTrue("Unable to start Shpurdp Server as user" in e.reason)
    ...
    try:
      ...
      self.assertTrue("Before starting Shpurdp Server" in e.reason)
    ...
    read_shpurdp_user_mock.return_value = "root"
    ...
    try:
      ...
      self.assertTrue("Shpurdp Server java process died" in e.reason)
      self.assertTrue(perform_housekeeping_mock.called)
    ...
    read_shpurdp_user_mock.return_value = "not-root-user"
    _shpurdp_server_.start(args)
    ...
    read_shpurdp_user_mock.return_value = "not-root-user"
    getuser_mock.return_value = read_shpurdp_user_mock.return_value
    ...
    try:
      _shpurdp_server_.start(args)
      self.fail("Can not start shpurdp-server as user non_custom_user.")
    except FatalException as e:
      # Expected
      self.assertTrue("Unable to start Shpurdp Server as user" in e.reason)
  ...
  @patch.object(_shpurdp_server_, "BackupRestore_main")
  def test_backup(self, bkrestore_mock):
    args = ["", "/some/path/file.zip"]
    _shpurdp_server_.backup(args)
  ...
  @patch("shpurdp_server.serverUpgrade.ensure_jdbc_driver_is_installed")
  @patch("shpurdp_server.serverUpgrade.get_jdbc_driver_path")
  @patch("shpurdp_server.serverUpgrade.ensure_can_start_under_current_user")
  @patch("shpurdp_server.serverUpgrade.generate_env")
  @patch("shpurdp_server.serverUpgrade.read_shpurdp_user")
  @patch("shpurdp_server.serverConfiguration.get_conf_dir")
  @patch("shpurdp_server.serverUpgrade.run_os_command")
  @patch("shpurdp_server.serverUpgrade.get_java_exe_path")
  @patch("shpurdp_server.serverUpgrade.get_shpurdp_properties")
  @patch("shpurdp_server.serverUpgrade.get_YN_input")
  def test_run_schema_upgrade(
    ...
    command = (
      ...
      + os.pathsep
      + "/path/to/jdbc.jar "
      "org.apache.shpurdp.server.upgrade.SchemaUpgradeHelper "
      "> "
      + os.sep
      + "var"
      + os.sep
    ...
    self.assertTrue(read_shpurdp_user_mock.called)
    self.assertTrue(run_os_command_mock.called)
  ...
  def test_setup_jdbc(
    ...
    # test getShpurdpProperties failed
    args.jdbc_db = "mysql"
    get_shpurdp_properties_mock.return_value = -1
    fail = False
    ...
    try:
      ...
      self.assertEqual("Error getting shpurdp properties", e.reason)
      fail = True
    self.assertTrue(fail)
    ...
    get_shpurdp_properties_mock.return_value = MagicMock()
  ...
  def test_setup_jdbc(
    ...
    # test getShpurdpProperties succeeded
    args.jdbc_db = "mssql"
    get_shpurdp_properties_mock.return_value = -1
    fail = False
  ...
  @patch("shpurdp_server.serverUpgrade.adjust_directory_permissions")
  @patch("shpurdp_server.serverUpgrade.print_warning_msg")
  @patch("shpurdp_server.serverUpgrade.read_shpurdp_user")
  @patch("shpurdp_server.serverUpgrade.run_schema_upgrade")
  @patch("shpurdp_server.dbConfiguration_linux.run_os_command")
  @patch("shpurdp_server.serverConfiguration.find_properties_file")
  @patch("shpurdp_server.serverUpgrade.update_shpurdp_properties")
  @patch("shpurdp_server.serverUpgrade.is_root")
  @patch("shpurdp_server.serverConfiguration.write_property")
  @patch("shpurdp_server.serverConfiguration.get_shpurdp_version")
  @patch("shpurdp_server.dbConfiguration.get_shpurdp_properties")
  @patch("shpurdp_server.serverConfiguration.get_shpurdp_properties")
  @patch("shpurdp_server.serverUpgrade.get_shpurdp_properties")
  @patch("shpurdp_server.serverUpgrade.move_user_custom_actions")
  @patch("shpurdp_server.serverUpgrade.update_krb_jaas_login_properties")
  @patch("shpurdp_server.serverUpgrade.update_shpurdp_env")
  @patch("shpurdp_server.setupMpacks.get_replay_log_file")
  @patch("shpurdp_server.serverUpgrade.logger")
  ...
  def test_upgrade_from_161(
    ...
    update_shpurdp_env_mock,
    update_krb_jaas_login_properties_mock,
    move_user_custom_actions_mock,
    get_shpurdp_properties_mock,
    get_shpurdp_properties_2_mock,
    get_shpurdp_properties_3_mock,
    get_shpurdp_version_mock,
    write_property_mock,
    is_root_mock,
    update_shpurdp_properties_mock,
    find_properties_file_mock,
    run_os_command_mock,
    ...
    is_root_mock.return_value = True
    update_shpurdp_properties_mock.return_value = 0
    update_shpurdp_env_mock.return_value = 0
    get_shpurdp_version_mock.return_value = "1.7.0"
    move_user_custom_actions_mock.return_value = None
    update_krb_jaas_login_properties_mock.return_value = -2

    # Local Postgres
    # In Shpurdp 1.6.1 for an embedded postgres database, the "server.jdbc.database" property stored the DB name,
    # and the DB type was assumed to be "postgres" if the "server.persistence.type" property was "local"
    properties = Properties()
    properties.process_pair(PERSISTENCE_TYPE_PROPERTY, "local")
    properties.process_pair(JDBC_DATABASE_PROPERTY, "shpurdp")
    properties.process_pair(RESOURCES_DIR_PROPERTY, "/tmp")
    get_shpurdp_properties_mock.return_value = properties

    properties2 = Properties()
    properties2.process_pair(PERSISTENCE_TYPE_PROPERTY, "local")
    properties2.process_pair(JDBC_DATABASE_NAME_PROPERTY, "shpurdp")
    properties2.process_pair(JDBC_DATABASE_PROPERTY, "postgres")
    get_shpurdp_properties_3_mock.side_effect = (
      get_shpurdp_properties_2_mock.side_effect
    ...
    read_shpurdp_user_mock.return_value = "custom_user"
    ...
    try:
      ...
      self.assertEqual(write_property_mock.call_args_list[0][0][1], "shpurdp")
      self.assertEqual(
    ...
    # External Postgres
    # In Shpurdp 1.6.1 for an external postgres database, the "server.jdbc.database" property stored the
    ...
    properties.process_pair(JDBC_RCA_SCHEMA_PROPERTY, "shpurdp")
    properties.process_pair(
      JDBC_URL_PROPERTY, "jdbc:postgresql://c6410.shpurdp.apache.org:5432/shpurdp"
    )
    ...
    properties2.process_pair(JDBC_DATABASE_NAME_PROPERTY, "shpurdp")
    properties2.process_pair(JDBC_DATABASE_PROPERTY, "postgres")
    properties2.process_pair(JDBC_RCA_SCHEMA_PROPERTY, "shpurdp")
    ...
    # External MySQL
    # In Shpurdp 1.6.1 for an external MySQL database, the "server.jdbc.database" property stored the DB type ("mysql"),
    ...
    properties.process_pair(JDBC_RCA_SCHEMA_PROPERTY, "shpurdp")
    properties.process_pair(
      JDBC_URL_PROPERTY, "jdbc:mysql://c6409.shpurdp.apache.org:3306/shpurdp"
    )
    ...
    try:
      ...
      self.assertTrue(
        ...
        == "/var/lib/shpurdp-server/resources/mysql-connector-java.jar"
      )
      self.assertTrue(
        os_symlink_mock.call_args_list[0][0][1]
        == "/var/lib/shpurdp-server/resources/mysql-jdbc-driver.jar"
      )
  ...
  @patch("shpurdp_server.serverUpgrade.run_schema_upgrade")
  @patch("shpurdp_server.serverUpgrade.update_shpurdp_properties")
  @patch("shpurdp_server.serverUpgrade.parse_properties_file")
  @patch("shpurdp_server.serverUpgrade.get_shpurdp_version")
  @patch("shpurdp_server.serverConfiguration.get_shpurdp_version")
  @patch("shpurdp_server.serverUpgrade.is_root")
  @patch("shpurdp_server.dbConfiguration.get_shpurdp_properties")
  @patch("shpurdp_server.serverConfiguration.get_shpurdp_properties")
  ...
  def test_upgrade(
    ...
    run_schema_upgrade_mock.return_value = 0
    isfile_mock.return_value = False
    get_shpurdp_version_2_mock.return_value = get_shpurdp_version_mock.return_value = (
      CURR_SHPURDP_VERSION
    )
    ...
    self.assertTrue("custom shpurdp user" in warning_args)
    self.assertTrue(move_user_custom_actions.called)

    args = reset_mocks()

    # Testing with defined custom user
    read_shpurdp_user_mock.return_value = "shpurdp-custom-user"
    upgrade(args)
    self.assertTrue(adjust_directory_permissions_mock.called)
    ...
    self.assertTrue(get_shpurdp_properties_mock.called)
    self.assertTrue(get_shpurdp_properties_2_mock.called)
    ...
    # Assert that move_user_custom_actions is called on upgrade to Shpurdp == 2.0.0
    get_shpurdp_version_2_mock.return_value = get_shpurdp_version_mock.return_value = (
      "2.0.0"
    )
    move_user_custom_actions.called = False
    ...
    # Assert that move_user_custom_actions is not called on upgrade to Shpurdp < 2.0.0
    get_shpurdp_version_2_mock.return_value = get_shpurdp_version_mock.return_value = (
      "1.6.0"
    )
    move_user_custom_actions.called = False
    ...
    p = MagicMock()
    get_shpurdp_properties_mock.reset_mock()
    get_shpurdp_properties_2_mock.reset_mock()
    get_shpurdp_properties_3_mock.reset_mock()
    get_shpurdp_properties_3_mock.return_value = (
      get_shpurdp_properties_2_mock.return_value
    ) = get_shpurdp_properties_mock.return_value = p
    ...
    get_shpurdp_properties_3_mock.return_value = (
      get_shpurdp_properties_2_mock.return_value
    ) = get_shpurdp_properties_mock.return_value = props
    exists_mock.return_value = True
    lexists_mock.return_value = True
  ...
  @patch("shpurdp_server.userInput.get_choice_string_input")
  ...
  def test_main_db_options(self, setup_mock):
    base_args = ["shpurdp-server.py", "setup"]
    db_args = [
      "--database",
      "postgres",
      "--databasehost",
  ...
  @patch("shpurdp_server.serverConfiguration.get_conf_dir")
  def _test_update_shpurdp_properties(self, get_conf_dir_mock):
    from shpurdp_server import (
      serverConfiguration,
    )  # need to modify constants inside the module

    properties = [
      "server.jdbc.user.name=shpurdp-server\n",
      "server.jdbc.user.passwd=/etc/shpurdp-server/conf/password.dat\n",
      "java.home=/usr/jdk64/jdk1.6.0_31\n",
      "server.jdbc.database_name=shpurdp\n",
      "shpurdp-server.user=shpurdp\n",
    ...
    configDefaults.SHPURDP_PROPERTIES_BACKUP_FILE = fn1
    os.close(tf1)
    serverConfiguration.SHPURDP_PROPERTIES_FILE = fn2
    os.close(tf2)

    with open(serverConfiguration.SHPURDP_PROPERTIES_FILE, "w") as f:
      f.write(NEW_PROPERTY)
      f.write(CHANGED_VALUE_PROPERTY)
      f.write(JAVA_RELEASES_NEW_PROPERTY)
      f.close()

    with open(configDefaults.SHPURDP_PROPERTIES_BACKUP_FILE, "w") as f:
    ...
    self.assertFalse(os.path.exists(configDefaults.SHPURDP_PROPERTIES_BACKUP_FILE))
    # Renamed RPMSAVE_FILE exists
    self.assertTrue(
      os.path.exists(
        configDefaults.SHPURDP_PROPERTIES_BACKUP_FILE
        + "."
        + timestamp.strftime("%Y%m%d%H%M%S")
      )
    )

    with open(serverConfiguration.SHPURDP_PROPERTIES_FILE, "r") as f:
      shpurdp_properties_content = f.readlines()

    for line in properties:
      if line == "agent.fqdn.service.url=URL\n":
        if (not GET_FQDN_SERVICE_URL + "=URL\n" in shpurdp_properties_content) and (
          line in shpurdp_properties_content
        ):
          self.fail()
      elif line == "java.releases=jdk1.7,jdk1.6\n":
        if not "java.releases=jdk1.8,jdk1.7\n" in shpurdp_properties_content:
          self.fail()
      else:
        if not line in shpurdp_properties_content:
          self.fail()

    if not NEW_PROPERTY in shpurdp_properties_content:
      self.fail()

    if CHANGED_VALUE_PROPERTY in shpurdp_properties_content:
      self.fail()

    # Command should not fail if *.rpmsave file is missing
    result = update_shpurdp_properties()
    self.assertEqual(result, 0)

    os.unlink(fn2)

    # if shpurdp.properties file is absent then "shpurdp-server upgrade" should
    # fail
    (tf, fn) = tempfile.mkstemp()
    configDefaults.SHPURDP_PROPERTIES_BACKUP_FILE = fn

    result = update_shpurdp_properties()
    self.assertNotEqual(result, 0)
    pass

  @patch("shpurdp_server.properties.Properties.__init__")
  @patch("shpurdp_server.serverConfiguration.search_file")
  def test_update_shpurdp_properties_negative_case(
    self, search_file_mock, properties_mock
  ):
    search_file_mock.return_value = None
    # Call tested method
    self.assertEqual(0, update_shpurdp_properties())
  ...
  @patch("shpurdp_server.serverConfiguration.get_conf_dir")
  def _test_update_shpurdp_properties_without_some_properties(self, get_conf_dir_mock):
    """
    Checks: update_shpurdp_properties call should add shpurdp-server.user property if
    it's absent
    """
    from shpurdp_server import (
      serverConfiguration,
    ...
    (tf2, fn2) = tempfile.mkstemp()
    os.close(tf2)
    serverConfiguration.SHPURDP_PROPERTIES_RPMSAVE_FILE = fn1
    serverConfiguration.SHPURDP_PROPERTIES_FILE = fn2

    with open(serverConfiguration.SHPURDP_PROPERTIES_RPMSAVE_FILE, "w") as f:
      for line in properties:
        f.write(line)
    ...
    shpurdp_properties = Properties()
    shpurdp_properties.load(open(fn2))

    self.assertTrue(NR_USER_PROPERTY in shpurdp_properties.keys())
    value = shpurdp_properties[NR_USER_PROPERTY]
    self.assertEqual(value, "root")
    self.assertTrue(OS_FAMILY_PROPERTY in shpurdp_properties.keys())
  ...
  @patch("shpurdp_server.dbConfiguration_linux.get_YN_input")
  @patch("shpurdp_server.dbConfiguration_linux.get_validated_string_input")
  @patch("shpurdp_server.dbConfiguration_linux.PGConfig._store_remote_properties")
  @patch(
    "shpurdp_server.dbConfiguration_linux.LinuxDBMSConfig.ensure_jdbc_driver_installed"
  )
  @patch("shpurdp_server.dbConfiguration_linux.read_password")
  ...
  @patch("resource_management.core.shell.call")
  @patch("sys.exit")
  @patch("shpurdp_server.userInput.get_YN_input")
  @patch("shpurdp_commons.os_utils.is_root")
  @patch("shpurdp_server.dbConfiguration_linux.store_password_file")
  @patch("builtins.input")
  def test_store_remote_properties(
    ...
    args.database_name = "shpurdp"
    args.postgres_schema = "shpurdp"
  ...
  @not_for_platform(PLATFORM_WINDOWS)
  @patch("shpurdp_server.serverConfiguration.find_properties_file")
  def test_get_shpurdp_properties(self, find_properties_file_mock):
    find_properties_file_mock.return_value = None
    rcode = get_shpurdp_properties()
  ...
  @only_for_platform(PLATFORM_WINDOWS)
  @patch("shpurdp_server.serverConfiguration.find_properties_file")
  def test_get_shpurdp_properties(self, find_properties_file):
  ...
  @patch("os.path.exists")
  @patch("os.remove")
  @patch("shpurdp_commons.os_utils.print_warning_msg")
  ...
  @patch("shpurdp_server.dbConfiguration.get_shpurdp_properties")
  @patch("shpurdp_server.dbConfiguration_linux.get_shpurdp_properties")
  @patch("shpurdp_server.dbConfiguration_linux.print_error_msg")
  @patch("shpurdp_server.dbConfiguration.print_error_msg")
  @patch("shpurdp_server.dbConfiguration_linux.print_warning_msg")
  @patch("builtins.input")
  @patch("glob.glob")
  @patch("os.path.isdir")
  ...
  @patch("shutil.copy")
  @patch("os.path.isfile")
  @patch("shpurdp_server.dbConfiguration.get_YN_input")
  @patch("shpurdp_server.dbConfiguration.update_properties")
  @patch("shpurdp_server.dbConfiguration.get_validated_string_input")
  ...
  @not_for_platform(PLATFORM_WINDOWS)
  @patch("shpurdp_server.serverConfiguration.check_database_name_property")
  @patch("shpurdp_server.serverConfiguration.find_properties_file")
  ...
  def test_configure_database_username_password_masterkey_persisted(
    ...
    get_shpurdp_properties_method.return_value = properties
  ...
  @patch("shpurdp_server.serverUtils.is_server_runing")
  @patch("shpurdp_commons.os_utils.run_os_command")
  @patch("shpurdp_server.setupSecurity.generate_env")
  @patch("shpurdp_server.setupSecurity.ensure_can_start_under_current_user")
  @patch("shpurdp_server.serverConfiguration.read_shpurdp_user")
  @patch("shpurdp_server.dbConfiguration.ensure_jdbc_driver_is_installed")
  @patch("shpurdp_server.serverConfiguration.parse_properties_file")
  @patch("shpurdp_server.serverConfiguration.get_shpurdp_properties")
  @patch("shpurdp_server.serverConfiguration.get_java_exe_path")
  @patch("sys.exit")
  def test_check_database(
    self,
    exitMock,
    getJavaExePathMock,
    getShpurdpPropertiesMock,
    ...
    getShpurdpPropertiesMock.return_value = properties
    readShpurdpUserMock.return_value = "test_user"
    ...
    self.assertTrue(readShpurdpUserMock.called)
    ...
    self.assertEqual(
      ...
      "/path/to/java -cp test:path12 org.apache.shpurdp.server.checks.DatabaseConsistencyChecker",
    )
  ...
  def _init_test_ldap_properties_map_invalid_input_1():
    ldap_properties_map = {
      "shpurdp.ldap.connectivity.server.host": "a",
      "shpurdp.ldap.connectivity.server.port": "3",
      "shpurdp.ldap.connectivity.secondary.server.host": "b",
      "shpurdp.ldap.connectivity.secondary.server.port": "2",
      "shpurdp.ldap.connectivity.use_ssl": "false",
      "shpurdp.ldap.attributes.user.name_attr": "user",
      "shpurdp.ldap.attributes.user.search_base": "uid",
  ...
  def _init_test_ldap_properties_map_invalid_input_1():
    ldap_properties_map = {
      ...
      "shpurdp.ldap.connectivity.secondary.server.port": "2",
      "shpurdp.ldap.connectivity.use_ssl": "false",
      "shpurdp.ldap.attributes.user.object_class": "user",
      "shpurdp.ldap.attributes.user.name_attr": "uid",
      "shpurdp.ldap.attributes.user.group_member_attr": "memberof",
      "shpurdp.ldap.attributes.group.object_class": "group",
      "shpurdp.ldap.attributes.group.name_attr": "cn",
      "shpurdp.ldap.attributes.group.member_attr": "member",
      "shpurdp.ldap.attributes.dn_attr": "dn",
      "shpurdp.ldap.attributes.user.search_base": "base",
      "shpurdp.ldap.advanced.referrals": "follow",
      "shpurdp.ldap.connectivity.anonymous_bind": "true",
  ...
  @patch("urllib.request.urlopen")
  @patch("builtins.input")
  @patch("shpurdp_server.userInput.get_password")
  @patch("shpurdp_server.setupSecurity.get_is_secure")
  @patch("shpurdp_server.setupSecurity.get_YN_input")
  @patch("shpurdp_server.setupSecurity.update_properties_2")
  @patch("shpurdp_server.setupSecurity.search_file")
  @patch("shpurdp_server.setupSecurity.get_shpurdp_properties")
  @patch("shpurdp_server.setupSecurity.logger")
  @patch("shpurdp_server.setupSecurity.is_server_runing")
  @patch("shpurdp_server.setupSecurity.get_ldap_properties_from_db")
  def test_setup_ldap_invalid_input(
    ...
    properties_updated_in_shpurdp_db = sorted(
      ldapProperties.items(), key=operator.itemgetter(0)
    )
    properties_should_be_updated_in_shpurdp_db = sorted(
      TestShpurdpServer._init_test_ldap_properties_map_invalid_input_1().items(),
      key=operator.itemgetter(0),
    )
    self.assertEqual(
      properties_should_be_updated_in_shpurdp_db, properties_updated_in_shpurdp_db
    )

    properties_updated_in_shpurdp_dot_properties = sorted(
      update_properties_method.call_args[0][1].items(), key=operator.itemgetter(0)
    )
    properties_should_be_updated_in_shpurdp_dot_properties = sorted(
      {"client.security": "ldap"}.items(), key=operator.itemgetter(0)
    )
    self.assertEqual(
      properties_should_be_updated_in_shpurdp_dot_properties,
      properties_updated_in_shpurdp_dot_properties,
    )
    ...
    properties_should_be_updated_in_shpurdp_db = sorted(
      TestShpurdpServer._init_test_ldap_properties_map_invalid_input_2().items(),
      key=operator.itemgetter(0),
    )
    self.assertEqual(
      properties_should_be_updated_in_shpurdp_db, properties_updated_in_shpurdp_db
  ...
  @patch("shpurdp_server.setupSecurity.is_server_runing")
  def test_setup_ldap_should_fail_if_shpurdp_server_is_not_running(
    ...
    try:
      ...
      self.assertTrue("Shpurdp Server is not running." == fe.reason)
      pass
  ...
  def _init_test_ldap_properties_map():
    ldap_properties_map = {
      "shpurdp.ldap.connectivity.server.host": "test",
      "shpurdp.ldap.connectivity.server.port": "1",
      "shpurdp.ldap.connectivity.secondary.server.host": "test",
      "shpurdp.ldap.connectivity.secondary.server.port": "2",
      "shpurdp.ldap.connectivity.use_ssl": "false",
      "shpurdp.ldap.attributes.user.name_attr": "test",
      "shpurdp.ldap.attributes.user.search_base": "test",
      "shpurdp.ldap.connectivity.anonymous_bind": "false",
      "shpurdp.ldap.advanced.collision_behavior": "skip",
      "shpurdp.ldap.advanced.force_lowercase_usernames": "false",
      "shpurdp.ldap.advanced.pagination_enabled": "false",
      "shpurdp.ldap.connectivity.bind_dn": "test",
      "shpurdp.ldap.advanced.referrals": "test",
  ...
  def _init_test_ldap_properties_map():
    ldap_properties_map = {
      ...
      "shpurdp.ldap.connectivity.server.port": "1",
      "shpurdp.ldap.connectivity.secondary.server.host": "test",
      "shpurdp.ldap.connectivity.secondary.server.port": "1",
      "shpurdp.ldap.connectivity.use_ssl": "false",
      "shpurdp.ldap.attributes.user.object_class": "test",
      "shpurdp.ldap.attributes.user.name_attr": "test",
      "shpurdp.ldap.attributes.user.group_member_attr": "test",
      "shpurdp.ldap.attributes.user.search_base": "test",
      "shpurdp.ldap.connectivity.anonymous_bind": "false",
      "shpurdp.ldap.advanced.collision_behavior": "skip",
      "shpurdp.ldap.advanced.force_lowercase_usernames": "false",
      "shpurdp.ldap.advanced.pagination_enabled": "false",
      "shpurdp.ldap.connectivity.bind_dn": "test",
      "shpurdp.ldap.attributes.group.object_class": "test",
      "shpurdp.ldap.attributes.group.member_attr": "test",
      "shpurdp.ldap.attributes.group.name_attr": "test",
      "shpurdp.ldap.attributes.dn_attr": "test",
      "shpurdp.ldap.advanced.referrals": "test",
      LDAP_MGR_PASSWORD_PROPERTY: "dummyPassword",
      "shpurdp.ldap.authentication.enabled": "true",
      "shpurdp.ldap.manage_services": "true",
      "shpurdp.ldap.enabled_services": "ZOOKEEPER",
    }
    return ldap_properties_map
  ...
  @patch("urllib.request.urlopen")
  @patch("shpurdp_server.setupSecurity.get_is_secure")
  @patch("shpurdp_server.setupSecurity.encrypt_password")
  @patch("shpurdp_server.setupSecurity.save_passwd_for_alias")
  @patch("shpurdp_server.setupSecurity.get_YN_input")
  @patch("shpurdp_server.setupSecurity.update_properties_2")
  @patch("shpurdp_server.setupSecurity.configure_ldap_password")
  @patch("shpurdp_server.setupSecurity.get_validated_string_input")
  @patch("shpurdp_server.serverConfiguration.search_file")
  @patch("shpurdp_server.setupSecurity.get_shpurdp_properties")
  @patch("shpurdp_server.setupSecurity.read_password")
  @patch("os.path.exists")
  @patch("shpurdp_server.setupSecurity.logger")
  @patch("shpurdp_server.setupSecurity.is_server_runing")
  @patch("shpurdp_server.setupSecurity.query_ldap_type")
  @patch("shpurdp_server.setupSecurity.get_ldap_properties_from_db")
  @patch("shpurdp_server.setupSecurity.get_eligible_services")
  @patch("shpurdp_server.setupSecurity.get_cluster_name")
  def test_setup_ldap(
    self,
    ...
    options = self._create_empty_options_mock()
    options.shpurdp_admin_username = "admin"
    options.shpurdp_admin_password = "admin"
    is_server_runing_method.return_value = (True, 0)
    ...
    def yn_input_side_effect(*args, **kwargs):
      ...
      ):  # note that LDAP is enabled for HDFS (see below) but we say we do not want it to be managed by Shpurdp
        return False
      else:
        return True
    ...
    properties_should_be_updated_in_shpurdp_db = sorted(
      TestShpurdpServer._init_test_ldap_properties_map().items(),
      key=operator.itemgetter(0),
    )
    self.assertEqual(
      properties_should_be_updated_in_shpurdp_db, properties_updated_in_shpurdp_db
  ...
  def test_setup_ldap_primary_host_and_port_with_ldap_url_option(
    ...
    self.assertTrue(isinstance(requestData, dict))
    ldapProperties = requestData["Configuration"]["properties"]
    self.assertTrue("shpurdp.ldap.connectivity.server.host" in ldapProperties)
    self.assertTrue("shpurdp.ldap.connectivity.server.port" in ldapProperties)
    self.assertEqual(ldapProperties["shpurdp.ldap.connectivity.server.host"], "a")
    self.assertEqual(ldapProperties["shpurdp.ldap.connectivity.server.port"], "1")
  ...
  @patch("shpurdp_server.setupSecurity.is_server_runing")
  @patch("shpurdp_server.setupSecurity.get_ldap_properties_from_db")
  def test_setup_ldap_with_shpurdp_admin_username_and_password_options(
    self,
    get_ldap_properties_from_db_method,
    is_server_runing_method,
    get_shpurdp_properties_method,
  ...
  @patch("shpurdp_server.setupSecurity.get_validated_string_input")
  @patch("shpurdp_server.setupSecurity.get_shpurdp_properties")
  @patch("shpurdp_server.setupSecurity.is_ldap_enabled")
  @patch("shpurdp_server.setupSecurity.is_server_runing")
  @patch("shpurdp_server.setupSecurity.is_root")
  @patch("shpurdp_server.setupSecurity.logger")
  def test_ldap_sync_all(
    ...
    response.read.side_effect = [
      '{"resources" : [{"href" : "http://c6401.shpurdp.apache.org:8080/api/v1/ldap_sync_events/16","Event" : {"id" : 16}}]}',
  ...
  def test_ldap_sync_ssl(
    ...
    response.getcode.side_effect = [201, 200, 200]
    response.read.side_effect = [
      '{"resources" : [{"href" : "https://c6401.shpurdp.apache.org:8443/api/v1/ldap_sync_events/16","Event" : {"id" : 16}}]}',
      '{"Event":{"status" : "RUNNING","summary" : {"groups" : {"created" : 0,"removed" : 0,"updated" : 0},"memberships" : {"created" : 0,"removed" : 0},"users" : {"created" : 0,"removed" : 0,"updated" : 0}}}}',
      '{"Event":{"status" : "COMPLETE","summary" : {"groups" : {"created" : 1,"removed" : 0,"updated" : 0},"memberships" : {"created" : 5,"removed" : 0},"users" : {"created" : 5,"removed" : 0,"updated" : 0}}}}',
    ]
  ...
  @patch("shpurdp_server.setupSecurity.is_server_runing")
  @patch("shpurdp_server.setupSecurity.logger")
  def test_sync_ldap_shpurdp_stopped(
    ...
    try:
      sync_ldap(options)
      self.fail("Should throw exception if shpurdp is stopped")
    except FatalException as fe:
      # Expected
  ...
  @patch("shpurdp_server.userInput.get_validated_string_input")
  ...
  @patch("builtins.open")
  @patch("shpurdp_server.serverConfiguration.search_file")
  @patch("shpurdp_server.serverConfiguration.backup_file_in_temp")
  def test_update_properties_2(
    self, backup_file_in_temp_mock, search_file_mock, open_mock
  ):
    conf_file = "shpurdp.properties"
    propertyMap = {"1": "1", "2": "2"}
    properties = MagicMock()
  ...
  @not_for_platform(PLATFORM_WINDOWS)
  @patch("shpurdp_server.dbConfiguration_linux.get_shpurdp_properties")
  def test_is_jdbc_user_changed(self, get_shpurdp_properties_mock):
  ...
  def test_check_database_name_property(
    self, get_shpurdp_version_mock, get_shpurdp_properties_mock, write_property_mock
  ):
    # negative case
    get_shpurdp_properties_mock.return_value = {JDBC_DATABASE_NAME_PROPERTY: ""}
    try:
    ...
    dbname = "shpurdp"
    get_shpurdp_properties_mock.reset_mock()
    get_shpurdp_properties_mock.return_value = {JDBC_DATABASE_NAME_PROPERTY: dbname}
    try:
      result = check_database_name_property()
    except FatalException:
      self.fail("Setup should be successful")

    # Check upgrade. In Shpurdp < 1.7.1 "database" property contained db name for local db
    dbname = "shpurdp"
    database = "shpurdp"
    persistence = "local"
    get_shpurdp_properties_mock.reset_mock()
  ...
  @patch("shpurdp_server.serverSetup.check_shpurdp_user")
  @patch("shpurdp_server.serverSetup.check_jdbc_drivers")
  @patch("shpurdp_server.serverSetup.check_selinux")
  @patch("shpurdp_server.serverSetup.is_root")
  @patch.object(PGConfig, "_setup_db")
  @patch("shpurdp_server.serverSetup.get_is_secure")
  @patch("shpurdp_server.dbConfiguration_linux.store_password_file")
  @patch("shpurdp_server.serverSetup.extract_views")
  @patch("shpurdp_server.serverSetup.adjust_directory_permissions")
  @patch("sys.exit")
  ...
  @patch("shpurdp_server.dbConfiguration_windows.MSSQLConfig._execute_db_script")
  @patch("shpurdp_server.dbConfiguration_windows.store_password_file")
  @patch("shpurdp_server.dbConfiguration_windows.MSSQLConfig._is_jdbc_driver_installed")
  @patch("shpurdp_server.serverSetup.verify_setup_allowed")
  @patch("shpurdp_server.serverSetup.get_YN_input")
  @patch("shpurdp_server.serverSetup.configure_os_settings")
  @patch("shpurdp_server.serverSetup.download_and_install_jdk")
  ...
  @patch("shpurdp_server.setupSecurity.search_file")
  @patch("shpurdp_server.setupSecurity.get_validated_string_input")
  def test_setup_shpurdp_krb5_jaas_with_options(
    ...
    self.assertEqual(None, setup_shpurdp_krb5_jaas(options))
    self.assertTrue(get_validated_string_input_mock.called)
    self.assertEqual(get_validated_string_input_mock.call_count, 2)
    get_validated_string_input_mock.assert_called_with(
      "Enter keytab path for shpurdp server's kerberos principal: ",
      "/etc/security/keytabs/shpurdp.keytab",
      ".*",
      False,
      False,
  ...
  def test_change_tables_owner_no_tables(
    ...
    self.assertEqual(
      ...
      "Failed to get list of shpurdp tables",
    )
  ...
  def test_change_tables_owner_fatal_psql(
    ...
    self.assertEqual(
      print_error_msg_mock.call_args_list[0][0][0],
      """Failed to get list of shpurdp tables. Message from psql:
 stdout:
...
""",
  ...
  @patch("sys.exit")
  @patch("shpurdp_server.userInput.get_YN_input")
  @patch("shpurdp_server.hostUpdate.logger")
  def test_update_host_names(
    ...
    self.assertEqual(
      ...
      "org.apache.shpurdp.server.update.HostUpdateHelper /testFileWithChanges > "
      "/var/log/shpurdp-server/shpurdp-server.out 2>&1",
    )
  ...
  def test_main_test_status_running(
    ...
    tmp_argv = sys.argv
    try:
      sys.argv = ["shpurdp-server", "status"]
  ...
  def test_status_extra_option(
    self, init_logging_mock, setup_logging_mock, get_shpurdp_properties_mock, logger_mock
  ):
    import sys

    tmp_argv = sys.argv
    try:
      sys.argv = ["shpurdp-server", "status", "--skip-database-check"]
      flag = False
      try:
        _shpurdp_server_.mainBody()
      except SystemExit as e:
  ...
  def test_web_server_startup_timeout(self):
    from shpurdp_server.serverConfiguration import get_web_server_startup_timeout
    from shpurdp_server.serverConfiguration import WEB_SERVER_STARTUP_TIMEOUT
  ...
  def _create_empty_options_mock(self):
    ...
    options.ldap_enabled_shpurdp = None
    ...
    options.shpurdp_admin_username = None
    options.shpurdp_admin_password = None
