...
class TestSetupAgent(TestCase):
  ...
  def test_runAgent(self, sleep_mock, environ_mock, run_os_command_mock):
    ...
    self.assertEqual(cmdStr, str((["cmd", "/c", "shpurdp-agent.cmd", "restart"],)))
    self.assertFalse("-v" in cmdStr)
    self.assertEqual(ret["exitstatus"], 0)
  ...
  @patch("shpurdp_commons.OSCheck.is_suse_family")
  @patch("shpurdp_commons.OSCheck.is_ubuntu_family")
  ...
  @patch.object(setup_agent, "get_shpurdp_repo_file_full_name")
  @patch.object(OSCheck, "os_distribution", new=MagicMock(return_value=os_distro_value))
  ...
  def test_setup_agent_main(
    ...
    # A version of shpurdp-agent was installed or available in the repo file
    ...
    get_shpurdp_repo_file_full_name_mock.return_value = "Shpurdp_Repo_File"
    ...
    exit_mock.reset_mock()
    # if "yum -y install --nogpgcheck shpurdp-agent" return not 0 result
    ...
    is_suse_family_mock.return_value = True
    is_ubuntu_family_mock.return_value = False
    # if "zypper install -y shpurdp-agent" return not 0 result
    installAgent_mock.return_value = {"log": "log", "exitstatus": 1}
    ret = setup_agent.main(
      ("setupAgent.py", "agents_host", "password", "server_hostname", "1.1.1", "8080")
    )
    ...
    # A version of shpurdp-agent was installed and shpurdp repo file was not found
    exit_mock.reset_mock()
    installAgent_mock.reset_mock()
    getOptimalVersion_mock.reset_mock()
    os_path_exists_mock.side_effect = None
    os_path_exists_mock.return_value = False  # shpurdp repo file was not found
    getOptimalVersion_mock.return_value = {"log": "1.1.1", "exitstatus": 0}
    isAgentPackageAlreadyInstalled_mock.return_value = False
    is_suse_family_mock.return_value = True
    is_ubuntu_family_mock.return_value = False
    ...
    )  # shpurdp repo file is missing, so installAgent() will not be called
    self.assertFalse(is_suse_family_mock.called)
    self.assertFalse(is_ubuntu_family_mock.called)
    self.assertTrue("exitstatus" in ret)
    self.assertEqual(ret["exitstatus"], 2)  # shpurdp repo file not found error code
    self.assertTrue(
      get_shpurdp_repo_file_full_name_mock() in ret["log"]
    )  # shpurdp repo file not found message

    # shpurdp-agent was not installed and shpurdp repo file was not found
    exit_mock.reset_mock()
    os_path_exists_mock.side_effect = None
    os_path_exists_mock.return_value = False  # shpurdp repo file not found
    getOptimalVersion_mock.reset_mock()
    isAgentPackageAlreadyInstalled_mock.reset_mock()
    is_suse_family_mock.reset_mock()
    is_ubuntu_family_mock.reset_mock()
