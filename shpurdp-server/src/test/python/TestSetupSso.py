            ...
            with patch.object(os_utils, "is_service_exist", return_value=True):
              with patch("glob.glob", return_value=["/etc/init.d/postgresql-9.3"]):
                ...
                with patch("builtins.open"):
                  ...
                  from shpurdp_server.setupSso import (
                    setup_sso,
                    SHPURDP_SSO_AUTH_ENABLED,
                    SSO_PROVIDER_URL,
                    SSO_CERTIFICATE,
                    JWT_COOKIE_NAME,
                    JWT_AUDIENCES,
...
class TestSetupSso(unittest.TestCase):
  @patch("shpurdp_server.setupSso.is_server_runing")
  ...
  @patch("shpurdp_server.setupSso.get_silent")
  @patch("shpurdp_server.setupSso.is_server_runing")
  ...
  def test_invalid_sso_provider_url_cli_option_when_enabling_sso_should_result_in_error(
    ...
    options.sso_provider_url = "The SSO provider URL is https://c7402.shpurdp.apache.org:8443/gateway/knoxsso/api/v1/websso"
    try:
      setup_sso(options)
      self.fail("Should fail with fatal exception")
    except FatalException as e:
    ...
    options.sso_provider_url = "https://c7402.shpurdp.apache.org:8443/gateway/knoxsso/api/v1/websso is the SSO provider URL"
    try:
      setup_sso(options)
      self.fail("Should fail with fatal exception")
    except FatalException as e:
  ...
  @patch("shpurdp_server.setupSso.perform_changes_via_rest_api")
  @patch("shpurdp_server.setupSso.get_shpurdp_properties")
  @patch("shpurdp_server.setupSso.get_silent")
  @patch("shpurdp_server.setupSso.is_server_runing")
  @patch("shpurdp_server.setupSso.get_json_via_rest_api")
  @patch("builtins.open")
  def test_all_cli_options_are_collected_when_enabling_sso(
    ...
    sso_enabled = "true"
    sso_enabled_services = "Shpurdp, SERVICE1, SERVICE2"
    sso_provider_url = (
      "https://c7402.shpurdp.apache.org:8443/gateway/knoxsso/api/v1/websso"
    )
    ...
    options.sso_enabled_shpurdp = "true"
    ...
    self.assertEqual(ssoProperties[SHPURDP_SSO_AUTH_ENABLED], sso_enabled)
  ...
  @patch("shpurdp_server.setupSso.perform_changes_via_rest_api")
  @patch("shpurdp_server.setupSso.get_YN_input")
  @patch("shpurdp_server.setupSso.get_shpurdp_properties")
  @patch("shpurdp_server.setupSso.get_silent")
  @patch("shpurdp_server.setupSso.is_server_runing")
  @patch("shpurdp_server.setupSso.get_json_via_rest_api")
  ...
  @patch("shpurdp_server.setupSso.get_cluster_name")
  @patch("shpurdp_server.setupSso.get_YN_input")
  @patch("shpurdp_server.setupSso.get_shpurdp_properties")
  @patch("shpurdp_server.setupSso.get_silent")
  @patch("shpurdp_server.setupSso.is_server_runing")
  ...
  def test_setup_sso_should_not_fail_when_sso_config_cannot_be_loaded_due_to_404_error(
    ...
    options = self._create_empty_options_mock()
    options.sso_enabled = sso_enabled
    options.sso_enabled_shpurdp = sso_enabled
    options.sso_manage_services = "true"
    options.sso_provider_url = sso_provider_url
    options.sso_public_cert_file = sso_public_cert_file
    options.sso_jwt_cookie_name = sso_jwt_cookie_name
  ...
  def _create_empty_options_mock(self):
    ...
    options.sso_enabled_shpurdp = None
