...
utils = __import__("shpurdp_server.utils").utils
        ...
        with patch.object(OSCheck, "os_distribution", return_value=os_distro_value):
          with patch.object(os_utils, "is_service_exist", return_value=True):
            with patch.object(utils, "get_postgre_hba_dir"):
              os.environ["ROOT"] = ""
              shpurdp_server = __import__("shpurdp-server")
...
class TestOSCheck(TestCase):
  ...
  @patch("shpurdp_commons.os_check._is_oracle_linux")
  ...
  def _test_update_shpurdp_properties_os(self, get_conf_dir_mock):
    ...
    f = open(configDefaults.SHPURDP_PROPERTIES_BACKUP_FILE, "w")
    try:
    ...
    f = open(serverConfiguration.SHPURDP_PROPERTIES_FILE, "r")
    try:
    ...
    count = 0
    for line in shpurdp_properties_content:
