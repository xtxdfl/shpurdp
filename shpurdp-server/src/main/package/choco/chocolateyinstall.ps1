...
$arguments = @{}
$shpurdpRoot = "C:\shpurdp"
$retries = 5
$lockTimeout = 60000

# Parse the packageParameters
#   /ShpurdpRoot:C:\shpurdp /Retries:5
if ($packageParameters) {
  ...
  if ($arguments.ContainsKey("ShpurdpRoot")) {
    Write-Debug "ShpurdpRoot Argument Found"
    $shpurdpRoot = $arguments["ShpurdpRoot"]
  }
...
$specificFolder = ""
$link = "$shpurdpRoot\$packageName"
$target = "$shpurdpRoot\$packageName-$packageVersion"
...
if($serverMutex.WaitOne($lockTimeout)) {
  try {
    Retry-Command -Command "Get-ChocolateyUnzip" -Arguments @{ FileFullPath = $zipFile; Destination = $shpurdpRoot; SpecificFolder = $specificFolder; PackageName = $packageName} -Retries $retries
