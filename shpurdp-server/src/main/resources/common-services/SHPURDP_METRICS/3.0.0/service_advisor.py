...
class SHPURDP_METRICSServiceAdvisor(service_advisor.ServiceAdvisor):
  def __init__(self, *args, **kwargs):
    self.as_super = super(SHPURDP_METRICSServiceAdvisor, self)
  ...
  def getServiceComponentLayoutValidations(self, services, hosts):
    ...
    return self.getServiceComponentCardinalityValidations(
      services, hosts, "SHPURDP_METRICS"
    )
  ...
  def getAmsMemoryRecommendation(self, services, hosts):
    ...
    return (
      self.round_to_n(collector_heapsize),
      self.round_to_n(hbase_heapsize),
      total_sinks_count,
    )
  ...
  def getServiceConfigurationRecommendations(
    ...
    recommender = SHPURDP_METRICSRecommender()
  ...
  def getServiceConfigurationsValidationItems(
    ...
    # validator = self.getSHPURDP_METRICSValidator()
    # Calls the methods of the validator using arguments,
    # method(siteProperties, siteRecommendations, configurations, services, hosts)
    # return validator.validateListOfConfigUsingMethod(configurations, recommendedDefaults, services, hosts, validator.validators)
    return []

  def getSHPURDP_METRICSValidator(self):
    return SHPURDP_METRICSValidator()


class SHPURDP_METRICSRecommender(service_advisor.ServiceAdvisor):
  """
  AMS Recommender suggests properties when adding the service for the first time or modifying configs via the UI.
  """

  def __init__(self, *args, **kwargs):
    self.as_super = super(SHPURDP_METRICSRecommender, self)
    self.as_super.__init__(*args, **kwargs)
  ...
  def getPreferredMountPoints(self, hostInfo):
    ...
    if hostInfo and "disk_info" in hostInfo:
      ...
      for mountpoint in hostInfo["disk_info"]:
        if not (
          mountpoint["mountpoint"] in undesirableMountPoints
          or mountpoint["mountpoint"].startswith(("/boot", "/mnt"))
  ...
  def recommendAmsConfigurationsFromHDP206(
    ...
    putAmsHbaseSiteProperty = self.putProperty(
      configurations, "ams-hbase-site", services
    )
    ...
    amsCollectorHosts = self.getComponentHostNames(
      services, "SHPURDP_METRICS", "METRICS_COLLECTOR"
    )

    serviceAdvisor = SHPURDP_METRICSServiceAdvisor()
    ...
    log_dir = "/var/log/shpurdp-metrics-collector"
    if "ams-env" in services["configurations"]:
      if (
        "metrics_collector_log_dir"
        in services["configurations"]["ams-env"]["properties"]
      ):
    ...
    if (
      ...
      and "fs.defaultFS" in services["configurations"]["core-site"]["properties"]
    ):
    ...
    if "ams-site" in services["configurations"]:
      if (
        ...
        in services["configurations"]["ams-site"]["properties"]
      ):
        operatingMode = services["configurations"]["ams-site"]["properties"][
          "timeline.metrics.service.operation.mode"
        ]
    ...
    rootDir = "file:///var/lib/shpurdp-metrics-collector/hbase"
    tmpDir = "/var/lib/shpurdp-metrics-collector/hbase-tmp"
    zk_port_default = []
    if "ams-hbase-site" in services["configurations"]:
      ...
      if "hbase.tmp.dir" in services["configurations"]["ams-hbase-site"]["properties"]:
        tmpDir = services["configurations"]["ams-hbase-site"]["properties"][
          "hbase.tmp.dir"
        ]
      if (
        "hbase.zookeeper.property.clientPort"
        in services["configurations"]["ams-hbase-site"]["properties"]
      ):
    ...
    if operatingMode == "embedded":
      if isLocalRootDir:
        ...
        putAmsHbaseSiteProperty(
          "hbase.rootdir", "file:///var/lib/shpurdp-metrics-collector/hbase"
        )
    ...
    if operatingMode == "distributed":
      ...
      putHbaseEnvProperty(
        "regionserver_xmn_size", serviceAdvisor.round_to_n(0.15 * hbase_heapsize, 64)
      )
    else:
      ...
      putHbaseEnvProperty(
        ...
        serviceAdvisor.round_to_n(0.15 * (hbase_heapsize + hbase_rs_heapsize), 64),
      )
...
class SHPURDP_METRICSValidator(service_advisor.ServiceAdvisor):
  """
  ...
  def __init__(self, *args, **kwargs):
    self.as_super = super(SHPURDP_METRICSValidator, self)
    self.as_super.__init__(*args, **kwargs)
  ...
  def validateAmsHbaseSiteConfigurationsFromHDP206(
    ...
    # TODO validate configuration for multiple SHPURDP_METRICS collectors
    if len(amsCollectorHosts) > 1:
      pass
    else:
      if total_sinks_count > 2000:
    ...
    validationItems.extend(
      [
        {"config-name": "hbase.rootdir", "item": rootdir_item},
        {"config-name": "hbase.cluster.distributed", "item": distributed_item},
        {
          "config-name": "hbase.zookeeper.property.clientPort",
          "item": hbase_zk_client_port_item,
        },
      ]
    )
      ...
      for host in hosts["items"]:
        if host["Hosts"]["host_name"] == collectorHostName:
          if op_mode == "embedded" or is_local_root_dir:
              ...
              [
                {
                  ...
                  "item": self.validatorEnoughDiskSpace(
                    properties, "hbase.rootdir", host["Hosts"], recommendedDiskSpace
                  ),
                }
              ]
            )
            validationItems.extend(
              [
                {
                  "config-name": "hbase.rootdir",
                  "item": self.validatorNotRootFs(
                    properties, recommendedDefaults, "hbase.rootdir", host["Hosts"]
                  ),
                }
              ]
            )
              ...
              [
                {
                  ...
                  "item": self.validatorNotRootFs(
                    properties, recommendedDefaults, "hbase.tmp.dir", host["Hosts"]
                  ),
                }
              ]
            )
          ...
          if is_local_root_dir:
            ...
            if (
              hbase_rootdir_mountpoint == hbase_tmpdir_mountpoint
              and len(preferred_mountpoints) > 1
            ):
              item = self.getWarnItem(
                "Consider not using {0} partition for storing metrics temporary data. "
                "{0} partition is already used as hbase.rootdir to store metrics data".format(
                  hbase_tmpdir_mountpoint
                )
              )
            ...
            dfs_datadirs = (
              hdfs_site.get("dfs.datanode.data.dir").split(",")
              if hdfs_site and "dfs.datanode.data.dir" in hdfs_site
              else []
            )
            if (
              dn_hosts
              and collectorHostName in dn_hosts
              and ams_site
              and dfs_datadirs
              and len(preferred_mountpoints) > len(dfs_datadirs)
            ):
              for dfs_datadir in dfs_datadirs:
                ...
                if dfs_datadir_mountpoint == hbase_rootdir_mountpoint:
                  item = self.getWarnItem(
                    ...
                    "{0} is already used by datanode to store HDFS data".format(
                      hbase_rootdir_mountpoint
                    )
                  )
                  validationItems.extend(
                    [{"config-name": "hbase.rootdir", "item": item}]
                  )
                  break
          # If no local DN in distributed mode
            ...
            validationItems.extend(
              [{"config-name": "hbase.cluster.distributed", "item": item}]
            )
          ...
          # if local DN installed
            ...
            validationItems.extend(
              [
                {
                  "config-name": "dfs.client.read.shortcircuit",
                  "item": self.validatorEqualsToRecommendedItem(
                    properties, recommendedDefaults, "dfs.client.read.shortcircuit"
                  ),
                }
              ]
            )
    ...
    if is_hbase_distributed:
      ...
      if hbase_regionserver_xmn_size < minRegionServerXmn:
        regionServerXmnItem = self.getWarnItem(
          "Value is lesser than the recommended minimum Xmn size of {0} "
          "(12% of hbase_regionserver_heapsize)".format(int(ceil(minRegionServerXmn)))
        )

      if hbase_regionserver_xmn_size > maxRegionServerXmn:
        regionServerXmnItem = self.getWarnItem(
          "Value is greater than the recommended maximum Xmn size of {0} "
          "(20% of hbase_regionserver_heapsize)".format(int(floor(maxRegionServerXmn)))
        )
    else:
      if (
        not hbaseMasterHeapsizeItem
        and (hbase_master_heapsize + hbase_regionserver_heapsize) > 32768
      ):
      ...
      if hbase_master_xmn_size < minMasterXmn:
        masterXmnItem = self.getWarnItem(
          "Value is lesser than the recommended minimum Xmn size of {0} "
          "(12% of hbase_master_heapsize + hbase_regionserver_heapsize)".format(
      ...
      if hbase_master_xmn_size > maxMasterXmn:
        masterXmnItem = self.getWarnItem(
          "Value is greater than the recommended maximum Xmn size of {0} "
          "(20% of hbase_master_heapsize + hbase_regionserver_heapsize)".format(
        ...
        for host in hosts["items"]:
          if host["Hosts"]["host_name"] == collectorHostName:
            ...
            if (
              ...
              masterHostMessage = (
                "Host {0} is used by multiple master components ({1}). "
                "It is recommended to use a separate host for the "
                "Shpurdp Metrics Collector component and ensure "
              ...
              hbaseMasterHeapsizeItem = self.getWarnItem(
                masterHostMessage.format(
                  collectorHostName,
                  str(", ".join(hostMasterComponents[collectorHostName])),
                )
              )
              if hbaseMasterHeapsizeItem:
                validationItems.extend(
  ...
  def validateAmsSiteConfigurationsFromHDP206(
    ...
    if op_mode not in ("embedded", "distributed"):
      ...
      len(self.getComponentHostNames(services, "SHPURDP_METRICS", "METRICS_COLLECTOR"))
      > 1
      and op_mode != "distributed"
    ):
    ...
    validationItems.extend(
      [
        {
          "config-name": "timeline.metrics.service.operation.mode",
