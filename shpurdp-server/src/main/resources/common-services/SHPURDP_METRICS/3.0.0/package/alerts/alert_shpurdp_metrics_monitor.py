...
def is_monitor_process_live(pid_file=None):
  ...
  """
  try:
    check_windows_service_status("ShpurdpMetricsHostMonitoring")
...
def execute(configurations={}, parameters={}, host_name=None):
  ...
  if set([AMS_MONITOR_PID_DIR]).issubset(configurations):
    AMS_MONITOR_PID_PATH = os.path.join(
      configurations[AMS_MONITOR_PID_DIR], "shpurdp-metrics-monitor.pid"
    )
  else:
  ...
  alert_label = (
    "Shpurdp Monitor is running on {0}"
    if ams_monitor_process_running
    else "Shpurdp Monitor is NOT running on {0}"
  )
