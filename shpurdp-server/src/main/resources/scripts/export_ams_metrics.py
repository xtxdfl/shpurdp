...
class Utils:
  ...
  def setup_logger(verbose, log_file):
    global logger
    logger = logging.getLogger("ShpurdpMetricsExport")
...
class AmsMetricsProcessor:
  ...
  def get_metrics_dirs(d):
    for o in os.listdir(d):
      if "shpurdp_metrics_export_" in o and os.path.isdir(os.path.join(d, o)):
...
def main():
  ...
  parser.set_description(
    ...
    "supports export of shpurdp metric data for an app "
    "from Shpurdp Metrics Service to a output dir. "
    ...
    "that implements a subset of the Shpurdp Metrics Service metrics server interfaces. "
  ...
  logfile = os.path.join("/tmp", "shpurdp_metrics_export.out")

  output_dir = os.path.join("/tmp", "shpurdp_metrics_export_" + time_suffix)
  ...
  if Params.ACTION == "export":
    ...
    Params.OUT_DIR = (
      ...
      else os.path.join(options.output_dir, "shpurdp_metrics_export_" + time_suffix)
    )
