...
os.environ["PATH"] += os.pathsep + "/var/lib/shpurdp-agent"
sys.path.append("/usr/lib/shpurdp-server/lib")
...
DEFAULT_SQL_DRIVER_PATH = "/var/lib/shpurdp-server/resources/sqljdbc41.jar"
...
with Environment() as env:
  ...
  parser.add_option(
    ...
    default=get_value_from_properties(
      get_shpurdp_properties(), JDBC_DRIVER_PATH_PROPERTY, DEFAULT_SQL_DRIVER_PATH
    ),
  ...
  def createHdfsResources():
    ...
    params.HdfsResource(
      format("{hdfs_path_prefix}/user/shpurdp-qa"),
      type="directory",
      action=["create_on_execute"],
      mode=0o770,
    )
  ...
  def putCreatedHdfsResourcesToIgnore(env):
    ...
    with open("/var/lib/shpurdp-agent/data/.hdfs_resource_ignore", "a+") as fp:
  ...
  Directory(
    "/var/lib/shpurdp-agent/tmp/hadoop_java_io_tmpdir",
  ...
  File(
    format("{shpurdp_libs_dir}/fast-hdfs-resource.jar"),
    mode=0o644,
    content=StaticFile(
      "/var/lib/shpurdp-agent/cache/stack-hooks/before-START/files/fast-hdfs-resource.jar"
    ),
  )
  ...
  try:
    ...
    os.remove("/var/lib/shpurdp-agent/data/.hdfs_resource_ignore")
    raise
  ...
  Logger.info("Shpurdp preupload script completed.")
