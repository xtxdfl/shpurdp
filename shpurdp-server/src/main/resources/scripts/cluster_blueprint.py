...
class ShpurdpBlueprint:
  ...
  def performPostOperation(self, url, data):
    ...
    req.add_header("X-Requested-By", "shpurdp_scripts")
...
def main():
  ...
  parser.set_description(
    "This python program is a Shpurdp thin client and "
    "supports import/export of Shpurdp managed clusters "
  ...
  parser.add_option(
    ...
    dest="port",
    default="8080",
    help="Shpurdp server port, when running silently. [default: 8080]",
  )
  parser.add_option(
    "-u",
    "--user",
    dest="user",
    default="admin",
    help="Shpurdp server username, when running silently. [default: admin]",
  )
  parser.add_option(
    "-p",
    "--password",
    dest="password",
    default="admin",
    help="Shpurdp server password, when running silently. [default: admin]",
  )
  parser.add_option(
    "-i",
    "--host",
    dest="hostname",
    default="localhost",
    help="Shpurdp server host, when running silently. [default: localhost]",
  )
  ...
  shpurdpBlueprint = ShpurdpBlueprint()

  if options.action == "import":
    shpurdpBlueprint.importBlueprint(options.blueprint, options.hosts, options.cluster)
  elif options.action == "export":
    shpurdpBlueprint.exportBlueprint(options.cluster, options.blueprint)
  else:
