...
<resource-agent version="rgmanager 2.0" name="hadoop">
  ...
  <parameters>
    ...
    <parameter name="shpurdpproperties" unique="0" required="0">
      <shortdesc lang="en">
        Shpurdp properties as comma separated key value pairs
      </shortdesc>
      <longdesc lang="en">
        Example property value:
        shpurdpproperties="server=localhost,port=8080,protocol=http,user=admin,password=admin,cluster=c1,output=/var/log/shpurdp_relocate.log"
      </longdesc>
      <content type="string"/>
    </parameter>
...
# Relocate Shpurdp managed master to current host on failover
execute_shpurdp_relocate_probe() {
  retval = parse_and_validate_shpurdp_properties
  if [ $retval -eq 2 ] ; then
    return 0
  ...
  if [ -z "$SHPURDP_RELOCATE_PROBE" ] ; then
    SHPURDP_RELOCATE_PROBE="relocate_resources.py"
  fi
  ...
  if [ -n "${SHPURDP_OUTPUT}" ]; then
    OUPUT_FILE_CMD="-o ${SHPURDP_OUTPUT}"
  fi

  "${SHPURDP_RELOCATE_PROBE}" -s ${SHPURDP_SERVER} -p ${SHPURDP_PORT} -r ${SHPURDP_PROTOCOL} -c ${SHPURDP_CLUSTER} -e "${SERVICE_NAME}" -m "${COMP_NAME}" -n "${NEW_HOSTNAME}" -u "${SHPURDP_USER}" -w "${SHPURDP_PASSWD} ${OUPUT_FILE_CMD}"

  retval=$?
  if [ $retval -eq 0 ] ; then
    ocf_log info "Shpurdp master successfully relocated."
  elif [ $retval -eq 1 ] ; then
    ocf_log error "Shpurdp relocate master failed. Continuing with failover..."
  elif [ $retval -eq 2 ] ; then
    ocf_log info "No action required from shpurdp probe."
  elif [ $retval -eq 3 ] ; then
    ocf_log err "Shpurdp relocate request verification failed. Exiting..."
    exit ${retval}
  else
    ocf_log error "Unknown return code from shpurdp probe ${retval}."
  fi

  return $retval
}

# Read Shpurdp properties as comma separated key value pairs from cluster.conf
# Property name: 'shpurdpproperties'.
# Example property value:
# shpurdpproperties="server=localhost,port=8080,protocol=http,user=admin,password=admin,cluster=c1,output=/var/log/shpurdp_relocate.log"
parse_and_validate_shpurdp_properties() {
  if [ -n "${OCF_RESKEY_shpurdpproperties}" ] ; then
    ocf_log info "Shpurdp properties found: ${OCF_RESKEY_shpurdpproperties}"

    IFS=',' read -ra properties <<< $OCF_RESKEY_shpurdpproperties

    for i in $properties; do
      if [[ "$i" == "server"* ]] ; then SHPURDP_SERVER=$(echo $i | cut -d"=" -f2); fi
      if [[ "$i" == "port"* ]] ; then SHPURDP_PORT=$(echo $i | cut -d"=" -f2); fi
      if [[ "$i" == "protocol"* ]] ; then SHPURDP_PROTOCOL=$(echo $i | cut -d"=" -f2); fi
      if [[ "$i" == "user"* ]] ; then SHPURDP_USER=$(echo $i | cut -d"=" -f2); fi
      if [[ "$i" == "password"* ]] ; then SHPURDP_PASSWD=$(echo $i | cut -d"=" -f2); fi
      if [[ "$i" == "cluster"* ]] ; then SHPURDP_CLUSTER=$(echo $i | cut -d"=" -f2); fi
      if [[ "$i" == "output"* ]] ; then SHPURDP_OUTPUT=$(echo $i | cut -d"=" -f2); fi
    done

    if [ -z "${SHPURDP_SERVER}" ] ; then
      ocf_log err "required shpurdp property 'server' is unset"
      return 1
    fi

    if [ -z "${SHPURDP_PORT}" ] ; then
      ocf_log err "required shpurdp property 'port' is unset"
      return 1
    fi

    if [ -z "${SHPURDP_PROTOCOL}" ] ; then
      ocf_log err "required shpurdp property 'protocol' is unset"
      return 1
    fi

    if [ -z "${SHPURDP_USER}" ] ; then
      ocf_log err "required shpurdp property 'user' is unset"
      return 1
    fi

    if [ -z "${SHPURDP_PASSWD}" ] ; then
      ocf_log err "required shpurdp property 'password' is unset"
      return 1
    fi

    if [ -z "${SHPURDP_CLUSTER}" ] ; then
      ocf_log err "required shpurdp property 'cluster' is unset"
      return 1
    fi

  else
    ocf_log info "No Shpurdp properties found."
    return 2
  fi
}
