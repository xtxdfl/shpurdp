...
logger = logging.getLogger("ShpurdpConfig")
...
def api_accessor(host, login, password, protocol, port, unsafe=None):
  def do_request(api_url, request_type=GET_REQUEST_TYPE, request_body=None):
    try:
      ...
      admin_auth = (
        base64.encodebytes(f"{login}:{password}".encode()).decode().replace("\n", "")
      )
...
def get_config_tag(cluster, config_type, accessor):
  ...
  try:
    ...
    raise Exception(
      f'"{config_type}" not found in server response. Response:\n{response}'
    )
  return current_config_tag


def create_new_desired_config(
  cluster, config_type, properties, attributes, accessor, version_note
):
...
def update_from_file(config_file):
  def update(cluster, config_type, accessor):
    ...
    try:
      ...
      raise Exception(
        f'File "{config_file}" should be in the following JSON format ("properties_attributes" is optional):\n{FILE_FORMAT}'
      )
...
def delete_properties(cluster, config_type, args, accessor, version_note):
  ...
  update_config(
    cluster, config_type, delete_specific_property(config_name), accessor, version_note
  )
  return 0
...
def main():
  ...
  parser.add_option(
    ...
    dest="port",
    default="8080",
    help="Optional port number for Shpurdp server. Default is '8080'. Provide empty string to not use port.",
  )
  parser.add_option(
    "-s",
    "--protocol",
  ...
  parser.add_option(
    ...
    "--config-type",
    dest="config_type",
    help="One of the various configuration types in Shpurdp. Ex: core-site, hdfs-site, mapred-queue-acls, etc.",
  )
  parser.add_option(
    "-b",
    "--version-note",
  ...
  if not options.credentials_file and (not options.user or not options.password):
    parser.error(
      "You should use option (-e) to set file with Shpurdp user credentials OR use (-u) username and (-p) password"
    )
  ...
  if options.credentials_file:
    if os.path.isfile(options.credentials_file):
      try:
        with open(options.credentials_file) as credentials_file:
          ...
          if len(login_lines) == 2:
            ...
            logger.error(
              f"Incorrect content of {options.credentials_file} file. File should contain Shpurdp username and password separated by new line."
            )
            return -1
      except Exception as e:
      ...
      logger.error(
        f"File {options.credentials_file} doesn't exist or you don't have permissions."
      )
      return -1
  else:
    user = options.user
  ...
  if action == SET_ACTION:
    if not options.file and (not options.key or options.value is None):
      parser.error(
        "You should use option (-f) to set file where entire configurations are saved OR (-k) key and (-v) value for one property"
      )
    if options.file:
    ...
    if not options.key:
      parser.error(
        "You should use option (-k) to set property name witch will be deleted"
      )
    else:
    ...
    logger.error(
      f'Action "{action}" is not supported. Supported actions: "get", "set", "delete".'
    )
    return -1
