...
logger = logging.getLogger("ShpurdpTakeoverConfigMerge")
...
"c6401.shpurdp.apache.org/etc/hive/conf/log4j.properties" : "hive-log4j"}
"""
...
class ConfigMerge:
  ...
  OUTPUT_DIR = "/tmp"
  OUT_FILENAME = "shpurdp_takeover_config_merge.out"
  JSON_FILENAME = "shpurdp_takeover_config_merge.json"
      ...
      for file in fileList:
        ...
        if ext in extensions:
          ...
          if (
            ...
            logger.warn(f"File {file_path} is not configurable by Shpurdp. Skipping...")
            continue
          config_name = None
          ...
          if not config_name:
            if file in ConfigMerge.NOT_MAPPED_FILES:
              if ConfigMerge.UNKNOWN_FILES_MAPPING_FILE:
                ...
                logger.error(
                  "Cannot map {0} to Shpurdp config type. Please use -u option to specify config mapping for this file. \n"
...
def main():
  ...
  parser.set_description(
    "This python program is an Shpurdp thin client and "
    "supports Shpurdp cluster takeover by generating a "
