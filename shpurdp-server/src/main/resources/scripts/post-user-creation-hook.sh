...
# wraps the command passed in as argument to be called via the shpurdp sudo
shpurdp_sudo(){

ARG_STR="$1"
CMD_STR="/var/lib/shpurdp-server/shpurdp-sudo.sh su '$HDFS_USR' -l -s /bin/bash -c '$ARG_STR'"
...
# check for hadoop
shpurdp_sudo "type hadoop > /dev/null 2>&1 || { echo >&2 \"hadoop client not installed\"; exit 1; }"

# check for the hdfs
shpurdp_sudo "hadoop fs -ls / > /dev/null 2>&1 || { echo >&2 \"hadoop dfs not available\"; exit 1; }"
...
shpurdp_sudo "yarn jar /var/lib/shpurdp-server/resources/stack-hooks/before-START/files/fast-hdfs-resource.jar $JSON_INPUT"
