...
##         vagrant environment documented at shpurdp wiki: https://cwiki.apache.org/confluence/display/SHPURDP/Quick+Start+Guide 
###########################

usage () {
echo "Usage: keytabs.sh <HOST_PRINCIPAL_KEYTABLE.csv> <SSH_LOGIN_KEY_PATH>";
echo "  <HOST_PRINCIPAL_KEYTABLE.csv>: CSV file generated by 'Enable Security Wizard' of Shpurdp";
...
processCSVFile () {
    ...
    echo "###########################################################################" >> generate_keytabs.sh;
    echo "## " >> generate_keytabs.sh;
    echo "## Shpurdp Security Script Generator" >> generate_keytabs.sh;
    echo "## "  >> generate_keytabs.sh;
    echo "## Shpurdp security script is generated which should be run on the" >> generate_keytabs.sh;
...
########################
getEnvironmentCMD () {
  os=`python -c 'import sys; sys.path.append("/usr/lib/shpurdp-server/lib/"); from shpurdp_commons import OSCheck; print OSCheck.get_os_family()'`
  version=`python -c 'import sys; sys.path.append("/usr/lib/shpurdp-server/lib/"); from shpurdp_commons import OSCheck; print OSCheck.get_os_major_version()'`
  os=$os$version;
  case $os in
  'ubuntu12' )
    pkgmgr='apt-get'
