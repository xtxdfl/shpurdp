...
if [[ -f ${mark_file} ]] ; then
  /var/lib/shpurdp-agent/shpurdp-sudo.sh rm -f ${mark_file}
  /var/lib/shpurdp-agent/shpurdp-sudo.sh mkdir -p ${mark_dir}
fi
...
if [[ ! -d $mark_dir ]] ; then
  ...
  if [[ $EXIT_CODE == 0 ]] ; then
    /var/lib/shpurdp-agent/shpurdp-sudo.sh su ${hdfs_user} - -s /bin/bash -c "export PATH=$PATH:$bin_dir ; yes Y | hdfs --config ${conf_dir} ${command}"
    (( EXIT_CODE = $EXIT_CODE | $? ))
  else
