...
def setup_kms_db(stack_version=None):
  ...
  if params.has_ranger_admin:
    ...
    dba_setup = format("shpurdp-python-wrap {kms_home}/dba_script.py -q")
    db_setup = format("shpurdp-python-wrap {kms_home}/db_setup.py")
...
def setup_java_patch():
  ...
  if params.has_ranger_admin:
    kms_home = params.kms_home
    setup_java_patch = format("shpurdp-python-wrap {kms_home}/db_setup.py -javapatch")
...
def kms(upgrade_type=None):
  ...
  if params.has_ranger_admin:
      ...
      if not os.path.isfile(path_to_jdbc):
        ...
        if not os.path.isfile(path_to_jdbc):
          ...
          error_message = (
            ...
            + " in ranger kms lib dir. So, db connection check can fail. Please run 'shpurdp-server setup --jdbc-db={db_name} --jdbc-driver={path_to_jdbc} on server host.'"
          )
    ...
    db_connection_check_command = format(
      "{shpurdp_java_home}/bin/java -cp {cp} org.apache.shpurdp.server.DBConnectionVerification '{ranger_kms_jdbc_connection_url}' {db_user} {db_password!p} {ranger_kms_jdbc_driver}"
    )
    env_dict = {}
    if params.db_flavor.lower() == "sqla":
...
def copy_jdbc_connector(kms_home):
  ...
  if params.jdbc_jar_name is None and params.driver_curl_source.endswith("/None"):
    ...
    Please run 'shpurdp-server setup --jdbc-db={params.db_name} --jdbc-driver={path_to_jdbc} on server host.'"
    Logger.error(error_message)
...
def check_ranger_service():
  ...
  if response_code is not None and response_code == 200:
    user_resp_code = ranger_adm_obj.create_shpurdp_admin_user(
      params.shpurdp_ranger_admin,
      params.shpurdp_ranger_password,
      params.admin_uname_password,
    )
    if user_resp_code is not None and user_resp_code == 200:
      get_repo_flag = get_repo(
        policymgr_mgr_url, params.repo_name, shpurdp_username_password_for_ranger
      )
      if not get_repo_flag:
        return create_repo(
          policymgr_mgr_url,
          json.dumps(params.kms_ranger_plugin_repo),
          shpurdp_username_password_for_ranger,
        )
      else:
        return True
    else:
