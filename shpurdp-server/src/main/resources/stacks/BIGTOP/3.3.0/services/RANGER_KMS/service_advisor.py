...
class RangerKMSRecommender(service_advisor.ServiceAdvisor):
  ...
  def recommendRangerKMSConfigurationsFromHDP23(
    ...
    if security_enabled:
      # adding for shpurdp user
      putRangerKmsSiteProperty(f"hadoop.kms.proxyuser.{shpurdp_user}.users", "*")
      putRangerKmsSiteProperty(f"hadoop.kms.proxyuser.{shpurdp_user}.hosts", "*")
      # adding for HTTP
      ...
      # deleting shpurdp user proxy properties
      putRangerKmsSitePropertyAttribute(
        f"hadoop.kms.proxyuser.{shpurdp_user}.hosts", "delete", "true"
      )
      putRangerKmsSitePropertyAttribute(
        f"hadoop.kms.proxyuser.{shpurdp_user}.users", "delete", "true"
      )
