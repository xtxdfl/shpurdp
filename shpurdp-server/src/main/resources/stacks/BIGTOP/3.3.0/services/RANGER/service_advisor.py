...
class RangerServiceAdvisor(service_advisor.ServiceAdvisor):
  ...
  def modifyComponentLayoutSchemes(self):
    ...
    self.componentLayoutSchemes.update(
      {"RANGER_USERSYNC": {3: 0, 6: 1, 31: 2, "else": 2}}
    )
...
class RangerRecommender(service_advisor.ServiceAdvisor):
  ...
  def recommendRangerConfigurationsFromHDP206(
    ...
    putRangerAdminProperty = self.putProperty(
      configurations, "admin-properties", services
    )
    ...
    if (
      ...
      if (
        ...
        port = services["configurations"]["ranger-admin-site"]["properties"][
          "ranger.service.https.port"
        ]
      # In Ranger-0.4.0 port stored in ranger-site https.service.port
      elif (
        "ranger-site" in services["configurations"]
        and "https.service.port"
        in services["configurations"]["ranger-site"]["properties"]
      ):
    ...
    if ranger_admin_hosts:
      if (
        len(ranger_admin_hosts) > 1
        and services["configurations"]
        and "admin-properties" in services["configurations"]
        and "policymgr_external_url"
        in services["configurations"]["admin-properties"]["properties"]
        and services["configurations"]["admin-properties"]["properties"][
        ...
        policymgr_external_url = services["configurations"]["admin-properties"][
          "properties"
        ]["policymgr_external_url"]
      else:
    ...
    if rangerServiceVersion == "0.4.0":
      ...
      if (
        "admin-properties" in services["configurations"]
        and "usersync-properties" in services["configurations"]
        and "SYNC_SOURCE"
        in services["configurations"]["usersync-properties"]["properties"]
      ):
        rangerUserSyncSource = services["configurations"]["usersync-properties"][
          "properties"
        ]["SYNC_SOURCE"]
        ...
        if ranger_services[item]["service_name"] in servicesList:
          ...
          if component_audit_file in services["configurations"]:
            ranger_audit_dict = [
              {
                ...
                "target_configname": "XAAUDIT.DB.IS_ENABLED",
              },
              {
                "filename": "ranger-env",
                "configname": "xasecure.audit.destination.hdfs",
                "target_configname": "XAAUDIT.HDFS.IS_ENABLED",
              },
              {
                "filename": "ranger-env",
                "configname": "xasecure.audit.destination.hdfs.dir",
                "target_configname": "XAAUDIT.HDFS.DESTINATION_DIRECTORY",
              },
            ]
            ...
            for item in ranger_audit_dict:
              if (
                item["filename"] in services["configurations"]
                and item["configname"]
                in services["configurations"][item["filename"]]["properties"]
              ):
                if (
                  item["filename"] in configurations
                  and item["configname"]
                  in configurations[item["filename"]]["properties"]
                ):
                  rangerAuditProperty = configurations[item["filename"]]["properties"][
                    item["configname"]
                  ]
                else:
                  rangerAuditProperty = services["configurations"][item["filename"]][
                    "properties"
                  ][item["configname"]]
  ...
  def recommendRangerUrlConfigurations(
    self, configurations, services, requiredServices
  ):
    servicesList = [
      service["StackServices"]["service_name"] for service in services["services"]
    ]
    ...
    if (
      ...
      if (
        "admin-properties" in configurations
        and "policymgr_external_url" in configurations["admin-properties"]["properties"]
      ):
        policymgr_external_url = configurations["admin-properties"]["properties"][
          "policymgr_external_url"
        ]
      else:
        policymgr_external_url = services["configurations"]["admin-properties"][
    ...
    for index in range(len(requiredServices)):
      if requiredServices[index]["service_name"] in servicesList:
        ...
        component_config_property = (
          f"ranger.plugin.{component_name.lower()}.policy.rest.url"
        )
        ...
        if (
          component_config_type in services["configurations"]
          and component_config_property
          in services["configurations"][component_config_type]["properties"]
        ):
  ...
  def recommendRangerConfigurationsFromHDP23(
    ...
    if (
      "admin-properties" in services["configurations"]
      and ("DB_FLAVOR" in services["configurations"]["admin-properties"]["properties"])
      and ("db_host" in services["configurations"]["admin-properties"]["properties"])
      and ("db_name" in services["configurations"]["admin-properties"]["properties"])
    ):
      rangerDbFlavor = services["configurations"]["admin-properties"]["properties"][
        "DB_FLAVOR"
      ]
      rangerDbHost = services["configurations"]["admin-properties"]["properties"][
        "db_host"
      ]
      rangerDbName = services["configurations"]["admin-properties"]["properties"][
        "db_name"
      ]
      ranger_db_url_dict = {
        "MYSQL": {
        ...
        "MSSQL": {
          "ranger.jpa.jdbc.driver": "com.microsoft.sqlserver.jdbc.SQLServerDriver",
          "ranger.jpa.jdbc.url": "jdbc:sqlserver://"
          + self.getDBConnectionHostPort(rangerDbFlavor, rangerDbHost)
          + ";databaseName="
          + rangerDbName,
        },
        "SQLA": {
          ...
          "ranger.jpa.jdbc.url": "jdbc:sqlanywhere:host="
          + self.getDBConnectionHostPort(rangerDbFlavor, rangerDbHost)
          + ";database="
          + rangerDbName,
        },
      ...
      if (
        ...
        ranger_db_privelege_url_dict = {
          "MYSQL": {
            "ranger_privelege_user_jdbc_url": "jdbc:mysql://"
            + self.getDBConnectionHostPort(rangerDbFlavor, rangerDbHost)
          },
          "ORACLE": {
          ...
          "POSTGRES": {
            "ranger_privelege_user_jdbc_url": "jdbc:postgresql://"
            + self.getDBConnectionHostPort(rangerDbFlavor, rangerDbHost)
            + "/postgres"
          },
          "MSSQL": {
            "ranger_privelege_user_jdbc_url": "jdbc:sqlserver://"
            + self.getDBConnectionHostPort(rangerDbFlavor, rangerDbHost)
            + ";"
          },
          "SQLA": {
            "ranger_privelege_user_jdbc_url": "jdbc:sqlanywhere:host="
            + self.getDBConnectionHostPort(rangerDbFlavor, rangerDbHost)
            + ";"
          },
        }
    ...
    if (
      "ranger-ugsync-site" in services["configurations"]
      and "ranger.usersync.source.impl.class"
      in services["configurations"]["ranger-ugsync-site"]["properties"]
    ):
      rangerUserSyncClass = services["configurations"]["ranger-ugsync-site"][
        "properties"
      ...
      if rangerUserSyncClass in authMap:
        ...
        putRangerAdminProperty(
          "ranger.authentication.method", rangerSqlConnectorProperty
        )
      ...
      if ranger_services[item]["service_name"] in servicesList:
        ...
        if component_audit_file in services["configurations"]:
          ranger_audit_dict = [
            ...
            {
              "filename": "ranger-admin-site",
              "configname": "ranger.audit.solr.urls",
              "target_configname": "xasecure.audit.destination.solr.urls",
            },
            {
              "filename": "ranger-admin-site",
              "configname": "ranger.audit.solr.zookeepers",
          ...
          for item in ranger_audit_dict:
            if (
              ...
              if (
                item["filename"] in configurations
                and item["configname"] in configurations[item["filename"]]["properties"]
              ):
                rangerAuditProperty = configurations[item["filename"]]["properties"][
                  item["configname"]
                ]
    ...
    if (
      "ranger-env" in services["configurations"]
      and "xasecure.audit.destination.solr"
      in services["configurations"]["ranger-env"]["properties"]
    ):
      audit_solr_flag = services["configurations"]["ranger-env"]["properties"][
        "xasecure.audit.destination.solr"
      ]
    if (
      "ranger-env" in services["configurations"]
      and "xasecure.audit.destination.db"
      in services["configurations"]["ranger-env"]["properties"]
    ):
      audit_db_flag = services["configurations"]["ranger-env"]["properties"][
    ...
    if "KNOX" in servicesList:
      ...
      if (
        "gateway-site" in services["configurations"]
        and "gateway.port" in services["configurations"]["gateway-site"]["properties"]
      ):
        knox_port = services["configurations"]["gateway-site"]["properties"][
          "gateway.port"
        ]
      putRangerAdminProperty(
        "ranger.sso.providerurl",
        f"https://{knox_host}:{knox_port}/gateway/knoxsso/api/v1/websso",
      )
  ...
  def recommendRangerConfigurationsFromHDP25(
    ...
    if "ATLAS" in servicesList and has_ranger_tagsync:
      ...
      if (
        ...
        protocol = "http"
        if (
          application_properties and "atlas.server.http.port" in application_properties
        ):
      ...
      putTagsyncSiteProperty(
        "ranger.tagsync.source.atlasrest.endpoint", atlas_rest_endpoint
      )
    ...
    if (
      "SHPURDP_INFRA_SOLR" in servicesList
      and zookeeper_host_port
      and is_solr_cloud_enabled
      and not is_external_solr_cloud_enabled
    ):
      ...
      if "infra-solr-env" in services["configurations"] and (
        ...
        infra_solr_znode = services["configurations"]["infra-solr-env"]["properties"][
          "infra_solr_znode"
        ]
        ranger_audit_zk_port = f"{zookeeper_host_port}{infra_solr_znode}"
      putRangerAdminProperty("ranger.audit.solr.zookeepers", ranger_audit_zk_port)
    elif (
      zookeeper_host_port and is_solr_cloud_enabled and is_external_solr_cloud_enabled
    ):
      ranger_audit_zk_port = f"{zookeeper_host_port}/ranger_audits"
      putRangerAdminProperty("ranger.audit.solr.zookeepers", ranger_audit_zk_port)
    else:
    ...
    for item in range(len(ranger_plugins_serviceuser)):
      if ranger_plugins_serviceuser[item]["service_name"] in servicesList:
        ...
        if (
          ...
          and config_name in services["configurations"][file_name]["properties"]
        ):
          service_user = services["configurations"][file_name]["properties"][
            config_name
          ]
    ...
    if "ATLAS" in servicesList:
      if "ranger-env" in services["configurations"]:
        ...
        if (
          ...
          in configurations["ranger-env"]["properties"]
        ):
          xasecure_audit_destination_hdfs = configurations["ranger-env"]["properties"][
            "xasecure.audit.destination.hdfs"
          ]
        else:
          xasecure_audit_destination_hdfs = services["configurations"]["ranger-env"][
            "properties"
          ]["xasecure.audit.destination.hdfs"]
        ...
        if (
          ...
          xasecure_audit_destination_solr = services["configurations"]["ranger-env"][
            "properties"
          ]["xasecure.audit.destination.solr"]

        putAtlasRangerAuditProperty(
          "xasecure.audit.destination.hdfs", xasecure_audit_destination_hdfs
        )
        putAtlasRangerAuditProperty(
          "xasecure.audit.destination.hdfs.dir", xasecure_audit_destination_hdfs_dir
        )
        putAtlasRangerAuditProperty(
          "xasecure.audit.destination.solr", xasecure_audit_destination_solr
        )
    required_services = [
  ...
  def recommendConfigurationsForSSO(self, configurations, clusterData, services, hosts):
    ...
    shpurdp_sso_details = (
      shpurdp_configuration.get_shpurdp_sso_details() if shpurdp_configuration else None
    )

    if shpurdp_sso_details and shpurdp_sso_details.is_managing_services():
      putRangerAdminSiteProperty = self.putProperty(
        configurations, "ranger-admin-site", services
      )

      # If SSO should be enabled for this service, continue
      if shpurdp_sso_details.should_enable_sso("RANGER"):
        putRangerAdminSiteProperty("ranger.sso.enabled", "true")
        putRangerAdminSiteProperty(
          "ranger.sso.providerurl", shpurdp_sso_details.get_sso_provider_url()
        )
        putRangerAdminSiteProperty(
      ...
      elif shpurdp_sso_details.should_disable_sso("RANGER"):
  ...
  def recommendConfigurationsForLDAP(
    self, configurations, clusterData, services, hosts
  ):
    shpurdp_configuration = self.get_shpurdp_configuration(services)
    ldap_properties = shpurdp_configuration.get_shpurdp_server_configuration_category(
      "ldap-configuration"
    )

    if (
      ldap_properties
      and "shpurdp.ldap.authentication.enabled" in ldap_properties
      and ldap_properties["shpurdp.ldap.authentication.enabled"].lower() == "true"
    ):
      putRangerUgsyncSite = self.putProperty(
        configurations, "ranger-ugsync-site", services
      )

      if "shpurdp.ldap.attributes.user.search_base" in ldap_properties:
        putRangerUgsyncSite(
          "ranger.usersync.ldap.searchBase",
          ldap_properties["shpurdp.ldap.attributes.user.search_base"],
        )
      if "shpurdp.ldap.attributes.group.member_attr" in ldap_properties:
        putRangerUgsyncSite(
          "ranger.usersync.group.memberattributename",
          ldap_properties["shpurdp.ldap.attributes.group.member_attr"],
        )
      if "shpurdp.ldap.attributes.group.name_attr" in ldap_properties:
        putRangerUgsyncSite(
          "ranger.usersync.group.nameattribute",
          ldap_properties["shpurdp.ldap.attributes.group.name_attr"],
        )
      if "shpurdp.ldap.attributes.group.object_class" in ldap_properties:
        putRangerUgsyncSite(
          "ranger.usersync.group.objectclass",
          ldap_properties["shpurdp.ldap.attributes.group.object_class"],
        )
      if "shpurdp.ldap.connectivity.bind_dn" in ldap_properties:
        putRangerUgsyncSite(
          "ranger.usersync.ldap.binddn",
          ldap_properties["shpurdp.ldap.connectivity.bind_dn"],
        )
      if "shpurdp.ldap.connectivity.server.host" in ldap_properties:
        ldap_protocol = "ldap://"
        if (
          "shpurdp.ldap.connectivity.use_ssl" in ldap_properties
          and ldap_properties["shpurdp.ldap.connectivity.use_ssl"] == "true"
        ):
          ldap_protocol = "ldaps://"
        ldap_port = "389"
        if "shpurdp.ldap.connectivity.server.port" in ldap_properties:
          ldap_port = ldap_properties["shpurdp.ldap.connectivity.server.port"]
        ldapUrl = (
          ldap_protocol
          + ldap_properties["shpurdp.ldap.connectivity.server.host"]
          + ":"
          + ldap_port
        )
        putRangerUgsyncSite("ranger.usersync.ldap.url", ldapUrl)
      if "shpurdp.ldap.attributes.user.object_class" in ldap_properties:
        putRangerUgsyncSite(
          "ranger.usersync.ldap.user.objectclass",
          ldap_properties["shpurdp.ldap.attributes.user.object_class"],
        )
      if "shpurdp.ldap.attributes.user.name_attr" in ldap_properties:
        putRangerUgsyncSite(
          "ranger.usersync.ldap.user.nameattribute",
          ldap_properties["shpurdp.ldap.attributes.user.name_attr"],
        )
...
class RangerValidator(service_advisor.ServiceAdvisor):
  ...
  def validateRangerAdminConfigurationsFromHDP23(
    ...
    if "RANGER" in servicesList and "policymgr_external_url" in ranger_site:
      ...
      if policymgr_mgr_url.endswith("/"):
        validationItems.append(
          {
            "config-name": "policymgr_external_url",
            "item": self.getWarnItem(
  ...
  def validateRangerTagsyncConfigurationsFromHDP25(
    ...
    if "RANGER" in servicesList:
      ...
      if (
        ...
        validationItems.append(
          {
            "config-name": "ranger.tagsync.source.atlas",
            "item": self.getWarnItem(
  ...
  def validateRangerUsersyncConfigurationsFromHDP26(
    ...
    if usersync_source_ldap_enabled and delta_sync_enabled and not group_sync_enabled:
      validationItems.append(
        {
          "config-name": "ranger.usersync.group.searchenabled",
          "item": self.getWarnItem(
  ...
  def validateRangerPasswordConfigurations(
    self, properties, recommendedDefaults, configurations, services, hosts
  ):
    ranger_env_properties = properties
    validationItems = []
