...
def execute(configurations={}, parameters={}, host_name=None):
  ...
  if DFS_DATA_DIR not in configurations:
    return (
      RESULT_STATE_UNKNOWN,
      [f"{DFS_DATA_DIR} is a required parameter for the script"],
    )
  ...
  if dfs_data_dir is None:
    return (
      RESULT_STATE_UNKNOWN,
      [f"{DFS_DATA_DIR} is a required parameter for the script and the value is null"],
    )
  ...
  for data_dir in normalized_data_dirs:
    ...
    if os.path.isdir(data_dir):
      ...
      curr_mount_point = (
        curr_mount_point.strip() if curr_mount_point else curr_mount_point
      )
  ...
  if data_dirs_not_exist:
    errors.append(
      "The following data dir(s) were not found: {0}\n".format(
  ...
  if data_dir_mount_file_exists:
    ...
    for data_dir in data_dirs_on_root:
      if (
        data_dir in expected_data_dir_to_mount
        and expected_data_dir_to_mount[data_dir] != "/"
      ):
  ...
  if len(errors) == 0:
    ...
    if len(normalized_data_dirs) > 0:
      messages.append(
        "The following data dir(s) are valid:\n{0}".format(
