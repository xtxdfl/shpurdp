...
class SparkServiceAdvisor(service_advisor.ServiceAdvisor):
  ...
  def getServiceConfigurationRecommendations(
    ...
    recommender.recommendSparkConfigurationsFromHDP25(
      configurations, clusterData, services, hosts
    )
    recommender.recommendSPARKConfigurationsFromHDP26(
      configurations, clusterData, services, hosts
    )
  ...
  def isKerberosEnabled(services, configurations):
    ...
    if (
      configurations
      and "spark-defaults" in configurations
      and "spark.history.kerberos.enabled"
      in configurations["spark-defaults"]["properties"]
    ):
      return (
      ...
      services
      and "spark-defaults" in services["configurations"]
      and "spark.history.kerberos.enabled"
      in services["configurations"]["spark-defaults"]["properties"]
    ):
      return (
...
class SparkRecommender(service_advisor.ServiceAdvisor):
  ...
  def recommendSparkConfigurationsFromHDP25(
    self, configurations, clusterData, services, hosts
  ):
    """
    :type configurations dict
    ...
    spark_queue = self.recommendYarnQueue(
      services, "spark-defaults", "spark.yarn.queue"
    )
    if spark_queue is not None:
      putSparkProperty("spark.yarn.queue", spark_queue)

  def recommendSPARKConfigurationsFromHDP26(
    self, configurations, clusterData, services, hosts
  ):
    """
    :type configurations dict
    ...
    if SparkServiceAdvisor.isKerberosEnabled(services, configurations):
      ...
      if spark_defaults:
        putSparkDafaultsProperty = self.putProperty(
          configurations, "spark-defaults", services
        )
  ...
  def setOrAddValueToProperty(
    self, putConfigProperty, config, propertyName, propertyValue, separator
  ):
    if (
      self.checkSiteProperties(config, propertyName)
      and len(str(config[propertyName]).strip()) > 0
    ):
      putConfigProperty(
        propertyName, str(config[propertyName]).strip() + separator + propertyValue
      )
    else:
      putConfigProperty(propertyName, propertyValue)

  def removeValueFromProperty(
    self, putConfigProperty, config, propertyName, propertyValue, separator
  ):
    ...
    if str(config[propertyName]).strip() == propertyValue:
      ...
      putConfigProperty(
        propertyName, str(config[propertyName]).replace(separator + propertyValue, "")
      )
...
class SparkValidator(service_advisor.ServiceAdvisor):
  ...
  def __init__(self, *args, **kwargs):
    ...
    self.validators = [
      ("spark-defaults", self.validateSparkDefaultsFromHDP25),
      (
  ...
  def validateSparkDefaultsFromHDP25(
    ...
    validationItems = [
      {
        ...
        "item": self.validatorYarnQueue(
          properties, recommendedDefaults, "spark.yarn.queue", services
        ),
      }
    ]
    return self.toConfigurationValidationProblems(validationItems, "spark-defaults")

  def validateSparkAtlasApplicationPropertiesFromHDP30(
    self, properties, recommendedDefaults, configurations, services, hosts
  ):
    validationItems = []
    servicesList = [
    ...
    if (
      not "ATLAS" in servicesList
      and "atlas.spark.enabled"
      in services["configurations"]["spark-atlas-application-properties-override"][
        "properties"
      ]
      and str(
      ...
      validationItems.append(
        {
          "config-name": "spark-atlas-application-properties-override",
          +"item": self.getErrorItem(
            "SAC could be enabled only if ATLAS service is available on cluster!"
          ),
        }
      )
    return self.toConfigurationValidationProblems(
      validationItems, "spark-atlas-application-properties-override"
    )
