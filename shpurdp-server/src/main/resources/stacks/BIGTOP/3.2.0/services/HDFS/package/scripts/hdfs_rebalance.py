...
class HdfsLine:
  ...
  def parseProgressLog(self, line, m):
    ...
    Throws ShpurdpException in case of parsing errors
    ...
    if m:
      ...
      raise ShpurdpException("Failed to parse line [%s]")

  def parseMemory(self, memorySize, multiplier_type):
    try:
      factor = self.MEMORY_SUFFIX.index(multiplier_type)
    except ValueError:
      raise ShpurdpException(f"Failed to memory value [{memorySize} {multiplier_type}]")
