...
class HDFSRecommender(service_advisor.ServiceAdvisor):
  ...
  def recommendConfigurationsForSSO(self, configurations, clusterData, services, hosts):
    ...
    if shpurdp_sso_details and shpurdp_sso_details.is_managing_services():
      ...
      if shpurdp_sso_details.should_enable_sso("HDFS"):
      ...
      elif shpurdp_sso_details.should_disable_sso("HDFS"):
        if self.is_kerberos_enabled(configurations, services):
...
class HDFSValidator(service_advisor.ServiceAdvisor):
  ...
  def validateHDFSCoreSiteFromHDP206(
    ...
    validationItems.extend(
      self.getShpurdpProxyUsersForHDFSValidationItems(properties, services)
    )
        ...
        validations.append(
          {
            ...
            "item": self.getNotApplicableItem(
              "Your Shpurdp Server has not been configured to download LZO and install it. "
              "LZO is GPL software and requires you to explicitly enable Shpurdp to install and download LZO. "
              "Please refer to the documentation to configure Shpurdp before proceeding."
            ),
          }
        )

  def getShpurdpProxyUsersForHDFSValidationItems(self, properties, services):
    validationItems = []
    ...
    if "HDFS" in servicesList:
      ...
      props = (
        f"hadoop.proxyuser.{shpurdp_user}.hosts",
        f"hadoop.proxyuser.{shpurdp_user}.groups",
      )
      for prop in props:
        validationItems.append(
