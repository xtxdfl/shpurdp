...
class YARNRecommender(service_advisor.ServiceAdvisor):
  ...
  def recommendYARNConfigurationsFromHDP206(
    ...
    if "TEZ" in servicesList:
      ...
      putYarnProperty(
        f"yarn.timeline-service.http-authentication.proxyuser.{shpurdp_user}.hosts",
        shpurdpHostName,
      )
      putYarnProperty(
        f"yarn.timeline-service.http-authentication.proxyuser.{shpurdp_user}.groups", "*"
      )
      old_shpurdp_user = self.getOldShpurdpUser(services)
      if old_shpurdp_user is not None:
        putYarnPropertyAttribute(
          f"yarn.timeline-service.http-authentication.proxyuser.{old_shpurdp_user}.hosts",
          "delete",
          "true",
        )
        putYarnPropertyAttribute(
          f"yarn.timeline-service.http-authentication.proxyuser.{old_shpurdp_user}.groups",
          "delete",
          "true",
        )
  ...
  def recommendConfigurationsForSSO(self, configurations, clusterData, services, hosts):
    ...
    if shpurdp_sso_details and shpurdp_sso_details.is_managing_services():
      ...
      if shpurdp_sso_details.should_enable_sso("YARN"):
        if self.is_kerberos_enabled(configurations, services):
          ...
          putYarnSiteProperty(
            ...
            shpurdp_sso_details.get_sso_provider_url(),
          )
          putYarnSiteProperty(
      ...
      elif shpurdp_sso_details.should_disable_sso("YARN"):
        if self.is_kerberos_enabled(configurations, services):
  ...
  def updateLlapConfigs(self, configurations, services, hosts, llap_queue_name):
    ...
    selected_queue_is_shpurdp_managed_llap = (
      None  # Queue named 'llap' at root level is Shpurdp managed.
    )
    ...
    # Update Visibility of 'num_llap_nodes' YARN Service. Visible only if selected queue is Shpurdp created 'llap'.
    capacity_scheduler_properties, received_as_key_value_pair = (
      self.getCapacitySchedulerProperties(services)
    )
    if capacity_scheduler_properties:
      ...
      if (
        ...
        selected_queue_is_shpurdp_managed_llap = True
        self.logger.info(
        ...
        selected_queue_is_shpurdp_managed_llap = False
    ...
    self.logger.info(
      f"DBG: selected_queue_is_shpurdp_managed_llap = {selected_queue_is_shpurdp_managed_llap}"
    )
    if not selected_queue_is_shpurdp_managed_llap:
      ...
      # Pouplate the 'num_llap_nodes_requested' in config 'num_llap_nodes', a read only config for non-Shpurdp managed queue case.
    ...
    else:  # Shpurdp managed 'llap' named queue at root level.
    ...
    # This can happen if we upgrade from Shpurdp 2.4 (with HDP 2.5) to Shpurdp 2.5, as this config is from 2.6 stack onwards only.
    if (
  ...
  def get_hive_tez_container_size(self, services):
    ...
    if not hive_container_size:
      ...
      # upgrade scenario from Shpurdp 2.4 to Shpurdp 2.5 with HDP 2.5 installed. Read it
      # from hive-site.
      #
      # If Shpurdp 2.5 after upgrade from 2.4 is managing HDP 2.6 here, this config would have
...
class MAPREDUCE2Recommender(YARNRecommender):
  ...
  def recommendConfigurationsForSSO(self, configurations, clusterData, services, hosts):
    ...
    if shpurdp_sso_details and shpurdp_sso_details.is_managing_services():
      ...
      # If SSO should be enabled for this service
      if shpurdp_sso_details.should_enable_sso("MAPREDUCE2"):
        if self.is_kerberos_enabled(configurations, services):
      ...
      # If SSO should be disabled for this service
      elif shpurdp_sso_details.should_disable_sso("MAPREDUCE2"):
        if self.is_kerberos_enabled(configurations, services):
