...
class KerberosServiceCheck(Script):
  def service_check(self, env):
    ...
    # If Shpurdp IS managing Kerberos identities (kerberos-env/manage_identities = true), it is
    ...
    #
    # If Shpurdp IS NOT managing Kerberos identities (kerberos-env/manage_identities = false), the
    # smoke test principal and its associated keytab file may not be available
    # **  If not available, this service check will execute
    # **  If available, this service check will execute

    if (
      (params.smoke_test_principal is not None)
      ...
      ccache_file_name = HASH_ALGORITHM(
        f"{params.smoke_test_principal}|{params.smoke_test_keytab_file}".encode("utf-8")
      ).hexdigest()
      ccache_file_path = (
        f"{params.tmp_dir}{os.sep}kerberos_service_check_cc_{ccache_file_name}"
      )
      ...
      # Shpurdp is not managing identities so if the smoke user does not exist, indicate why....
      print(
        "Skipping this service check since Shpurdp is not managing Kerberos identities and the smoke user "
