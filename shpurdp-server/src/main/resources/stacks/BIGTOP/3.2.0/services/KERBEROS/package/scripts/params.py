...
from shpurdp_commons.kerberos.utils import get_property_value, get_unstructured_data
...
kadm5_acl_dir = (
  kdc_conf_dir  # Typically kadm5.acl and kdc.conf exist in the same directory
)
...
smoke_user = "shpurdp-qa"

manage_identities = "true"

artifact_dir = format("{tmp_dir}/SHPURDP-artifacts/")
jce_policy_zip = default(
  "/shpurdpLevelParams/jce_name", None
)  # None when jdk is already installed by user
jce_location = config["shpurdpLevelParams"]["jdk_location"]
jdk_name = default("/shpurdpLevelParams/jdk_name", None)
...
if config is not None:
  ...
  if configurations is not None:
    ...
    if cluster_env is not None:
      ...
      smoke_user = get_property_value(
        cluster_env, "smokeuser", smoke_user, True, smoke_user
      )
    ...
    if krb5_conf_data is not None:
      ...
      admin_keytab = get_property_value(
        krb5_conf_data, "admin_keytab", admin_keytab, True, None
      )

      test_principal = get_property_value(
        krb5_conf_data, "test_principal", test_principal, True, None
      )
      test_password = get_property_value(
        krb5_conf_data, "test_password", test_password, True, None
      )
      test_keytab = get_property_value(
        krb5_conf_data, "test_keytab", test_keytab, True, None
      )
      test_keytab_file = get_property_value(
        krb5_conf_data, "test_keytab_file", test_keytab_file, True, None
      )

      krb5_conf_template = get_property_value(
        krb5_conf_data, "content", krb5_conf_template
      )
    ...
    # template in krb5-conf/content had not be updated during the Shpurdp upgrade to 2.4.0 - which
    ...
    if kdc_conf_data is not None:
      ...
      kdcdefaults_kdc_tcp_ports = get_property_value(
        kdc_conf_data, "kdcdefaults_kdc_tcp_ports", kdcdefaults_kdc_tcp_ports
      )

      kdc_conf_template = get_property_value(
        kdc_conf_data, "content", kdc_conf_template
      )
    ...
    if kadm5_acl_data is not None:
      kadm5_acl_template = get_property_value(
        kadm5_acl_data, "content", kadm5_acl_template
      )
