...
from shpurdp_commons.shell import RepoCallContext
from shpurdp_commons.str_utils import cbool, cint
...
class InstallPackages(Script):
  ...
  def actionexecute(self, env):
    ...
    try:
      if not command_repository.items:
        Logger.warning(
          "Repository list is empty. Shpurdp may not be managing the repositories for {0}.".format(
  ...
  def _fix_default_links_for_current(self):
    """
    If a prior version of Shpurdp did not correctly reverse the conf symlinks, then they would
