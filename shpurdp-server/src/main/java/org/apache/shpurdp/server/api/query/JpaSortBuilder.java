...
/**
 * The {@link JpaSortBuilder} class is used to convert and Shpurdp
...
public class JpaSortBuilder<T> {
   ...
   *
   * @param sortRequest
   *          the Shpurdp sort request properties to turn into a JPA sort
   *          request. If {@code null} or the {@link SortRequestProperty} list
   *          is null, an empty list is returned.
   * @param visitor
   *          a visitor that knows how to convert the Shpurdp properties into
  ...
  public List<Order> buildSortOrders(SortRequest sortRequest,
    ...
    for (SortRequestProperty sort : sortProperties) {
      ...
      if (sort.getOrder() == org.apache.shpurdp.server.controller.spi.SortRequest.Order.ASC) {
