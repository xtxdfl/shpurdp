...
/**
 * Provides LDAP user authorization logic for Shpurdp Server
 */
public class ShpurdpLdapAuthenticationProvider extends ShpurdpAuthenticationProvider {
  ...
  private static Logger LOG = LoggerFactory.getLogger(ShpurdpLdapAuthenticationProvider.class);

  final ShpurdpLdapConfigurationProvider ldapConfigurationProvider;

  private ShpurdpLdapAuthoritiesPopulator authoritiesPopulator;
  ...
  @Inject
  public ShpurdpLdapAuthenticationProvider(Users users, Configuration configuration, ShpurdpLdapConfigurationProvider ldapConfigurationProvider,
                                          ShpurdpLdapAuthoritiesPopulator authoritiesPopulator) {
    ...
    if (isLdapEnabled()) {
      ...
      try {
        ...
        if (userEntity == null) {
          ...
          return new ShpurdpUserAuthentication(null, userDetails, true);
        }
        ...
        String message = ldapConfigurationProvider.get().isLdapAlternateUserSearchEnabled()
            ...
            : "Login Failed: More than one user with that username found, please work with your Shpurdp Administrator to adjust your LDAP configuration";
  ...
  LdapAuthenticationProvider loadLdapAuthenticationProvider(String userName) {
    ...
    if (ldapConfigPropertiesChanged || !ldapUserSearchFilter.equals(ldapUserSearchFilterThreadLocal.get())) {
      ...
      ShpurdpLdapBindAuthenticator bindAuthenticator = new ShpurdpLdapBindAuthenticator(springSecurityContextSource, ldapConfigurationProvider.get());
  ...
  private String getLdapUserSearchFilter(String userName) {
    return ldapServerProperties.get()
        .getUserSearchFilter(ldapConfigurationProvider.get().isLdapAlternateUserSearchEnabled() && ShpurdpLdapUtils.isUserPrincipalNameFormat(userName));
  }
