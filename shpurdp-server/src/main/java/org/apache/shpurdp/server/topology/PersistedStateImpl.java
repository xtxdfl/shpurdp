...
import org.apache.shpurdp.server.orm.dao.TopologyLogicalRequestDAO;
...
/**
 * Implementation which uses Shpurdp Database DAO and Entity objects for persistence
...
public class PersistedStateImpl implements PersistedState {
  ...
  public LogicalRequest getProvisionRequest(long clusterId) {
    ...
    for (TopologyRequestEntity entity : entities) {
      if(TopologyRequest.Type.PROVISION == TopologyRequest.Type.valueOf(entity.getAction())) {
        ...
        try {
          ClusterTopology clusterTopology = new ClusterTopologyImpl(shpurdpContext, replayedRequest);
  ...
  public Map<ClusterTopology, List<LogicalRequest>> getAllRequests() {
    //todo: we only currently support a single request per shpurdp instance so there should only
    ...
    for (TopologyRequestEntity entity : entities) {
      ...
      if (clusterTopology == null) {
        try {
          clusterTopology = new ClusterTopologyImpl(shpurdpContext, replayedRequest);
      ...
      if (logicalRequestEntity != null) {
        try {
          ...
          shpurdpContext.getNextRequestId();
