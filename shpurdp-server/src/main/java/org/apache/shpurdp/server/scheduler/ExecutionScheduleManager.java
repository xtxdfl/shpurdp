...
package org.apache.shpurdp.server.scheduler;

import static org.apache.shpurdp.server.state.scheduler.RequestExecution.Status.ABORTED;
import static org.apache.shpurdp.server.state.scheduler.RequestExecution.Status.PAUSED;
import static org.apache.shpurdp.server.state.scheduler.RequestExecution.Status.SCHEDULED;
...
import org.apache.shpurdp.server.controller.internal.RequestImpl;
...
import org.apache.shpurdp.server.security.authorization.internal.InternalTokenClientFilter;
import org.apache.shpurdp.server.security.authorization.internal.InternalTokenStorage;
import org.apache.shpurdp.server.state.Cluster;
import org.apache.shpurdp.server.state.Clusters;
import org.apache.shpurdp.server.state.scheduler.Batch;
import org.apache.shpurdp.server.state.scheduler.BatchRequest;
import org.apache.shpurdp.server.state.scheduler.BatchRequestJob;
import org.apache.shpurdp.server.state.scheduler.BatchRequestResponse;
import org.apache.shpurdp.server.state.scheduler.BatchSettings;
import org.apache.shpurdp.server.state.scheduler.RequestExecution;
import org.apache.shpurdp.server.state.scheduler.Schedule;
...
public class ExecutionScheduleManager {
  ...
  protected Client shpurdpClient;
  protected WebTarget shpurdpWebResource;
  ...
  protected void buildApiClient() throws NoSuchAlgorithmException, KeyManagementException {
    ...
    this.shpurdpClient = client;
    this.shpurdpWebResource = client.target(url);
    ...
    shpurdpWebResource.register(csrfFilter);
    shpurdpWebResource.register(tokenFilter);
  ...
  private long getFirstJobOrderId(RequestExecution requestExecution) throws ShpurdpException {
    ...
    if (firstBatchOrderId == null) {
      throw new ShpurdpException("Can't schedule RequestExecution with no batches");
    }
   ...
   */
  public void scheduleAllBatches(RequestExecution requestExecution) throws ShpurdpException {
  ...
  public void scheduleBatch(RequestExecution requestExecution, long startingBatchOrderId)
    ...
    if (!isSchedulerAvailable()) {
      throw new ShpurdpException("Scheduler unavailable.");
    }
    ...
    if (firstJobDetail == null) {
      throw new ShpurdpException("Unable to schedule jobs. firstJobDetail = "
        + firstJobDetail);
    }
    ...
    if (schedule != null) {
      ...
      try {
        ...
        throw new ShpurdpException(e.getMessage());
      }
  ...
  private Integer countFailedTasksBeforeStartingBatch(RequestExecution requestExecution, long startingBatchOrderId) throws ShpurdpException {
    int result = 0;
    Batch batch = requestExecution.getBatch();
    if (batch != null) {
      List<BatchRequest> batchRequests = batch.getBatchRequests();
   ...
   * @return
   */
  public void validateSchedule(Schedule schedule) throws ShpurdpException {
    Date startDate = null;
    Date endDate = null;
    if (!schedule.isEmpty()) {
      if (schedule.getStartTime() != null && !schedule.getStartTime().isEmpty()) {
        try {
          ...
          throw new ShpurdpException("Start time in invalid format. startTime "
      ...
      if (schedule.getEndTime() != null && !schedule.getEndTime().isEmpty()) {
        try {
          ...
          throw new ShpurdpException("End time in invalid format. endTime "
      ...
      if (endDate != null) {
        if (endDate.before(new Date())) {
          throw new ShpurdpException("End date should be in the future. " +
            "endDate = " + endDate);
        }
        if (startDate != null && endDate.before(startDate)) {
          throw new ShpurdpException("End date cannot be before start date. " +
      ...
      if (cronExpression != null && !cronExpression.trim().isEmpty()) {
        if (!CronExpression.isValidExpression(cronExpression)) {
          throw new ShpurdpException("Invalid non-empty cron expression " +
   ...
   * @throws ShpurdpException
   */
  public void deleteAllJobs(RequestExecution requestExecution) throws ShpurdpException {
    Long firstBatchOrderId = getFirstJobOrderId(requestExecution);
   ...
   * @throws ShpurdpException
   */
  public void deleteJobs(RequestExecution requestExecution, Long startingBatchOrderId) throws ShpurdpException {
    if (!isSchedulerAvailable()) {
      throw new ShpurdpException("Scheduler unavailable.");
    }
  ...
  public Long executeBatchRequest(long executionId,
                                  long batchId,
                                  String clusterName) throws ShpurdpException {
    ...
    try {
      ...
      throw new ShpurdpException("Exception occurred while performing request", e);
    }
  ...
  protected RequestStatus abortRequestById(RequestExecution requestExecution, Long requestId) throws ShpurdpException {
    ...
    org.apache.shpurdp.server.controller.spi.Request request = new RequestImpl(Collections.emptySet(),
    ...
    try{
     ...
     throw new ShpurdpException("Error while aborting the request.", e);
    }
  }
  ...
  public String getBatchRequestStatus(Long executionId, String clusterName) throws ShpurdpException {
    Cluster cluster = clusters.getCluster(clusterName);
    RequestExecution requestExecution = cluster.getAllRequestExecutions().get(executionId);

    if (requestExecution == null) {
      throw new ShpurdpException("Unable to find request schedule with id = "
          + executionId);
    }
  ...
  public void updateBatchRequest(long executionId, long batchId, String clusterName,
                                 ...
                                 boolean statusOnly) throws ShpurdpException {
...
/*
  protected BatchRequestResponse performUriRequest(String url, String body, String method) {
    ...
    try {
      response = shpurdpClient.resource(url).entity(body).method(method, ClientResponse.class);
  ...
  /*protected BatchRequestResponse performApiGetRequest(String relativeUri, boolean queryAllFields) {
    WebResource webResource = extendApiResource(shpurdpWebResource, relativeUri);
    if (queryAllFields) {
  ...
  public boolean hasToleranceThresholdExceeded(Long executionId,
      String clusterName, Map<String, Integer> taskCounts) throws ShpurdpException {
      ...
      if (jobDetail != null) {
        try {
          ...
          if (triggers != null && triggers.size() > 0) {
            if (triggers.size() > 1) {
              throw new ShpurdpException("Too many triggers defined for job. " +
                "job: " + jobKey);
            }
   ...
   * @param webTarget Shpurdp WebResource as provided by the client {@link #shpurdpWebResource}
   ...
   */
  public void pauseAfterBatchIfNeeded(long executionId, long batchId, String clusterName) throws ShpurdpException {
    Cluster cluster = clusters.getCluster(clusterName);
    RequestExecution requestExecution = cluster.getAllRequestExecutions().get(executionId);
