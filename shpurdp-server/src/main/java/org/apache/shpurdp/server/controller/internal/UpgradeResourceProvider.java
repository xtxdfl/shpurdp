...
import org.apache.shpurdp.server.controller.ActionExecutionContext;
import org.apache.shpurdp.server.controller.ShpurdpActionExecutionHelper;
...
import org.apache.shpurdp.server.controller.ExecuteCommandJson;
import org.apache.shpurdp.server.controller.KerberosHelper;
import org.apache.shpurdp.server.controller.KerberosHelperImpl.SupportedCustomOperation;
...
import org.apache.shpurdp.server.orm.entities.UpgradeGroupEntity;
import org.apache.shpurdp.server.orm.entities.UpgradeHistoryEntity;
import org.apache.shpurdp.server.orm.entities.UpgradeItemEntity;
...
import org.apache.shpurdp.server.serveraction.upgrades.PluginUpgradeServerAction;
import org.apache.shpurdp.server.stack.upgrade.AddComponentTask;
...
import org.apache.shpurdp.server.stack.upgrade.CreateAndConfigureTask;
import org.apache.shpurdp.server.stack.upgrade.Direction;
import org.apache.shpurdp.server.stack.upgrade.ManualTask;
import org.apache.shpurdp.server.stack.upgrade.ServerSideActionTask;
import org.apache.shpurdp.server.stack.upgrade.Task;
import org.apache.shpurdp.server.stack.upgrade.UpdateStackGrouping;
import org.apache.shpurdp.server.stack.upgrade.UpgradePack;
import org.apache.shpurdp.server.stack.upgrade.UpgradeScope;
import org.apache.shpurdp.server.stack.upgrade.orchestrate.StageWrapper;
import org.apache.shpurdp.server.stack.upgrade.orchestrate.TaskWrapper;
...
import org.apache.shpurdp.server.stack.upgrade.orchestrate.UpgradeGroupHolder;
...
import org.apache.shpurdp.spi.upgrade.UpgradeAction;
...
public class UpgradeResourceProvider extends AbstractControllerResourceProvider {
   ...
   * "host_order": [
   *   { "hosts":
   *       [ "c6401.shpurdp.apache.org, "c6402.shpurdp.apache.org", "c6403.shpurdp.apache.org" ],
   *     "service_checks": ["ZOOKEEPER"]
   *   },
   *   {
   *     "hosts": [ "c6404.shpurdp.apache.org, "c6405.shpurdp.apache.org"],
  ...
  @Inject
  private static Provider<ShpurdpMetaInfo> s_metaProvider = null;
  ...
  @Inject
  private static Provider<ShpurdpActionExecutionHelper> s_actionExecutionHelper;

  @Inject
  private static Provider<ShpurdpCustomCommandExecutionHelper> s_commandExecutionHelper;
   ...
   */
  @Inject
  public UpgradeResourceProvider(@Assisted ShpurdpManagementController controller) {
  ...
  public RequestStatus createResources(final Request request) throws SystemException,
    ...
    UpgradeEntity entity = createResources(new Command<UpgradeEntity>() {
      @Override
      public UpgradeEntity invoke() throws ShpurdpException, AuthorizationException {
  ...
  public RequestStatus updateResources(final Request request, Predicate predicate)
    ...
    if (null != requestStatus) {
      ...
      try {
        ...
        throw new SystemException(shpurdpException.getMessage(), shpurdpException);
      }
    }
   ...
   * {@link org.apache.shpurdp.server.orm.entities.UpgradeItemEntity}, whose
  ...
  protected UpgradeEntity createUpgrade(UpgradeContext upgradeContext)
    ...
    if (groups.isEmpty()) {
      throw new ShpurdpException("There are no groupings available");
    }
    ...
    if (pack.getType() == UpgradeType.NON_ROLLING) {
      ...
      if (!foundUpdateDesiredRepositoryIdGrouping) {
        throw new ShpurdpException(String.format(
    ...
    The Upgrade Pack is responsible for calling {@link org.apache.shpurdp.server.serveraction.upgrades.UpdateDesiredRepositoryAction}
        ...
        switch(wrapper.getType()) {
          ...
          case REGENERATE_KEYTABS: {
            try {
              ...
              throw new ShpurdpException("Unable to build keytab regeneration stage",
  ...
  UpgradeEntity createUpgradeInsideTransaction(Cluster cluster,
      ...
      UpgradeEntity upgradeEntity, UpgradeContext upgradeContext) throws ShpurdpException {
  ...
  private RequestStageContainer createRequest(UpgradeContext upgradeContext) throws ShpurdpException {
  ...
  private void makeActionStage(UpgradeContext context, RequestStageContainer request,
      ...
      boolean allowRetry) throws ShpurdpException {
    ...
    Stage stage = s_stageFactory.get().createNew(request.getId().longValue(), "/tmp/shpurdp",
  ...
  private boolean makeServerSideStage(UpgradeGroupHolder group, UpgradeContext context,
    ...
    ActionExecutionContext actionContext = buildActionExecutionContext(cluster, context,
        Role.SHPURDP_SERVER_ACTION.toString(), stackId, Collections.emptyList(),
    ...
    // is not one, then default to Shpurdp's classloader
    ...
    try {
      ...
      if (UpgradeAction.class.isAssignableFrom(clazz)) {
        ...
        throw new ShpurdpException("The class " + taskClass
    ...
    stage.addServerActionCommand(classToSchedule,
        getManagementController().getAuthName(), Role.SHPURDP_SERVER_ACTION, RoleCommand.EXECUTE,
  ...
  void setUpgradeRequestStatus(Cluster cluster, long requestId, HostRoleStatus status,
      boolean suspended, Map<String, Object> propertyMap) throws ShpurdpException {
  ...
  private void addComponentHistoryToUpgrade(Cluster cluster, UpgradeEntity upgrade,
      UpgradeContext upgradeContext) throws ShpurdpException {
