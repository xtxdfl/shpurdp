...
public class AlertServiceStateListener {
   ...
   * {@link ShpurdpServer#main(String[])} and the ordering of
   * {@link ControllerModule} and {@link GuiceJpaInitializer}.
   */
  @Inject
  private Provider<ShpurdpMetaInfo> m_metaInfoProvider;
   ...
   */
  @Inject
  public AlertServiceStateListener(ShpurdpEventPublisher publisher) {
      ...
      if (null == m_alertDispatchDao.findDefaultServiceGroup(clusterId, serviceName)) {
        try {
          ...
          LOG.error("Unable to create a default alert group for {}",
            event.getServiceName(), shpurdpException);
        }
      }
  ...
  @Subscribe
  @AllowConcurrentEvents
  public void onShpurdpEvent(ServiceRemovedEvent event) {
    LOG.debug("Received event {}", event);
