 ...
 * advertised by the {@link org.apache.shpurdp.server.state.ServiceComponentHost}
 * that bubble up to the
 * {@link org.apache.shpurdp.server.orm.entities.HostVersionEntity}.
 */
@Singleton
@EagerSingleton
public class StackVersionListener {
  ...
  public StackVersionListener(VersionEventPublisher eventPublisher,
    ...
    this.shpurdpMetaInfoProvider = shpurdpMetaInfoProvider;
    this.m_hostLevelParamsHolder = m_hostLevelParamsHolder;
    eventPublisher.register(this);
  }

  @Subscribe
  public void onShpurdpEvent(HostComponentVersionAdvertisedEvent event) {
    ...
    try {
      ...
      ShpurdpMetaInfo shpurdpMetaInfo = shpurdpMetaInfoProvider.get();
      ComponentInfo componentInfo = shpurdpMetaInfo.getComponent(desiredStackId.getStackName(),
  ...
  private void processComponentAdvertisedVersion(Cluster cluster, ServiceComponent sc,
      ServiceComponentHost sch, String newVersion) throws ShpurdpException {
    ...
    if (previousVersion == null || StringUtils.equalsIgnoreCase(UNKNOWN_VERSION, previousVersion)) {
      // value may be "UNKNOWN" when upgrading from older Shpurdp versions
  ...
  /**
   * Possible situation after upgrade from older Shpurdp version. Just use
  ...
  private void processUnknownDesiredVersion(Cluster cluster, ServiceComponent sc,
                                            ...
                                            String newVersion) throws ShpurdpException {
    sch.setUpgradeState(UpgradeState.NONE);
    sch.setVersion(newVersion);
    sch.recalculateHostVersionState();
  }
  ...
  private void setUpgradeStateAndRecalculateHostVersions(ServiceComponentHost sch,
      UpgradeState upgradeState) throws ShpurdpException {
