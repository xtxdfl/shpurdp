...
import static org.apache.shpurdp.server.controller.ShpurdpManagementControllerImpl.CLUSTER_PHASE_INITIAL_INSTALL;
import static org.apache.shpurdp.server.controller.ShpurdpManagementControllerImpl.CLUSTER_PHASE_INITIAL_START;
import static org.apache.shpurdp.server.controller.ShpurdpManagementControllerImpl.CLUSTER_PHASE_PROPERTY;
import static org.apache.shpurdp.server.controller.internal.RequestResourceProvider.CONTEXT;
...
import org.apache.shpurdp.server.controller.predicate.NotPredicate;
...
import org.apache.shpurdp.server.state.svccomphost.ServiceComponentHostDisableEvent;
import org.apache.shpurdp.server.state.svccomphost.ServiceComponentHostRestoreEvent;
...
public class HostComponentResourceProvider extends AbstractControllerResourceProvider {
   ...
   */
  @AssistedInject
  public HostComponentResourceProvider(@Assisted ShpurdpManagementController managementController) {
  ...
  private Set<Resource> findResources(Request request, final Predicate predicate,
    ...
    Set<ServiceComponentHostResponse> responses = getResources(new Command<Set<ServiceComponentHostResponse>>() {
      @Override
      public Set<ServiceComponentHostResponse> invoke() throws ShpurdpException {
  ...
  protected RequestStatus deleteResourcesAuthorized(Request request, Predicate predicate)
    ...
    DeleteStatusMetaData deleteStatusMetaData = modifyResources(new Command<DeleteStatusMetaData>() {
      @Override
      public DeleteStatusMetaData invoke() throws ShpurdpException, AuthorizationException {
   ...
   */
  //todo: This was moved from ShpurdpManagementController and needs a lot of refactoring.
  ...
  protected RequestStageContainer updateHostComponents(RequestStageContainer stages,
                                                                    ...
                                                                    boolean useClusterHostInfo) throws ShpurdpException, AuthorizationException {
    ...
    for (ServiceComponentHostRequest request : requests) {
      ...
      if (! isValidStateTransition(stages, oldSchState, newState, sch)) {
        throw new ShpurdpException("Invalid state transition for host component"
  ...
  public RequestStageContainer doUpdateResources(final RequestStageContainer stages, final Request request,
    ...
    RequestStageContainer requestStages = modifyResources(new Command<RequestStageContainer>() {
      @Override
      public RequestStageContainer invoke() throws ShpurdpException {
...
//    return State.isValidStateTransition(startState, desiredState);
  ...
  private void doDirectTransitions(Map<ServiceComponentHost, State> directTransitionScHosts) throws ShpurdpException {
    for (Map.Entry<ServiceComponentHost, State> entry : directTransitionScHosts.entrySet()) {
      ...
      switch (newState) {
        ...
        case INSTALLED:
          ...
          break;
        default:
          throw new ShpurdpException("Direct transition from " + componentHost.getState() + " to " + newState + " not supported");
      }
      try {
        componentHost.handleEvent(event);
      } catch (InvalidStateTransitionException e) {
        //Should not occur, must be covered by previous checks
        throw new ShpurdpException("Internal error - not supported transition", e);
      }
    }
  }
