...
import org.apache.shpurdp.server.controller.PrivilegeResponse;
import org.apache.shpurdp.server.controller.UserAuthenticationSourceRequest;
import org.apache.shpurdp.server.controller.UserAuthenticationSourceResponse;
...
public class UserAuthenticationSourceResourceProvider extends AbstractAuthorizedResourceProvider {
  ...
  public UserAuthenticationSourceResourceProvider() {
    ...
    EnumSet<RoleAuthorization> requiredAuthorizations = EnumSet.of(RoleAuthorization.SHPURDP_MANAGE_USERS);
  ...
  public Set<Resource> getResources(Request request, Predicate predicate)
    ...
    Set<UserAuthenticationSourceResponse> responses = getResources(new Command<Set<UserAuthenticationSourceResponse>>() {
      @Override
      public Set<UserAuthenticationSourceResponse> invoke() throws ShpurdpException, AuthorizationException {
   ...
   * @throws ShpurdpException when the user authentication source cannot be created.
   */
  private void createUserAuthenticationSources(Set<UserAuthenticationSourceRequest> requests) throws ShpurdpException {
    for (UserAuthenticationSourceRequest request : requests) {
      ...
      if (authenticationType == null) {
        throw new ShpurdpException("A value authentication type must be supplied.");
      }

      UserEntity userEntity = users.getUserEntity(username);
      if (userEntity == null) {
        throw new ShpurdpException("There is no user with the supplied username");
      }
   ...
   * @throws ShpurdpException if the user authentication sources could not be read
   */
  ...
  private void updateUserAuthenticationSources(Set<UserAuthenticationSourceRequest> requests) throws AuthorizationException, ShpurdpException {
    ...
    for (UserAuthenticationSourceRequest request : requests) {
       ...
       *  ************************************************** */
      if (!isSelf && !AuthorizationHelper.isAuthorized(ResourceType.SHPURDP, null, RoleAuthorization.SHPURDP_MANAGE_USERS)) {
  ...
  private Resource toResource(UserAuthenticationSourceResponse response, Set<String> requestedIds) {
    ...
    // any sensitive information.  Once set that data should stay internal to Shpurdp.
