 ...
 */
@Singleton
public class ShpurdpActionExecutionHelper {
  private final static Logger LOG =
      LoggerFactory.getLogger(ShpurdpActionExecutionHelper.class);
  ...
  public void validateAction(ExecuteActionRequest actionRequest) throws ShpurdpException {
    if (actionRequest.getActionName() == null || actionRequest.getActionName().isEmpty()) {
      throw new ShpurdpException("Action name must be specified");
    }

    ActionDefinition actionDef = shpurdpMetaInfo.getActionDefinition(actionRequest.getActionName());
    if (actionDef == null) {
      throw new ShpurdpException("Action " + actionRequest.getActionName() + " does not exist");
    }
      ...
      for (String input : inputs) {
        ...
        if (!inputName.isEmpty()) {
          ...
          if (mandatory && !actionRequest.getParameters().containsKey(inputName)) {
            throw new ShpurdpException("Action " + actionRequest.getActionName() + " requires input '" +
    ...
    if (resourceFilters != null && !resourceFilters.isEmpty()) {
      if (resourceFilters.size() > 1) {
        throw new ShpurdpException("Custom action definition only allows one " +
    ...
    if (null != actionRequest.getClusterName()) {
      ...
      if (cluster == null) {
        throw new ShpurdpException("Unable to find cluster. clusterName = " +
      ...
      if (!expectedService.isEmpty() && !actualService.isEmpty() && !expectedService.equals(actualService)) {
        throw new ShpurdpException("Action " + actionRequest.getActionName() + " targets service " + actualService +
      ...
      if (StringUtils.isNotBlank(targetService)) {
        ...
        try {
          serviceInfo = shpurdpMetaInfo.getService(stackId.getStackName(), stackId.getStackVersion(),
            targetService);
        ...
        if (serviceInfo == null) {
          throw new ShpurdpException("Action " + actionRequest.getActionName() +
      ...
      if (!expectedComponent.isEmpty() && !actualComponent.isEmpty() && !expectedComponent.equals(actualComponent)) {
        throw new ShpurdpException("Action " + actionRequest.getActionName() + " targets component " + actualComponent +
      ...
      if (StringUtils.isNotBlank(targetComponent) && StringUtils.isBlank(targetService)) {
        throw new ShpurdpException("Action " + actionRequest.getActionName() + " targets component " + targetComponent +
      ...
      if (StringUtils.isNotBlank(targetComponent)) {
        ...
        try {
          compInfo = shpurdpMetaInfo.getComponent(stackId.getStackName(), stackId.getStackVersion(),
    ...
    if (TargetHostType.SPECIFIC.equals(targetHostType)
      ...
      if ((resourceFilter == null || resourceFilter.getHostNames().size() == 0) && !isTargetHostTypeAllowsEmptyHosts(targetHostType)) {
        throw new ShpurdpException("Action " + actionRequest.getActionName() + " requires explicit target host(s)" +
   ...
   * @throws ShpurdpException if the task can not be added
   */
  public void addExecutionCommandsToStage(final ActionExecutionContext actionContext, Stage stage,
                                          Map<String, String> requestParams) throws ShpurdpException {
   ...
   * @param stage
   * @param requestParams
   * @param checkHostIsMemberOfCluster if true ShpurdpException will be thrown in case host is not member of cluster.
   * @throws ShpurdpException
   */
  public void addExecutionCommandsToStage(final ActionExecutionContext actionContext, Stage stage,
...
//      StackId stackId = cluster.getCurrentStackVersion();
      if (serviceName != null && !serviceName.isEmpty()) {
        if (componentName != null && !componentName.isEmpty()) {
          ...
          try {
            componentInfo = shpurdpMetaInfo.getComponent(stackId.getStackName(),
    ...
    if (resourceFilter.getHostNames().isEmpty() && candidateHosts.isEmpty()) {
      throw new ShpurdpException("Suitable hosts not found, component="
      ...
      if (!resourceFilter.getHostNames().isEmpty() && !candidateHosts.isEmpty()) {
        for (String hostname : resourceFilter.getHostNames()) {
          if (!candidateHosts.contains(hostname)) {
            throw new ShpurdpException("Request specifies host " + hostname +
    ...
    if (targetHosts.isEmpty()) {
      ...
      switch (hostType) {
        ...
        case MAJORITY:
          ...
          break;
        default:
          throw new ShpurdpException("Unsupported target type = " + hostType);
      }
    }
  ...
  * */
  ...
  private void setAdditionalParametersForStageAccordingToAction(Stage stage, ActionExecutionContext actionExecutionContext) throws ShpurdpException {
    if (actionExecutionContext.getActionName().equals(ACTION_UPDATE_REPO)) {
      ...
      if (params != null && params.containsKey(SUCCESS_FACTOR_PARAMETER)) {
        try{
          ...
          throw new ShpurdpException("Failed to cast success_factor value to float!", ex.getCause());
        }
      }
