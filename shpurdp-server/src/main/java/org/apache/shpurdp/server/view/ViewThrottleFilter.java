 ...
 * do not cause a loss of service for Shpurdp. The underlying problem is that
 * views are accessed off of the REST endpoint (/api/v1/views). This means that
 * the Shpurdp REST API connector is going to handle the request from its own
 ...
 * and their assumption that if they run on the Shpurdp server they can share the
 * same session.
...
public class ViewThrottleFilter implements Filter {
  ...
  public void init(FilterConfig filterConfig) throws ServletException {
    ...
    LOG.info("Shpurdp Views will be able to utilize {} concurrent REST API threads",
        viewSemaphoreCount);
