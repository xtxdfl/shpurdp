 ...
 */
package org.apache.shpurdp.server.state.host;
...
import org.apache.shpurdp.server.agent.AgentEnv;
import org.apache.shpurdp.server.agent.DiskInfo;
import org.apache.shpurdp.server.agent.HostInfo;
import org.apache.shpurdp.server.agent.RecoveryReport;
...
import org.apache.shpurdp.server.events.HostStateUpdateEvent;
import org.apache.shpurdp.server.events.HostStatusUpdateEvent;
...
import org.apache.shpurdp.server.orm.cache.HostConfigMappingImpl;
...
import org.apache.shpurdp.server.orm.entities.HostStateEntity;
...
import org.apache.shpurdp.server.state.HostConfig;
import org.apache.shpurdp.server.state.HostEvent;
import org.apache.shpurdp.server.state.HostEventType;
...
import org.apache.shpurdp.server.state.fsm.SingleArcTransition;
import org.apache.shpurdp.server.state.fsm.StateMachine;
import org.apache.shpurdp.server.state.fsm.StateMachineFactory;
...
public class HostImpl implements Host {
  ...
  public void setState(HostState state) {
    ...
    shpurdpEventPublisher.publish(new HostStateUpdateEvent(getHostName(), state));
  ...
  public void handleEvent(HostEvent event)
    ...
    if (oldState != getState()) {
      shpurdpEventPublisher.publish(new HostStateUpdateEvent(getHostName(), getState()));
      if (LOG.isDebugEnabled()) {
  ...
  public void setStatus(String status) {
    if (!Objects.equals(this.status, status)) {
      shpurdpEventPublisher.publish(new HostStatusUpdateEvent(getHostName(), status));
    }
    this.status = status;
  }
  ...
  public Map<String, HostConfig> getDesiredHostConfigs(Cluster cluster,
      Map<String, DesiredConfig> clusterDesiredConfigs) throws ShpurdpException {
   ...
   */
  @Override
  public boolean hasComponentsAdvertisingVersions(StackId stackId) throws ShpurdpException {
    HostEntity hostEntity = getHostEntity();

    for (HostComponentStateEntity componentState : hostEntity.getHostComponentStateEntities()) {
      ComponentInfo component = shpurdpMetaInfo.getComponent(stackId.getStackName(),
  ...
  @Override
  public void calculateHostStatus(Long clusterId) throws ShpurdpException {
    ...
    for (ServiceComponentHost scHost : scHosts) {
      ComponentInfo componentInfo =
          shpurdpMetaInfo.getComponent(stackId.getStackName(),
