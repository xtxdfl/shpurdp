...
import org.apache.shpurdp.server.controller.predicate.ArrayPredicate;
...
import org.apache.shpurdp.server.controller.spi.RequestStatusMetaData;
...
/**
 * Abstract resource provider implementation that maps to an Shpurdp management controller.
 */
public abstract class AbstractResourceProvider extends BaseProvider implements ResourceProvider, ObservableResourceProvider {
  ...
  /**
   * Invoke a command against the Shpurdp backend to create resources and map
   * any {@link ShpurdpException} to the types appropriate for the
  ...
  protected <T> T createResources(Command<T> command)
    ...
    try {
      ...
      if (LOG.isErrorEnabled()) {
        LOG.error("Caught ShpurdpException when creating a resource", e);
      }
  ...
  /**
   * Invoke a command against the Shpurdp backend to get resources and map
   * any {@link ShpurdpException} to the types appropriate for the
   * {@link ResourceProvider} interface.
   *
   * @param command  the command to invoke
  ...
  protected <T> T getResources (Command<T> command)
    ...
    try {
      ...
      if (LOG.isErrorEnabled()) {
        LOG.error("Caught ShpurdpException when getting a resource", e);
      }
      throw new SystemException("An internal system exception occurred: " + e.getMessage(), e);
    }
  }
  ...
  /**
   * Invoke a command against the Shpurdp backend to modify resources and map
   * any {@link ShpurdpException} to the types appropriate for the
   * {@link ResourceProvider} interface.
   *
   * @param command  the command to invoke
  ...
  protected <T> T modifyResources (Command<T> command)
    ...
    try {
      ...
      if (LOG.isErrorEnabled()) {
        LOG.error("Caught ShpurdpException when modifying a resource", e);
      }
      throw new SystemException("An internal system exception occurred: " + e.getMessage(), e);
    }
  }
  ...
  private <T> T invokeWithRetry(Command<T> command) throws ShpurdpException, AuthorizationException {
  ...
  protected interface Command<T> {
     ...
     */
    T invoke() throws ShpurdpException, AuthorizationException;
  }
}
