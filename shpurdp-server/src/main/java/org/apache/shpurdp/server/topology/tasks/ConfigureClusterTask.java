 ...
 */
package org.apache.shpurdp.server.topology.tasks;
...
import org.apache.shpurdp.server.topology.AsyncCallableService;
import org.apache.shpurdp.server.topology.ClusterConfigurationRequest;
...
public class ConfigureClusterTask implements Callable<Boolean> {
  ...
  public ConfigureClusterTask(@Assisted ClusterTopology topology, @Assisted ClusterConfigurationRequest configRequest,
                              @Assisted ShpurdpEventPublisher shpurdpEventPublisher) {
  ...
  private void notifyListeners() throws ShpurdpException {
    long clusterId = topology.getClusterId();
    String clusterName = topology.getShpurdpContext().getClusterName(clusterId);
    shpurdpEventPublisher.publish(new ClusterConfigFinishedEvent(clusterId, clusterName));
  }
