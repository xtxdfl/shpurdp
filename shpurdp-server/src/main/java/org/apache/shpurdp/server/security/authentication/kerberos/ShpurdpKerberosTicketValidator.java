...
/**
 * ShpurdpKerberosTicketValidator is a {@link KerberosTicketValidator} implementation that delegates
 ...
 * <ul>
 * <li>{@link SunJaasKerberosTicketValidator#setServicePrincipal(String)} using the Shpurdp server property from {@link Configuration#KERBEROS_AUTH_SPNEGO_PRINCIPAL}</li>
 * <li>{@link SunJaasKerberosTicketValidator#setKeyTabLocation(Resource)} using the Shpurdp server property from {@link Configuration#KERBEROS_AUTH_SPNEGO_KEYTAB_FILE}</li>
 * </ul>
 */
@Component
public class ShpurdpKerberosTicketValidator implements KerberosTicketValidator, InitializingBean {

  private final SunJaasKerberosTicketValidator kerberosTicketValidator;

  /**
   * Creates a new ShpurdpKerberosTicketValidator
   *
   * @param configuration the Shpurdp server configuration
   */
  public ShpurdpKerberosTicketValidator(Configuration configuration) {

    ShpurdpKerberosAuthenticationProperties properties = (configuration == null)
        ? null
