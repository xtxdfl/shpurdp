...
/**
 * An authenticator which binds as a user and checks if user should get shpurdp
 * admin authorities according to LDAP group membership
 */
public class ShpurdpLdapBindAuthenticator extends AbstractLdapAuthenticator {
  private static final Logger LOG = LoggerFactory.getLogger(ShpurdpLdapBindAuthenticator.class);
  private static final String SHPURDP_ADMIN_LDAP_ATTRIBUTE_KEY = "shpurdp_admin";

  private final ShpurdpLdapConfiguration ldapConfiguration;


  public ShpurdpLdapBindAuthenticator(BaseLdapPathContextSource contextSource, ShpurdpLdapConfiguration ldapConfiguration) {
    super(contextSource);
  ...
  public DirContextOperations authenticate(Authentication authentication) {
    ...
    if (StringUtils.isNotEmpty(ldapServerProperties.getAdminGroupMappingRules())) {
      setShpurdpAdminAttr(user, ldapServerProperties);
    }

    // Users stored locally in shpurdp are matched against LDAP users by the ldap attribute configured to be used as user name.
    // (e.g. uid, sAMAccount -> shpurdp user name )
    ...
    if (ldapUserName == null) {
      ...
      // with a login name that is different (e.g. user principal name) from the shpurdp user name stored in
      // shpurdp db. In this case add the user login name  as login alias for shpurdp user name.
  ...
  private DirContextOperations bind(DirContextOperations user, String password) {
    ...
    Name fullDn = ShpurdpLdapUtils.getFullDn(userDistinguishedName, baseLdapPathContextSource.getBaseLdapName());
  ...
  /**
   * Checks weather user is a member of shpurdp administrators group in LDAP. If
   * yes, sets user's shpurdp_admin attribute to true
   *
   * @param user the user details
   * @return the updated user details
   */
  private DirContextOperations setShpurdpAdminAttr(DirContextOperations user, LdapServerProperties ldapServerProperties) {
    ...
    String setShpurdpAdminAttrFilter = resolveShpurdpAdminAttrFilter(ldapServerProperties, memberValue);
    LOG.debug("LDAP login - set admin attr filter: {}", setShpurdpAdminAttrFilter);
    ...
    List<String> shpurdpAdminGroups = ldapTemplate.search(groupBase, setShpurdpAdminAttrFilter, attributesMapper);

    //user has admin role granted, if user is a member of at least 1 group,
    // which matches the rules in configuration
    if (shpurdpAdminGroups.size() > 0) {
      user.setAttributeValue(SHPURDP_ADMIN_LDAP_ATTRIBUTE_KEY, true);
    }

    return user;
  }

  private String resolveShpurdpAdminAttrFilter(LdapServerProperties ldapServerProperties, String memberValue) {
    ...
    String setShpurdpAdminAttrFilter;
    if (StringUtils.isEmpty(groupSearchFilter)) {
      ...
      setShpurdpAdminAttrFilter = String.format("(&(%s=%s)(objectclass=%s)(|%s))",
          groupMembershipAttr,
          memberValue,
          groupObjectClass,
          adminGroupMappingRegex);
    } else {
      setShpurdpAdminAttrFilter = String.format("(&(%s=%s)%s)",
          groupMembershipAttr,
          memberValue,
          groupSearchFilter);
    }
    return setShpurdpAdminAttrFilter;
  }
