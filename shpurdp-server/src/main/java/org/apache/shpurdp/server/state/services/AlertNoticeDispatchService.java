...
import org.apache.shpurdp.server.notifications.DispatchCredentials;
import org.apache.shpurdp.server.notifications.DispatchFactory;
import org.apache.shpurdp.server.notifications.DispatchRunnable;
...
public class AlertNoticeDispatchService extends AbstractScheduledService {
   ...
   */
  private static final String VELOCITY_LOG_TAG = "shpurdp-alerts";

  /**
   * The internal Shpurdp templates that ship.
   */
  private static final String SHPURDP_ALERT_TEMPLATES = "alert-templates.xml";

  /**
   * The property containing the dispatch authentication username.
   */
  public static final String SHPURDP_DISPATCH_CREDENTIAL_USERNAME = "shpurdp.dispatch.credential.username";

  /**
   * The property containing the dispatch authentication password.
   */
  public static final String SHPURDP_DISPATCH_CREDENTIAL_PASSWORD = "shpurdp.dispatch.credential.password";

  /**
   * The property containing the dispatch recipients
   */
  public static final String SHPURDP_DISPATCH_RECIPIENTS = "shpurdp.dispatch.recipients";

  /**
   * The context key for Shpurdp information to be passed to Velocity.
   */
  private static final String VELOCITY_SHPURDP_KEY = "shpurdp";
  ...
  /**
   * The configuration instance to get Shpurdp properties.
   */
  @Inject
  private Configuration m_configuration;

  /**
   * Shpurdp meta information used fro alert {@link Notification}s.
   */
  @Inject
  private Provider<ShpurdpMetaInfo> m_metaInfo;
  ...
  protected void startUp() throws Exception {
    ...
    try {
      ...
      // the internal one
      if (null == inputStream) {
        inputStream = ClassLoader.getSystemResourceAsStream(SHPURDP_ALERT_TEMPLATES);
      }
      ...
      LOG.error(
          ...
          SHPURDP_ALERT_TEMPLATES, exception);
    } finally {
      if (null != inputStream) {
  ...
  private AlertNotification buildNotificationFromTarget(AlertTargetEntity target) {
    ...
    if (properties.containsKey(SHPURDP_DISPATCH_CREDENTIAL_USERNAME)
        && properties.containsKey(SHPURDP_DISPATCH_CREDENTIAL_PASSWORD)) {
      DispatchCredentials credentials = new DispatchCredentials();
      credentials.UserName = properties.get(SHPURDP_DISPATCH_CREDENTIAL_USERNAME);
      credentials.Password = properties.get(SHPURDP_DISPATCH_CREDENTIAL_PASSWORD);
  ...
  private void renderDigestNotificationContent(NotificationDispatcher dispatcher,
    ...
    ShpurdpInfo shpurdp = new ShpurdpInfo(m_metaInfo.get(), m_configuration);
    ...
    if (dispatcher.isNotificationContentGenerationRequired()) {
      if (null != template) {
        ...
        velocityContext.put(VELOCITY_SHPURDP_KEY, shpurdp);
        ...
        for (AlertHistoryEntity alert : histories) {
          subjectWriter.write("Apache Shpurdp Alert Summary");
   ...
   * {@link AlertNoticeDispatchService#SHPURDP_DISPATCH_RECIPIENTS} into a list.
   */
  private static final class AlertTargetPropertyDeserializer implements
    ...
    public AlertTargetProperties deserialize(JsonElement json, Type typeOfT,
      ...
      for (Entry<String, JsonElement> entry : entrySet) {
        ...
        if (entryKey.equals(SHPURDP_DISPATCH_RECIPIENTS)) {
  ...
  /**
   * The {@link ShpurdpInfo} class is used to provide the template engine with
   * information about the Shpurdp installation.
   */
  public final static class ShpurdpInfo {
     ...
     * @param metaInfo
     */
    protected ShpurdpInfo(ShpurdpMetaInfo metaInfo, Configuration m_configuration) {
      m_url = m_configuration.getShpurdpDisplayUrl();
    ...
    /**
     * Gets the Shpurdp server version.
     *
     * @return the version
     */
