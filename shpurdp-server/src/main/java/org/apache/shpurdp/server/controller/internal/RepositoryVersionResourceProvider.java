...
import org.apache.shpurdp.server.api.resources.OperatingSystemResourceDefinition;
import org.apache.shpurdp.server.api.resources.RepositoryResourceDefinition;
...
import org.apache.shpurdp.server.state.repository.ManifestServiceInfo;
...
public class RepositoryVersionResourceProvider extends AbstractAuthorizedResourceProvider {
  ...
  public RepositoryVersionResourceProvider() {
    ...
    setRequiredCreateAuthorizations(EnumSet.of(RoleAuthorization.SHPURDP_MANAGE_STACK_VERSIONS));
    setRequiredDeleteAuthorizations(EnumSet.of(RoleAuthorization.SHPURDP_MANAGE_STACK_VERSIONS));
    setRequiredUpdateAuthorizations(EnumSet.of(RoleAuthorization.SHPURDP_MANAGE_STACK_VERSIONS, RoleAuthorization.SHPURDP_EDIT_STACK_REPOS));

    setRequiredGetAuthorizations(EnumSet.of(
        RoleAuthorization.SHPURDP_MANAGE_STACK_VERSIONS,
        RoleAuthorization.SHPURDP_EDIT_STACK_REPOS,
        ...
        public Void invoke() throws ShpurdpException {
          ...
          for (String propertyName : requiredProperties) {
            if (properties.get(propertyName) == null) {
              throw new ShpurdpException("Property " + propertyName + " should be provided");
            }
          }
          ...
          validateRepositoryVersion(repositoryVersionDAO, shpurdpMetaInfo, entity);
  ...
  protected Set<Resource> getResourcesAuthorized(Request request, Predicate predicate)
    ...
    for (RepositoryVersionEntity entity: requestedEntities) {
      ...
      try {
        stack = shpurdpMetaInfo.getStack(entity.getStackName(), entity.getStackVersion());
  ...
  protected RequestStatus updateResourcesAuthorized(Request request, Predicate predicate)
    ...
    modifyResources(new Command<Void>() {
      @Override
      public Void invoke() throws ShpurdpException, AuthorizationException {
        for (Map<String, Object> propertyMap : propertyMaps) {
          final Long id;
          try {
            id = Long.parseLong(propertyMap.get(REPOSITORY_VERSION_ID_PROPERTY_ID).toString());
          } catch (Exception ex) {
            throw new ShpurdpException("Repository version should have numerical id");
          }
          ...
          if (StringUtils.isNotBlank(ObjectUtils.toString(propertyMap.get(SUBRESOURCE_OPERATING_SYSTEMS_PROPERTY_ID)))) {
            if (!AuthorizationHelper.isAuthorized(ResourceType.SHPURDP, null, RoleAuthorization.SHPURDP_EDIT_STACK_REPOS)) {
            ...
            try {
              ...
              throw new ShpurdpException("Json structure for operating systems is incorrect", ex);
            }
          }
   ...
   *
   * @throws ShpurdpException exception with error message
   */
  protected static void validateRepositoryVersion(RepositoryVersionDAO dao,
      ShpurdpMetaInfo metaInfo, RepositoryVersionEntity repositoryVersion) throws ShpurdpException {
   ...
   */
  protected static void validateRepositoryVersion(RepositoryVersionDAO dao,
      ShpurdpMetaInfo metaInfo, RepositoryVersionEntity repositoryVersion, boolean skipUrlCheck) throws ShpurdpException {
    ...
    for (RepoOsEntity os : repositoryVersion.getRepoOsEntities()) {
      ...
      for (RepoDefinitionEntity repositoryEntity : os.getRepoDefinitionEntities()) {
        ...
        if (!skipUrlCheck && os.isShpurdpManaged() && existingRepoUrls.contains(baseUrl)) {
    ...
    if (osRepositoryVersion.isEmpty()) {
      throw new ShpurdpException("At least one set of repositories for OS should be provided");
    }

    for (String os: osRepositoryVersion) {
      if (!osSupported.contains(os)) {
        throw new ShpurdpException("Operating system type " + os + " is not supported by stack " + requiredStackId);
      }
    }

    if (!RepositoryVersionEntity.isVersionInStack(repositoryVersion.getStackId(), repositoryVersion.getVersion())) {
      throw new ShpurdpException(MessageFormat.format("Version {0} needs to belong to stack {1}",
   ...
   * @throws ShpurdpException if some properties are missing or json has incorrect structure
   */
  protected RepositoryVersionEntity toRepositoryVersionEntity(Map<String, Object> properties) throws ShpurdpException {
