...
import org.apache.shpurdp.server.agent.stomp.dto.ExecutionCommandsCluster;
import org.apache.shpurdp.server.events.ExecutionCommandEvent;
import org.apache.shpurdp.server.orm.dao.HostRoleCommandDAO;
import org.apache.shpurdp.server.serveraction.kerberos.KerberosIdentityDataFileReader;
...
public class AgentCommandsPublisher {
  ...
  public void sendAgentCommand(Multimap<Long, AgentCommand> agentCommands) throws ShpurdpRuntimeException {
        ...
        threadPools.getAgentPublisherCommandsPool().submit(() -> {
          agentCommands.entries().stream().parallel().forEach(acHostEntry -> {
            ...
            if (ac instanceof ExecutionCommand) {
              ...
              } catch (NumberFormatException|ShpurdpException e) {
  ...
  public void sendAgentCommand(Long hostId, AgentCommand agentCommand) throws ShpurdpRuntimeException {
  ...
  private void populateExecutionCommandsClusters(
    ...
    @Nullable Map<String, DesiredConfig> desiredConfigs) throws ShpurdpRuntimeException {

    if (LOG.isDebugEnabled()) {
      try {
        LOG.debug("Sending command string = " + StageUtils.jaxbToString(ac));
      } catch (Exception e) {
        throw new ShpurdpRuntimeException("Could not get jaxb string for command", e);
      }
    }
        ...
        if (hlp != null) {
          ...
          if (SET_KEYTAB.equalsIgnoreCase(customCommand) || REMOVE_KEYTAB.equalsIgnoreCase(customCommand) || CHECK_KEYTABS.equalsIgnoreCase(customCommand)) {
            ...
            try {
              ...
              throw new ShpurdpRuntimeException("Could not inject keytab into command", e);
            }
          }
        }
  ...
  private static abstract class KerberosCommandParameterProcessor {
        ...
        resolvedPrincipal -> {
          ...
          if (targetHost.equalsIgnoreCase(hostName)) {
            try {
              ...
              throw new ShpurdpRuntimeException("Could not inject keytabs to enable kerberos", e);
            }
          }
        }
      ));
     ...
     * @throws ShpurdpException
     */
    protected Map<String, ? extends Collection<String>> getServiceComponentFilter(Map<String, ? extends Collection<String>> serviceComponentFilter) throws ShpurdpException {
