...
import org.apache.shpurdp.server.state.services.AlertNoticeDispatchService.AlertInfo;
...
public class AlertScriptDispatcher implements NotificationDispatcher {
  ...
  /**
   * The key in {@code shpurdp.properties} which contains the script to execute.
   */
  public static final String SCRIPT_CONFIG_DEFAULT_KEY = "notification.dispatch.alert.script";

  /**
   * The key in {@code shpurdp.properties} which contains the timeout, in
   ...
   */
  public static final String DISPATCH_PROPERTY_SCRIPT_CONFIG_KEY = "shpurdp.dispatch-property.script";

  /**
   * A dispatch property that instructs this dispatcher to lookup script by filename
   * from {@link org.apache.shpurdp.server.state.alert.AlertTarget}.
   */
  public static final String DISPATCH_PROPERTY_SCRIPT_FILENAME_KEY  = "shpurdp.dispatch-property.script.filename";
  ...
  /**
   * Configuration data from the shpurdp.properties file.
   */
  @Inject
  ...
  /**
   * Gets the dispatch script location from shpurdp.properties and notification.
   *
   * @param notification
  ...
  ProcessBuilder getProcessBuilder(String script, AlertNotification notification) {
    ...
    // sh -c '/foo/sys_logger.py shpurdp_server_agent_heartbeat "Agent Heartbeat"
    // SHPURDP CRITICAL "Something went wrong with the host" 1111111 host222'
