...
public class TaskWrapperBuilder {
  ...
  public static List<TaskWrapper> getTaskList(String service, String component, HostsType hostsType, List<Task> tasks, Map<String, String> params) {
    // Ok if Shpurdp Server is not part of the cluster hosts since this is only used in the calculation of how many batches
    // to create.
    ...
    for (Task t : tasks) {
      ...
      if (t.getType().isServerAction() && CollectionUtils.isNotEmpty(hostsType.getHosts())) {
        collection.add(new TaskWrapper(service, component, Collections.singleton(shpurdpServerHostname), params, t));
        continue;
      }
