...
import static org.apache.shpurdp.server.events.AlertDefinitionEventType.CREATE;
...
import org.apache.shpurdp.server.events.listeners.alerts.AlertDefinitionsUIUpdateListener;
...
public class AlertDefinitionsHolder extends AgentHostDataHolder<AlertDefinitionsAgentUpdateEvent> {
  ...
  @Inject
  public AlertDefinitionsHolder(ShpurdpEventPublisher eventPublisher) {
    eventPublisher.register(this);
  }

  @Override
  protected AlertDefinitionsAgentUpdateEvent getCurrentData(Long hostId) throws ShpurdpException {
    ...
    for (Map.Entry<Long, Map<Long, AlertDefinition>> e : alertDefinitions.entrySet()) {
      ...
      AlertDefinitionEntity shpurdpStaleAlert = alertDefinitionDAO.findByName(clusterId, AlertDefinitionsUIUpdateListener.SHPURDP_STALE_ALERT_NAME);
      Integer staleIntervalMultiplier = alertHelper.getWaitFactorMultiplier(alertDefinitionFactory.coerce(shpurdpStaleAlert));
  ...
  public AlertDefinitionsAgentUpdateEvent getDeleteCluster(Long clusterId, Long hostId) throws ShpurdpException {
    Map<Long, AlertCluster> result = new TreeMap<>();
  ...
  @Override
  protected AlertDefinitionsAgentUpdateEvent handleUpdate(AlertDefinitionsAgentUpdateEvent current, AlertDefinitionsAgentUpdateEvent update) throws ShpurdpException {
    ...
    switch (update.getEventType()) {
      ...
      case CREATE:
        if (!Sets.intersection(existingClusters.keySet(), updateClusters.keySet()).isEmpty()) {
          throw new ShpurdpException("Existing clusters in create");
        }
  ...
  @Subscribe
  public void onHostToClusterAssign(HostsAddedEvent hostsAddedEvent) throws ShpurdpException {
  ...
  private void safelyUpdateData(AlertDefinitionsAgentUpdateEvent event) throws ShpurdpException {
    try {
      updateData(event);
  ...
  public void provideAlertDefinitionAgentUpdateEvent(AlertDefinitionEventType eventType, Long clusterId,
                                                     Map<Long, AlertDefinition> alertDefinitions, String hostName) throws ShpurdpException {
    Long hostId = clusters.get().getHost(hostName).getHostId();
  ...
  public void provideStaleAlertDefinitionUpdateEvent(AlertDefinitionEventType eventType, Long clusterId,
                                                     Integer staleIntervalMultiplier, String hostName) throws ShpurdpException {
    Long hostId = clusters.get().getHost(hostName).getHostId();
