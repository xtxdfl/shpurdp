package org.apache.shpurdp.server.topology.validators;
...
import org.apache.shpurdp.server.topology.TopologyValidator;
  ...
  private Map<String, Map<String, Collection<String>>> validateRequiredPasswords(ClusterTopology topology) {
    ...
    for (Map.Entry<String, HostGroupInfo> groupEntry: topology.getHostGroupInfo().entrySet()) {
      ...
      for (String component : hostGroup.getComponentNames()) {
        //for now, SHPURDP is not recognized as a service in Stacks
        if (component.equals(RootComponent.SHPURDP_SERVER.name())) {
          continue;
        }
