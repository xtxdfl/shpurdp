...
public class RepositoryVersionDAO extends CrudDAO<RepositoryVersionEntity, Long> {
  ...
  public RepositoryVersionEntity create(StackEntity stackEntity,
                                        ...
                                        List<RepoOsEntity> repoOsEntities) throws ShpurdpException {
  ...
  public RepositoryVersionEntity create(StackEntity stackEntity,
                                        ...
                                        RepositoryType type) throws ShpurdpException {

    if (stackEntity == null || version == null || version.isEmpty()
        || displayName == null || displayName.isEmpty()) {
      throw new ShpurdpException("At least one of the required properties is null or empty");
    }

    RepositoryVersionEntity existingByDisplayName = findByDisplayName(displayName);

    if (existingByDisplayName != null) {
      throw new ShpurdpException("Repository version with display name '" + displayName + "' already exists");
    }
    ...
    if (existingVersionInStack != null) {
      throw new ShpurdpException(MessageFormat.format("Repository Version for version {0} already exists, in stack {1}-{2}",
    ...
    if (!RepositoryVersionEntity.isVersionInStack(stackId, version)) {
      throw new ShpurdpException(MessageFormat.format("Version {0} needs to belong to stack {1}", version, stackEntity.getStackName() + "-" + stackEntity.getStackVersion()));
    }
