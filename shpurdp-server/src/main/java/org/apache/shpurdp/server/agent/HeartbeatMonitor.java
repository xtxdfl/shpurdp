...
import org.apache.shpurdp.server.events.MessageNotDelivered;
...
import org.apache.shpurdp.server.state.host.HostHeartbeatLostEvent;
...
public class HeartbeatMonitor implements Runnable {
  ...
  public HeartbeatMonitor(Clusters clusters, ActionManager am,
    ...
    shpurdpManagementController = injector.getInstance(
            ShpurdpManagementController.class);
    configuration = injector.getInstance(Configuration.class);
    agentRequests = new AgentRequests();
    shpurdpEventPublisher = injector.getInstance(ShpurdpEventPublisher.class);
  ...
  public void start() {
    monitorThread = new Thread(this, "shpurdp-hearbeat-monitor");
    monitorThread.start();
  }
  ...
  private void doWork() throws InvalidStateTransitionException, ShpurdpException {
   ...
   */
  public List<StatusCommand> generateStatusCommands(String hostname) throws ShpurdpException {
  ...
  private StatusCommand createStatusCommand(String hostname, Cluster cluster,
      ServiceComponentHost sch, Map<String, DesiredConfig> desiredConfigs) throws ShpurdpException {
    ...
    hostLevelParams.put(JDK_LOCATION, shpurdpManagementController.getJdkResourceUrl());
    hostLevelParams.put(STACK_NAME, stackId.getStackName());
    hostLevelParams.put(STACK_VERSION, stackId.getStackVersion());

    if (statusCmd.getPayloadLevel() == StatusCommand.StatusCommandPayload.EXECUTION_COMMAND) {
      ExecutionCommand ec = shpurdpManagementController.getExecutionCommand(cluster, sch, RoleCommand.START);
  ...
  private void handleHeartbeatLost(Long hostId) throws ShpurdpException, InvalidStateTransitionException {
