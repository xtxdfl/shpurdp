...
import static org.apache.shpurdp.server.controller.metrics.MetricsServiceProvider.MetricsService.GANGLIA;
...
import org.apache.shpurdp.server.controller.jmx.JMXHostProvider;
...
import org.apache.shpurdp.server.controller.metrics.MetricsReportPropertyProvider;
...
import org.apache.shpurdp.server.controller.spi.ProviderModule;
...
public abstract class AbstractProviderModule implements ProviderModule,
  ...
  static {
    ...
    serviceConfigTypes.put(Service.Type.SHPURDP_METRICS, "ams-site");
   ...
   */
  @Inject
  protected ShpurdpManagementController managementController;
   ...
   */
  @Inject
  protected ShpurdpEventPublisher eventPublisher;
  ...
  public AbstractProviderModule() {
    if (managementController == null) {
      managementController = ShpurdpServer.getController();
    }
    ...
    if (null == eventPublisher && null != managementController) {
      eventPublisher = managementController.getShpurdpEventPublisher();
    ...
    if (service.equals(GANGLIA)) {
      ...
      try {
        if (clusterMetricServerVipPort == null) {
          String configType = serviceConfigTypes.get(Service.Type.SHPURDP_METRICS);
          String currentConfigVersion = getDesiredConfigVersion(clusterName, configType);
          String oldConfigVersion = serviceConfigVersions.get(configType);
          if (!currentConfigVersion.equals(oldConfigVersion)) {
            serviceConfigVersions.put(configType, currentConfigVersion);
  ...
  private String getDesiredConfigVersion(String clusterName,
    ...
    try {
      ...
      LOG.error(
          ...
          configType, clusterName, versionTag, shpurdpException);
    }
