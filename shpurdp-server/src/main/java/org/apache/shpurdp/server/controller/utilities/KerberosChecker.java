...
public class KerberosChecker {
  ...
  /**
   * Checks Shpurdp Server with a Kerberos principal and keytab to allow views
   * to authenticate via SPNEGO against cluster components.
   *
   * @throws ShpurdpException
   */
  public static void checkJaasConfiguration() throws ShpurdpException {

    if (config.isKerberosJaasConfigurationCheckEnabled()) {
      LOG.info("Checking Shpurdp Server Kerberos credentials.");
      ...
      try {
        ...
        throw new ShpurdpException(
            "Shpurdp Server Kerberos credentials check failed. \n" +
                "Check KDC availability and JAAS configuration in " + jaasConfPath);
      }

      LOG.info("Shpurdp Server Kerberos credentials check passed.");
    } else {
      LOG.info("Skipping Shpurdp Server Kerberos credentials check.");
    }
  }
