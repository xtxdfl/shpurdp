...
import org.apache.shpurdp.server.api.resources.ResourceInstanceFactoryImpl;
import org.apache.shpurdp.server.api.resources.SubResourceDefinition;
import org.apache.shpurdp.server.api.resources.ViewExternalSubResourceDefinition;
import org.apache.shpurdp.server.api.services.ShpurdpMetaInfo;
import org.apache.shpurdp.server.api.services.views.ViewExternalSubResourceService;
import org.apache.shpurdp.server.api.services.views.ViewSubResourceService;
...
import org.apache.shpurdp.server.events.ClusterConfigFinishedEvent;
...
import org.apache.shpurdp.server.orm.dao.ViewURLDAO;
...
import org.apache.shpurdp.server.orm.entities.ViewEntityEntity;
...
import org.apache.shpurdp.server.orm.entities.ViewInstancePropertyEntity;
import org.apache.shpurdp.server.orm.entities.ViewParameterEntity;
import org.apache.shpurdp.server.orm.entities.ViewResourceEntity;
import org.apache.shpurdp.server.orm.entities.ViewURLEntity;
...
import org.apache.shpurdp.server.utils.Closeables;
import org.apache.shpurdp.server.utils.VersionUtils;
import org.apache.shpurdp.server.view.configuration.AutoInstanceConfig;
import org.apache.shpurdp.server.view.configuration.EntityConfig;
import org.apache.shpurdp.server.view.configuration.InstanceConfig;
import org.apache.shpurdp.server.view.configuration.ParameterConfig;
import org.apache.shpurdp.server.view.configuration.PermissionConfig;
import org.apache.shpurdp.server.view.configuration.PersistenceConfig;
import org.apache.shpurdp.server.view.configuration.PropertyConfig;
import org.apache.shpurdp.server.view.configuration.ResourceConfig;
import org.apache.shpurdp.server.view.configuration.ViewConfig;
import org.apache.shpurdp.server.view.validation.ValidationException;
import org.apache.shpurdp.view.ShpurdpStreamProvider;
import org.apache.shpurdp.view.ClusterType;
import org.apache.shpurdp.view.Masker;
import org.apache.shpurdp.view.SystemException;
import org.apache.shpurdp.view.View;
import org.apache.shpurdp.view.ViewContext;
import org.apache.shpurdp.view.ViewDefinition;
import org.apache.shpurdp.view.ViewInstanceDefinition;
import org.apache.shpurdp.view.ViewResourceHandler;
import org.apache.shpurdp.view.cluster.Cluster;
import org.apache.shpurdp.view.events.Event;
import org.apache.shpurdp.view.events.Listener;
import org.apache.shpurdp.view.migration.ViewDataMigrationException;
import org.apache.shpurdp.view.validation.Validator;
...
public class ViewRegistry {
  ...
  private static final String VIEW_SHPURDP_VERSION_REGEXP = "^((\\d+\\.)?)*(\\*|\\d+)$";
  private static final String VIEW_LOG_FILE = "view.log4j.properties";
  private static final String SHPURDP_LOG_FILE = "log4j.properties";
   ...
   */
  @Inject
  Provider<ShpurdpMetaInfo> shpurdpMetaInfoProvider;

  /**
   * Shpurdp configuration.
   */
  @Inject
   ...
   */
  @Inject
  ShpurdpSessionManager shpurdpSessionManager;

  /**
   * Registry for Remote Shpurdp Cluster
   */
  @Inject
  RemoteShpurdpClusterRegistry remoteShpurdpClusterRegistry;

  /**
   * Remote Shpurdp Cluster Dao
   */
  @Inject
  RemoteShpurdpClusterDAO remoteShpurdpClusterDAO;
   ...
   */
  @Inject
  public ViewRegistry(ShpurdpEventPublisher publisher) {
...
// ----- ViewRegistry ------------------------------------------------------
  ...
  public Cluster getCluster(ViewInstanceDefinition viewInstance) {
    if (viewInstance != null) {
      ...
      if (clusterId != null && viewInstance.getClusterType() == ClusterType.LOCAL_SHPURDP) {
        try {
      ...
      } else if (clusterId != null && viewInstance.getClusterType() == ClusterType.REMOTE_SHPURDP) {
        try {
          return remoteShpurdpClusterRegistry.get(clusterId);
  ...
  public void onShpurdpEvent(ServiceInstalledEvent event) {

    Clusters clusters = clustersProvider.get();
    Long clusterId = event.getClusterId();

    try {
      org.apache.shpurdp.server.state.Cluster cluster = clusters.getClusterById(clusterId);
  ...
  private static org.apache.shpurdp.view.ResourceProvider getProvider(
      Class<? extends org.apache.shpurdp.view.ResourceProvider> clazz,
      final ViewContext viewInstanceContext) {
    Injector viewInstanceInjector = Guice.createInjector(new AbstractModule() {
      @Override
      protected void configure() {
  ...
  // remove undeployed views from the shpurdp db
  ...
  private boolean checkAuthorization(ResourceEntity resourceEntity) {
    ...
    return (resourceId == null)
        ? AuthorizationHelper.isAuthorized(ResourceType.SHPURDP, null, RoleAuthorization.SHPURDP_MANAGE_VIEWS)
  ...
  public void readViewArchive(Path path) {
    ...
    if (extractor.ensureExtractedArchiveDirectory(extractedArchivesPath)) {
      try {
        ...
        readViewArchive(viewDefinition, archiveFile, extractedArchiveDirFile, shpurdpMetaInfoProvider.get().getServerVersion());
      } catch (Exception e) {
  ...
  private void readNonSystemViewViewArchives() {
    try {
      ...
      if (files != null) {
        final String serverVersion = shpurdpMetaInfoProvider.get().getServerVersion();
  ...
  /**
   * copies non-log4j properties (like shpurdp.log.dir) from shpurdp's log4j.properties into view's log4j properties
   * and removes log4j specific properties (log4j.rootLogger) inside shpurdp's log4j.properties from view's log4j properties
  ...
  private void configureViewLogging(ViewEntity viewDefinition, ClassLoader cl) {
    ...
    InputStream shpurdpLog4jStream = null;
    if (null != viewLog4jStream) {
      try {
        ...
        shpurdpLog4jStream = cl.getResourceAsStream(SHPURDP_LOG_FILE);
        if (null != shpurdpLog4jStream) {
          Properties shpurdpLog4jConfig = new Properties();
          shpurdpLog4jConfig.load(shpurdpLog4jStream);

          // iterate through all the shpurdp configs and get the once not starting from log4j
          // set them into view properties and remove any log4j property which view might be overriding.
          for (Object property : shpurdpLog4jConfig.keySet()) {
            String prop = (String) property;
            if (prop.startsWith(LOG4J)) {
              viewLog4jConfig.remove(prop);
            } else {
              viewLog4jConfig.put(prop, shpurdpLog4jConfig.getProperty(prop));
            }
          }
        }
        ...
        Closeables.closeSilently(shpurdpLog4jStream);
  ...
  private void addAutoInstanceDefinition(ViewEntity viewEntity) {
    ...
    Map<String, org.apache.shpurdp.server.state.Cluster> allClusters = clustersProvider.get().getClusters();
    for (org.apache.shpurdp.server.state.Cluster cluster : allClusters.values()) {
  ...
  /**
   * Check the configured view max and min Shpurdp versions for the given view entity
   * against the given Shpurdp server version.
   *
   * @param view          the view
  ...
  protected boolean checkViewVersions(ViewEntity view, String serverVersion) {
    ...
    return checkViewVersion(view, config.getMinShpurdpVersion(), serverVersion, "minimum", -1, "less than") &&
        checkViewVersion(view, config.getMaxShpurdpVersion(), serverVersion, "maximum", 1, "greater than");

  }

  // check the given version against the actual Shpurdp server version
  private boolean checkViewVersion(ViewEntity view, String version, String serverVersion, String label,
    ...
    if (version != null && !version.isEmpty()) {
      ...
      if (!version.matches(VIEW_SHPURDP_VERSION_REGEXP)) {
        String msg = "The configured " + label + " Shpurdp version " + version + " for view " +
      ...
      if (compVal == errValue) {
        String msg = "The Shpurdp server version " + serverVersion + " is " + errMsg + " the configured " + label +
            " Shpurdp version " + version + " for view " + view.getName();
  ...
  protected ViewURLStreamProvider createURLStreamProvider(ViewContext viewContext) {
    ...
    org.apache.shpurdp.server.controller.internal.URLStreamProvider streamProvider =
        new org.apache.shpurdp.server.controller.internal.URLStreamProvider(
  ...
  /**
   * Factory method to create a view Shpurdp stream provider.
   *
   * @return a new view Shpurdp stream provider
   */
  protected ViewShpurdpStreamProvider createShpurdpStreamProvider() {
    ComponentSSLConfiguration sslConfiguration = ComponentSSLConfiguration.instance();
    org.apache.shpurdp.server.controller.internal.URLStreamProvider streamProvider =
        new org.apache.shpurdp.server.controller.internal.URLStreamProvider(
            configuration.getViewShpurdpRequestConnectTimeout(),
            configuration.getViewShpurdpRequestReadTimeout(),
            sslConfiguration.getTruststorePath(),
            sslConfiguration.getTruststorePassword(),
            sslConfiguration.getTruststoreType());
    return new ViewShpurdpStreamProvider(streamProvider, shpurdpSessionManager, ShpurdpServer.getController());
  }

  /**
   * Get Remote Shpurdp Cluster Stream provider
   *
   * @param clusterId
   * @return
   */
  protected ShpurdpStreamProvider createRemoteShpurdpStreamProvider(Long clusterId) {
    RemoteShpurdpClusterEntity clusterEntity = remoteShpurdpClusterDAO.findById(clusterId);
    if (clusterEntity != null) {
      return new RemoteShpurdpStreamProvider(getBaseurl(clusterEntity.getUrl()),
          clusterEntity.getUsername(), clusterEntity.getPassword(),
          configuration.getViewShpurdpRequestConnectTimeout(), configuration.getViewShpurdpRequestReadTimeout());
    }
    return null;
  }
  ...
  private ViewInstanceEntity getLatestUnregisteredInstance(String serverVersion, ViewInstanceEntity instance)
    ...
    // find all view archives from previous Shpurdp versions
