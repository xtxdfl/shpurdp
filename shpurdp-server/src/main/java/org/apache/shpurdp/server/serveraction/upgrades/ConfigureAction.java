...
import org.apache.shpurdp.server.stack.upgrade.ConfigUpgradeChangeDefinition.ConditionalField;
import org.apache.shpurdp.server.stack.upgrade.ConfigUpgradeChangeDefinition.ConfigurationKeyValue;
import org.apache.shpurdp.server.stack.upgrade.ConfigUpgradeChangeDefinition.IfValueMatchType;
import org.apache.shpurdp.server.stack.upgrade.ConfigUpgradeChangeDefinition.Insert;
import org.apache.shpurdp.server.stack.upgrade.ConfigUpgradeChangeDefinition.Masked;
import org.apache.shpurdp.server.stack.upgrade.ConfigUpgradeChangeDefinition.Replace;
import org.apache.shpurdp.server.stack.upgrade.ConfigUpgradeChangeDefinition.Transfer;
import org.apache.shpurdp.server.stack.upgrade.ConfigureTask;
import org.apache.shpurdp.server.stack.upgrade.Direction;
import org.apache.shpurdp.server.stack.upgrade.PropertyKeyState;
import org.apache.shpurdp.server.stack.upgrade.TransferOperation;
...
import org.apache.shpurdp.server.state.ConfigMergeHelper;
import org.apache.shpurdp.server.state.ConfigMergeHelper.ThreeWayValue;
...
public class ConfigureAction extends AbstractUpgradeServerAction {
   ...
   */
  @Inject
  private ShpurdpManagementController m_controller;
   ...
   */
  @Inject
  private Provider<ShpurdpMetaInfo> m_shpurdpMetaInfo;
  ...
  public CommandReport execute(
    ...
    if (desiredConfig == null) {
      throw new ShpurdpException("Could not find desired config type with name " + configType);
    }

    Config config = cluster.getConfig(configType, desiredConfig.getTag());
    if (config == null) {
      throw new ShpurdpException("Could not find config type with name " + configType);
    }
  ...
  private List<String> findValuesToPreserve(String clusterName, Config config)
    ...
    for (String serviceName : cluster.getServices().keySet()) {
      Set<PropertyInfo> serviceProperties = m_shpurdpMetaInfo.get().getServiceProperties(
    ...
    // match
    Set<PropertyInfo> stackProperties = m_shpurdpMetaInfo.get().getStackProperties(
