...
import org.apache.shpurdp.server.agent.stomp.ShpurdpSubscriptionRegistry;
import org.apache.shpurdp.server.api.ShpurdpSendToMethodReturnValueHandler;
...
public class RootStompConfig {
  ...
  private final org.apache.shpurdp.server.configuration.Configuration configuration;

  public RootStompConfig(ServletContext servletContext, Injector injector) {
    this.servletContext = servletContext;
    configuration = injector.getInstance(org.apache.shpurdp.server.configuration.Configuration.class);
  }

  @Bean
  public DefaultMessageEmitter defaultMessageEmitter(Injector injector) {
    org.apache.shpurdp.server.configuration.Configuration configuration =
        injector.getInstance(org.apache.shpurdp.server.configuration.Configuration.class);
    return new DefaultMessageEmitter(injector.getInstance(AgentSessionManager.class),
        brokerTemplate,
        injector.getInstance(ShpurdpEventPublisher.class),
  ...
  public void configureRegistryCacheSize(SimpleBrokerMessageHandler simpleBrokerMessageHandler) throws NoSuchFieldException, IllegalAccessException {
    ShpurdpSubscriptionRegistry defaultSubscriptionRegistry =
        new ShpurdpSubscriptionRegistry(configuration.getSubscriptionRegistryCacheSize());
  ...
  public void configureGlobal(SimpAnnotationMethodMessageHandler messageHandler) {
    ...
    for (HandlerMethodReturnValueHandler handler : handlers) {
      if (handler instanceof SendToMethodReturnValueHandler && !handlerReplaced) {
        ...
        ShpurdpSendToMethodReturnValueHandler shpurdpSendToMethodReturnValueHandler =
          new ShpurdpSendToMethodReturnValueHandler(brokerTemplate, true);
        shpurdpSendToMethodReturnValueHandler.setHeaderInitializer(sendHandler.getHeaderInitializer());
        changedHandlers.add(shpurdpSendToMethodReturnValueHandler);
  ...
  public static class ExceptionHandlingAdvice{
    ...
    public ErrorMessage handle(Exception e) {
      ...
      return new ErrorMessage(new ShpurdpException(e.getMessage()));
    }
  }
