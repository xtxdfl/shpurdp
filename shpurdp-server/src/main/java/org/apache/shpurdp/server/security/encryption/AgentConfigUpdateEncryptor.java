...
import org.apache.shpurdp.server.agent.AgentEncryptionKey;
...
/**
 * This encryptor encrypts the sensitive data in config updates sent to shpurdp-agent.
...
public class AgentConfigUpdateEncryptor extends PropertiesEncryptor implements Encryptor<AgentConfigsUpdateEvent> {
  ...
  private Cluster getCluster(long clusterId) throws ShpurdpRuntimeException {
    try {
      return clusters.get().getCluster(clusterId);
    } catch (ShpurdpException e) {
      throw new ShpurdpRuntimeException("Cannot load cluster: " + clusterId, e);
    }
  }
