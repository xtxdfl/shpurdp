...
import org.apache.shpurdp.server.agent.DiskInfo;
...
import org.apache.shpurdp.server.events.HostsAddedEvent;
...
import org.apache.shpurdp.server.orm.dao.HostStateDAO;
...
import org.apache.shpurdp.server.orm.dao.RequestOperationLevelDAO;
...
import org.apache.shpurdp.server.orm.dao.TopologyLogicalTaskDAO;
...
import org.apache.shpurdp.server.security.authorization.ShpurdpGrantedAuthority;
import org.apache.shpurdp.server.security.authorization.ResourceType;
import org.apache.shpurdp.server.state.AgentVersion;
...
import org.apache.shpurdp.server.state.HostHealthStatus.HealthStatus;
...
public class ClustersImpl implements Clusters {
  ...
  private void loadClustersAndHosts() {
    ...
    for (Long hostId : hostsById.keySet()) {
      ...
      } catch (ShpurdpRuntimeException e) {
  ...
  public void addCluster(String clusterName, StackId stackId, SecurityType securityType)
    ...
    try {
      ...
      throw new ShpurdpException("Unable to create cluster " + clusterName, e);
    }
    ...
    m_metadataHolder.get().updateData(m_shpurdpManagementController.get().getClusterMetadata(cluster));
  }
  ...
  @Override
  public Cluster getClusterById(long id) throws ShpurdpException {
  ...
  @Override
  public Host getHost(String hostname) throws ShpurdpException {
  ...
  @Override
  public Host getHostById(Long hostId) throws ShpurdpException {
   ...
   */
  @Override
  public void addHost(String hostname) throws ShpurdpException {
    if (getHostsByName().containsKey(hostname)) {
      throw new ShpurdpException(MessageFormat.format("Duplicate entry for Host {0}", hostname));
    }
  ...
  public void updateHostWithClusterAndAttributes(
      ...
      Map<String, Map<String, String>> hostAttributes) throws ShpurdpException {
   ...
   */
  @Override
  public void mapAndPublishHostsToCluster(Set<String> hostnames, String clusterName) throws ShpurdpException {
  ...
  private void publishAddingHostsToCluster(Set<String> hostnames, String clusterName) throws ShpurdpException {
   ...
   * @param clusterName Cluster name
   * @throws ShpurdpException May throw a DuplicateResourceException.
   */
  @Override
  ...
  public void deleteCluster(String clusterName)
    ...
    if (!cluster.canBeRemoved()) {
      throw new ShpurdpException("Could not delete cluster" + ", clusterName=" + clusterName);
    }
  ...
  @Override
  public void unmapHostFromCluster(String hostname, String clusterName) throws ShpurdpException {
  ...
  @Transactional
  void unmapHostFromClusters(Host host, Set<Cluster> clusters) throws ShpurdpException {
  ...
  @Transactional
  void deleteConfigGroupHostMapping(Long hostId) throws ShpurdpException {
   ...
   * {@link org.apache.shpurdp.server.HostNotFoundException}.
   *
   * @param hostname
   * @throws ShpurdpException
   */
  @Override
  public void deleteHost(String hostname) throws ShpurdpException {
  ...
  @Override
  public void publishHostsDeletion(Set<Long> hostIds, Set<String> hostNames) throws ShpurdpException {
   ...
   */
  @Transactional
  void deleteHostEntityRelationships(String hostname) throws ShpurdpException {
    if (!getHostsByName().containsKey(hostname)) {
  ...
  private boolean checkPermission(Cluster cluster, boolean readOnly) {
    for (GrantedAuthority grantedAuthority : securityHelper.getCurrentAuthorities()) {
      if (grantedAuthority instanceof ShpurdpGrantedAuthority) {

        ShpurdpGrantedAuthority authority       = (ShpurdpGrantedAuthority) grantedAuthority;
        PrivilegeEntity        privilegeEntity = authority.getPrivilegeEntity();
        Integer                permissionId    = privilegeEntity.getPermission().getId();

        // admin has full access
        if (permissionId.equals(PermissionEntity.SHPURDP_ADMINISTRATOR_PERMISSION)) {
          return true;
        }
        if (cluster != null) {
