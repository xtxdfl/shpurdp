...
import org.apache.shpurdp.server.serveraction.AbstractServerAction;
 ...
 * {@link org.apache.shpurdp.server.serveraction.ServerAction} implementations.
 * <p/>
 ...
 * (see {@link org.apache.shpurdp.server.serveraction.kerberos.KerberosIdentityDataFileReader}).
 */
public abstract class KerberosServerAction extends AbstractServerAction {
   ...
   * {@link org.apache.shpurdp.server.serveraction.kerberos.KDCType} for valid values
   */
   ...
   */
  public static final String DATA_DIRECTORY_PREFIX = ".shpurdp_";
  ...
  /**
   * Key used in kerberosCommandParams in ExecutionCommand to indicate whether to include Shpurdp server indetity
   * ("true") or ignore it ("false")
   */
  public static final String INCLUDE_SHPURDP_IDENTITY = "include_shpurdp_identity";
   ...
   * <p/>
   * If not found, {@link org.apache.shpurdp.server.serveraction.kerberos.KDCType#MIT_KDC} will be
   ...
   * @throws ShpurdpException if the cluster's name is not available
   */
  protected String getClusterName() throws ShpurdpException {
    ...
    if ((clusterName == null) || clusterName.isEmpty()) {
      throw new ShpurdpException("Failed to retrieve the cluster name from the execution command");
    }
   ...
   * @throws ShpurdpException if the Cluster object cannot be retrieved
   */
  protected Cluster getCluster() throws ShpurdpException {
    Cluster cluster = clusters.getCluster(getClusterName());

    if (cluster == null) {
      throw new ShpurdpException(String.format("Failed to retrieve cluster for %s", getClusterName()));
    }
   ...
   * {@link org.apache.shpurdp.server.serveraction.kerberos.KerberosIdentityDataFileReader} and calls
          ...
          kerberosKeytabController.getFilteredKeytabs(serviceIdentities, getHostFilter(), getIdentityFilter())
            .stream().flatMap(rkk -> rkk.getPrincipals().stream()).map(principal -> (Callable<CommandReport>) () -> {
              try {
                ...
                throw new ShpurdpRuntimeException(shpurdpException);
              }
            }
  ...
  protected Long shpurdpServerHostID() {
    String shpurdpServerHostName = StageUtils.getHostName();
    HostEntity shpurdpServerHostEntity = hostDAO.findByName(shpurdpServerHostName);
    return (shpurdpServerHostEntity == null)
        ? null
        : shpurdpServerHostEntity.getHostId();
  }
   ...
   * @throws ShpurdpException if an error occurs retrieving the relevant cluster details
   */
  protected Map<String, String> getConfigurationProperties(String configType) throws ShpurdpException {
