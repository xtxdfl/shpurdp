...
import org.apache.shpurdp.server.audit.event.kerberos.CreateKeyTabKerberosAuditEvent;
...
public class CreateKeytabFilesServerAction extends KerberosServerAction {
   ...
   * {@link org.apache.shpurdp.server.serveraction.kerberos.KerberosServerAction#processIdentities(java.util.Map)} )}
   * to iterate through the Kerberos identity metadata and call
   * {@link org.apache.shpurdp.server.serveraction.kerberos.CreateKeytabFilesServerAction#processIdentities(java.util.Map)}
   ...
   * <p/>
   * It is expected that the {@link org.apache.shpurdp.server.serveraction.kerberos.CreatePrincipalsServerAction}
   ...
   * {@link org.apache.shpurdp.server.serveraction.kerberos.KerberosOperationHandler} to generate
            ...
            try {
              ...
              if ((visitedPrincipalKeys == null) || !visitedPrincipalKeys.contains(visitationKey)) {
                ...
                if (!hostDirectory.exists() && hostDirectory.mkdirs()) {
                  // Make sure only Shpurdp has access to this directory.
                  ensureShpurdpOnlyAccess(hostDirectory);
                }
                ...
                if (hostDirectory.exists()) {
                  ...
                  if (password == null) {
                    if (!regenerateKeytabs && hostName.equalsIgnoreCase(KerberosHelper.SHPURDP_SERVER_HOST_NAME)) {
                    ...
                    if (keytab != null) {
                      try {
                        if (operationHandler.createKeytabFile(keytab, destinationKeytabFile)) {
                          ensureShpurdpOnlyAccess(destinationKeytabFile);
  ...
  public Keytab createKeytab(String principal, KerberosPrincipalEntity principalEntity, String password,
                             ...
                             boolean canCache, ActionLog actionLog) throws ShpurdpException {
   ...
   * The file is the set readable by only the Shpurdp server process owner.
   *
   ...
   * @throws ShpurdpException if a failure occurs while creating the cache file containing the the keytab data
   */
  private File cacheKeytab(String principal, Keytab keytab) throws ShpurdpException {
    File cacheDirectory = configuration.getKerberosKeytabCacheDir();

    if (cacheDirectory == null) {
      String message = "The Kerberos keytab cache directory is not configured in the Shpurdp properties";
      LOG.error(message);
      throw new ShpurdpException(message);
    }

    if (!cacheDirectory.exists()) {
      // If the cache directory does not exist, create it and ensure only Shpurdp has access to it
      if (cacheDirectory.mkdirs()) {
        ensureShpurdpOnlyAccess(cacheDirectory);
    ...
    ensureShpurdpOnlyAccess(cachedKeytabFile);

    return cachedKeytabFile;
  }

  /**
   * Ensures that the owner of the Shpurdp server process is the only local user account able to
   ...
   */
  protected void ensureShpurdpOnlyAccess(File file) throws ShpurdpException {
    if (file.exists()) {
      if (!file.setReadable(false, false) || !file.setReadable(true, true)) {
        String message = String.format("Failed to set %s readable only by Shpurdp", file.getAbsolutePath());
        LOG.warn(message);
        throw new ShpurdpException(message);
      }

      if (!file.setWritable(false, false) || !file.setWritable(true, true)) {
        String message = String.format("Failed to set %s writable only by Shpurdp", file.getAbsolutePath());
        LOG.warn(message);
        throw new ShpurdpException(message);
      }
      ...
      if (file.isDirectory()) {
        if (!file.setExecutable(false, false) || !file.setExecutable(true, true)) {
          String message = String.format("Failed to set %s executable by Shpurdp", file.getAbsolutePath());
          LOG.warn(message);
          throw new ShpurdpException(message);
        }
      } else {
