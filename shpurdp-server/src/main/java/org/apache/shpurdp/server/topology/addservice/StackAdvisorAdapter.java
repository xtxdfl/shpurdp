...
import org.apache.shpurdp.server.controller.internal.UnitUpdater;
...
public class StackAdvisorAdapter {
  ...
  @Inject
  private org.apache.shpurdp.server.configuration.Configuration serverConfig;
  ...
  AddServiceInfo recommendLayout(AddServiceInfo info) {
    ...
    catch (ShpurdpException|StackAdvisorException ex) {
   ...
   */
  Map<String, Map<String, Set<String>>> getAllServices(AddServiceInfo info) throws ShpurdpException {
  ...
  private Cluster getCluster(AddServiceInfo info) throws ShpurdpException {
  ...
  AddServiceInfo recommendConfigurations(AddServiceInfo info) {
    ...
    if (info.getRequest().getRecommendationStrategy().shouldUseStackAdvisor()) {
      ...
      catch (StackAdvisorException|ShpurdpException ex) {
