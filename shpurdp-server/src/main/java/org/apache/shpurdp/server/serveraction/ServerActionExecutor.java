...
package org.apache.shpurdp.server.serveraction;
...
import org.apache.shpurdp.server.stack.upgrade.orchestrate.UpgradeServiceSummary;
import org.apache.shpurdp.server.stack.upgrade.orchestrate.UpgradeSummary;
 ...
 * <p/>
 * The ServerActionExecutor executes in its own thread, polling for SHPURDP_SERVER_ACTION
...
public class ServerActionExecutor {
  ...
  /**
   * The Shpurdp configuration used to obtain configured details such as the default server-side action
   * timeout.
   */
  @Inject
  ...
  public void doWork() throws InterruptedException {
    List<HostRoleCommand> tasks = db.getTasksByRoleAndStatus(Role.SHPURDP_SERVER_ACTION.name(),
  ...
  private class Worker implements Runnable {
    ...
    private CommandReport execute(HostRoleCommand hostRoleCommand, ExecutionCommand executionCommand)
        throws ShpurdpException, InterruptedException {

      if (hostRoleCommand == null) {
        throw new ShpurdpException("Missing HostRoleCommand data");
      } else if (executionCommand == null) {
        throw new ShpurdpException("Missing ExecutionCommand data");
      } else {
        Map<String, String> roleParams = executionCommand.getRoleParams();
        if (roleParams == null) {
          throw new ShpurdpException("Missing RoleParams data");
        } else {
          String actionClassname = roleParams.get(ServerAction.ACTION_NAME);

          if (actionClassname == null) {
            throw new ShpurdpException("Missing action classname for server action");
          } else {
            ...
            if (upgradeSummary != null) {
              ...
              ShpurdpMetaInfo shpurdpMetaInfo = shpurdpManagementController.getShpurdpMetaInfo();
              ...
              if (serviceName != null && !serviceName.isEmpty()){
                ...
                addServiceInfo(services, shpurdpMetaInfo, serviceSummary.sourceStackId, serviceName);
              } else {
                ...
                for(String key: upgradeServiceSummaries.keySet()){
                  ...
                  addServiceInfo(services, shpurdpMetaInfo, serviceSummary.sourceStackId, key);
                }
              }
            ...
            if (action == null) {
              throw new ShpurdpException("Failed to create server action: " + actionClassname);
            } else {
    ...
    private void addServiceInfo(Map<String, ServiceInfo> services, ShpurdpMetaInfo shpurdpMetaInfo, String stackId, String serviceName) {
      List<String> stackInfo = getStackInfo(stackId);
      LOG.debug(String.format("Stack info list: %s", stackInfo));
      if (stackInfo.size() > 1) {
        try {
          ServiceInfo service = shpurdpMetaInfo.getService(stackInfo.get(0), stackInfo.get(1), serviceName);
     ...
     * @throws ShpurdpException
     */
    private ServerAction createServerAction(String classname, Map<String, ServiceInfo> services) throws ShpurdpException {
      Class<?> actionClass = null;
      actionClass = getServerActionClass(classname);
      if (actionClass == null) {
        LOG.debug(String.format("Did not find %s in Shpurdp, try to load it from external directories", classname));
        actionClass = getServiceLevelServerActionClass(classname, services);
      }

      if (actionClass == null) {
        throw new ShpurdpException("Unable to load server action class: " + classname);
      } else {
        ...
        if (serverActionClass == null) {
          throw new ShpurdpException("Unable to execute server action class, invalid type: " + classname);
        } else {
    ...
    /**
     * Load server action classes defined in Shpurdp source code
     * */
    private Class<?> getServerActionClass(String classname) throws ShpurdpException{
      Class<?> actionClass = null;
      try {
        actionClass = Class.forName(classname);
        if (actionClass == null) {
          LOG.warn(String.format("Unable to load server action class: %s from Shpurdp", classname));
        }
