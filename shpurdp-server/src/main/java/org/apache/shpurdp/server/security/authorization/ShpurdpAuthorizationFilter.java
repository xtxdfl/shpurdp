...
import org.apache.shpurdp.server.audit.event.AccessUnauthorizedAuditEvent;
import org.apache.shpurdp.server.audit.event.AuditEvent;
import org.apache.shpurdp.server.audit.event.LoginAuditEvent;
...
@Component
public class ShpurdpAuthorizationFilter implements Filter {
  ...
  private static final String API_CREDENTIALS_SHPURDP_PATTERN = API_VERSION_PREFIX + "/clusters/.*?/credentials/shpurdp\\..*";
  ...
  /**
   * The Shpurdp authentication entry point
   */
  private final ShpurdpEntryPoint entryPoint;

  /**
   * Access to Shpurdp configuration data
   */
   ...
   * @param configuration    the Shpurdp configuration
   * @param users            Shpurdp user access
   * @param auditLogger      the Audit logger
   * @param permissionHelper the permission helper
   */
  public ShpurdpAuthorizationFilter(ShpurdpEntryPoint entryPoint,
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
    ...
    if (authentication == null || authentication instanceof AnonymousAuthenticationToken ||
      ...
      if (token != null) {
        ...
        // for view access, we should redirect to the Shpurdp login
      ...
      if (requestURI.matches(API_BOOTSTRAP_PATTERN_ALL)) {
        ...
        for (GrantedAuthority grantedAuthority : authentication.getAuthorities()) {
          if (grantedAuthority instanceof ShpurdpGrantedAuthority) {
            ...
            if (!"GET".equalsIgnoreCase(httpRequest.getMethod()) && requestURI.matches(API_CREDENTIALS_SHPURDP_PATTERN)) {
              // Only the administrator can operate on credentials where the alias starts with "shpurdp."
              if (permissionId.equals(PermissionEntity.SHPURDP_ADMINISTRATOR_PERMISSION)) {
                authorized = true;
                break;
              }
