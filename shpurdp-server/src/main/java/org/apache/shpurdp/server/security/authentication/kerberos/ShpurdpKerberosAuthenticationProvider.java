...
/**
 * ShpurdpKerberosAuthenticationProvider is an {@link org.springframework.security.authentication.AuthenticationProvider}
 ...
 * using the <code>shpurdp-server setup-kerberos</code> CLI.
 * <p>
 * This implementation supports Trusted Proxy authentication.
 */
public class ShpurdpKerberosAuthenticationProvider implements AuthenticationProvider, InitializingBean {
  private static final Logger LOG = LoggerFactory.getLogger(ShpurdpKerberosAuthenticationProvider.class);

  private ShpurdpAuthToLocalUserDetailsService authToLocalUserDetailsService;
  private ShpurdpProxiedUserDetailsService proxiedUserDetailsService;
  private KerberosTicketValidator ticketValidator;

  @Inject
  public ShpurdpKerberosAuthenticationProvider(ShpurdpAuthToLocalUserDetailsService authToLocalUserDetailsService,
    ...
    if (authentication == null) {
      ...
      if (requestDetails instanceof TrustedProxyAuthenticationDetails) {
        ...
        if (StringUtils.isNotEmpty(proxiedUserName)) {
          ...
          userDetails = new ShpurdpProxiedUserDetailsImpl(
              proxiedUserDetailsService.loadProxiedUser(proxiedUserName, localProxyUserName, trustedProxyAuthenticationDetails),
              new ShpurdpProxyUserKerberosDetailsImpl(ticketValidation.username(), localProxyUserName));
        } else {
  ...
  public void setAuthToLocalUserDetailsService(ShpurdpAuthToLocalUserDetailsService authToLocalUserDetailsService) {
    this.authToLocalUserDetailsService = authToLocalUserDetailsService;
  }

  public void setProxiedUserDetailsService(ShpurdpProxiedUserDetailsService proxiedUserDetailsService) {
    this.proxiedUserDetailsService = proxiedUserDetailsService;
  }
