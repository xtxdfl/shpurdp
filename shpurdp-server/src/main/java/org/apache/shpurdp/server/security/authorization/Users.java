...
import org.apache.shpurdp.server.orm.dao.UserAuthenticationDAO;
...
public class Users {
  ...
  @Inject
  protected ShpurdpLdapConfigurationProvider ldapConfigurationProvider;
   ...
   * @throws ShpurdpException if user does not exist
   */
  public synchronized void setUserActive(String userName, boolean active) throws ShpurdpException {
    UserEntity userEntity = userDAO.findUserByName(userName);
    if (userEntity != null) {
      setUserActive(userEntity, active);
    } else {
      throw new ShpurdpException("User " + userName + " doesn't exist");
    }
  }
   ...
   * @throws ShpurdpException if user does not exist
   */
  public synchronized void setUserActive(UserEntity userEntity, final boolean active) throws ShpurdpException {
    if (userEntity != null) {
   ...
   * @param userName   the Shpurdp username
   */
   ...
   * @throws ShpurdpException if group does not exist
   */
  public synchronized void setGroupLdap(String groupName) throws ShpurdpException {
    ...
    if (groupEntity != null) {
      ...
      throw new ShpurdpException("Group " + groupName + " doesn't exist");
    }
  }
   ...
   * @throws ShpurdpException if user already exists
   */
  public UserEntity createUser(String userName, String localUserName, String displayName) throws ShpurdpException {
   ...
   */
  @Transactional
  public synchronized UserEntity createUser(String userName, String localUserName, String displayName, Boolean active) throws ShpurdpException {
    ...
    if (userDAO.findUserByName(validatedUserName) != null) {
      throw new ShpurdpException("User already exists");
    }
  ...
  /**
   * Removes a user from the Shpurdp database.
   * <p>
   ...
   */
  @Transactional
  public synchronized void removeUser(User user) throws ShpurdpException {
    UserEntity userEntity = userDAO.findByPK(user.getUserId());
    if (userEntity != null) {
      removeUser(userEntity);
    } else {
      throw new ShpurdpException("User " + user + " doesn't exist");
    }
  }
   ...
   */
  @Transactional
  public synchronized void removeUser(UserEntity userEntity) throws ShpurdpException {
    if (userEntity != null) {
      if (!isUserCanBeRemoved(userEntity)) {
        throw new ShpurdpException("Could not remove user " + userEntity.getUserName() +
   ...
   */
  public List<String> getAllMembers(String groupName) throws ShpurdpException {
  ...
  @Transactional
  public synchronized void removeGroup(Group group) throws ShpurdpException {
    final GroupEntity groupEntity = groupDAO.findByPK(group.getGroupId());
    if (groupEntity != null) {
      groupDAO.remove(groupEntity);
    } else {
      throw new ShpurdpException("Group " + group + " doesn't exist");
    }
  }

  /**
   * Test the user for Shpurdp Admistrator privileges.
   *
   * @param userEntity the user to test
   * @return true if the user has Shpurdp Administrator privileges; otherwise false
   */
  public synchronized boolean hasAdminPrivilege(UserEntity userEntity) {
    ...
    if (principalEntity != null) {
      ...
      if (roles != null) {
        PermissionEntity adminPermission = permissionDAO.findShpurdpAdminPermission();
  ...
  /**
   * Grants Shpurdp Administrator privilege to provided user.
   *
  ...
  public synchronized void grantAdminPrivilege(UserEntity userEntity) {
    ...
    adminPrivilege.setPermission(permissionDAO.findShpurdpAdminPermission());
    adminPrivilege.setPrincipal(userEntity.getPrincipal());
    adminPrivilege.setResource(resourceDAO.findShpurdpResource());
  ...
  /**
   * Revokes Shpurdp Administrator privileges from provided user.
   *
   * @param userId user id
   */
  ...
  public synchronized void revokeAdminPrivilege(UserEntity userEntity) {
    for (PrivilegeEntity privilege : userEntity.getPrincipal().getPrivileges()) {
      if (privilege.getPermission().getPermissionName().equals(PermissionEntity.SHPURDP_ADMINISTRATOR_PERMISSION_NAME)) {
  ...
  public synchronized boolean isUserCanBeRemoved(UserEntity userEntity) {
    List<PrincipalEntity> adminPrincipals = principalDAO.findByPermissionId(PermissionEntity.SHPURDP_ADMINISTRATOR_PERMISSION);
  ...
  private void processLdapAdminGroupMappingRules(Set<MemberEntity> membershipsToCreate) {
    ...
    ShpurdpLdapConfiguration ldapConfiguration = ldapConfigurationProvider.get();
    ...
    // LDAP users to become shpurdp administrators
    Set<UserEntity> shpurdpAdminProspects = Sets.newHashSet();

    // gathering all the users that need to be shpurdp admins
    for (MemberEntity memberEntity : membershipsToCreate) {
      if (ldapAdminGroups.contains(memberEntity.getGroup().getGroupName())) {
        LOG.debug("Shpurdp admin user prospect: [{}] ", memberEntity.getUser().getUserName());
        shpurdpAdminProspects.add(memberEntity.getUser());
      }
    }

    // granting admin privileges to the admin prospects
    for (UserEntity userEntity : shpurdpAdminProspects) {
      LOG.info("Granting shpurdp admin roles to the user: {}", userEntity.getUserName());
   ...
   */
  public Collection<ShpurdpGrantedAuthority> getUserAuthorities(String userName) {
   ...
   * @return the users collection of implicit and explicit granted authorities
   */
  public Collection<ShpurdpGrantedAuthority> getUserAuthorities(UserEntity userEntity) {
    if (userEntity == null) {
      return Collections.emptyList();
    }
    ...
    Set<ShpurdpGrantedAuthority> authorities = new HashSet<>(privilegeEntities.size());

    for (PrivilegeEntity privilegeEntity : privilegeEntities) {
      authorities.add(new ShpurdpGrantedAuthority(privilegeEntity));
    }
   ...
   */
  @Transactional
  public synchronized void modifyAuthentication(UserAuthenticationEntity userAuthenticationEntity, String currentKey, String newKey, boolean isSelf) throws ShpurdpException {
      ...
      if (userAuthenticationEntity.getAuthenticationType() == UserAuthenticationType.LOCAL) {
        ...
        String expectedCurrentKey = isSelf
          ...
          : getAuthenticatedUserLocalAuthenticationMethod().
              orElseThrow(() -> new ShpurdpException("Authentication error")).getAuthenticationKey();

        if (StringUtils.isEmpty(currentKey) || !passwordEncoder.matches(currentKey, expectedCurrentKey)) {
          throw new ShpurdpException("Wrong current password provided");
        }
   ...
   */
  public void addAuthentication(UserEntity userEntity, UserAuthenticationType authenticationType, String key) throws ShpurdpException {
    switch (authenticationType) {
      ...
      case KERBEROS:
        ...
        break;
      default:
        throw new ShpurdpException("Unexpected user authentication type");
    }
  }
   ...
   */
  public void addJWTAuthentication(UserEntity userEntity, String key) throws ShpurdpException {
   ...
   * @throws ShpurdpException
   */
  public void addJWTAuthentication(UserEntity userEntity, String key, boolean persist) throws ShpurdpException {
            ...
            (user, authKey) -> {
              ...
              for (UserAuthenticationEntity entity : authenticationEntities) {
                if ((entity.getAuthenticationType() == UserAuthenticationType.JWT) &&
                  ...
                  throw new ShpurdpException("The authentication type already exists for this user");
                }
              }
            },
        persist);
   ...
   */
  public void addKerberosAuthentication(UserEntity userEntity, String principalName) throws ShpurdpException {
   ...
   * @throws ShpurdpException
   */
  public void addKerberosAuthentication(UserEntity userEntity, String principalName, boolean persist) throws ShpurdpException {
    addAuthentication(userEntity,
        ...
        principalName,
            (user, key) -> {
              ...
              if (!CollectionUtils.isEmpty(userAuthenticationDAO.findByTypeAndKey(UserAuthenticationType.KERBEROS, key))) {
                throw new ShpurdpException("The authentication type already exists for this principal");
              }
            },
        persist);
  }
  ...
  /**
   * Adds the ability for a user to authenticate using a password stored in Shpurdp's database
   * <p>
   ...
   */
  public void addLocalAuthentication(UserEntity userEntity, String password) throws ShpurdpException {
   ...
   * @throws ShpurdpException
   */
  public void addLocalAuthentication(UserEntity userEntity, String password, boolean persist) throws ShpurdpException {
   ...
   */
  public void addPamAuthentication(UserEntity userEntity, String userName) throws ShpurdpException {
   ...
   * @throws ShpurdpException
   */
  public void addPamAuthentication(UserEntity userEntity, String userName, boolean persist) throws ShpurdpException {
    addAuthentication(userEntity,
   ...
   */
  public void addLdapAuthentication(UserEntity userEntity, String dn) throws ShpurdpException {
   ...
   * @throws ShpurdpException
   */
  public void addLdapAuthentication(UserEntity userEntity, String dn, boolean persist) throws ShpurdpException {
    addAuthentication(userEntity,
  ...
  private void addAuthentication(UserEntity userEntity, UserAuthenticationType type, String key,
    ...
    if (userEntity == null) {
      throw new ShpurdpException("Missing user");
    }
  ...
  /**
   * Validates the password meets configured requirements according shpurdp.properties.
   *
   * <p>
   *
  ...
  public void validatePassword(String password) {
    ...
    if (!StringUtils.isEmpty(regexp) && (!Pattern.matches(regexp,password))) {
      final String msg = "The password does not meet the Shpurdp user password policy : " + configuration.getPasswordPolicyDescription();
      throw new IllegalArgumentException(msg);
    }
  }

  /**
   * Validates the password meets configured requirements according shpurdp.properties and it does not contain the username.
   *
   * <p>
   *
   * @param password the password
  ...
  public void validatePassword(String password, String userName) {
    ...
    if (StringUtils.isNotEmpty(userName) && Pattern.compile(Pattern.quote(userName), Pattern.CASE_INSENSITIVE).matcher(password).find()) {
      final String msg = "The password does not meet the Shpurdp user password policy : password cannot contain the username";
      throw new IllegalArgumentException(msg);
    }
  }

  /**
   * Validates the password meets configured requirements according shpurdp.properties, does not contain the username and different from previous passwords
   *
   * <p>
   *
   * @param password the password
  ...
  public void validatePassword(String password, String userName, String authenticationKey) {
    ...
    if (isMatched) {
      final String msg = "The password does not meet the Shpurdp user password policy : new password can not be same as previous " + configuration.getPasswordPolicyHistoryCount() + " passwords.";
      throw new IllegalArgumentException(msg);
    }
  }
  ...
  private interface Validator {
     ...
     * @throws ShpurdpException
     */
    void validate(UserEntity userEntity, String key) throws ShpurdpException;
  }
