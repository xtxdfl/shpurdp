...
import org.apache.shpurdp.server.controller.jmx.JMXMetricHolder;
...
/**
 * I represent a "SERVER" {@link org.apache.shpurdp.server.state.alert.SourceType} alert
...
public class JmxServerSideAlert extends AlertRunnable {
  ...
  private Optional<JMXMetricHolder> jmxMetric(ServerSource serverSource, Cluster cluster) throws ShpurdpException {
  ...
  private URI jmxUrl(Cluster cluster, ServerSource serverSource) throws ShpurdpException {
    return serverSource.getUri().resolve(config(cluster)).resolve(serverSource.getJmxInfo().getUrlSuffix());
  }

  private Map<String, Map<String, String>> config(Cluster cluster) throws ShpurdpException {
