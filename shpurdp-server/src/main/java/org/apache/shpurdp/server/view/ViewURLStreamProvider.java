 ...
 */
public class ViewURLStreamProvider implements org.apache.shpurdp.view.URLStreamProvider, URLConnectionProvider {
  ...
  private HostPortRestrictionHandler getHostPortRestrictionHandler() {
    if (hostPortRestrictionHandler == null) {
      HostPortRestrictionHandler hostPortRestrictionHandlerTmp =
          new HostPortRestrictionHandler(viewContext.getShpurdpProperty(Configuration.PROXY_ALLOWED_HOST_PORTS.getKey()));
      hostPortRestrictionHandler = hostPortRestrictionHandlerTmp;
    }
  ...
  private InputStream getInputStream(String spec, String requestMethod, Map<String, String> headers, byte[] info)
    ...
    if (!isProxyCallAllowed(spec)) {
      LOG.warn("Call to " + spec + " is not allowed. See shpurdp.properties proxy.allowed.hostports.");
      throw new IOException("Call to " + spec + " is not allowed. See shpurdp.properties proxy.allowed.hostports.");
    }
