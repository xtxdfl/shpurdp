...
/**
 * ShpurdpBasicAuthenticationFilter extends a {@link BasicAuthenticationFilter} to allow for auditing
...
@Component
@Order(3)
public class ShpurdpBasicAuthenticationFilter extends BasicAuthenticationFilter implements ShpurdpAuthenticationFilter {
  private static final Logger LOG = LoggerFactory.getLogger(ShpurdpBasicAuthenticationFilter.class);
   ...
   */
  public ShpurdpBasicAuthenticationFilter(AuthenticationManager authenticationManager,
                                         ShpurdpEntryPoint shpurdpEntryPoint,
                                         ShpurdpAuthenticationEventHandler eventHandler) {
    super(authenticationManager, shpurdpEntryPoint);
  ...
  /**
   * Tests to see if this {@link ShpurdpBasicAuthenticationFilter} should be applied in the authentication
   * filter chain.
   * <p>
  ...
  protected void onUnsuccessfulAuthentication(HttpServletRequest servletRequest,
    ...
    if (eventHandler != null) {
      ...
      if (authException instanceof ShpurdpAuthenticationException) {
        cause = (ShpurdpAuthenticationException) authException;
      } else {
        ...
        cause = new ShpurdpAuthenticationException(username, authException.getMessage(), false, authException);
      }
