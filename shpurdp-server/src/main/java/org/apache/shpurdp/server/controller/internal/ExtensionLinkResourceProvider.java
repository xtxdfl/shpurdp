...
import org.apache.shpurdp.server.controller.ExtensionLinkRequest;
...
public class ExtensionLinkResourceProvider extends AbstractControllerResourceProvider {
  ...
  protected ExtensionLinkResourceProvider(ShpurdpManagementController managementController) {
  ...
  protected RequestStatus deleteResourcesAuthorized(Request request, Predicate predicate)
    ...
    RequestStatusResponse response = modifyResources(new Command<RequestStatusResponse>() {
      @Override
      public RequestStatusResponse invoke() throws ShpurdpException {
        for (ExtensionLinkRequest extensionLinkRequest : requests) {
  ...
  private void verifyStackAndExtensionExist(ExtensionLinkRequest request) throws NoSuchParentResourceException {
    try {
      if (request.getStackName() != null && request.getStackVersion() != null) {
        getManagementController().getShpurdpMetaInfo().getStack(request.getStackName(), request.getStackVersion());
      }
      if (request.getExtensionName() != null && request.getExtensionVersion() != null) {
        getManagementController().getShpurdpMetaInfo().getExtension(request.getExtensionName(), request.getExtensionVersion());
      }
    }
    catch (ShpurdpException shpurdpException) {
      throw new NoSuchParentResourceException(shpurdpException.getMessage());
    }
  }
