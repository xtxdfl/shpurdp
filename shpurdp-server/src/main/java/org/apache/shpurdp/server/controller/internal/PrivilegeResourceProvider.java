...
public abstract class PrivilegeResourceProvider<T> extends AbstractAuthorizedResourceProvider {
   ...
   *
   * @return the entities
   * @throws ShpurdpException if resource entities were not found
   */
  public abstract Map<Long, T> getResourceEntities(Map<String, Object> properties) throws ShpurdpException;
   ...
   *
   * @throws ShpurdpException if the the privilege permission is not allowable for the resource type
   */
  protected boolean checkResourceTypes(PrivilegeEntity entity) throws ShpurdpException {
  ...
  protected PrivilegeEntity toEntity(Map<String, Object> properties, Long resourceId)
    ...
    if (permission == null) {
      throw new ShpurdpException("Can't find a permission named " + permissionName +
          " for the resource.");
    }
    ...
    if (StringUtils.equalsIgnoreCase(PrincipalTypeEntity.GROUP_PRINCIPAL_TYPE_NAME, principalType)) {
      ...
      throw new ShpurdpException("Unknown principal type " + principalType);
    }
    if (entity.getPrincipal() == null) {
      throw new ShpurdpException("Could not find " + principalType + " named " + principalName);
    }
    return entity;
  }
  ...
  private Command<Void> getCreateCommand(final Map<String, Object> properties) {
    return new Command<Void>() {
      ...
      public Void invoke() throws ShpurdpException {
        ...
        if (entity.getPrincipal() == null) {
          throw new ShpurdpException("Can't find principal " + properties.get(PRINCIPAL_TYPE) +
        ...
        if (!checkResourceTypes(entity)) {
          throw new ShpurdpException("Can't grant " + entity.getPermission().getResourceType().getName() +
        ...
        try {
          for (Map<String, Object> resource : getPropertyMaps(predicate)) {
            if (resource.get(PRIVILEGE_ID) == null) {
              throw new ShpurdpException("Privilege ID should be provided for this request");
            }
            PrivilegeEntity entity = privilegeDAO.findById(Integer.valueOf(resource.get(PRIVILEGE_ID).toString()));
            if (entity != null) {
              if (!checkResourceTypes(entity)) {
                throw new ShpurdpException("Can't remove " + entity.getPermission().getResourceType().getName() +
          ...
          throw new ShpurdpException("Caught exception deleting privilege.", e);
        }
        return null;
      }
    };
      ...
      public Void invoke() throws ShpurdpException {
        ...
        for (Map<String, Object> properties: request.getProperties()) {
          ...
          if (resource != null && ! resourceId.equals(resource)) {
            throw new ShpurdpException("Can't update privileges of multiple resources in one request");
          }
          resource = resourceId;
        ...
        for (PrivilegeEntity requiredPrivilege: requiredEntities) {
          ...
          if (!isInBothLists) {
            if (!checkResourceTypes(requiredPrivilege)) {
              throw new ShpurdpException("Can't grant " + requiredPrivilege.getPermission().getResourceType().getName() +
        ...
        for (PrivilegeEntity currentPrivilege: currentPrivileges) {
          ...
          if (!isInBothLists) {
            if (!checkResourceTypes(currentPrivilege)) {
              throw new ShpurdpException("Can't remove " + currentPrivilege.getPermission().getResourceType().getName() +
