...
public class SchemaUpgradeHelper {
   ...
   * @return
   */
  protected String getShpurdpServerVersion() {
  ...
  protected List<UpgradeCatalog> getUpgradePath(String sourceVersion,
                                                       String targetVersion) throws ShpurdpException {
  ...
  public void executeUpgrade(List<UpgradeCatalog> upgradeCatalogs) throws ShpurdpException {
  ...
  public void executePreDMLUpdates(List<UpgradeCatalog> upgradeCatalogs) throws ShpurdpException {
  ...
  public void executeDMLUpdates(List<UpgradeCatalog> upgradeCatalogs, String shpurdpUpgradeConfigUpdatesFileName) throws ShpurdpException {
    LOG.info("Executing DML changes.");

    if (upgradeCatalogs != null && !upgradeCatalogs.isEmpty()) {
      for (UpgradeCatalog upgradeCatalog : upgradeCatalogs) {
        try {
          upgradeCatalog.setConfigUpdatesFileName(shpurdpUpgradeConfigUpdatesFileName);
  ...
  public void resetUIState() throws ShpurdpException {
    ...
    try {
      ...
      throw new ShpurdpException("Unable to reset UI state", e);
    }
  }
  ...
  public void cleanUpRCATables() {
    ...
    try {
      ...
      LOG.warn("Error cleaning rca tables from shpurdprca db", e);
    }
  }
  ...
  /**
   * Upgrade Shpurdp DB schema to the target version passed in as the only
   * argument.
    ...
    try {
      ...
      if (javaVersion < Configuration.JDK_MIN_VERSION) {
        LOG.error(String.format("Oracle JDK version is lower than %.1f It can cause problems during upgrade process. Please," +
                " use 'shpurdp-server setup' command to upgrade JDK!", Configuration.JDK_MIN_VERSION));
        System.exit(1);
      }
      ...
      String targetVersion = schemaUpgradeHelper.getShpurdpServerVersion();
      ...
      if (!schemaUpgradeHelper.verifyUpgradePath(minimalRequiredUpgradeVersion, sourceVersion)){
        throw new ShpurdpException(String.format("Database version does not meet minimal upgrade requirements. Expected version should be not less than %s, current version is %s",
      ...
      String shpurdpUpgradeConfigUpdatesFileName = "shpurdp_upgrade_config_changes_" + date + ".json";
      ...
      schemaUpgradeHelper.executeDMLUpdates(upgradeCatalogs, shpurdpUpgradeConfigUpdatesFileName);
