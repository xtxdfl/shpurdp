 ...
 *            "type": "{@link org.apache.shpurdp.server.state.kerberos.KerberosIdentityDescriptor}"
 *          }
 *        },
 *        "configurations": {
 ...
 *            "type": "{@link org.apache.shpurdp.server.state.kerberos.KerberosConfigurationDescriptor}"
 *          }
 *        },
 *        "auth_to_local": {
 ...
 * {@link org.apache.shpurdp.server.state.kerberos.AbstractKerberosDescriptor#name} value, it is
...
public abstract class AbstractKerberosDescriptorContainer extends AbstractKerberosDescriptor {
  ...
  public List<KerberosIdentityDescriptor> getIdentities() {
    try {
      ...
      // ShpurdpException will not be thrown unless an error occurs while trying to dereference
   ...
   */
  public List<KerberosIdentityDescriptor> getIdentities(boolean resolveReferences, Map<String, Object> contextForFilter) throws ShpurdpException {
    if (identities == null) {
      return null;
    } else {
      ...
      if (path.startsWith("/")) {
        ...
        switch (pathParts.length) {
          ...
          case 1:
            ...
            break;
          default:
            throw new ShpurdpException(String.format("Unexpected path length in %s", path));
        }
   ...
   * {@link org.apache.shpurdp.server.state.kerberos.AbstractKerberosDescriptorContainer}
   *
   * @return a Map of values for this AbstractKerberosDescriptorContainer
   * @see org.apache.shpurdp.server.state.kerberos.AbstractKerberosDescriptorContainer
   */
  @Override
   ...
   * @throws ShpurdpException
   */
  private KerberosIdentityDescriptor dereferenceIdentity(KerberosIdentityDescriptor identity) throws ShpurdpException {
    ...
    if (identity != null) {
      ...
      try {
        ...
        throw new ShpurdpException(String.format("Invalid Kerberos identity reference: %s", identity.getReference()), e);
      }
