...
import static org.apache.shpurdp.server.controller.KerberosHelperImpl.CHECK_KEYTABS;
import static org.apache.shpurdp.server.controller.KerberosHelperImpl.SET_KEYTAB;
...
import org.apache.shpurdp.server.agent.stomp.dto.ComponentVersionReport;
import org.apache.shpurdp.server.agent.stomp.dto.ComponentVersionReports;
...
import org.apache.shpurdp.server.events.ActionFinalReportReceivedEvent;
import org.apache.shpurdp.server.events.AlertEvent;
import org.apache.shpurdp.server.events.AlertReceivedEvent;
import org.apache.shpurdp.server.events.HostComponentVersionAdvertisedEvent;
import org.apache.shpurdp.server.events.publishers.AlertEventPublisher;
...
import org.apache.shpurdp.server.state.Alert;
...
import org.apache.shpurdp.server.state.host.HostStatusUpdatesReceivedEvent;
...
public class HeartbeatProcessor extends AbstractService{
  ...
  @Inject
  ShpurdpEventPublisher shpurdpEventPublisher;
  ...
  public HeartbeatProcessor(Clusters clusterFsm, ActionManager am, HeartbeatMonitor heartbeatMonitor,
    ...
    actionManager = am;
    ThreadFactory threadFactory = new ThreadFactoryBuilder().setNameFormat("shpurdp-heartbeat-processor-%d").build();
   ...
   */
  public void processHeartbeat(HeartBeat heartbeat) throws ShpurdpException {
   ...
   * @throws ShpurdpException
   */
  protected void processHostStatus(HeartBeat heartbeat) throws ShpurdpException {
   ...
   * @throws ShpurdpException
   */
  protected void processCommandReports(HeartBeat heartbeat, long now) throws ShpurdpException {
  ...
  protected void processCommandReports(List<CommandReport> reports, String hostName, Long now)
    ...
    for (CommandReport report : reports) {
      ...
      if (RoleCommand.valueOf(report.getRoleCommand()) == RoleCommand.ACTIONEXECUTE &&
        ...
        shpurdpEventPublisher.publish(event);
      }
      ...
      if (service == null || service.isEmpty()) {
        throw new ShpurdpException("Invalid command report, service: " + service);
      }
      if (actionMetadata.getActions(service.toLowerCase()).contains(report.getRole())) {
        ...
        try {
          ...
          if (report.getStatus().equals(HostRoleStatus.COMPLETED.toString())) {
            ...
            if (!scHost.getState().equals(org.apache.shpurdp.server.state.State.UPGRADING) &&
   ...
   * @throws ShpurdpException
   */
  protected void processStatusReports(HeartBeat heartbeat) throws ShpurdpException {
   ...
   * @throws ShpurdpException
   */
  public void processVersionReports(ComponentVersionReports versionReports, String hostname) throws ShpurdpException {
   ...
   * @throws ShpurdpException
   */
  public void processStatusReports(List<ComponentStatus> componentStatuses, String hostname) throws ShpurdpException {
    Set<Cluster> clusters = clusterFsm.getClustersForHost(hostname);
    for (Cluster cl : clusters) {
          ...
          try {
            ...
            if (svc.getServiceComponents().containsKey(componentName)) {
              ...
              if (status.getStatus() != null) {
                org.apache.shpurdp.server.state.State prevState = scHost.getState();
                org.apache.shpurdp.server.state.State liveState =
                    org.apache.shpurdp.server.state.State.valueOf(org.apache.shpurdp.server.state.State.class,
                        status.getStatus());
                //ignore reports from status commands if component is in INIT or any "in progress" state
                if (prevState.equals(org.apache.shpurdp.server.state.State.INSTALLED)
                    || prevState.equals(org.apache.shpurdp.server.state.State.STARTED)
                    || prevState.equals(org.apache.shpurdp.server.state.State.UNKNOWN)) {
