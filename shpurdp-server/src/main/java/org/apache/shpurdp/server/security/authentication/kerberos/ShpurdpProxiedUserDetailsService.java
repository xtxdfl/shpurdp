...
import org.apache.shpurdp.server.security.authentication.tproxy.ShpurdpTProxyConfiguration;
import org.apache.shpurdp.server.security.authentication.tproxy.TrustedProxyAuthenticationDetails;
import org.apache.shpurdp.server.security.authentication.tproxy.TrustedProxyAuthenticationNotAllowedException;
...
/**
 * ShpurdpProxiedUserDetailsService is a {@link UserDetailsService} that handles proxied users via the
 ...
 */
@Component
public class ShpurdpProxiedUserDetailsService implements UserDetailsService {
  private static final Logger LOG = LoggerFactory.getLogger(ShpurdpProxiedUserDetailsService.class);
  ...
  @Inject
  private Provider<ShpurdpTProxyConfiguration> shpurdpTProxyConfigurationProvider;
   ...
   * Constructor.
   * <p>
   * Given the Shpurdp {@link Configuration}, initializes the {@link KerberosName} class using
   * the <code>auth-to-local</code> rules from {@link ShpurdpKerberosAuthenticationProperties#getAuthToLocalRules()}.
   *
   * @param configuration the Shpurdp configuration data
   * @param users         the Shpurdp users access object
   */
  ShpurdpProxiedUserDetailsService(Configuration configuration, Users users) {
  ...
  public UserDetails loadProxiedUser(String proxiedUserName,
    ...
    ShpurdpTProxyConfiguration tProxyConfiguration = shpurdpTProxyConfigurationProvider.get();
  ...
  boolean validateGroup(ShpurdpTProxyConfiguration tProxyConfiguration, String proxyUserName, UserEntity userEntity) {
  ...
  boolean validateUser(ShpurdpTProxyConfiguration tProxyConfiguration, String proxyUserName, String proxiedUserName) {
  ...
  boolean validateHost(ShpurdpTProxyConfiguration tProxyConfiguration, String proxyUserName, String remoteAddress) {
  ...
  private UserDetails createUserDetails(UserEntity userEntity) {
    ...
    return new ShpurdpUserDetailsImpl(new User(userEntity), null, users.getUserAuthorities(userEntity));
  }
}
