...
package org.apache.shpurdp.server.security.authorization.internal;
...
public class InternalAuthenticationToken implements Authentication {
  ...
  private static final Collection<? extends GrantedAuthority> AUTHORITIES =
    Collections.singleton(new ShpurdpGrantedAuthority(ADMIN_PRIV_ENTITY));
  ...
  private static void createAdminPrivilegeEntity(PrivilegeEntity entity) {
    ...
    pe.setId(PermissionEntity.SHPURDP_ADMINISTRATOR_PERMISSION);
    pe.setPermissionName(PermissionEntity.SHPURDP_ADMINISTRATOR_PERMISSION_NAME);
    ...
    rte.setId(ResourceType.SHPURDP.getId());
    rte.setName(ResourceType.SHPURDP.name());
