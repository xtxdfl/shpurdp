...
import org.apache.shpurdp.server.controller.SettingResponse;
...
public class SettingResourceProvider extends AbstractAuthorizedResourceProvider {
  ...
  protected SettingResourceProvider() {
    ...
    EnumSet<RoleAuthorization> requiredAuthorizations = EnumSet.of(RoleAuthorization.SHPURDP_MANAGE_SETTINGS);
  ...
  private Command<SettingResponse> newCreateCommand(final Request request, final Map<String, Object> properties) {
    return new Command<SettingResponse>() {
      @Override
      public SettingResponse invoke() throws ShpurdpException, AuthorizationException {
      ...
      public Void invoke() throws ShpurdpException {
        ...
        for (Map<String, Object> propertyMap : propertyMaps) {
          if (propertyMap.containsKey(SETTING_NAME_PROPERTY_ID)) {
            ...
            SettingEntity entity = dao.findByName(name);
            if (entity == null) {
              throw new ShpurdpException(String.format("There is no setting with name: %s ", name));
            }
  ...
  private void updateEntity(SettingEntity entity, Map<String, Object> propertyMap) throws ShpurdpException {
    String name = propertyMap.get(SETTING_NAME_PROPERTY_ID).toString();
    if (!Objects.equals(name, entity.getName())) {
      throw new ShpurdpException("Name for Setting is immutable, cannot change name.");
    }
  ...
  private SettingEntity toEntity(final Map<String, Object> properties) throws ShpurdpException {
    for (String propertyName: REQUIRED_PROPERTIES) {
      if (properties.get(propertyName) == null) {
        throw new ShpurdpException(String.format("Property %s should be provided", propertyName));
      }
    }
