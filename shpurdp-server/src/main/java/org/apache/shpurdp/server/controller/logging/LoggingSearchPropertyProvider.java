...
public class LoggingSearchPropertyProvider implements PropertyProvider {
  ...
  public Set<Resource> populateResources(Set<Resource> resources, Request request, Predicate predicate) throws SystemException {
    ...
    for (Resource resource : resources) {
      ...
      final String mappedComponentNameForLogSearch =
        getMappedComponentNameForSearch(clusterName, componentName, shpurdpManagementController);
      ...
      if (mappedComponentNameForLogSearch != null) {
        ...
        if ((logFileNames != null) && (!logFileNames.isEmpty())) {
          ...
          for (String fileName : logFileNames) {
            ...
            final String searchEngineURI = shpurdpManagementController.getShpurdpServerURI(getFullPathToSearchEngine(clusterName));
  ...
  private Long getClusterResourceID(String clusterName) {
    ...
    if(!StringUtils.isEmpty(clusterName)) {
      try {
        Cluster cluster = shpurdpManagementController.getClusters().getCluster(clusterName);
  ...
  private boolean logSearchServerRunning(String clusterName) {
    return loggingRequestHelperFactory.getHelper(shpurdpManagementController, clusterName) != null;
  }

  private String getMappedComponentNameForSearch(String clusterName, String componentName, ShpurdpManagementController controller) {
    try {
      ShpurdpMetaInfo metaInfo = controller.getShpurdpMetaInfo();
  ...
  protected void setShpurdpManagementController(ShpurdpManagementController shpurdpManagementController) {
