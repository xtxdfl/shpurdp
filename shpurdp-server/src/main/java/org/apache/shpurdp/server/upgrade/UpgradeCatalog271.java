...
import static org.apache.shpurdp.server.upgrade.UpgradeCatalog270.YARN_SERVICE;
...
public class UpgradeCatalog271 extends AbstractUpgradeCatalog {
   ...
   */
  protected void updateRangerLogDirConfigs() throws ShpurdpException {
   ...
   * @throws ShpurdpException
   */
  protected void updateRangerKmsDbUrl() throws ShpurdpException {
    ShpurdpManagementController shpurdpManagementController = injector.getInstance(ShpurdpManagementController.class);
    Clusters clusters = shpurdpManagementController.getClusters();
    if (clusters != null) {
      Map<String, Cluster> clusterMap = clusters.getClusters();
   ...
   * In Shpurdp 2.7 these properties are set to {{timeline_reader_address_http}} and {{timeline_reader_address_https}} and the stack code substitutes the proper host names.
   * In Shpurdp 2.7.1 the stack code no longer does this but the stack advisor and a SingleHostTopologyUpdater is responsible for replacing the hosts.
   */
  protected void updateTimelineReaderAddress() throws ShpurdpException {
    Clusters clusters = injector.getInstance(ShpurdpManagementController.class).getClusters();
  ...
  private void updateProperty(Cluster cluster, String configType, String propertyName, String newValue) throws ShpurdpException {
  ...
  private Optional<String> hostNameOf(Cluster cluster, String serviceName, String componentName) throws ShpurdpException {
    try {
  ...
  protected void renameShpurdpInfraService() {
    LOG.info("Renaming service SHPURDP_INFRA to SHPURDP_INFRA_SOLR in config group records");
    ShpurdpManagementController shpurdpManagementController = injector.getInstance(ShpurdpManagementController.class);
    Clusters clusters = shpurdpManagementController.getClusters();
    if (clusters == null)
      return;
    ...
    executeInTransaction(() -> {
      ...
      upgradeHistoryUpdate.setParameter("newServiceName", SHPURDP_INFRA_NEW_NAME);
      upgradeHistoryUpdate.setParameter("oldServiceName", SHPURDP_INFRA_OLD_NAME);
