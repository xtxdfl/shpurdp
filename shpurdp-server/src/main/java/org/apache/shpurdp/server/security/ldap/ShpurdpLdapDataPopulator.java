 ...
 */
public class ShpurdpLdapDataPopulator {
  /**
   * Log.
   */
  private static final Logger LOG = LoggerFactory.getLogger(ShpurdpLdapDataPopulator.class);

  /**
   * Shpurdp configuration.
   */
  private Provider<ShpurdpLdapConfiguration> configurationProvider;
  ...
  /**
   * Construct an ShpurdpLdapDataPopulator.
   *
   * @param configurationProvider the Shpurdp configuration
   * @param users                 utility that provides access to Users
   */
  @Inject
  public ShpurdpLdapDataPopulator(Provider<ShpurdpLdapConfiguration> configurationProvider, Users users) {
   ...
   * @throws ShpurdpException if synchronization failed for any reason
   */
  public LdapBatchDto synchronizeAllLdapGroups(LdapBatchDto batchInfo, boolean collectIgnoredUsers) throws ShpurdpException {
   ...
   * @throws ShpurdpException if synchronization failed for any reason
   */
  public LdapBatchDto synchronizeAllLdapUsers(LdapBatchDto batchInfo, boolean collectIgnoredUsers) throws ShpurdpException {
   ...
   * @throws ShpurdpException if synchronization failed for any reason
   */
  public LdapBatchDto synchronizeLdapGroups(Set<String> groups, LdapBatchDto batchInfo, boolean collectIgnoredUsers) throws ShpurdpException {
    ...
    for (String group : groups) {
      ...
      if (groupDtos.isEmpty()) {
        throw new ShpurdpException("Couldn't sync LDAP group " + group
            + ", it doesn't exist");
      }
   ...
   * @throws ShpurdpException if synchronization failed for any reason
   */
  public LdapBatchDto synchronizeLdapUsers(Set<String> users, LdapBatchDto batchInfo, boolean collectIgnoredUsers) throws ShpurdpException {
    ...
    for (String user : users) {
      ...
      if (userDtos.isEmpty()) {
        throw new ShpurdpException("Couldn't sync LDAP user " + user
            + ", it doesn't exist");
      }
   ...
   * @throws ShpurdpException if synchronization failed for any reason
   */
  public LdapBatchDto synchronizeExistingLdapGroups(LdapBatchDto batchInfo, boolean collectIgnoredUsers) throws ShpurdpException {
   ...
   * @throws ShpurdpException if synchronization failed for any reason
   */
  public LdapBatchDto synchronizeExistingLdapUsers(LdapBatchDto batchInfo, boolean collectIgnoredUsers) throws ShpurdpException {
   ...
   * @param collectIgnoredUsers   true, to collect the set of existing users that would normally be ignored;
   *                              false, to continue to ignore them
   * @throws ShpurdpException if group refresh failed
   */
   ...
   * @throws ShpurdpException
   */
  protected void cleanUpLdapUsersWithoutGroup() throws ShpurdpException {
  ...
  protected Map<String, User> getInternalUsers() {
    ...
    LOG.trace("Get all users from Shpurdp Server.");
  ...
  protected static class LdapGroupContextMapper implements ContextMapper {
    ...
    public Object mapFromContext(Object ctx) {
      ...
      boolean outOfScope = ShpurdpLdapUtils.isLdapObjectOutOfScopeFromBaseDn(adapter, ldapServerProperties.getBaseDN());
      if (outOfScope) {
  ...
  private ShpurdpLdapConfiguration getConfiguration() {
