...
import org.apache.shpurdp.server.controller.ConfigGroupRequest;
...
import org.apache.shpurdp.server.controller.internal.AbstractResourceProvider;
import org.apache.shpurdp.server.controller.internal.ComponentResourceProvider;
import org.apache.shpurdp.server.controller.internal.ConfigGroupResourceProvider;
...
import org.apache.shpurdp.server.controller.internal.VersionDefinitionResourceProvider;
...
/**
 * Provides topology related information as well as access to the core Shpurdp functionality.
 */
public class ShpurdpContext {
  ...
  private final static Logger LOG = LoggerFactory.getLogger(ShpurdpContext.class);
  ...
  public HostRoleCommand createShpurdpTask(long requestId, long stageId, String component, String host,
  ...
  //todo: change return type to a topology abstraction
  public HostRoleCommand createShpurdpTask(long taskId, long requestId, long stageId,
  ...
  public void createShpurdpResources(ClusterTopology topology, String clusterName, SecurityType securityType,
    ...
    createShpurdpClusterResource(clusterName, stack.getName(), stack.getVersion(), securityType);
    createShpurdpServiceAndComponentResources(topology, clusterName, stackId, repoVersion.getId());
  }

  public void createShpurdpClusterResource(String clusterName, String stackName, String stackVersion, SecurityType securityType) {
  ...
  public void createShpurdpServiceAndComponentResources(ClusterTopology topology, String clusterName,
    ...
    } catch (ShpurdpException | AuthorizationException e) {
      throw new RuntimeException("Failed to persist service and component resources: " + e, e);
    }
  ...
  public void createShpurdpHostResources(long  clusterId, String hostName, Map<String, Collection<String>> components)  {
    Host host;
    try {
    ...
    for (Map.Entry<String, Collection<String>> entry : components.entrySet()) {
      ...
      for (String component : entry.getValue()) {
        ...
        try {
          if (cluster.getService(service) != null && !component.equals(RootComponent.SHPURDP_SERVER.name())) {
            requests.add(new ServiceComponentHostRequest(clusterName, service, component, hostName, null));
          }
        } catch(ShpurdpException se) {
  ...
  public synchronized static ShpurdpManagementController getController() {
    if (controller == null) {
      controller = ShpurdpServer.getController();
    }
    return controller;
  }
  ...
  /**
   * Persist cluster state for the shpurdp UI.  Setting this state informs that UI that a cluster has been
   ...
   *
   * @throws ShpurdpException upon any system-level error that occurs
   */
  public void waitForConfigurationResolution(String clusterName, Set<String> updatedConfigTypes) throws ShpurdpException {
    Cluster cluster = getController().getClusters().getCluster(clusterName);
  ...
  public long getClusterId(String clusterName) throws ShpurdpException {
    return getController().getClusters().getCluster(clusterName).getClusterId();
  }

  public String getClusterName(long clusterId) throws ShpurdpException {
   ...
   */
  private void createConfigGroupsAndRegisterHost(ClusterTopology topology, String groupName) throws ShpurdpException {
