...
public abstract class ServiceDirectory extends StackDefinitionDirectory {
   ...
   * @throws ShpurdpException if unable to parse the service directory
   */
  public ServiceDirectory(String servicePath) throws ShpurdpException {
    super(servicePath);
    parsePath();
  }
   ...
   */
  protected void parsePath() throws ShpurdpException {
    ...
    File kdf = new File(directory, ShpurdpMetaInfo.KERBEROS_DESCRIPTOR_FILE_NAME);
   ...
   *
   * @throws ShpurdpException if the metainfo file doesn't exist or
   *                         unable to unmarshal the metainfo file
   */
  protected void parseMetaInfoFile() throws ShpurdpException {
    File f = new File(getAbsolutePath() + File.separator + SERVICE_METAINFO_FILE_NAME);
    if (! f.exists()) {
      throw new ShpurdpException(String.format("Stack Definition Service at '%s' doesn't contain a metainfo.xml file",
          f.getAbsolutePath()));
    }
