...
public class ExportBlueprintRequest implements TopologyRequest {
  ...
  public ExportBlueprintRequest(TreeNode<Resource> clusterNode, ShpurdpManagementController controller) throws InvalidTopologyTemplateException {
  ...
  public class ExportedHostGroup {
    ...
    public ExportedHostGroup(TreeNode<Resource> host) {
      ...
      addShpurdpComponentIfLocalhost((String) host.getObject().getPropertyValue(
          PropertyHelper.getPropertyId("Hosts", "host_name")));
    ...
    /**
     * Add the SHPURDP_SERVER component if the host is the local host.
     *
     * @param hostname  host to check
     */
    private void addShpurdpComponentIfLocalhost(String hostname) {
      try {
        InetAddress hostAddress = InetAddress.getByName(hostname);
        try {
          if (hostAddress.equals(InetAddress.getLocalHost())) {
            getComponents().add("SHPURDP_SERVER");
          }
