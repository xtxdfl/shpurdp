...
import static org.apache.shpurdp.server.controller.RootComponent.SHPURDP_AGENT;
...
public class AlertDefinition {
   ...
   * In case of SHPURDP server alerts or AGGREGATE alerts, an empty host set is returned.
   * @return matching host names
   */
  public Set<String> matchingHosts(Clusters clusters) throws ShpurdpException {
    Cluster cluster = clusters.getCluster(clusterId);
    if (source.getType() == SourceType.AGGREGATE) {
      return emptySet();
    }
    if (SHPURDP.name().equals(serviceName)) {
      return SHPURDP_AGENT.name().equals(componentName) ? cluster.getHostNames() : emptySet();
    }
