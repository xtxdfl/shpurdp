...
public interface KerberosHelper {
  ...
  /**
   * The hostname used to hold the place of the actual hostname of the host that the Shpurdp server
   * is on.
   */
  String SHPURDP_SERVER_HOST_NAME = "shpurdp_server";
  /**
   * The name of the Kerberos environment configuration type
   */
  String KERBEROS_ENV = "kerberos-env";
  /**
   * The name of the Shpurdp server's Kerberos identities as defined in the Kerberos descriptor
   */
  String SHPURDP_SERVER_KERBEROS_IDENTITY_NAME = "shpurdp-server";
  /**
   * The kerberos-env property name declaring whether Shpurdp should manage its own required
   * identities or not
   */
  String CREATE_SHPURDP_PRINCIPAL = "create_shpurdp_principal";
  /**
   * The kerberos-env property name declaring whether Shpurdp should manage the cluster's required
   * identities or not
   */
   ...
   *
   * @see org.apache.shpurdp.server.serveraction.kerberos.KDCType
   */
   ...
   *
   * @see org.apache.shpurdp.server.serveraction.kerberos.PreconfigureServiceType
   */
  ...
  void deleteIdentities(Cluster cluster, List<Component> components, Set<String> identities) throws ShpurdpException, KerberosOperationException;
  ...
  Set<String> getHostsWithValidKerberosClient(Cluster cluster) throws ShpurdpException;
   ...
   */
  KerberosDescriptor getKerberosDescriptor(Cluster cluster, boolean includePreconfigureData) throws ShpurdpException;
   ...
   * <p/>
   * See {@link org.apache.shpurdp.server.state.kerberos.VariableReplacementHelper#replaceVariables(String, java.util.Map)}
   ...
   * @throws ShpurdpException if an issue occurs
   */
   ...
   * <p/>
   * Most of this was borrowed from {@link org.apache.shpurdp.server.actionmanager.ExecutionCommandWrapper#getExecutionCommand()}
   *
   * @param cluster                      the relevant Cluster
   * @param hostname                     the relevant hostname
  ...
  /**
   * Gets the Shpurdp server Kerberos identities found in the Kerberos descriptor.
   *
   * @param kerberosDescriptor the kerberos descriptor
   */
  List<KerberosIdentityDescriptor> getShpurdpServerIdentities(KerberosDescriptor kerberosDescriptor) throws ShpurdpException;

  /**
   * Determines if the Shpurdp identities should be created when enabling Kerberos.
   * <p>
   ...
   * @return true if the Shpurdp identities should be created; otherwise false
   */
  boolean createShpurdpIdentities(Map<String, String> kerberosEnvProperties);
   ...
   */
  PrincipalKeyCredential getKDCAdministratorCredentials(String clusterName) throws ShpurdpException;
   ...
   */
  File createTemporaryDirectory() throws ShpurdpException;
  ...
  /**
   * Command to invoke against the Shpurdp backend.
   *
  ...
  interface Command<T, A> {
     ...
     *
     * @return the response
     * @throws ShpurdpException thrown if a problem occurred during invocation
     */
    T invoke(A arg) throws ShpurdpException;
  }
}
