...
import org.apache.shpurdp.server.state.scheduler.GuiceJobFactory;
...
public class ExecutionSchedulerImpl implements ExecutionScheduler {
  ...
  @Override
  public synchronized void startScheduler(Integer delay) throws ShpurdpException {
    try {
      if (!isInitialized) {
        initializeScheduler();
        isInitialized = true;
  ...
  @Override
  public synchronized void stopScheduler() throws ShpurdpException {
    if (scheduler == null) {
      throw new ShpurdpException("Scheduler not instantiated !");
    }
    try {
      scheduler.shutdown();
    } catch (SchedulerException e) {
