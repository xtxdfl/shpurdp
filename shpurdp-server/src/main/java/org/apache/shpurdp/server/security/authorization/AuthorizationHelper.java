...
import org.apache.shpurdp.server.security.authentication.ShpurdpProxiedUserDetailsImpl;
...
public class AuthorizationHelper {
  ...
  public static String getProxyUserName(Authentication authentication) {
    ...
    if (userDetails instanceof ShpurdpProxiedUserDetailsImpl) {
      ShpurdpProxiedUserDetailsImpl shpurdpProxiedUserDetails = (ShpurdpProxiedUserDetailsImpl) userDetails;
      return shpurdpProxiedUserDetails.getProxyUserDetails().getUsername();
    }
    return null;
  }
  ...
  public static int getAuthenticatedId() {
    ...
    if (principal instanceof ShpurdpUserDetails) {
      return ((ShpurdpUserDetails) principal).getUserId();
    }
   ...
   * <p/>
   * The specified resource type is a high-level resource such as {@link ResourceType#SHPURDP Shpurdp},
  ...
  public static boolean isAuthorized(Authentication authentication, ResourceType resourceType,
    ...
    if ((requiredAuthorizations == null) || requiredAuthorizations.isEmpty()) {
      ...
      for (GrantedAuthority grantedAuthority : authentication.getAuthorities()) {
        ShpurdpGrantedAuthority shpurdpGrantedAuthority = (ShpurdpGrantedAuthority) grantedAuthority;
        PrivilegeEntity privilegeEntity = shpurdpGrantedAuthority.getPrivilegeEntity();
        ...
        if (ResourceType.SHPURDP == privilegeResourceType) {
   ...
   * locally in shpurdp. These external login names act as an alias to
   * shpurdp users name. This method stores in the current http session a mapping
   * of alias user name to local shpurdp user name to make possible resolving
   * login alias to shpurdp user name.
   * @param shpurdpUserName shpurdp user name for which the alias is to be stored in the session
   * @param loginAlias The Name with which user logged in Shpurdp UI.
   */
  public static void addLoginNameAlias(String shpurdpUserName, String loginAlias) {
    ServletRequestAttributes attr = (ServletRequestAttributes) RequestContextHolder.getRequestAttributes();
    if (attr != null) {
      LOG.info("Adding login alias '{}' for user name '{}'", loginAlias, shpurdpUserName);
      attr.setAttribute(loginAlias, shpurdpUserName, RequestAttributes.SCOPE_SESSION);
      //save Vice Versa Too
      attr.setAttribute(shpurdpUserName, loginAlias, RequestAttributes.SCOPE_SESSION);
    }
  }

  /**
   * Looks up the provided loginAlias in the current http session and return the shpurdp
   * user name that the alias is defined for.
   * @param loginAlias the login alias to resolve to shpurdp user name
   * @return the shpurdp user name if the alias is found otherwise returns the passed in loginAlias
   */
