...
import org.apache.shpurdp.server.controller.internal.ConfigurationTopologyException;
...
public class ClusterConfigurationRequest {
  ...
  public ClusterConfigurationRequest(ShpurdpContext shpurdpContext, ClusterTopology topology, boolean setInitial, StackAdvisorBlueprintProcessor stackAdvisorBlueprintProcessor, boolean configureSecurity) {
    this(shpurdpContext, topology, setInitial, stackAdvisorBlueprintProcessor);
    this.configureSecurity = configureSecurity;
  }

  public ClusterConfigurationRequest(ShpurdpContext shpurdpContext, ClusterTopology clusterTopology, boolean setInitial,
                                     StackAdvisorBlueprintProcessor stackAdvisorBlueprintProcessor) {
    this.shpurdpContext = shpurdpContext;
  ...
  public void process() throws ShpurdpException, ConfigurationTopologyException {
  ...
  private Set<String> configureKerberos(Configuration clusterConfiguration, Map<String, Map<String, String>> existingConfigurations) throws ShpurdpException {
    ...
    try {
      KerberosHelper kerberosHelper = ShpurdpContext.getController().getKerberosHelper();
  ...
  private Collection<String> getRequiredHostgroupsForKerberosConfiguration() {
    ...
    try {
      ...
      // apply Kerberos specific configurations
      Map<String, Map<String, String>> updatedConfigs = ShpurdpContext.getController().getKerberosHelper()
    ...
    } catch (KerberosInvalidConfigurationException | ShpurdpException e) {
      LOG.error("An exception occurred while doing Kerberos related configuration update: " + e, e);
    }
    return requiredHostGroups;
  }

  private Cluster getCluster() throws ShpurdpException {
    String clusterName = shpurdpContext.getClusterName(clusterTopology.getClusterId());
    return ShpurdpContext.getController().getClusters().getCluster(clusterName);
  }
   ...
   *   to comply with the ServiceConfigVersioning framework in Shpurdp.
   *
   * This method will also send these requests to the management controller.
   *
   * @param configurationRequests a list of requests to send to the ShpurdpManagementController.
   */
  private void setConfigurationsOnCluster(List<BlueprintServiceConfigRequest> configurationRequests,
    ...
    Cluster cluster;
    try {
      clusterName = shpurdpContext.getClusterName(clusterTopology.getClusterId());
      cluster = ShpurdpContext.getController().getClusters().getCluster(clusterName);
    ...
    for (BlueprintServiceConfigRequest blueprintConfigRequest : configurationRequests) {
      ...
      for (BlueprintServiceConfigElement blueprintElement : blueprintConfigRequest.getConfigElements()) {
        ...
        List<ConfigurationRequest> listOfRequests = shpurdpContext.createConfigurationRequests(clusterProperties);
      ...
      if (clusterRequest != null) {
        ...
        shpurdpContext.setConfigurationOnCluster(clusterRequest);
      } else {
    ...
    shpurdpContext.notifyAgentsAboutConfigsChanges(clusterName);
    ...
    if (tag.equals(TopologyManager.TOPOLOGY_RESOLVED_TAG)) {
      ...
      try {
        ...
        shpurdpContext.waitForConfigurationResolution(clusterName, updatedConfigTypes);
      } catch (ShpurdpException e) {
   ...
   * Shpurdp Blueprints is used to deploy a cluster.
   */
