...
import org.apache.shpurdp.server.api.query.render.AlertSummaryGroupedRenderer;
...
import org.apache.shpurdp.server.events.AlertUpdateEvent;
import org.apache.shpurdp.server.events.InitialAlertEvent;
...
public class AlertReceivedListener {
  ...
  @RequiresSession
  public void onAlertEvent(AlertReceivedEvent event) throws ShpurdpException {
  ...
  private void clearStaleAlerts(String hostName, Long definitionId) throws ShpurdpException {
  ...
  private boolean isValid(Alert alert) {
    ...
    // SHPURDP/SHPURDP_SERVER is always a valid service/component combination
    String shpurdpServiceName = RootService.SHPURDP.name();
    String shpurdpServerComponentName = RootComponent.SHPURDP_SERVER.name();
    String shpurdpAgentComponentName = RootComponent.SHPURDP_AGENT.name();
    if (shpurdpServiceName.equals(serviceName) && shpurdpServerComponentName.equals(componentName)) {
      return true;
    }
    ...
    // - a cluster exists
    // - this is not for SHPURDP_SERVER component
    ...
    try {
      ...
      LOG.error(msg, shpurdpException);
      return false;
    }
    ...
    // - this is not for SHPURDP_SERVER component
    //
    // if the alert is for SHPURDP/SHPURDP_AGENT, then it's valid IFF
    // the agent's host is still a part of the reported cluster
    if (shpurdpServiceName.equals(serviceName) && shpurdpAgentComponentName.equals(componentName)) {
      ...
      // SHPURDP/SHPURDP_AGENT and valid host; return true
      return true;
    }

    // at this point the following criteria is guaranteed
    // - a cluster exists
    // - not for the SHPURDP service
