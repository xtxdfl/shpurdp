...
/**
 * ShpurdpJwtAuthenticationFilter is used to validate JWT token and authenticate users.
 * <p>
 * This authentication filter is expected to be used withing an {@link ShpurdpDelegatingAuthenticationFilter}.
 *
 * @see ShpurdpDelegatingAuthenticationFilter
 */
@Component
@Order(1)
public class ShpurdpJwtAuthenticationFilter implements ShpurdpAuthenticationFilter {
  private static final Logger LOG = LoggerFactory.getLogger(ShpurdpJwtAuthenticationFilter.class);
   ...
   */
  private final AuthenticationEntryPoint shpurdpEntryPoint;
   ...
   * Constructor.
   *
   * @param shpurdpEntryPoint   the Spring entry point
   * @param propertiesProvider a provider for the SSO-related Shpurdp configuration
   * @param eventHandler       the Shpurdp authentication event handler
   */
  ShpurdpJwtAuthenticationFilter(AuthenticationEntryPoint shpurdpEntryPoint,
                                JwtAuthenticationPropertiesProvider propertiesProvider,
                                ShpurdpJwtAuthenticationProvider authenticationProvider,
                                ShpurdpAuthenticationEventHandler eventHandler) {
    if (eventHandler == null) {
      throw new IllegalArgumentException("The ShpurdpAuthenticationEventHandler must not be null");
    }

    this.shpurdpEntryPoint = shpurdpEntryPoint;
  ...
  public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain chain) throws IOException, ServletException {
    ...
    try {
      ...
      if (e instanceof ShpurdpAuthenticationException) {
        ...
        cause = new ShpurdpAuthenticationException(null, e.getMessage(), false, e);
      }
      ...
      shpurdpEntryPoint.commence(httpServletRequest, httpServletResponse, e);
    }
  }
  ...
  private boolean isAuthenticationRequired(String token) {
    ...
    if (existingAuth instanceof ShpurdpUserAuthentication && !StringUtils.equals(token, (String) existingAuth.getCredentials())) {
      return true;
    }
  ...
  String getJWTFromCookie(HttpServletRequest req) {
    ...
    if (cookies != null) {
      ...
      if (StringUtils.isEmpty(jwtCookieName)) {
        jwtCookieName = ShpurdpServerConfigurationKey.SSO_JWT_COOKIE_NAME.getDefaultValue();
      }
