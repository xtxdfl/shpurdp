...
import org.apache.shpurdp.server.utils.HTTPUtils;
import org.apache.shpurdp.server.utils.HostAndPort;
...
abstract class KDCKerberosOperationHandler extends KerberosOperationHandler {
  ...
  protected Keytab createKeytab(String principal, String password, Integer keyNumber)
    ...
    try {
      try {
        keytabFile = File.createTempFile("shpurdp_tmp", ".keytab");
  ...
  protected boolean init(Map<String, String> kerberosConfiguration) throws KerberosOperationException {
    ...
    try {
      credentialsCacheFile = File.createTempFile("shpurdp_krb_", "cc");
      credentialsCacheFile.deleteOnExit();
      ensureShpurdpOnlyAccess(credentialsCacheFile);
    } catch (IOException e) {
   ...
   */
  private void ensureShpurdpOnlyAccess(File file) throws ShpurdpException {
    if (file.exists()) {
