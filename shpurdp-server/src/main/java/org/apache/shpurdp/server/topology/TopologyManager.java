...
import org.apache.shpurdp.server.controller.internal.ScaleClusterRequest;
...
import org.apache.shpurdp.server.events.ClusterProvisionStartedEvent;
import org.apache.shpurdp.server.events.ClusterProvisionedEvent;
import org.apache.shpurdp.server.events.HostsRemovedEvent;
import org.apache.shpurdp.server.events.RequestFinishedEvent;
...
import org.apache.shpurdp.server.topology.addservice.ResourceProviderAdapter;
...
import org.apache.shpurdp.server.utils.ManagedThreadPoolExecutor;
...
public class TopologyManager {
  ...
  @Inject
  private ShpurdpEventPublisher shpurdpEventPublisher;
  ...
  private void setPersistedState() {
    persistedState = shpurdpContext.getPersistedTopologyState();
  }
  ...
  public void onRequestFinished(RequestFinishedEvent event) {
    if(event.getType() != ShpurdpEvent.ShpurdpEventType.REQUEST_FINISHED
    ...
    if(isClusterProvisionWithBlueprintFinished(event.getClusterId())) {
      ...
      if(isLogicalRequestSuccessful(provisionRequest)) {
        LOG.info("Cluster creation request id={} using Blueprint {} successfully completed for cluster id={}",
                ...
                event.getClusterId());
        shpurdpEventPublisher.publish(new ClusterProvisionedEvent(event.getClusterId()));
      } else {
  ...
  public RequestStatusResponse provisionCluster(final ProvisionClusterRequest request) throws InvalidTopologyException, ShpurdpException {
    ensureInitialized();

    final ClusterTopology topology = new ClusterTopologyImpl(shpurdpContext, request);
    ...
    // get the id prior to creating shpurdp resources which increments the counter
    final Long provisionId = shpurdpContext.getNextRequestId();
    ...
    // create resources
    shpurdpContext.createShpurdpResources(topology, clusterName, securityType, repoVersion, repoVersionID);
    ...
    long clusterId = shpurdpContext.getClusterId(clusterName);
    ...
    addClusterConfigRequest(logicalRequest, topology, new ClusterConfigurationRequest(shpurdpContext, topology, true,
    ...
    shpurdpContext.persistInstallStateForUI(clusterName, stack.getName(), stack.getVersion());
    clusterProvisionWithBlueprintCreateRequests.put(clusterId, logicalRequest);
    shpurdpEventPublisher.publish(new ClusterProvisionStartedEvent(clusterId));
  ...
  private boolean isKerberosClientInstallAllowed(final ClusterTopology topology) {
    final org.apache.shpurdp.server.topology.Configuration topologyConfig = topology.getBlueprint().getConfiguration();
  ...
  /**
   * Saves the quick links profile to the DB as an Shpurdp setting. Creates a new setting entity or updates the existing
   * one.
  ...
  void saveOrUpdateQuickLinksProfile(String quickLinksProfileJson) {
    ...
    if (null == settingEntity) {
      ...
      settingEntity.setSettingType(QuickLinksProfile.SETTING_TYPE_SHPURDP_SERVER);
  ...
  private static void submitCredential(String clusterName, Credential credential) {
    ResourceProvider provider = ShpurdpContext.getClusterController().ensureResourceProvider(Resource.Type.Credential);
  ...
  private void submitKerberosDescriptorAsArtifact(String clusterName, Map<?,?> descriptor) {
    ...
    ResourceProvider artifactProvider =
        shpurdpContext.getClusterController().ensureResourceProvider(Resource.Type.Artifact);
  ...
  public RequestStatusResponse scaleHosts(final ScaleClusterRequest request)
      throws InvalidTopologyException, ShpurdpException {
    ...
    if (topology == null) {
      throw new InvalidTopologyException("Unable to retrieve cluster topology for cluster. This is most likely a " +
          ...
          "the Shpurdp UI. At this time only clusters created via the API using a " +
    ...
    request.setClusterId(clusterId);
    if (shpurdpContext.isTopologyResolved(clusterId)) {
    ...
    final Long requestId = shpurdpContext.getNextRequestId();
    LogicalRequest logicalRequest = RetryHelper.executeWithRetry(new Callable<LogicalRequest>() {
         @Override
         public LogicalRequest call() throws Exception {
  ...
  public void removePendingHostRequests(String clusterName, long requestId) {
    ...
    long clusterId = 0;
    try {
      clusterId = shpurdpContext.getClusterId(clusterName);
  ...
  public void onHostHeartBeatLost(Host host) {
    if (ShpurdpServer.getController() == null) {
      return;
    }
    ensureInitialized();
    synchronized (availableHosts) {
  ...
  private void queueHostTasks(ClusterTopology topology, HostOfferResponse response, String hostName) {
    ...
    response.executeTasks(executorService, hostName, topology, shpurdpContext);
  }
  ...
  private void updateHostWithRackInfo(ClusterTopology topology, HostOfferResponse response, HostImpl host) {
    ...
    if (null != rackInfoFromTemplate) {
      ...
      try {
        ...
        shpurdpContext.getController().registerRackChange(shpurdpContext.getClusterName(topology.getClusterId()));
      } catch (ShpurdpException e) {
  ...
  private void replayRequests(Map<ClusterTopology, List<LogicalRequest>> persistedRequests) {
    ...
    for (Map.Entry<ClusterTopology, List<LogicalRequest>> requestEntry : persistedRequests.entrySet()) {
      ...
      if (!configChecked) {
        configChecked = true;
        if (!shpurdpContext.isTopologyResolved(topology.getClusterId())) {
          if (provisionRequest == null) {
            ...
            ClusterConfigurationRequest configRequest = new ClusterConfigurationRequest(shpurdpContext, topology, false, stackAdvisorBlueprintProcessor);
  ...
  private void addClusterConfigRequest(final LogicalRequest logicalRequest, ClusterTopology topology, ClusterConfigurationRequest configurationRequest) {
    ConfigureClusterTask task = configureClusterTaskFactory.createConfigureClusterTask(topology, configurationRequest, shpurdpEventPublisher);
