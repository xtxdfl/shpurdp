...
package org.apache.shpurdp.server.controller.jmx;
...
import org.apache.shpurdp.server.state.services.MetricsRetrievalService.MetricSourceType;
...
public class JMXPropertyProvider extends ThreadPoolEnabledPropertyProvider {
  ...
  /**
   * When Shpurdp queries NameNode's HA state (among other metrics), it retrieves all metrics from "NN_URL:port/jmx".
  ...
  protected Resource populateResource(Resource resource, Request request, Predicate predicate, Ticket ticket)
    ...
    for (String hostName : hostNames) {
      try {
        ...
        ShpurdpException detailedException = new ShpurdpException(String.format(
