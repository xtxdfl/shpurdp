...
import org.apache.shpurdp.server.state.ServicePropertyInfo;
import org.apache.shpurdp.server.state.ThemeInfo;
...
public class ServiceModule extends BaseModule<ServiceModule, ServiceInfo> implements Validable{
  ...
  public ServiceModule(
    ...
    serviceInfo.setSchemaVersion(ShpurdpMetaInfo.SCHEMA_VERSION_2);
  ...
  public void resolveCommonService(Map<String, StackModule> allStacks, Map<String, ServiceModule> commonServices, Map<String, ExtensionModule> extensions)
      throws ShpurdpException {
    if(!isCommonService) {
      throw new ShpurdpException("Not a common service");
    }
    ...
    if(parentString != null) {
      ...
      if(parentToks.length != 3) {
        throw new ShpurdpException("The common service '" + serviceInfo.getName() + serviceInfo.getVersion()
      ...
      if (parentToks[0].equalsIgnoreCase(StackManager.COMMON_SERVICES)) {
        ...
        if (baseModuleState == ModuleState.INIT) {
          ...
          throw new ShpurdpException("Cycle detected while parsing common service");
        }
        resolveExplicit(baseService, allStacks, commonServices, extensions);
      } else {
        throw new ShpurdpException("Common service cannot inherit from a non common service");
      }
    }
  ...
  private void mergeThemes(ServiceModule parent, Map<String, StackModule> allStacks,
                           Map<String, ServiceModule> commonServices, Map<String, ExtensionModule> extensions) throws ShpurdpException {
