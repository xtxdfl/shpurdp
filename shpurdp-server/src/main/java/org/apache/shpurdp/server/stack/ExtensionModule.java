...
public class ExtensionModule extends BaseModule<ExtensionModule, ExtensionInfo> implements Validable {
   ...
   *
   * @throws ShpurdpException if an exception occurs during extension resolution
   */
  @Override
  public void resolve(
  ...
  private void mergeExtensionWithParent(
    ...
    if (parentExtension == null) {
      throw new ShpurdpException("Extension '" + extensionInfo.getName() + ":" + extensionInfo.getVersion() +
   ...
   * @param extensions         all extensions
   *
   * @throws ShpurdpException if an exception occurs merging the child services with the parent extension
   */
  ...
  private void mergeServiceWithExplicitParent(
    ...
    if(isCommonServiceParent(parent)) {
      ...
      throw new ShpurdpException("The service '" + service.getModuleInfo().getName() + "' in extension '" + extensionInfo.getName() + ":"
   ...
   * @param allStacks        all stacks specified in the stack definition
   *
   * @throws ShpurdpException if the extension name is the same as any of the stacks
   */
  private void checkExtensionName(Map<String, StackModule> allStacks)
    ...
    for (StackModule stack : allStacks.values()) {
      ...
      if (name.equals(stackName)) {
        throw new ShpurdpException("The extension '" + name + "' has a name which matches a stack name");
      }
    }
  }
  ...
  private void mergeServiceWithCommonServiceParent(
    ...
    if(parentToks.length != 3 || !parentToks[0].equalsIgnoreCase(StackManager.COMMON_SERVICES)) {
      throw new ShpurdpException("The service '" + serviceInfo.getName() + "' in extension '" + extensionInfo.getName() + ":"
          + extensionInfo.getVersion() + "' extends an invalid parent: '" + parent + "'");
    }
   ...
   * @param commonServices     all common services specified in the stack definition
   * @param extensions         all extensions
   * @throws ShpurdpException if unable to resolve the extension
   */
