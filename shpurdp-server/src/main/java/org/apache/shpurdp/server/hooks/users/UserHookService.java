...
package org.apache.shpurdp.server.hooks.users;
...
import org.apache.shpurdp.server.serveraction.users.PostUserCreationHookServerAction;
...
public class UserHookService implements HookService {
  ...
  @Inject
  private ShpurdpEventFactory eventFactory;
  ...
  public boolean execute(HookContext hookContext) {
    ...
    shpurdpEventPublisher.publish(userCreatedEvent);

    return true;
  }

  @Subscribe
  public void onUserCreatedEvent(UserCreatedEvent event) throws ShpurdpException {
    ...
    try {
      ...
      ServiceComponentHostServerActionEvent serverActionEvent = new ServiceComponentHostServerActionEvent("shpurdp-server-host", System.currentTimeMillis());
      Map<String, String> commandParams = prepareCommandParams(ctx, clsData);

      stage.addServerActionCommand(PostUserCreationHookServerAction.class.getName(), "shpurdp", Role.SHPURDP_SERVER_ACTION,
      ...
      throw new ShpurdpException("Failed to assemble stage for server action", e);
    }
