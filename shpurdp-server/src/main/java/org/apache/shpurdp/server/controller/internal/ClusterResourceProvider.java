...
import org.apache.shpurdp.server.controller.ServiceConfigVersionRequest;
...
import org.apache.shpurdp.server.topology.TopologyRequestFactory;
...
public class ClusterResourceProvider extends AbstractControllerResourceProvider {
   ...
   */
  ClusterResourceProvider(ShpurdpManagementController managementController) {
    super(Resource.Type.Cluster, propertyIds, keyPropertyIds, managementController);

    setRequiredCreateAuthorizations(EnumSet.of(RoleAuthorization.SHPURDP_ADD_DELETE_CLUSTERS));
    setRequiredDeleteAuthorizations(EnumSet.of(RoleAuthorization.SHPURDP_ADD_DELETE_CLUSTERS));
  ...
  protected boolean isAuthorizedToCreateResources(Authentication authentication, Request request) {
    return AuthorizationHelper.isAuthorized(authentication, ResourceType.SHPURDP, null, getRequiredCreateAuthorizations());
  }

  @Override
  protected boolean isAuthorizedToDeleteResources(Authentication authentication, Predicate predicate) throws SystemException {
    return AuthorizationHelper.isAuthorized(authentication, ResourceType.SHPURDP, null, getRequiredDeleteAuthorizations());
  }
  ...
  public Set<Resource> getResources(Request request, Predicate predicate)
    ...
    Set<ClusterResponse> responses = getResources(new Command<Set<ClusterResponse>>() {
      @Override
      public Set<ClusterResponse> invoke() throws ShpurdpException, AuthorizationException {
