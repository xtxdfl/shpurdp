...
import org.apache.shpurdp.server.controller.logging.LogQueryResponse;
import org.apache.shpurdp.server.controller.logging.LoggingRequestHelper;
...
public class LoggingService extends BaseService {
   ...
   * partial-response syntax support provided by the Shpurdp REST framework.
   *
   ...
   * ShpurdpManagementController.  This facilitates simpler unit testing.
   *
   */
  interface ControllerFactory {
    ShpurdpManagementController getController();
  }

  /**
   * Default implementation of the internal ControllerFactory interface,
   * which uses the ShpurdpServer static method to obtain the controller.
   */
  private static class DefaultControllerFactory implements ControllerFactory {
    @Override
    public ShpurdpManagementController getController() {
      return ShpurdpServer.getController();
    }
  }
}
