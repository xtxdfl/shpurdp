 ...
 */
public class ShpurdpViewErrorHandlerProxy extends ErrorHandler implements MethodHandler {

  private final static Logger LOGGER = LoggerFactory.getLogger(ShpurdpViewErrorHandlerProxy.class);

  private final ErrorHandler webAppErrorHandler;
  private final ShpurdpErrorHandler shpurdpErrorHandler;

  public ShpurdpViewErrorHandlerProxy(ErrorHandler webAppErrorHandler, ShpurdpErrorHandler shpurdpErrorHandler) {
    this.webAppErrorHandler = webAppErrorHandler;
    this.shpurdpErrorHandler = shpurdpErrorHandler;
  }
  ...
  public void handle(String target, Request baseRequest, HttpServletRequest request, HttpServletResponse response) throws IOException {
    ...
    if (isInternalError(request, response)) {
      //invoke the shpurdp error handler
      shpurdpErrorHandler.handle(target, baseRequest, request, response);
    } else {
  ...
  public void setShowStacks(boolean showStacks) {
    shpurdpErrorHandler.setShowStacks(showStacks);
