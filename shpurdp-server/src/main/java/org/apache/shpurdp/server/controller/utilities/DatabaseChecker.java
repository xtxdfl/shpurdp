...
public class DatabaseChecker {
  ...
  static ShpurdpMetaInfo shpurdpMetaInfo;
  static MetainfoDAO metainfoDAO;

  public static void checkDBConsistency() throws ShpurdpException {
    ...
    if (checkPassed) {
      ...
      String errorMessage = "DB consistency check failed. Run \"shpurdp-server start --skip-database-validation\" to skip validation.";
      LOG.error(errorMessage);
      throw new ShpurdpException(errorMessage);
    }
  }
   ...
   */
  public static void checkDBConfigsConsistency() throws ShpurdpException {
    ...
    if (clusters != null) {
      for (ClusterEntity clusterEntity : clusters) {
        ...
        if (clusterStateEntity != null) {
          ...
          StackInfo stack = shpurdpMetaInfo.getStack(currentStack.getStackName(),
          ...
          for (ClusterServiceEntity clusterServiceEntity : clusterServiceEntities) {
            if (!State.INIT.equals(
              ...
              ServiceInfo serviceInfo = shpurdpMetaInfo.getService(stack.getName(),
    ...
    if (checkPassed) {
      ...
      String errorMessage = "DB configs consistency check failed. Run \"shpurdp-server start --skip-database-validation\" to skip validation.";
      LOG.error(errorMessage);
      throw new ShpurdpException(errorMessage);
    }
  }

  public static void checkDBVersion() throws ShpurdpException {
