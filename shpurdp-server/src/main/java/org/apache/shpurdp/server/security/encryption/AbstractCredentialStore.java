...
import org.apache.shpurdp.server.security.credential.CredentialFactory;
...
public abstract class AbstractCredentialStore implements CredentialStore {
   ...
   */
  @Override
  public void addCredential(String alias, Credential credential) throws ShpurdpException {
   ...
   * @throws ShpurdpException if an error occurs while retrieving the new credential
   */
  @Override
  public Credential getCredential(String alias) throws ShpurdpException {
    if (alias == null) {
      return null;
    } else {
      lock.lock();
   ...
   *
   * @param alias a string declaring the alias (or name) of the credential
   * @throws ShpurdpException if an error occurs while removing the new credential
   */
  @Override
  public void removeCredential(String alias) throws ShpurdpException {
    if ((alias != null) && !alias.isEmpty()) {
      lock.lock();
      try {
        KeyStore ks = loadCredentialStore();
        if (ks != null) {
          try {
            ...
            throw new ShpurdpException("Failed to delete the KeyStore entry - the key store may not have been initialized", e);
          }
        }
      } finally {
        lock.unlock();
   ...
   * @throws ShpurdpException if an error occurs while searching forthe credential
   */
  @Override
  public Set<String> listCredentials() throws ShpurdpException {
    ...
    try {
      ...
      if (ks != null) {
        try {
          ...
          throw new ShpurdpException("Failed to read KeyStore - the key store may not have been initialized", e);
        }
      }
    } finally {
      lock.unlock();
   ...
   */
  @Override
  public boolean containsCredential(String alias) throws ShpurdpException {
      ...
      try {
        ...
        if (ks != null) {
          try {
            ...
            throw new ShpurdpException("Failed to search the KeyStore for the requested entry - the key store may not have been initialized", e);
          }
        }
      } finally {
        lock.unlock();
   ...
   * @throws ShpurdpException if an error occurs while storing the new credential
   */
  protected void addCredential(KeyStore keyStore, String alias, Credential credential) throws ShpurdpException {
    if (keyStore != null) {
      try {
        Key key;
        ...
        throw new ShpurdpException("The key store has not been initialized", e);
      }
    }
  }
   ...
   * @throws ShpurdpException if an error occurs while retrieving the new credential
   */
  protected Credential getCredential(KeyStore keyStore, String alias) throws ShpurdpException {
    char[] value = null;
    ...
    if (keyStore != null) {
      try {
        ...
        throw new ShpurdpException("The key cannot be recovered (e.g., the given password is wrong)", e);
      } catch (KeyStoreException e) {
        throw new ShpurdpException("The key store has not been initialized", e);
      } catch (NoSuchAlgorithmException e) {
        throw new ShpurdpException(" if the algorithm for recovering the key cannot be found", e);
      }
    }
   ...
   * @throws ShpurdpException if an error occurs while persisting the key store data
   */
  protected abstract void persistCredentialStore(KeyStore keyStore) throws ShpurdpException;

  /**
   * Calls the implementation-specific facility to load the KeyStore
   *
   * @throws ShpurdpException if an error occurs while loading the key store data
   */
  protected abstract KeyStore loadCredentialStore() throws ShpurdpException;
   ...
   * @throws ShpurdpException if an error occurs while loading the key store data from the InputStream
   */
  protected KeyStore loadKeyStore(InputStream inputStream, String keyStoreType) throws ShpurdpException {
    if (masterKeyService == null) {
      throw new ShpurdpException("Master Key Service is not set for this Credential store.");
    }
    ...
    try {
      ...
      throw new ShpurdpException(String.format("No provider supports a key store implementation for the specified type: %s", keyStoreType), e);
    }

    try {
      keyStore.load(inputStream, masterKeyService.getMasterSecret());
    } catch (CertificateException e) {
      throw new ShpurdpException(String.format("One or more credentials from the key store could not be loaded: %s", e.getLocalizedMessage()), e);
    } catch (NoSuchAlgorithmException e) {
      throw new ShpurdpException(String.format("The algorithm used to check the integrity of the key store cannot be found: %s", e.getLocalizedMessage()), e);
    } catch (IOException e) {
      if (e.getCause() instanceof UnrecoverableKeyException) {
        throw new ShpurdpException(String.format("The password used to decrypt the key store is incorrect: %s", e.getLocalizedMessage()), e);
      } else {
        throw new ShpurdpException(String.format("Failed to read the key store: %s", e.getLocalizedMessage()), e);
      }
    }
   ...
   * @throws ShpurdpException if an error occurs while writing the key store data
   */
  protected void writeKeyStore(KeyStore keyStore, OutputStream outputStream) throws ShpurdpException {
    if (masterKeyService == null) {
      throw new ShpurdpException("Master Key Service is not set for this Credential store.");
    }
    ...
    try {
      ...
      throw new ShpurdpException(String.format("A credential within in the key store data could not be stored: %s", e.getLocalizedMessage()), e);
    } catch (NoSuchAlgorithmException e) {
      throw new ShpurdpException(String.format("The appropriate data integrity algorithm could not be found: %s", e.getLocalizedMessage()), e);
    } catch (KeyStoreException e) {
      throw new ShpurdpException(String.format("The key store has not been initialized: %s", e.getLocalizedMessage()), e);
    } catch (IOException e) {
      throw new ShpurdpException(String.format("Failed to write the key store: %s", e.getLocalizedMessage()), e);
    }
  }
