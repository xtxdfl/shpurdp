...
final class HostOfferResponse {
  ...
  void executeTasks(Executor executor, final String hostName, final ClusterTopology topology, final ShpurdpContext shpurdpContext) {
        ...
        public void run() {
          for (TopologyHostTask task : tasks) {
            try {
              ...
              hostRequest.markHostRequestFailed(HostRoleStatus.ABORTED, e, shpurdpContext.getPersistedTopologyState());
              break;
            }
          }
        }
