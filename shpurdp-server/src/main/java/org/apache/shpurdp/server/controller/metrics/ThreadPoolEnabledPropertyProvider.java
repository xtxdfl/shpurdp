...
import org.apache.shpurdp.server.controller.utilities.BufferedThreadPoolExecutorCompletionService;
...
public abstract class ThreadPoolEnabledPropertyProvider extends AbstractPropertyProvider {
  ...
  private static ThreadPoolExecutor initExecutorService() {
    ...
    ThreadFactory threadFactory = new ThreadFactoryBuilder().setDaemon(true).setNameFormat(
        "shpurdp-property-provider-thread-%d").build();
   ...
   * @param throwable  the caught exception
   *
   * @throws org.apache.shpurdp.server.controller.spi.SystemException always around the given exception
   */
