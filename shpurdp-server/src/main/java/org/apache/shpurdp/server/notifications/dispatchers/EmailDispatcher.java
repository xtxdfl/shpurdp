...
public class EmailDispatcher implements NotificationDispatcher {
  ...
  protected Transport getMailTransport(Map<String, Object> properties) throws NoSuchProviderException {
    ...
    if (properties.containsKey(AlertNoticeDispatchService.SHPURDP_DISPATCH_CREDENTIAL_USERNAME)) {
      credentials = new DispatchCredentials();
      credentials.UserName = (String) properties.get(AlertNoticeDispatchService.SHPURDP_DISPATCH_CREDENTIAL_USERNAME);
      credentials.Password = (String) properties.get(AlertNoticeDispatchService.SHPURDP_DISPATCH_CREDENTIAL_PASSWORD);
    }
