...
public class AgentEncryptionKey implements Credential {
    ...
    try {
      if (!credentialStoreService.containsCredential(null, AgentEncryptionKey.ALIAS)) {
        if (createIfNotFound) {
          ...
          throw new ShpurdpRuntimeException("AgentEncryptionKey with alias: " + ALIAS + " doesn't exist in credential store.");
        }
      } else {
        return loadKey(credentialStoreService);
      }
    } catch (ShpurdpException e) {
      throw new ShpurdpRuntimeException("Cannot load agent encryption key: " + e.getMessage(), e);
    }
  }

  private static AgentEncryptionKey loadKey(CredentialStoreService credentialStoreService) throws ShpurdpException {
  ...
  public void saveToCredentialStore(CredentialStoreService credentialStoreService) {
    try {
      ...
      throw new ShpurdpRuntimeException("Cannot save agent encryption key: " + e.getMessage(), e);
    }
  }
}
