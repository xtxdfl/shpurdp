...
/**
 * ShpurdpAuthToLocalUserDetailsService is a {@link UserDetailsService} that translates
 * a Kerberos principal name into a local username that may be used when looking up
 * and Shpurdp user account.
 */
@Component
public class ShpurdpAuthToLocalUserDetailsService implements UserDetailsService {
  private static final Logger LOG = LoggerFactory.getLogger(ShpurdpAuthToLocalUserDetailsService.class);
   ...
   */
  ShpurdpAuthToLocalUserDetailsService(Configuration configuration, Users users) {
    ShpurdpKerberosAuthenticationProperties properties = configuration.getKerberosAuthenticationProperties();
  ...
  public UserDetails loadUserByUsername(String principal) throws UsernameNotFoundException {
    ...
    if (CollectionUtils.isEmpty(entities)) {
      ...
      throw new ShpurdpAuthenticationException("", "Unexpected error due to collisions on the principal name", false);
    }
  }

  /**
   * Given a username, finds an appropriate account in the Shpurdp database.
   *
   ...
   * in the <code>shpurdp.properties</code> file, translate the supplied principal name to a local name.
   *
