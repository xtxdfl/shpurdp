...
public class ComponentResourceProvider extends AbstractControllerResourceProvider {
   ...
   */
  @AssistedInject
  ComponentResourceProvider(@Assisted ShpurdpManagementController managementController,
  ...
  public Set<Resource> getResources(Request request, Predicate predicate)
    ...
    Set<ServiceComponentResponse> responses = getResources(new Command<Set<ServiceComponentResponse>>() {
      @Override
      public Set<ServiceComponentResponse> invoke() throws ShpurdpException {
  ...
  public void createComponents(Set<ServiceComponentRequest> requests)
    ...
    ShpurdpMetaInfo shpurdpMetaInfo = getManagementController().getShpurdpMetaInfo();
    ...
    for (ServiceComponentRequest request : requests) {
      ...
      setServiceNameIfAbsent(request, cluster, shpurdpMetaInfo);
      ...
      if (!shpurdpMetaInfo.isValidServiceComponent(stackId.getStackName(),
    ...
    if (!duplicates.isEmpty()) {
      //Java8 has StringJoiner library but shpurdp is not on Java8 yet.
    ...
    for (ServiceComponentRequest request : requests) {
      ...
      if (StringUtils.isNotEmpty(request.getRecoveryEnabled())) {
        ...
        if (componentInfo == null) {
            throw new ShpurdpException("Could not get component information from stack definition: Stack=" +
  ...
  protected Set<ServiceComponentResponse> getComponents(Set<ServiceComponentRequest> requests) throws ShpurdpException {
  ...
  private Set<ServiceComponentResponse> getComponents(ServiceComponentRequest request) throws ShpurdpException {

    final ShpurdpMetaInfo shpurdpMetaInfo = getManagementController().getShpurdpMetaInfo();
  ...
  protected RequestStatusResponse updateComponents(Set<ServiceComponentRequest> requests,
    ...
    for (ServiceComponentRequest request : requests) {
      ...
      if (sc.isClientComponent() && !newState.isValidClientComponentState()) {
        throw new ShpurdpException("Invalid desired state for a client component");
      }
  ...
  protected RequestStatusResponse deleteComponents(Set<ServiceComponentRequest> requests) throws ShpurdpException, AuthorizationException {
    Clusters clusters = getManagementController().getClusters();
    ShpurdpMetaInfo shpurdpMetaInfo = getManagementController().getShpurdpMetaInfo();
  ...
  private void deleteHostComponentsForServiceComponent(ServiceComponent sc, ServiceComponentRequest request,
                                                       DeleteHostComponentStatusMetaData deleteMetaData) throws ShpurdpException {
    for (ServiceComponentHost sch : sc.getServiceComponentHosts().values()) {
      if (!sch.getDesiredState().isRemovableState()) {
        deleteMetaData.setShpurdpException(new ShpurdpException("Found non removable host component when trying to delete service component." +
  ...
  private Cluster getClusterForRequest(final ServiceComponentRequest request, final Clusters clusters) throws ShpurdpException {
  ...
  private Service getServiceFromCluster(final ServiceComponentRequest request, final Cluster cluster) throws ShpurdpException {
    try {
  ...
  private Cluster getCluster(final ServiceComponentRequest request, final Clusters clusters) throws ShpurdpException {
    Validate.notEmpty(request.getClusterName(), "cluster name should be non-empty");
  ...
  private void setServiceNameIfAbsent(final ServiceComponentRequest request,
                                      ...
                                      final ShpurdpMetaInfo shpurdpMetaInfo) throws ShpurdpException {
    if (StringUtils.isEmpty(request.getServiceName())) {
      ...
      if (StringUtils.isEmpty(serviceName)) {
        throw new ShpurdpException("Could not find service for component"
