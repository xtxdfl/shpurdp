...
import org.apache.shpurdp.server.controller.ShpurdpManagementHelper;
...
import org.apache.shpurdp.server.orm.entities.ExtensionEntity;
...
public class StackManager {
   ...
   * @param helper
   *          Shpurdp management helper automatically injected
   *
  ...
  public StackManager(@Assisted("stackRoot") File stackRoot,
      ...
      ExtensionDAO extensionDao, ExtensionLinkDAO linkDao, ShpurdpManagementHelper helper)
      throws ShpurdpException {
  ...
  protected void parseDirectories(File stackRoot, File commonServicesRoot, File extensionRoot) throws ShpurdpException {
  ...
  private void populateDB(StackDAO stackDao, ExtensionDAO extensionDao) throws ShpurdpException {
   ...
   * @throws ShpurdpException if unable to resolve all stacks
   */
   ...
   * @throws ShpurdpException if unable to resolve all common services
   */
   ...
   * @param stackModules          map of stack id which contains name and version to stack module.
   * @param commonServiceModules  map of common service id which contains name and version to common service module.
   * @throws ShpurdpException if unable to resolve all extensions
   */
   ...
   * @throws ShpurdpException if the specified common services root directory is invalid
   */
  private void validateCommonServicesDirectory(File commonServicesRoot) throws ShpurdpException {
    if(commonServicesRoot != null) {
      ...
      if (!commonServicesRoot.isDirectory() && !commonServicesRoot.exists()) {
        throw new ShpurdpException("" + Configuration.COMMON_SERVICES_DIR_PATH
   ...
   * @throws ShpurdpException if the specified stack root directory is invalid
   */
  private void validateStackDirectory(File stackRoot) throws ShpurdpException {
    ...
    if (!stackRoot.isDirectory() && !stackRoot.exists()) {
      throw new ShpurdpException("" + Configuration.METADATA_DIR_PATH
  ...
  public static Validator getPropertySchemaValidator() throws ShpurdpException {
    ...
    try {
      ...
      throw new ShpurdpException(String.format("Failed to parse property schema file %s", PROPERTY_SCHEMA_PATH), e);
    }
    return schema.newValidator();
  }

  public static void validateAllPropertyXmlsInFolderRecursively(File stackRoot, Validator validator) throws ShpurdpException {
   ...
   * @throws ShpurdpException if the specified extension root directory is invalid
   */
  private void validateExtensionDirectory(File extensionRoot) throws ShpurdpException {
   ...
   * @throws ShpurdpException if unable to parse all common services
   */
  private Map<String, ServiceModule> parseCommonServicesDirectory(File commonServicesRoot) throws ShpurdpException {
   ...
   * @throws ShpurdpException if unable to parse all stacks
   */
  private Map<String, StackModule> parseStackDirectory(File stackRoot) throws ShpurdpException {
    ...
    if (stackMap.isEmpty()) {
      throw new ShpurdpException("Unable to find stack definitions under " +
          "stackRoot = " + stackRoot.getAbsolutePath());
    }
    return stackModules;
  }

  public void linkStackToExtension(StackInfo stack, ExtensionInfo extension) throws ShpurdpException {
    stack.addExtension(extension);
  }

  public void unlinkStackAndExtension(StackInfo stack, ExtensionInfo extension) throws ShpurdpException {
   ...
   * @throws ShpurdpException if unable to parse all extensions
   */
  private Map<String, ExtensionModule> parseExtensionDirectory(File extensionRoot) throws ShpurdpException {
    ...
    if (stackMap.isEmpty()) {
      throw new ShpurdpException("Unable to find extension definitions under " +
