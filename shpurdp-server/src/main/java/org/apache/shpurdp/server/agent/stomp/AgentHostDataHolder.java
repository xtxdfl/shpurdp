...
import org.apache.shpurdp.server.events.STOMPHostEvent;
...
public abstract class AgentHostDataHolder<T extends STOMPHostEvent & Hashable> extends AgentDataHolder<T> {
  ...
  protected abstract T getCurrentData(Long hostId) throws ShpurdpException;
  protected abstract T handleUpdate(T current, T update) throws ShpurdpException;

  public T getUpdateIfChanged(String agentHash, Long hostId) throws ShpurdpException {
    T hostData = initializeDataIfNeeded(hostId, true);
    return !Objects.equals(agentHash, hostData.getHash()) ? hostData : getEmptyData();
  }

  public T initializeDataIfNeeded(Long hostId, boolean regenerateHash) throws ShpurdpRuntimeException {
  ...
  private T initializeData(Long hostId, boolean regenerateHash) {
    ...
    try {
      ...
      throw new ShpurdpRuntimeException("Error during retrieving initial value for host: " + hostId + " and class: " + getClass().getName(), e);
    }
    if (regenerateHash) {
   ...
   * event to listeners.
   */
  public void updateData(T update) throws ShpurdpException {
    try {
      data.compute(update.getHostId(), (id, current) -> {
        ...
        try {
          ...
          throw new ShpurdpRuntimeException("Error during handling update for host: " + id + " and class: " + getClass().getName(), e);
        }
        if (updated == null) {
          return current;
        } else {
   ...
   */
  public final void resetData(Long hostId) throws ShpurdpException {
