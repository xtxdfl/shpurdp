...
public class ShortTaskStatus {
  ...
  /**
   * If the hostname is null (or empty), returns the hostname of the Shpurdp Server; else returns the
   * supplied hostname value.
   *
   * @param hostName a hostname
   * @return the hostname of the Shpurdp Server if the hostname is null (or empty); else supplied hostname value
   */
  private String translateHostName(String hostName) {
    // if the hostname in the command is null, replace it with the hostname of the Shpurdp Server
    // This is because commands (to be) handled by the Shpurdp Server have a null value for its
    // host designation.
