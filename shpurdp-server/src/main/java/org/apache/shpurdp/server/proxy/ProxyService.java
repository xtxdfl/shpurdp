...
package org.apache.shpurdp.server.proxy;
...
import org.apache.shpurdp.server.view.ImpersonatorSettingImpl;
...
public class ProxyService {
  ...
  private static final String SHPURDP_PROXY_PREFIX = "ShpurdpProxy-";
  ...
  private Map<String, List<String>> getHeaderParamsToForward(HttpHeaders headers) {
    ...
    for (String paramName: headers.getRequestHeaders().keySet()) {
      if (paramName.startsWith(SHPURDP_PROXY_PREFIX)) {
        headerParamsToForward.put(paramName.replaceAll(SHPURDP_PROXY_PREFIX,""), headers.getRequestHeader(paramName));
      }
    }
