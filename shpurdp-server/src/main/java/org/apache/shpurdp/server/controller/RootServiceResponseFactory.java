...
public class RootServiceResponseFactory extends
  ...
  private String getComponentVersion(String componentName, HostResponse host) {
    ...
    switch (component) {
      case SHPURDP_SERVER:
        componentVersion = shpurdpMetaInfo.getServerVersion();
        break;
      case SHPURDP_AGENT:
  ...
  private Map<String, String> getComponentProperties(String componentName){
    ...
    if (componentName != null) {
      ...
      switch (component) {
      case SHPURDP_SERVER:
        response = configs.getShpurdpProperties();
  ...
  @Override
  public Set<RootServiceHostComponentResponse> getRootServiceHostComponent(RootServiceHostComponentRequest request, Set<HostResponse> hosts) throws ShpurdpException {
    ...
    for (RootServiceComponentResponse component : rootServiceComponents) {
      ...
      if (component.getComponentName().equals(RootComponent.SHPURDP_SERVER.name())) {
