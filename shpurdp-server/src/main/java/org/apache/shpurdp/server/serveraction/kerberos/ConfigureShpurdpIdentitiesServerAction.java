...
/**
 * ConfigureShpurdpIdentitiesServerAction is a ServerAction implementation that creates keytab files as
 * instructed.
 * <p/>
 ...
 */
public class ConfigureShpurdpIdentitiesServerAction extends KerberosServerAction {


  private static final String KEYTAB_PATTERN = "keyTab=\"(.+)?\"";
  private static final String PRINCIPAL_PATTERN = "principal=\"(.+)?\"";

  private final static Logger LOG = LoggerFactory.getLogger(ConfigureShpurdpIdentitiesServerAction.class);
   ...
   * {@link ConfigureShpurdpIdentitiesServerAction#processIdentities(Map)}
  ...
  /**
   * Creates keytab file for shpurdp-server identity.
   * <p/>
   ...
   * @throws ShpurdpException if an error occurs while processing the identity record
   */
  @Override
  protected CommandReport processIdentity(ResolvedKerberosPrincipal resolvedPrincipal,
    ...
    if (includedInFilter && resolvedPrincipal != null && StageUtils.getHostName().equals(resolvedPrincipal.getHostName())) {
      ...
      for (Map.Entry<String, String> serviceMappingEntry : resolvedPrincipal.getServiceMapping().entries()) {
        ...
        if (RootService.SHPURDP.name().equals(serviceMappingEntry.getKey())) {
          ...
          try {
            if (srcKeytabFile.exists()) {
              ...
              installShpurdpServerIdentity(resolvedPrincipal, srcKeytabFile.getAbsolutePath(), destKeytabFilePath,
                keytab.getOwnerName(), ownerAccess,
                keytab.getGroupName(), groupAccess, actionLog);

              if (serviceMappingEntry.getValue().contains("SHPURDP_SERVER_SELF")) {
  ...
  /**
   * Installs the Shpurdp Server Kerberos identity by copying its keytab file to the specified location
   * and then creating the Shpurdp Server JAAS File.
   *
   * @param principal          the shpurdp server principal name
   * @param srcKeytabFilePath  the source location of the shpurdp server keytab file
   * @param destKeytabFilePath the destination location of the shpurdp server keytab file
   ...
   * @throws ShpurdpException
   */
  public boolean installShpurdpServerIdentity(ResolvedKerberosPrincipal principal,
                                             String srcKeytabFilePath,
                                             String destKeytabFilePath,
                                             String ownerName, String ownerAccess,
                                             String groupName, String groupAccess,
                                             ActionLog actionLog) throws ShpurdpException {
    ...
    try {
      ...
      Long shpurdpServerHostID = shpurdpServerHostID();
      HostEntity hostEntity = null;
      if (shpurdpServerHostID != null) {
        hostEntity = hostDAO.findById(shpurdpServerHostID);
      }
      ...
      if (actionLog != null) {
        actionLog.writeStdOut(String.format("Created Shpurdp server keytab file for %s at %s", principal, destKeytabFilePath));
      }
    } catch (InterruptedException | IOException e) {
      throw new ShpurdpException(e.getLocalizedMessage(), e);
    }
  ...
  /**
   * Configure Shpurdp's JAAS file to reflect the principal name and keytab file for Shpurdp's Kerberos
   * identity.
   *
   * @param principal      the Shpurdp server's principal name
   * @param keytabFilePath the absolute path to the Shpurdp server's keytab file
   * @param actionLog      the logger
   */
  public void configureJAAS(String principal, String keytabFilePath, ActionLog actionLog) {
    ...
    if (jaasConfPath != null) {
      ...
      String message = String.format("Failed to configure JAAS, config file should be passed to Shpurdp server as: " +
  ...
  void copyFile(String srcKeytabFilePath, String destKeytabFilePath)
    ...
    if (!result.isSuccessful()) {
      throw new ShpurdpException(result.getStderr());
    }
   ...
   * @throws ShpurdpException if an error occurs setting the permissions on the fils
   */
  ...
  /**
   * Gets the location of Shpurdp's JAAS config file.
   * <p>
   ...
   *
   * @return the path to Shpurdp's JAAS config file
   */
