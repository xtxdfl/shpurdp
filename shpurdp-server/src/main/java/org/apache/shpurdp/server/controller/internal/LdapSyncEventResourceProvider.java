...
import org.apache.shpurdp.server.controller.LdapSyncRequest;
...
import org.apache.shpurdp.server.orm.entities.LdapSyncEventEntity;
...
public class LdapSyncEventResourceProvider extends AbstractControllerResourceProvider {
   ...
   */
  public LdapSyncEventResourceProvider(ShpurdpManagementController managementController) {
    ...
    EnumSet<RoleAuthorization> roleAuthorizations =
        EnumSet.of(RoleAuthorization.SHPURDP_MANAGE_GROUPS, RoleAuthorization.SHPURDP_MANAGE_USERS);
  ...
  private Command<LdapSyncEventEntity> getCreateCommand(final Map<String, Object> properties) {
    return new Command<LdapSyncEventEntity>() {
      @Override
      public LdapSyncEventEntity invoke() throws ShpurdpException, AuthorizationException {
        ...
        for (LdapSyncSpecEntity ldapSyncSpecEntity : eventEntity.getSpecs()) {
          if (ldapSyncSpecEntity.getPrincipalType() == LdapSyncSpecEntity.PrincipalType.USERS) {
            ...
            if (!AuthorizationHelper.isAuthorized(ResourceType.SHPURDP, null, RoleAuthorization.SHPURDP_MANAGE_GROUPS)) {
   ...
   *
   * @throws ShpurdpException if the sync could not be completed
   */
  private LdapBatchDto syncLdap(LdapSyncEventEntity event) throws ShpurdpException {
