...
public class MissingOsInRepoVersionCheck extends ClusterCheck {
  ...
  private Cluster cluster(UpgradeCheckRequest prerequisiteCheck) throws ShpurdpException {
   ...
   */
  private Set<String> sourceOsFamilies(UpgradeCheckRequest request) throws ShpurdpException {
    StackId stackId = new StackId(request.getTargetRepositoryVersion().getStackId());
    return shpurdpMetaInfo.get().getStack(stackId).getRepositoriesByOs().keySet();
  }
