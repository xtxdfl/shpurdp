...
/**
 * Provider of an input stream for a request to the Remote Shpurdp server.
 */
public class RemoteShpurdpStreamProvider implements ShpurdpStreamProvider {
  ...
  public RemoteShpurdpStreamProvider(String baseUrl, String username, String password, int connectTimeout, int readTimeout) {
  ...
  @Override
  public InputStream readFrom(String path, String requestMethod, String body, Map<String, String> headers) throws IOException, ShpurdpHttpException {
    return getInputStream(urlStreamProvider.processURL(getUrl(path), requestMethod, body, addHeaders(headers)));
  }

  @Override
  public InputStream readFrom(String path, String requestMethod, InputStream body, Map<String, String> headers) throws IOException, ShpurdpHttpException {
    return getInputStream(urlStreamProvider.processURL(getUrl(path), requestMethod, body, addHeaders(headers)));
  }
  ...
  private InputStream getInputStream(HttpURLConnection connection) throws IOException, ShpurdpHttpException {
    ...
    if(responseCode >= ProxyService.HTTP_ERROR_RANGE_START){
      throw new ShpurdpHttpException(IOUtils.toString(connection.getErrorStream()),responseCode);
    }
  ...
  private void addRequestedByHeaders(HashMap<String, String> newHeaders) {
    newHeaders.put("X-Requested-By", "SHPURDP");
  }
