...
import org.apache.shpurdp.server.orm.ShpurdpJpaLocalTxnInterceptor;
import org.apache.shpurdp.server.orm.ShpurdpLocalSessionInterceptor;
 ...
 */
public class ShpurdpJpaPersistModule extends PersistModule {
  private final String jpaUnit;

  public ShpurdpJpaPersistModule(String jpaUnit) {
  ...
  @Override protected void configurePersistence() {
    ...
    bind(ShpurdpJpaPersistService.class).in(Singleton.class);

    bind(PersistService.class).to(ShpurdpJpaPersistService.class);
    bind(UnitOfWork.class).to(ShpurdpJpaPersistService.class);
    bind(EntityManager.class).toProvider(ShpurdpJpaPersistService.class);
    bind(EntityManagerFactory.class).toProvider(JpaPersistService.EntityManagerFactoryProvider.class);

    transactionInterceptor = new ShpurdpJpaLocalTxnInterceptor();
    requestInjection(transactionInterceptor);

    MethodInterceptor sessionInterceptor = new ShpurdpLocalSessionInterceptor();
   ...
   */
  public ShpurdpJpaPersistModule properties(Map<?, ?> properties) {
   ...
   */
  public <T> ShpurdpJpaPersistModule addFinder(Class<T> iface) {
