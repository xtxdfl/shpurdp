...
class KerberosPropertyTemplate:
  def __init__(
    ...
    self.kerberos_prop_name = get_value_from_properties(
      properties, i_prop_name, i_prop_name_default
    )
    self.kerberos_prop_val_prompt = i_prop_val_pattern.format(
      get_prompt_default(self.kerberos_prop_name)
    )
...
def init_kerberos_properties_list(properties, options):
  kerberos_properties = [
    KerberosPropertyTemplate(
      properties,
      options.kerberos_enabled,
      "authentication.kerberos.enabled",
      "Enable Kerberos authentication [true|false] {0}: ",
      REGEX_TRUE_FALSE,
      False,
      "false",
    ),
    KerberosPropertyTemplate(
      properties,
      options.kerberos_spnego_principal,
    ...
    KerberosPropertyTemplate(
      properties,
      options.kerberos_spnego_keytab_file,
      "authentication.kerberos.spnego.keytab.file",
      "SPNEGO keytab file {0}: ",
      REGEX_ANYTHING,
      False,
    ...
    KerberosPropertyTemplate(
      properties,
      options.kerberos_auth_to_local_rules,
      "authentication.kerberos.auth_to_local.rules",
      "Auth-to-local rules {0}: ",
      REGEX_ANYTHING,
      False,
      "DEFAULT",
    ),
...
def setup_kerberos(options):
  ...
  if not is_root():
    err = "shpurdp-server setup-kerberos should be run with root-level privileges"
  ...
  print("Properties to be updated / written into shpurdp properties:")
