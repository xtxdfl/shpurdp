...
SHPURDP_SSO_AUTH_ENABLED = "shpurdp.sso.authentication.enabled"
SSO_MANAGE_SERVICES = "shpurdp.sso.manage_services"
SSO_ENABLED_SERVICES = "shpurdp.sso.enabled_services"
SSO_PROVIDER_URL = "shpurdp.sso.provider.url"
SSO_CERTIFICATE = "shpurdp.sso.provider.certificate"
SSO_PROVIDER_ORIGINAL_URL_QUERY_PARAM = "shpurdp.sso.provider.originalUrlParamName"
JWT_AUDIENCES = "shpurdp.sso.jwt.audiences"
JWT_COOKIE_NAME = "shpurdp.sso.jwt.cookieName"
...
SSO_CONFIG_API_ENTRYPOINT = (
  "services/SHPURDP/components/SHPURDP_SERVER/configurations/sso-configuration"
)
...
def validate_options(options):
  ...
  if options.sso_enabled == "true":
    ...
    if options.sso_enabled_shpurdp and not re.match(
      REGEX_TRUE_FALSE, options.sso_enabled_shpurdp
    ):
      errors.append("--sso-enabled-shpurdp should be to either 'true' or 'false'")
...
def populate_shpurdp_requires_sso(options, properties):
  if options.sso_enabled_shpurdp is None:
    enabled = get_boolean_from_dictionary(properties, SHPURDP_SSO_AUTH_ENABLED, False)
    enabled = get_YN_input(
      f"Use SSO for Shpurdp [y/n] ({'y' if enabled else 'n'})? ", enabled
    )
  else:
    enabled = "true" == options.sso_enabled_shpurdp

  properties[SHPURDP_SSO_AUTH_ENABLED] = "true" if enabled else "false"
...
def remove_sso_conf(shpurdp_properties, admin_login, admin_password):
  perform_changes_via_rest_api(
    shpurdp_properties, admin_login, admin_password, SSO_CONFIG_API_ENTRYPOINT, "DELETE"
  )


def update_sso_conf(
  shpurdp_properties, sso_configuration_properties, admin_login, admin_password
):
...
def setup_sso(options):
  ...
  if not get_silent():
    ...
    properties = get_sso_properties(shpurdp_properties, admin_login, admin_password)

    if not options.sso_enabled:
      shpurdp_auth_enabled = get_value_from_dictionary(
        properties, SHPURDP_SSO_AUTH_ENABLED
      )
      manage_services = get_value_from_dictionary(properties, SSO_MANAGE_SERVICES)

      if shpurdp_auth_enabled or manage_services:
        if (shpurdp_auth_enabled and "true" == shpurdp_auth_enabled) or (
    ...
    if enable_sso:
      ...
      populate_shpurdp_requires_sso(options, properties)
      ...
      update_sso_conf(shpurdp_properties, properties, admin_login, admin_password)
    else:
      remove_sso_conf(shpurdp_properties, admin_login, admin_password)
