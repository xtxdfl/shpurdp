...
HOST_UPDATE_HELPER_CMD = (
  "{0} -cp {1} "
  + "org.apache.shpurdp.server.update.HostUpdateHelper {2}"
  + " > "
...
def update_host_names(args, options):
  ...
  services_stopped = userInput.get_YN_input(
    "Please, confirm Shpurdp services are stopped [y/n] (n)? ", False
  )
  ...
  pending_commands = userInput.get_YN_input(
    "Please, confirm there are no pending commands on cluster [y/n] (n)? ", False
  )
  ...
  db_backup_done = userInput.get_YN_input(
    "Please, confirm you have made backup of the Shpurdp db [y/n] (n)? ", False
  )
  ...
  if status:
    raise FatalException(1, "Shpurdp Server should be stopped")
  ...
  print_info_msg(
    "Return code from update host names command, retcode = " + str(retcode)
  )
