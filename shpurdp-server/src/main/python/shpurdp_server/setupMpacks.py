...
from shpurdp_commons.inet_utils import download_file_anyway
...
MPACK_INSTALL_CHECKER_CMD = (
  "{0} -cp {1} "
  + "org.apache.shpurdp.server.checks.MpackInstallChecker --mpack-stacks {2}"
)
...
def get_mpack_properties():
  """
  Read shpurdp properties required for management packs
  :return: (stack_location, service_definitions_location, mpacks_staging_location)
  """
  # Get shpurdp config properties
  properties = get_shpurdp_properties()
  if properties == -1:
    print_error_msg("Error getting shpurdp properties")
    return -1
...
def remove_symlinks(
  ...
  :param extension_location: Path to extensions folder
                             (/var/lib/shpurdp-server/resources/extension)
  ...
  :param dashboard_location: Path to the custom service dashboard folder
                             (/var/lib/shpurdp-server/resources/dashboards)
  :param staged_mpack_dir: Path to management pack staging location
                           (/var/lib/shpurdp-server/resources/mpacks/mpack_name-mpack_version)
  """
  for location in [
    stack_location,
    extension_location,
...
def run_mpack_install_checker(options, mpack_stacks):
  ...
  if not jdk_path or not database_type:
    # Shpurdp Server has not been setup, so no cluster would be present
    return (0, "", "")
...
def validate_purge(options, purge_list, mpack_dir, mpack_metadata, replay_mode=False):
  ...
  """
  # Get shpurdp mpacks config properties
  if not purge_list:
    return
  ...
  if STACK_DEFINITIONS_RESOURCE_NAME in purge_list:
    ...
    if not mpack_stacks:
      ...
      err = (
        ...
        "does not contain a stack, the --purge option with --purge-list={1} would cause your existing Shpurdp "
...
def process_stack_definitions_artifact(artifact, artifact_source_dir, options):
  ...
  for file in sorted(os.listdir(artifact_source_dir)):
    if os.path.isfile(os.path.join(artifact_source_dir, file)):
      # Example: /var/lib/shpurdp-server/resources/stacks/stack_advisor.py
...
def create_dashboard_symlinks(
  ...
  :param dashboard_location: Location of the dashboards directory in shpurdp-server/resources
  :param options: Command line options
  """
...
def process_extension_definitions_artifact(artifact, artifact_source_dir, options):
  ...
  for file in sorted(os.listdir(artifact_source_dir)):
    if os.path.isfile(os.path.join(artifact_source_dir, file)):
      # Example: /var/lib/shpurdp-server/resources/extensions/README.txt
...
def validate_mpack_prerequisites(mpack_metadata):
  ...
  if properties == -1:
    ...
    return -1
  stack_location = get_stack_location(properties)
  current_shpurdp_version = get_shpurdp_version(properties)
  fail = False

  mpack_prerequisites = mpack_metadata.prerequisites
  if "min_shpurdp_version" in mpack_prerequisites:
    min_shpurdp_version = mpack_prerequisites.min_shpurdp_version
    if compare_versions(min_shpurdp_version, current_shpurdp_version, format=True) > 0:
      print_error_msg(
        "Prerequisite failure! Current Shpurdp Version = {0}, "
        "Min Shpurdp Version = {1}".format(current_shpurdp_version, min_shpurdp_version)
      )
      fail = True
  if "max_shpurdp_version" in mpack_prerequisites:
    max_shpurdp_version = mpack_prerequisites.max_shpurdp_version
    if compare_versions(max_shpurdp_version, current_shpurdp_version, format=True) < 0:
      print_error_msg(
        "Prerequisite failure! Current Shpurdp Version = {0}, "
        "Max Shpurdp Version = {1}".format(current_shpurdp_version, max_shpurdp_version)
      )
...
def _install_mpack(options, replay_mode=False, is_upgrade=False):
  ...
  # Stage management pack (Stage at /var/lib/shpurdp-server/resources/mpacks/mpack_name-mpack_version)
  ...
  if shpurdp_user:
    # This is required when a non-admin user is configured to setup shpurdp-server
  ...
  print_info_msg(
    f"Management pack {mpack_name}-{mpack_version} successfully installed! Please restart shpurdp-server."
  )
  return (
    mpack_metadata,
    mpack_name,
      ...
      if hook_name == hook.name:
        ...
        if os.path.exists(hook_script):
          ...
          if hook.type == PYTHON_HOOK_TYPE:
            command = ["/usr/bin/shpurdp-python-wrap", hook_script]
...
def add_replay_log(
  ...
  :param mpack_archive_path: mpack archive path (/var/lib/shpurdp-server/resources/mpacks/mpack.tar.gz)
...
def replay_mpack_logs():
  """
  Replay mpack logs during shpurdp-server upgrade
  """
  replay_log_file = get_replay_log_file()
  if os.path.exists(replay_log_file):
    with open(replay_log_file, "r") as f:
