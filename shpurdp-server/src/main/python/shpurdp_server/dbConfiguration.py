...
from shpurdp_commons.str_utils import cbool
from shpurdp_server.serverClassPath import JDBC_DRIVER_PATH_PROPERTY
...
SHPURDP_DATABASE_NAME = "shpurdp"
SHPURDP_DATABASE_TITLE = "shpurdp"
...
class DBMSDesc:
  def __init__(
    self, i_dbms_key, i_storage_key, i_dbms_name, i_storage_name, i_fn_create_config
  ):
...
class DbAuthenticationKeys:
  def __init__(
    self, i_user_name_key, i_password_key, i_password_alias, i_password_filename
  ):
...
class DBMSConfig(object):
  def __init__(self, options, properties, storage_type):
    ...
    self.db_title = SHPURDP_DATABASE_TITLE

    self.must_set_database_options = DBMSConfig._init_member_with_default(
      options, "must_set_database_options", False
    )

    self.JDBC_DRIVER_INSTALL_MSG = (
      "Before starting Shpurdp Server, you must install the JDBC driver."
    )
  ...
  def ensure_jdbc_driver_installed(self, properties):
    ...
    if default_driver_path and os.path.isfile(default_driver_path):
      shpurdp_should_use_existing_default_jdbc = get_YN_input(
        f"Should shpurdp use existing default jdbc {default_driver_path} [y/n] (y)? ",
        True,
      )
      if shpurdp_should_use_existing_default_jdbc:
    ...
    path_to_custom_jdbc_driver = get_validated_string_input(
      "Enter full path to custom jdbc driver: ", None, None, None, False, False
    )
    if path_to_custom_jdbc_driver and os.path.isfile(path_to_custom_jdbc_driver):
      try:
        ...
        if not path_to_custom_jdbc_driver == os.path.join(
          ...
          if os.path.isfile(
            os.path.join(configDefaults.JAVA_SHARE_PATH, custom_jdbc_name)
          ):
            replace_jdbc_in_share_dir = get_YN_input(
              f"You already have file {custom_jdbc_name} in /usr/share/java/. Should it be replaced? [y/n] (y)? ",
              True,
            )
          ...
          print(
            f"Copying {path_to_custom_jdbc_driver} to {configDefaults.JAVA_SHARE_PATH}"
          )
      except Exception as e:
        err = (
          "Can not copy file {0} to {1} due to: {2} . Please check file "
          "permissions and free disk space.".format(
            path_to_custom_jdbc_driver, configDefaults.JAVA_SHARE_PATH, str(e)
          )
        )
        raise FatalException(1, err)
  ...
  def _read_password_from_properties(properties, options):
    ...
    if password_file:
      if is_alias_string(password_file):
        database_password = decrypt_password_for_alias(
          properties, JDBC_RCA_PASSWORD_ALIAS, options
        )
      else:
  ...
  @staticmethod
  def _init_member_with_prop_default(
    options, attr_name, properties, property_key, default_val
  ):
    val = DBMSConfig._init_member_with_properties(
      options, attr_name, properties, property_key
    )
...
class DBMSConfigFactory(object):
  ...
  def create(self, options, properties, dbId="Shpurdp"):
    """
...
class DBMSConfigFactoryWindows(DBMSConfigFactory):
  def __init__(self):
    from shpurdp_server.dbConfiguration_windows import DATABASE_DBMS_MSSQL
  ...
  def create(self, options, properties, dbId="Shpurdp"):
    ...
    # dbId = additional information, that helps distinguish between various database connections, if applicable
    """
    from shpurdp_server.dbConfiguration_windows import createMSSQLConfig
...
class DBMSConfigFactoryLinux(DBMSConfigFactory):
  def __init__(self):
    ...
    self.DBMS_LIST = [
      ...
      DBMSDesc(
        self.DBMS_KEYS_LIST[1], STORAGE_TYPE_REMOTE, "Oracle", "", createOracleConfig
      ),
      DBMSDesc(
        self.DBMS_KEYS_LIST[2],
        STORAGE_TYPE_REMOTE,
        "MySQL / MariaDB",
        "",
        createMySQLConfig,
      ...
      DBMSDesc(
        self.DBMS_KEYS_LIST[3], STORAGE_TYPE_REMOTE, "PostgreSQL", "", createPGConfig
      ),
      DBMSDesc(
        self.DBMS_KEYS_LIST[4],
        STORAGE_TYPE_REMOTE,
  ...
  def select_dbms(self, options):
    try:
      ...
      db_name = (
        get_value_from_properties(get_shpurdp_properties(), JDBC_DATABASE_PROPERTY, "")
        .strip()
        .lower()
      )
      persistence_type = (
        get_value_from_properties(
          get_shpurdp_properties(), PERSISTENCE_TYPE_PROPERTY, ""
        )
        .strip()
        .lower()
      )
  ...
  def get_default_dbms_name(self):
    ...
    default_dbms_name = (
      get_value_from_properties(properties, DEFAULT_DBMS_PROPERTY, "").strip().lower()
    )
  ...
  def _get_default_dbms_index(self, options):
    ...
    try:
      ...
      raise FatalException(
        15,
        "Invalid database selection: {0} {1}".format(
...
def check_jdbc_drivers(args):
  ...
  # SHPURDP-5696 Validate the symlinks for each supported driver, in case various back-end HDP services happen to
...
def ensure_dbms_is_running(options, properties, scmStatus=None):
  ...
  if result == -1:
    raise FatalException(
      -1, "JDBC driver is not installed. Run shpurdp-server setup and try again."
    )
