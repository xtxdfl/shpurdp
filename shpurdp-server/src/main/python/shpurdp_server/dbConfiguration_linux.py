...
from shpurdp_commons.exceptions import NonFatalException, FatalException
...
from shpurdp_server.serverConfiguration import (
  ...
  JDBC_CONNECTION_POOL_MAX_AGE,
  JDBC_CONNECTION_POOL_MAX_IDLE_TIME,
  JDBC_CONNECTION_POOL_MAX_IDLE_TIME_EXCESS,
  JDBC_SQLA_SERVER_NAME,
  LOCAL_DATABASE_ADMIN_PROPERTY,
)
from shpurdp_commons.inet_utils import wait_for_port_opened
...
from shpurdp_server.utils import get_postgre_hba_dir, get_postgre_running_status
...
class LinuxDBMSConfig(DBMSConfig):
  def __init__(self, options, properties, storage_type):
    ...
    self.database_host = DBMSConfig._init_member_with_prop_default(
      options, "database_host", properties, JDBC_HOSTNAME_PROPERTY, "localhost"
    )
    ...
    self.local_admin_user = DBMSConfig._init_member_with_prop_default(
      options, "local_admin_user", properties, LOCAL_DATABASE_ADMIN_PROPERTY, "postgres"
    )
    self.database_password = getattr(options, "database_password", "")

    self.jdbc_connection_pool_type = DBMSConfig._init_member_with_prop_default(
      options,
      "jdbc_connection_pool_type",
      properties,
      JDBC_CONNECTION_POOL_TYPE,
    ...
    self.jdbc_connection_pool_acquisition_size = (
    ...
    self.jdbc_connection_pool_idle_test_interval = (
      DBMSConfig._init_member_with_prop_default(
        options,
        "jdbc_connection_pool_idle_test_interval",
        properties,
    ...
    self.jdbc_connection_pool_max_idle_time = DBMSConfig._init_member_with_prop_default(
      options,
      "jdbc_connection_pool_max_idle_time",
      properties,
      JDBC_CONNECTION_POOL_MAX_IDLE_TIME,
    ...
    self.jdbc_connection_pool_max_idle_time_excess = (
      DBMSConfig._init_member_with_prop_default(
        options,
        "jdbc_connection_pool_max_idle_time_excess",
        properties,
    ...
    self.jdbc_connection_pool_max_age = DBMSConfig._init_member_with_prop_default(
      options,
      "jdbc_connection_pool_max_age",
      properties,
      JDBC_CONNECTION_POOL_MAX_AGE,
  ...
  def _prompt_db_properties(self):
    if self.must_set_database_options:
      ...
      self.database_password = LinuxDBMSConfig._configure_database_password(
        True, self.database_password
      )
  ...
  def _reset_remote_database(self):
    ...
    print_warning_msg(
      "To reset Shpurdp Server schema "
      + "you must run the following DDL directly from the database shell to "
      + "drop the schema:"
      + os.linesep
      + client_usage_cmd_drop
      + os.linesep
      + "Then you must run the following DDL "
      + "directly from the database shell to create the schema: "
      + os.linesep
  ...
  def _configure_database_password(showDefault=True, defaultPassword=DEFAULT_PASSWORD):
    ...
    passwordDescr = (
      "Invalid characters in password. Use only alphanumeric or "
  ...
  def _get_validated_db_name(database_storage_name, database_name):
    return get_validated_string_input(
      database_storage_name + " name (" + database_name + "): ",
      database_name,
      ".*",
  ...
  def _display_db_properties(self):
    print_info_msg(
      "Using database options: {database},{host},{port},{schema},{user},{password}".format(
        database=self.dbms,
  ...
  def _setup_remote_db(self):
    setup_msg = (
      "Before starting Shpurdp Server, you must run the following DDL "
      "directly from the database shell to create the schema: {0}".format(
        self.init_script_file
      )
    )
    ...
    proceed = get_YN_input(
      "Proceed with configuring remote database connection properties [y/n] (y)? ", True
    )
  ...
  def _store_password_property(self, properties, property_name, options):
    properties.process_pair(
      property_name, store_password_file(self.database_password, JDBC_PASSWORD_FILENAME)
    )
    if self.isSecure:
      encrypted_password = encrypt_password(
        JDBC_RCA_PASSWORD_ALIAS, self.database_password, options
      )
  ...
  def _get_jdbc_connection_string(self):
    ...
    return connectionStringFormat.format(
      jdbc_hostname, self.database_port, self.database_name
    )
  ...
  def _store_connection_pool_properties(self, properties):
    ...
    if self.jdbc_connection_pool_type == "c3p0":
      ...
      properties.process_pair(
        JDBC_CONNECTION_POOL_MAX_IDLE_TIME, self.jdbc_connection_pool_max_idle_time
      )
      properties.process_pair(
        JDBC_CONNECTION_POOL_MAX_IDLE_TIME_EXCESS,
        self.jdbc_connection_pool_max_idle_time_excess,
      ...
      properties.process_pair(
        JDBC_CONNECTION_POOL_MAX_AGE, self.jdbc_connection_pool_max_age
      )
...
class PGConfig(LinuxDBMSConfig):
  ...
  EXECUTE_SCRIPT_AS_USER = [
    SHPURDP_SUDO_BINARY,
    "bash",
    "-c",
    "export PGPASSWORD={0} && psql -U {1} -f {2}",
  ]

  EXECUTE_QUERY_AS_POSTGRES_FOR_DB_SILENT = [
    SHPURDP_SUDO_BINARY,
    "su",
    "postgres",
    "-",
    '--command=psql -qAt -c "{0}" {1}',
  ]
  EXECUTE_QUERY_AS_POSTGRES_FOR_DB = [
    SHPURDP_SUDO_BINARY,
    "su",
    "postgres",
    "-",
  ...
  if (
    ...
    and OSCheck.get_os_major_version() in OSConst.systemd_redhat_os_major_versions
  ):
    ...
    PG_START_CMD = SHPURDP_SUDO_BINARY + f" {SERVICE_CMD} start {PG_SERVICE_NAME}"
    PG_RESTART_CMD = SHPURDP_SUDO_BINARY + f" {SERVICE_CMD} restart {PG_SERVICE_NAME}"
    PG_HBA_RELOAD_CMD = SHPURDP_SUDO_BINARY + f" {SERVICE_CMD} reload {PG_SERVICE_NAME}"
  else:
    ...
    if os.path.isfile("/usr/bin/postgresql-setup"):
      ...
      if OSCheck.is_suse_family() and not is_service_exist(PG_SERVICE_NAME):
        ...
        if versioned_script_paths:
          versioned_script_path_tps = [
            (re.search(r"postgresql-([0-9]+\.?[0-9]*)", path).group(1), path)
            for path in versioned_script_paths
          ]
    ...
    PG_START_CMD = SHPURDP_SUDO_BINARY + f" {SERVICE_CMD} {PG_SERVICE_NAME} start"
    PG_RESTART_CMD = SHPURDP_SUDO_BINARY + f" {SERVICE_CMD} {PG_SERVICE_NAME} restart"
    PG_HBA_RELOAD_CMD = SHPURDP_SUDO_BINARY + f" {SERVICE_CMD} {PG_SERVICE_NAME} reload"

  PG_HBA_CONF_FILE = None
  PG_HBA_CONF_FILE_BACKUP = None
  POSTGRESQL_CONF_FILE = None

  POSTGRES_EMBEDDED_INIT_FILE = ShpurdpPath.get(
    "/var/lib/shpurdp-server/resources/Shpurdp-DDL-Postgres-EMBEDDED-CREATE.sql"
  )
  POSTGRES_EMBEDDED_DROP_FILE = ShpurdpPath.get(
    "/var/lib/shpurdp-server/resources/Shpurdp-DDL-Postgres-EMBEDDED-DROP.sql"
  )

  POSTGRES_INIT_FILE = ShpurdpPath.get(
    "/var/lib/shpurdp-server/resources/Shpurdp-DDL-Postgres-CREATE.sql"
  )
  POSTGRES_DROP_FILE = ShpurdpPath.get(
    "/var/lib/shpurdp-server/resources/Shpurdp-DDL-Postgres-DROP.sql"
  )
  ...
  def __init__(self, options, properties, storage_type):
    ...
    self.JDBC_DRIVER_INSTALL_MSG = (
      "Before starting Shpurdp Server, "
      'you must copy the {0} JDBC driver JAR file to {1} and set property "server.jdbc.driver.path=[path/to/custom_jdbc_driver]" in shpurdp.properties.'.format(
    ...
    self.client_tool_usage_pattern = (
      "su -postgres --command=psql -f {0} -v username='\"{1}\"' -v password=\"'{2}'\""
    )
  ...
  def ensure_dbms_is_running(self, options, properties, scmStatus=None):
    if self._is_local_database():
      if is_root():
        ...
        print(
          "Unable to check PostgreSQL server status when starting "
  ...
  def _create_postgres_lock_directory(self):
    ...
    try:
      ...
      print(
        "WARNING: Unable to create /var/run/postgresql directory, because user [postgres] doesn't exist. Potentially,"
    ...
    try:
      ...
      print(
        "WARNING: Unable to create /var/run/postgresql directory. Potentially,"
        " postgresql service start can be failed."
      )
  ...
  def _reset_remote_database(self):
    ...
    raise NonFatalException(
      "Please set DB password to PGPASSWORD env variable before running DDL`s!"
    )
  ...
  def _configure_database_name(self):
    self.database_name = LinuxDBMSConfig._get_validated_db_name(
      self.database_storage_name, self.database_name
    )
  ...
  def _check_postgre_up():
    ...
    if pg_status == PGConfig.PG_STATUS_RUNNING:
      ...
      try:
        ...
        wait_for_port_opened(
          "127.0.0.1", PG_PORT, PG_PORT_CHECK_TRIES_COUNT, PG_PORT_CHECK_INTERVAL
        )
  ...
  def _setup_db(self):
    # password access to shpurdp-server and mapred
    dbname = self.database_name
    ...
    retcode, outdata, errdata = self.run_with_retries(
      command, "Creating schema and user..."
    )
    if retcode == 0:
      ddl_command = PGConfig.EXECUTE_SCRIPT_AS_USER[:]
      ddl_command[-1] = ddl_command[-1].format(
        password, username, PGConfig.POSTGRES_INIT_FILE
      )
      retcode, outdata, errdata = self.run_with_retries(
        ddl_command, "Creating tables..."
      )
  ...
  @staticmethod
  def _configure_pg_hba_shpurdpdb_users(conf_file, database_username):
    ...
    conf_file_content_out += "\n"
    conf_file_content_out += (
      "host  all   " + database_username + ",mapred 0.0.0.0/0  md5"
    )
    conf_file_content_out += "\n"
  ...
  def _configure_postgresql_conf():
    ...
    postgresql_conf_file_out = re.sub(
      "#+listen_addresses.*?(#|$)", listenAddress, postgresql_conf_file_in.decode()
    )
  ...
  def _configure_postgres(self):
    ...
    PGConfig._configure_pg_hba_shpurdpdb_users(
      PGConfig.PG_HBA_CONF_FILE, self.database_username
    )
    sudo.chmod(PGConfig.PG_HBA_CONF_FILE, 0o644)
  ...
  def _change_db_files_owner(self):
    ...
    if not self._change_tables_owner():
      print_error_msg(
        """Shpurdp is unable to change ownership of the database tables in {database} to {user}.
...
Make sure that all tables returned by following SQL are owned by {user}:
  "SELECT tablename FROM pg_tables WHERE schemaname = 'shpurdp';",
  "SELECT sequence_name FROM information_schema.sequences WHERE sequence_schema = 'shpurdp';",
  "SELECT table_name FROM information_schema.views WHERE table_schema = 'shpurdp';
""".format(
          database=self.database_name,
          admin_user=self.local_admin_user,
          user=self.database_username,
        )
      )
  ...
  def _change_tables_owner(self):
    ...
    if not tables:
      print_error_msg("Failed to get list of shpurdp tables")
      return False

    for tbl in tables:
      retcode, stdout, stderr = self._execute_psql_query(
        "select u.usename from information_schema.tables t "
        "join pg_catalog.pg_class c on (t.table_name = c.relname) "
        "join pg_catalog.pg_user u on (c.relowner = u.usesysid) "
        "where t.table_schema='shpurdp' and t.table_name='" + tbl + "';",
        self.database_name,
      )
      owner = stdout.strip()
  ...
  def _execute_psql_query(query, databse, silent=True):
    ...
    """
    cmd = (
      PGConfig.EXECUTE_QUERY_AS_POSTGRES_FOR_DB_SILENT[:]
      if silent
      else PGConfig.EXECUTE_QUERY_AS_POSTGRES_FOR_DB[:]
    )
...
class OracleConfig(LinuxDBMSConfig):
  def __init__(self, options, properties, storage_type):
    ...
    if (hasattr(options, "sid_or_sname") and options.sid_or_sname == "sname") or (
      hasattr(options, "jdbc_url")
      and options.jdbc_url
      and re.match(ORACLE_SNAME_PATTERN, options.jdbc_url)
    ):
    ...
    self.init_script_file = ShpurdpPath.get(
      "/var/lib/shpurdp-server/resources/Shpurdp-DDL-Oracle-CREATE.sql'"
    )
    self.drop_tables_script_file = ShpurdpPath.get(
      "/var/lib/shpurdp-server/resources/Shpurdp-DDL-Oracle-DROP.sql"
    )
  ...
  def _reset_remote_database(self):
    ...
    raise NonFatalException(
      "Please replace '*' symbols with password before running DDL`s!"
    )
  ...
  def _configure_database_name(self):
    if self.persistence_type != STORAGE_TYPE_LOCAL:
      ...
      idType = get_validated_string_input(
        ...
        + ORACLE_DB_ID_TYPES[0]
        + "\n2 - "
        + ORACLE_DB_ID_TYPES[1]
        + "\n("
        + idType
        + "): ",
        idType,
      ...
      self.database_name = OracleConfig._get_validated_service_name(
        self.database_name, IDTYPE_INDEX
      )
    else:
      self.database_name = LinuxDBMSConfig._get_validated_db_name(
        self.database_storage_name, self.database_name
...
class MySQLConfig(LinuxDBMSConfig):
  def __init__(self, options, properties, storage_type):
    ...
    self.init_script_file = ShpurdpPath.get(
      "/var/lib/shpurdp-server/resources/Shpurdp-DDL-MySQL-CREATE.sql"
    )
    self.drop_tables_script_file = ShpurdpPath.get(
      "/var/lib/shpurdp-server/resources/Shpurdp-DDL-MySQL-DROP.sql"
    )
  ...
  def _get_remote_script_line(self, scriptFile):
    MYSQL_INIT_SCRIPT = ShpurdpPath.get(
      "/var/lib/shpurdp-server/resources/Shpurdp-DDL-MySQL-CREATE.sql"
    )
    MYSQL_EXEC_ARGS_WITH_USER_VARS = (
      "mysql --host={0} --port={1} --user={2} --password={3} {4} "
    ...
    MYSQL_EXEC_ARGS = (
      ...
      if MYSQL_INIT_SCRIPT == scriptFile
      else MYSQL_EXEC_ARGS_WITH_USER_VARS
    )
...
class MSSQLConfig(LinuxDBMSConfig):
  def __init__(self, options, properties, storage_type):
    ...
    self.init_script_file = ShpurdpPath.get(
      "/var/lib/shpurdp-server/resources/Shpurdp-DDL-SQLServer-CREATE.sql"
    )
    self.drop_tables_script_file = ShpurdpPath.get(
      "/var/lib/shpurdp-server/resources/Shpurdp-DDL-SQLServer-DROP.sql"
    )
    self.client_tool_usage_pattern = ""
...
class SQLAConfig(LinuxDBMSConfig):
  ...
  def __init__(self, options, properties, storage_type):
    ...
    self.driver_class_name = (
      "sap.jdbc4.sqlanywhere.IDriver"  # TODO sybase.* for v < 17, check requirements
    )
    self.driver_file_name = "sajdbc4.jar"
    self.server_name = DBMSConfig._init_member_with_prop_default(
      options, "sqla_server_name", properties, JDBC_SQLA_SERVER_NAME, "shpurdp"
    )
    ...
    self.database_storage_name = "Database"
    self.database_port = DBMSConfig._init_member_with_prop_default(
      options, "database_port", properties, JDBC_PORT_PROPERTY, "2638"
    )
    ...
    self.JDBC_DRIVER_INSTALL_MSG = (
      "Before starting Shpurdp Server, "
      'you must copy the {0} jdbc client tarball to {1} and set property "server.jdbc.driver.path=[path/to/custom_jdbc_driver]" in shpurdp.properties.'.format(
        self.dbms_full_name, configDefaults.SHARE_PATH
      )
    )

    self.init_script_file = ShpurdpPath.get(
      "/var/lib/shpurdp-server/resources/Shpurdp-DDL-SQLAnywhere-CREATE.sql"
    )
    self.drop_tables_script_file = ShpurdpPath.get(
      "/var/lib/shpurdp-server/resources/Shpurdp-DDL-SQLAnywhere-DROP.sql"
    )
  ...
  def _get_jdbc_connection_string(self):
    ...
    connectionStringFormat = self.database_url_pattern
    return connectionStringFormat.format(
      self.server_name, self.database_name, jdbc_hostname, self.database_port
    )
  ...
  def _is_jdbc_driver_installed(self, properties):
    drivers = []
    drivers.extend(
      glob.glob(configDefaults.SHARE_PATH + os.sep + self.client_tarball_pattern)
    )
    if drivers:
      return drivers
    return -1
  ...
  def _configure_database_name(self):
    self.server_name = get_validated_string_input(
      "Server name (" + str(self.server_name) + "): ",
      self.server_name,
      ".*",
      "Invalid server name",
      False,
  ...
  def _extract_client_tarball(self, properties):
    ...
    if retcode != 0:
      raise FatalException(
        -1, "Error extracting SQL Anywhere client tarball: " + str(err)
      )

  def _get_native_libs(self, properties):
    return os.path.join(
      get_resources_location(properties), self.client_folder, "native", "lib64"
    )
