...
from shpurdp_commons.os_utils import (
  ...
  is_valid_filepath,
  change_owner,
  get_shpurdp_repo_file_full_name,
  get_file_owner,
)
...
from shpurdp_server.serverConfiguration import (
  ...
  get_is_persisted,
  get_is_secure,
  get_master_key_location,
  get_db_type,
  write_property,
  get_original_master_key,
  ...
  CHECK_SHPURDP_KRB_JAAS_CONFIGURATION_PROPERTY,
)
from shpurdp_server.serverUtils import (
  is_server_runing,
  get_shpurdp_server_api_base,
...
from shpurdp_server.setupActions import SETUP_ACTION, LDAP_SETUP_ACTION
...
REGEX_ANYTHING = ".*"
LDAP_TO_PAM_MIGRATION_HELPER_CMD = (
  "{0} -cp {1} "
  + "org.apache.shpurdp.server.security.authentication.LdapToPamMigrationHelper"
  + " >> "
...
SETUP_LDAP_CONFIG_URL = (
  "services/SHPURDP/components/SHPURDP_SERVER/configurations/ldap-configuration"
)

PAM_CONFIG_FILE = "pam.configuration"

LDAP_MGR_USERNAME_PROPERTY = "shpurdp.ldap.connectivity.bind_dn"
LDAP_MGR_PASSWORD_FILENAME = "ldap-password.dat"
LDAP_ANONYMOUS_BIND = "shpurdp.ldap.connectivity.anonymous_bind"
LDAP_USE_SSL = "shpurdp.ldap.connectivity.use_ssl"
LDAP_DISABLE_ENDPOINT_IDENTIFICATION = (
  "shpurdp.ldap.advanced.disable_endpoint_identification"
)
NO_AUTH_METHOD_CONFIGURED = "no auth method"

SHPURDP_LDAP_AUTH_ENABLED = "shpurdp.ldap.authentication.enabled"
LDAP_MANAGE_SERVICES = "shpurdp.ldap.manage_services"
LDAP_ENABLED_SERVICES = "shpurdp.ldap.enabled_services"
...
def read_master_key(isReset=False, options=None):
  ...
  while input:
    ...
    masterKey2 = get_validated_string_input(
      "Re-enter master key: ",
      passwordDefault,
      passwordPattern,
      passwordDescr,
...
def save_master_key(options, master_key, key_location, persist=True):
  if master_key:
    ...
    command = SECURITY_PROVIDER_KEY_CMD.format(
      get_java_exe_path(),
...
def adjust_directory_permissions(shpurdp_user):
  properties = get_shpurdp_properties()

  bootstrap_dir = os.path.abspath(
    get_value_from_properties(properties, BOOTSTRAP_DIR_PROPERTY)
  )
  ...
  if masterKeyFile:
    configDefaults.NR_ADJUST_OWNERSHIP_LIST.append(
      (masterKeyFile, configDefaults.MASTER_KEY_FILE_PERMISSIONS, "{0}", False)
    )
  credStoreFile = get_credential_store_location(properties)
  if os.path.exists(credStoreFile):
    configDefaults.NR_ADJUST_OWNERSHIP_LIST.append(
      (credStoreFile, configDefaults.CREDENTIALS_STORE_FILE_PERMISSIONS, "{0}", False)
    )
  ...
  if trust_store_location:
    configDefaults.NR_ADJUST_OWNERSHIP_LIST.append(
      (
        ...
        configDefaults.TRUST_STORE_LOCATION_PERMISSIONS,
        "{0}",
        False,
      )
    )
  ...
  if jdk_file_name:
    ...
    if os.path.exists(jdk_file_path):
      configDefaults.NR_ADJUST_OWNERSHIP_LIST.append(
        (jdk_file_path, "644", "{0}", False)
      )
  if jce_file_name:
    jce_file_path = os.path.abspath(os.path.join(resources_dir, jce_file_name))
    if os.path.exists(jce_file_path):
      configDefaults.NR_ADJUST_OWNERSHIP_LIST.append(
        (jce_file_path, "644", "{0}", False)
      )
  if java_home:
    jdk_security_dir = os.path.abspath(
      os.path.join(java_home, configDefaults.JDK_SECURITY_DIR)
    )
    if os.path.exists(jdk_security_dir):
      configDefaults.NR_ADJUST_OWNERSHIP_LIST.append(
        (jdk_security_dir + "/*", "644", "{0}", True)
      )
      configDefaults.NR_ADJUST_OWNERSHIP_LIST.append(
        (jdk_security_dir, "755", "{0}", False)
      )

  # Grant read permissions to all users. This is required when a non-admin user is configured to setup shpurdp-server.
  # However, do not change ownership of the repo file to shpurdp user.

  shpurdp_repo_file = get_shpurdp_repo_file_full_name()

  if shpurdp_repo_file:
    if os.path.exists(shpurdp_repo_file):
      shpurdp_repo_file_owner = get_file_owner(shpurdp_repo_file)
      configDefaults.NR_ADJUST_OWNERSHIP_LIST.append(
        (shpurdp_repo_file, "644", shpurdp_repo_file_owner, False)
      )

  print("Adjusting shpurdp-server permissions and ownership...")

  for pack in configDefaults.NR_ADJUST_OWNERSHIP_LIST:
    file = pack[0]
    mod = pack[1]
    user = pack[2].format(shpurdp_user)
    recursive = pack[3]
  ...
  for pack in configDefaults.NR_CHANGE_OWNERSHIP_LIST:
    path = pack[0]
    user = pack[1].format(shpurdp_user)
    recursive = pack[2]
...
def configure_ldap_password(ldap_manager_password_option, interactive_mode):
  ...
  password = (
    read_password(
      ...
      password_prompt,
      password_descr,
      ldap_manager_password_option,
      confirm_password_prompt,
    )
    if interactive_mode
    else ldap_manager_password_option
  )
...
class LdapSyncOptions:
  def __init__(self, options):
    ...
    try:
      self.ldap_sync_post_process_existing_users = (
  ...
  def no_ldap_sync_options_set(self):
    return (
      not self.ldap_sync_all
      and not self.ldap_sync_existing
      and self.ldap_sync_users is None
      and self.ldap_sync_groups is None
    )


def get_ldap_property_from_db(properties, admin_login, admin_password, property_name):
  ldap_properties_from_db = get_ldap_properties_from_db(
...
def get_ldap_properties_from_db(properties, admin_login, admin_password):
  ...
  url = get_shpurdp_server_api_base(properties) + SETUP_LDAP_CONFIG_URL
  admin_auth = (
    base64.b64encode(f"{admin_login}:{admin_password}".encode())
    .decode()
    .replace("\n", "")
  )
  ...
  while request_in_progress:
    ...
    if num_of_tries == 60:
      raise FatalException(
        1, "Could not fetch LDAP configuration within a minute; giving up!"
      )
    sys.stdout.write(".")
    sys.stdout.flush()
    ...
    try:
      with closing(
        ...
        if response_status_code != 200:
          ...
          err = "Error while fetching LDAP configuration. Http status code - " + str(
...
def is_ldap_enabled(properties, admin_login, admin_password):
  ldap_enabled = get_ldap_property_from_db(
    properties, admin_login, admin_password, SHPURDP_LDAP_AUTH_ENABLED
  )
...
def sync_ldap(options):
  ...
  if ldap_sync_options.no_ldap_sync_options_set():
    err = "Must specify a sync option (all, existing, users or groups).  Please invoke shpurdp-server.py --help to print the options."
    raise FatalException(1, err)

  # TODO: use serverUtils.get_shpurdp_admin_username_password_pair (requires changes in shpurdp-server.py too to modify option names)
  admin_login = (
    ldap_sync_options.ldap_sync_admin_name
    if ldap_sync_options.ldap_sync_admin_name is not None
    and ldap_sync_options.ldap_sync_admin_name
    else get_validated_string_input(
      prompt="Enter Shpurdp Admin login: ",
      default=None,
      pattern=None,
      description=None,
      is_pass=False,
  ...
  admin_password = (
    ldap_sync_options.ldap_sync_admin_password
    if ldap_sync_options.ldap_sync_admin_password is not None
    and ldap_sync_options.ldap_sync_admin_password
    else get_validated_string_input(
      prompt="Enter Shpurdp Admin password: ",
      default=None,
      pattern=None,
      description=None,
      is_pass=True,
  ...
  if is_ldap_enabled(properties, admin_login, admin_password) != "true":
    err = "LDAP is not configured. Run 'shpurdp-server setup-ldap' first."
    raise FatalException(1, err)

  url = get_shpurdp_server_api_base(properties) + SERVER_API_LDAP_URL
  admin_auth = (
    ...
    bodies = [
      {
        "Event": {
          "specs": [
            {"principal_type": "users", "sync_type": "existing"},
...
def setup_sensitive_data_encryption(options):
  if not is_root():
    warn = (
      "shpurdp-server encrypt-passwords is run as "
  ...
  if isSecure:
    ...
    decrypt = get_YN_input(
      "Do you want to decrypt passwords managed by Shpurdp? [y/n] (n): ", False
    )
    if not decrypt:
  ...
  db_password, ts_password = deryptPasswordsConfigs(
    db_password, db_sql_auth, masterKey, ts_password
  )
  save_decrypted_shpurdp_properties(db_password, properties, ts_password)

  if not decrypt:
    if resetKey or not isSecure:
      # Read master key and encrypt sensitive data, if non-secure or reset is true
      masterKey, isPersisted = setup_master_key(
        masterKeyFile, options, properties, resetKey
      )
    else:
      if not isPersisted:
    ...
    encrypt_sensitive_data(
      db_password, masterKey, options, isPersisted, properties, ts_password
    )
  ...
  if shpurdp_user:
    adjust_directory_permissions(shpurdp_user)
  return 0


def save_decrypted_shpurdp_properties(db_password, properties, ts_password):
...
def encrypt_sensitive_data(
  db_password, masterKey, options, persist, properties, ts_password
):
  ...
  if db_password and not is_alias_string(db_password):
    ...
    if retCode != 0:
      ...
      if properties.get_property(JDBC_RCA_PASSWORD_FILE_PROPERTY):
        propertyMap[JDBC_RCA_PASSWORD_FILE_PROPERTY] = get_alias_string(
          JDBC_RCA_PASSWORD_ALIAS
        )

  if ts_password and not is_alias_string(ts_password):
    retCode = save_passwd_for_alias(
      SSL_TRUSTSTORE_PASSWORD_ALIAS, ts_password, masterKey
    )
    if retCode != 0:
      print("Failed to save secure TrustStore password.")
    else:
      propertyMap[SSL_TRUSTSTORE_PASSWORD_PROPERTY] = get_alias_string(
        SSL_TRUSTSTORE_PASSWORD_ALIAS
      )
...
def setup_master_key(masterKeyFile, options, properties, resetKey):
  ...
  persist = get_YN_input(
    "Do you want to persist master key. If you choose "
    "not to persist, you need to provide the Master "
    "Key while starting the shpurdp server as an env "
    "variable named "
    + SECURITY_KEY_ENV_VAR_NAME
  ...
  if persist:
    save_master_key(
      options,
      masterKey,
      get_master_key_location(properties) + os.sep + SECURITY_MASTER_KEY_FILENAME,
      persist,
    )
...
def printManualDecryptionWarning(db_password, db_sql_auth, ts_password):
  ...
  print(
    "- Edit "
    + os.path.join(get_conf_dir(), SHPURDP_PROPERTIES_FILE)
    + " and set "
    + SECURITY_IS_ENCRYPTION_ENABLED
    + " = "
    + "false."
  ...
  err = (
    "{0} is already encrypted. Please call {1} to store unencrypted"
...
def setup_shpurdp_krb5_jaas(options):
  ...
  if os.path.exists(jaas_conf_file):
    ...
    write_property(CHECK_SHPURDP_KRB_JAAS_CONFIGURATION_PROPERTY, "true")
  else:
...
class LdapPropTemplate:
  ...
  def get_default_value(self, ldap_type):
    return (
      self.default_value.get_default_value(ldap_type) if self.default_value else None
    )
  ...
  def get_input(self, ldap_type, interactive_mode):
    ...
    return (
      get_validated_string_input(
        self.get_prompt_text(ldap_type),
        default_value,
        self.prompt_regex,
        "Invalid characters in the input!",
        False,
        self.allow_empty_prompt,
        answer=self.option,
      )
      if interactive_mode
      else self.option
  ...
  def should_query_ldap_type(self):
    return (
      not self.allow_empty_prompt
      and not self.option
      and self.default_value
      and self.default_value.depends_on_ldap_type()
    )
...
class LdapDefaultMap(LdapDefault):
  ...
  def get_default_value(self, ldap_type):
    return (
      self.default_value_map[ldap_type]
      if self.default_value_map and ldap_type in self.default_value_map
      else None
    )
...
def init_ldap_properties_list_reqd(properties, options):
  ...
  ldap_properties = [
    LdapPropTemplate(
      ...
      "shpurdp.ldap.connectivity.server.host",
      "Primary LDAP Host{0}: ",
      REGEX_HOSTNAME,
      False,
      LdapDefaultMap(
        {LDAP_IPA: "ipa.shpurdp.apache.org", LDAP_GENERIC: "ldap.shpurdp.apache.org"}
      ),
    ),
    LdapPropTemplate(
      properties,
      options.ldap_primary_port,
      "shpurdp.ldap.connectivity.server.port",
      "Primary LDAP Port{0}: ",
      REGEX_PORT,
      False,
    ...
    LdapPropTemplate(
      ...
      "shpurdp.ldap.connectivity.secondary.server.host",
    ...
    LdapPropTemplate(
      ...
      "shpurdp.ldap.connectivity.secondary.server.port",
    ...
    LdapPropTemplate(
      properties,
      options.ldap_ssl,
      "shpurdp.ldap.connectivity.use_ssl",
    ...
    LdapPropTemplate(
      ...
      "shpurdp.ldap.attributes.user.name_attr",
      "User ID attribute{0}: ",
      REGEX_ANYTHING,
      False,
    ...
    LdapPropTemplate(
      ...
      "shpurdp.ldap.attributes.user.search_base",
      "Search Base{0}: ",
      REGEX_ANYTHING,
      False,
      LdapDefault("dc=shpurdp,dc=apache,dc=org"),
    ),
    LdapPropTemplate(
      properties,
      options.ldap_referral,
      "shpurdp.ldap.advanced.referrals",
    ...
    LdapPropTemplate(
      ...
      "shpurdp.ldap.connectivity.anonymous_bind" "Bind anonymously [true/false]{0}: ",
      REGEX_TRUE_FALSE,
      False,
      LdapDefault("false"),
    ),
...
def init_ldap_properties_list_reqd(properties, options):
  ldap_properties = [
    ...
    LdapPropTemplate(
      ...
      "shpurdp.ldap.attributes.user.object_class",
      "User object class{0}: ",
      REGEX_ANYTHING,
      False,
      LdapDefaultMap(
    ...
    LdapPropTemplate(
      ...
      "shpurdp.ldap.attributes.user.group_member_attr",
    ...
    LdapPropTemplate(
      ...
      "shpurdp.ldap.attributes.group.object_class",
    ...
    LdapPropTemplate(
      ...
      "shpurdp.ldap.attributes.group.name_attr",
    ...
    LdapPropTemplate(
      ...
      "shpurdp.ldap.attributes.group.member_attr",
    ...
    LdapPropTemplate(
      properties,
      options.ldap_dn,
      "shpurdp.ldap.attributes.dn_attr",
    ...
    LdapPropTemplate(
      ...
      LdapDefaultMap(
        {
          LDAP_AD: "dc=shpurdp,dc=apache,dc=org",
          LDAP_IPA: "cn=accounts,dc=shpurdp,dc=apache,dc=org",
          LDAP_GENERIC: "dc=shpurdp,dc=apache,dc=org",
        }
      ),
    ),
    LdapPropTemplate(
    ...
    LdapPropTemplate(
      properties,
      options.ldap_bind_anonym,
      "shpurdp.ldap.connectivity.anonymous_bind",
    ...
    LdapPropTemplate(
      ...
      "shpurdp.ldap.advanced.collision_behavior",
    ...
    LdapPropTemplate(
      ...
      "shpurdp.ldap.advanced.force_lowercase_usernames",
    ...
    LdapPropTemplate(
      ...
      "shpurdp.ldap.advanced.pagination_enabled",
...
def update_ldap_configuration(
  ...
  request_data = {"Configuration": {"category": "ldap-configuration", "properties": {}}}
  request_data["Configuration"]["properties"] = ldap_property_value_map
  perform_changes_via_rest_api(
    properties, admin_login, admin_password, SETUP_LDAP_CONFIG_URL, "PUT", request_data
  )
...
def query_ldap_type(ldap_type_option):
  return get_validated_string_input(
    f"Please select the type of LDAP you want to use [{'/'.join(LDAP_TYPES)}]({LDAP_GENERIC}):",
    LDAP_GENERIC,
    REGEX_LDAP_TYPE,
...
def setup_ldap(options):
  ...
  ldap_bind_dn_template = LdapPropTemplate(
    ...
    LdapDefaultMap(
      {
        LDAP_AD: "cn=ldapbind,dc=shpurdp,dc=apache,dc=org",
        LDAP_IPA: "uid=ldapbind,cn=users,cn=accounts,dc=shpurdp,dc=apache,dc=org",
        LDAP_GENERIC: "uid=ldapbind,cn=users,dc=shpurdp,dc=apache,dc=org",
      }
    ),
  )
  ldap_type = (
  ...
  ldap_property_values_in_shpurdp_properties = {}
    ...
    if ldap_prop.prop_name == LDAP_ANONYMOUS_BIND:
      ...
      if ldaps:
        ...
        if not custom_trust_store:
          custom_trust_store = (
            get_YN_input(
              "Do you want to provide custom TrustStore for Shpurdp [y/n] ({0})?".format(
                truststore_default
              ),
              truststore_set,
            )
        ...
        if custom_trust_store:
          ...
          ldap_property_values_in_shpurdp_properties[SSL_TRUSTSTORE_TYPE_PROPERTY] = (
            ts_type
          )
          ldap_property_values_in_shpurdp_properties[SSL_TRUSTSTORE_PATH_PROPERTY] = (
            ts_path
          )
          ldap_property_values_in_shpurdp_properties[
            SSL_TRUSTSTORE_PASSWORD_PROPERTY
  ...
  populate_shpurdp_requires_ldap(options, ldap_property_value_map)
  ...
  for property in ldap_property_list_opt:
    if property in ldap_property_values_in_shpurdp_properties:
      if property not in ldap_property_list_passwords:
        print(f"{property}: {ldap_property_values_in_shpurdp_properties[property]}")
      else:
        print(f"{property}: {BLIND_PASSWORD}")
  ...
  if save_settings:
    ...
    # Saving LDAP configuration in Shpurdp DB using the REST API
    update_ldap_configuration(
      admin_login, admin_password, properties, ldap_property_value_map
    )

    # The only properties we want to write out in Shpurdp.properties are the client.security type being LDAP and the custom Truststore related properties (if any)
    ldap_property_values_in_shpurdp_properties[CLIENT_SECURITY] = "ldap"
    update_properties_2(properties, ldap_property_values_in_shpurdp_properties)
...
@OsFamilyFuncImpl(OsFamilyImpl.DEFAULT)
def generate_env(options, shpurdp_user, current_user):
  properties = get_shpurdp_properties()
  isSecure = get_is_secure(properties)
  (isPersisted, masterKeyFile) = get_is_persisted(properties)
  environ = os.environ.copy()
  ...
  if isSecure and not masterKeyFile:
    ...
    if masterKey is not None and masterKey != "":
      ...
      if keyLocation is not None:
        try:
          ...
          print_warning_msg(
            "Cannot read Master key from path specified in " "environemnt."
          )
          prompt = True
      else:
    ...
    if prompt:
      ...
      if shpurdp_user != current_user:
        uid = pwd.getpwnam(shpurdp_user).pw_uid
        gid = pwd.getpwnam(shpurdp_user).pw_gid
...
@OsFamilyFuncImpl(OSConst.WINSRV_FAMILY)
def ensure_can_start_under_current_user(shpurdp_user):
...
def ensure_can_start_under_current_user(shpurdp_user):
  ...
  if shpurdp_user is None:
    err = "Unable to detect a system user for Shpurdp Server.\n" + SETUP_OR_UPGRADE_MSG
    raise FatalException(1, err)
  if current_user != shpurdp_user and not is_root():
    err = (
      'Unable to start Shpurdp Server as user {0}. Please either run "shpurdp-server start" '
      'command as root, as sudo or as user "{1}"'.format(current_user, shpurdp_user)
    )
    raise FatalException(1, err)
  return current_user
...
class PamPropTemplate:
  def __init__(
    ...
    self.prop_name = i_prop_name
    self.option = i_option
    self.pam_prop_name = get_value_from_properties(
      properties, i_prop_name, i_prop_name_default
    )
    self.pam_prop_val_prompt = i_prop_val_pattern.format(
      get_prompt_default(self.pam_prop_name)
    )
    self.prompt_regex = i_prompt_regex
    self.allow_empty_prompt = i_allow_empty_prompt
...
def init_pam_properties_list_reqd(properties, options):
  properties = [
    PamPropTemplate(
      properties,
      options.pam_config_file,
      PAM_CONFIG_FILE,
      "PAM configuration file* {0}: ",
      REGEX_ANYTHING,
      False,
      "/etc/pam.d/shpurdp",
    ),
    PamPropTemplate(
      properties,
      options.pam_auto_create_groups,
      AUTO_GROUP_CREATION,
      "Do you want to allow automatic group creation* [true/false] {0}: ",
      REGEX_TRUE_FALSE,
      False,
      "false",
    ),
...
def setup_pam(options):
  if not is_root():
    err = "Shpurdp-server setup-pam should be run with root-level privileges"
  ...
  for pam_prop in pam_property_list_reqd:
    input = get_validated_string_input(
      pam_prop.pam_prop_val_prompt,
      pam_prop.pam_prop_name,
      pam_prop.prompt_regex,
      "Invalid characters in the input!",
      False,
      pam_prop.allow_empty_prompt,
  ...
  if not os.path.exists(pam_config_file):
    print_warning_msg(
      "The PAM configuration file, {0} does not exist.  "
      "Please create it before restarting Shpurdp.".format(pam_config_file)
    )
...
def migrate_ldap_pam(args):
  ...
  confirm = get_YN_input(
    "Shpurdp Server configured for %s. Confirm "
    "you have made a backup of the Shpurdp Server database [y/n] (y)? " % db_title,
    True,
  )
  ...
  # At this point, the args does not have the shpurdp database information.
  # Augment the args with the correct shpurdp database information
  ...
  environ = generate_env(args, shpurdp_user, current_user)

  (retcode, stdout, stderr) = run_os_command(command, env=environ)
  print_info_msg(
    "Return code from LDAP to PAM migration command, retcode = " + str(retcode)
  )
  if stdout:
  ...
  if retcode > 0:
    print_error_msg(
      "Error executing LDAP to PAM migration, please check the server logs."
    )
  else:
...
def populate_shpurdp_requires_ldap(options, properties):
  if options.ldap_enabled_shpurdp is None:
    enabled = get_boolean_from_dictionary(properties, SHPURDP_LDAP_AUTH_ENABLED, False)
    enabled = get_YN_input(
      f"Use LDAP authentication for Shpurdp [y/n] ({'y' if enabled else 'n'})? ", enabled
    )
  else:
    enabled = "true" == options.ldap_enabled_shpurdp

  properties[SHPURDP_LDAP_AUTH_ENABLED] = "true" if enabled else "false"


def populate_service_management(
  options, properties, shpurdp_properties, admin_login, admin_password
):
  services = ""
  if options.ldap_enabled_services is None:
    if options.ldap_manage_services is None:
      manage_services = get_boolean_from_dictionary(
        properties, LDAP_MANAGE_SERVICES, False
      )
      manage_services = get_YN_input(
        f"Manage LDAP configurations for eligible services [y/n] ({'y' if manage_services else 'n'})? ",
        manage_services,
      )
    else:
      ...
      print(
        f"Manage LDAP configurations for eligible services [y/n] ({'y' if stored_manage_services else 'n'})? {'y' if manage_services else 'n'}"
      )

    if manage_services:
      enabled_services = (
        get_value_from_dictionary(properties, LDAP_ENABLED_SERVICES, "")
        .upper()
        .split(",")
      )
      ...
      configure_for_all_services = get_YN_input(
        f" Manage LDAP for all services [y/n] ({'y' if all else 'n'})? ", all
      )
      if configure_for_all_services:
        services = WILDCARD_FOR_ALL_SERVICES
      else:
        cluster_name = get_cluster_name(shpurdp_properties, admin_login, admin_password)
        ...
        if cluster_name:
          eligible_services = get_eligible_services(
            ...
            admin_password,
            cluster_name,
            FETCH_SERVICES_FOR_LDAP_ENTRYPOINT,
            "LDAP",
          )
          ...
          if eligible_services and len(eligible_services) > 0:
            ...
            for service in eligible_services:
              ...
              question = (
                f"   Manage LDAP for {service} [y/n] ({'y' if enabled else 'n'})? "
              )
    ...
    services = (
      options.ldap_enabled_services.upper() if options.ldap_enabled_services else ""
    )
