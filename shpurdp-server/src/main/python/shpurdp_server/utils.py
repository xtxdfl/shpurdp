...
from shpurdp_commons.logging_utils import print_error_msg
    ...
    if pids:
      ...
      for item in pids:
        if pid_exists(item["pid"]) and (item["exe"] not in exclude_list):
          ...
          pid_saved = item["pid"]
          logger.info("Shpurdp server started with PID " + str(item["pid"]))
        if (
          pid_exists(item["pid"])
...
def wait_for_ui_start(shpurdp_server_ui_port, pid, timeout=1):
  tstart = time.time()
  while int(time.time() - tstart) <= timeout:
    try:
      sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
      sock.settimeout(1)
      sock.connect(("localhost", shpurdp_server_ui_port))
      print("\nServer started listening on " + str(shpurdp_server_ui_port))
      return True
    except Exception as e:
      # print str(e)
      pass
...
# parse shpurdp repo file and get the value of '#json.url = http://...'
def get_json_url_from_repo_file():
  repo_file_path = get_shpurdp_repo_file_full_name()
