...
from shpurdp_commons.logging_utils import get_silent, print_warning_msg, print_error_msg
...
# keytool commands
KEYTOOL_IMPORT_CERT_CMD = (
  "{0}"
  + os.sep
  + "bin"
  + os.sep
  + "keytool -import -alias '{1}' -storetype '{2}' -file '{3}' -storepass '{4}' -noprompt"
)
KEYTOOL_DELETE_CERT_CMD = (
  "{0}"
  + os.sep
  + "bin"
  + os.sep
  + "keytool -delete -alias '{1}' -storepass '{2}' -noprompt"
)
...
# keytool commands
KEYTOOL_IMPORT_CERT_CMD = (
  "{0} -import -alias '{1}' -storetype '{2}' -file '{3}' -storepass '{4}' -noprompt"
)
...
def get_and_persist_truststore_password(properties, options):
  ...
  if truststore_password:
    if isSecure:
      truststore_password = decrypt_password_for_alias(
        properties, SSL_TRUSTSTORE_PASSWORD_ALIAS, options
      )
  else:
    truststore_password = read_password(
      "",
      ".*",
    ...
    if truststore_password:
      encrypted_password = get_encrypted_password(
        SSL_TRUSTSTORE_PASSWORD_ALIAS, truststore_password, properties, options
      )
...
def get_import_cert_command(
  ...
  cmd = KEYTOOL_IMPORT_CERT_CMD.format(
    get_keytool_path(jdk_path),
    alias,
    truststore_type,
    import_cert_path,
    truststore_password,
...
def get_delete_cert_command(jdk_path, alias, truststore_path, truststore_password):
  cmd = KEYTOOL_DELETE_CERT_CMD.format(
    get_keytool_path(jdk_path), alias, truststore_password
  )
  if truststore_path:
    cmd += KEYTOOL_KEYSTORE.format(truststore_path)
  return cmd
...
def import_cert_and_key(security_server_keys_dir, options):
  ...
  import_key_path = get_validated_filepath_input(
    ...
    answer=options.import_key_path,
  )
  ...
  if retcode == 0:
    ...
    set_file_permissions(passFilePath, "660", read_shpurdp_user(), False)
...
def import_file_to_keystore(source, destination):
  ...
  set_file_permissions(destination, "660", read_shpurdp_user(), False)
...
def get_fqdn(timeout=2):
  ...
  if properties == -1:
    print("Error reading shpurdp properties")
    return None
...
def is_valid_https_port(port):
  properties = get_shpurdp_properties()
  if properties == -1:
    print("Error getting shpurdp properties")
    return False
  ...
  if port.strip() == one_way_port.strip():
    print(
      "Port for https can't match the port for one way authentication port("
      + one_way_port
      + ")"
    )
    return False
  ...
  if port.strip() == two_way_port.strip():
    print(
      "Port for https can't match the port for two way authentication port("
      + two_way_port
      + ")"
    )
    return False
...
def setup_https(options):
  if not is_root():
    warn = (
      "shpurdp-server setup-https is run as "
  ...
  if not get_silent():
    ...
    try:
      ...
      disable_https = (
        options.api_ssl in ["false"] if options.api_ssl is not None else None
      )
      configure_https = (
        options.api_ssl in ["true"] if options.api_ssl is not None else None
      )
      ...
      if api_ssl:
        disable_https = (
          ...
          else get_YN_input("Do you want to disable HTTPS [y/n] (n)? ", False)
        )
        if disable_https:
          ...
          properties.process_pair(
            ...
            get_validated_string_input(
              ...
              "Invalid port.",
              False,
              validatorFunction=is_valid_https_port,
              answer=options.api_ssl_port,
            ),
          )
          cert_was_imported = import_cert_and_key_action(
            security_server_keys_dir, properties, options
          )
      else:
        if get_YN_input(
          "Do you want to configure HTTPS [y/n] (y)? ", True, configure_https
        ):
          properties.process_pair(
            SSL_API_PORT,
            get_validated_string_input(
      ...
      f = open(conf_file, "w")
      properties.store(f, "Changed by 'shpurdp-server setup-https' command")

      if api_ssl_old_value != properties.get_property(
        SSL_API
      ) or client_api_ssl_port_old_value != properties.get_property(SSL_API_PORT):
        print(
          "Shpurdp server URL changed. To make use of the Tez View in Shpurdp "
...
def setup_truststore(options, import_cert=False):
  if not get_silent():
    ...
    truststore_reconfigure = (
      True if options.trust_store_reconfigure is not None else False
    )

    if truststore_confirm or get_YN_input(
      "Do you want to configure a truststore [y/n] (y)? ", True
    ):
      # Re-configuration enabled only for option "Setup truststore"
      if (
        not import_cert
        and properties.get_property(SSL_TRUSTSTORE_TYPE_PROPERTY)
        and (
          truststore_reconfigure
          or get_YN_input(
            "The truststore is already configured. Do you want to re-configure "
            "the truststore [y/n] (y)? ",
            True,
          )
        )
      ):
      ...
      if import_cert:
        import_cert_confirm = (
          True
          if options.import_cert_path is not None
          else get_YN_input("Do you want to import a certificate [y/n] (y)? ", True)
        )
        if import_cert_confirm:
          aliasOption = (
            options.import_cert_alias
            if options.import_cert_alias is not None and options.import_cert_alias
            else None
          )
          alias = (
            aliasOption
            if aliasOption is not None
          ...
          run_os_command(
            get_delete_cert_command(
              jdk_path, alias, truststore_path, truststore_password
            )
          )
    ...
    conf_file = find_properties_file()
    f = open(conf_file, "w")
    properties.store(f, "Changed by 'shpurdp-server setup-security' command")
  else:
