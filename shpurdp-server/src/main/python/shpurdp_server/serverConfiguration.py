...
import tempfile
import getpass
import shpurdp_server.serverClassPath
...
from shpurdp_server.utils import compare_versions, locate_file, on_powerpc
...
PID_NAME = "shpurdp-server.pid"

# Non-root user setup commands
NR_USER_PROPERTY = "shpurdp-server.user"
...
SHPURDP_PROPERTIES_FILE = "shpurdp.properties"
SHPURDP_ENV_FILE = "shpurdp-env.sh"
SHPURDP_KRB_JAAS_LOGIN_FILE = "krb5JAASLogin.conf"
GET_FQDN_SERVICE_URL = "server.fqdn.service.url"

SERVER_OUT_FILE_KEY = "shpurdp.output.file.path"
VERBOSE_OUTPUT_KEY = "shpurdp.output.verbose"

DEBUG_MODE_KEY = "shpurdp.server.debug"
SUSPEND_START_MODE_KEY = "shpurdp.server.debug.suspend.start"
...
# shpurdp JDK
SHPURDP_JAVA_HOME_PROPERTY = "shpurdp.java.home"
SHPURDP_JDK_NAME_PROPERTY = "shpurdp.jdk.name"
SHPURDP_JCE_NAME_PROPERTY = "shpurdp.jce.name"
SHPURDP_JAVA_VERSION = "shpurdp.java.version"
...
JDBC_DATABASE_NAME_PROPERTY = (
  "server.jdbc.database_name"  # E.g., shpurdp. Not used on Windows.
)
...
JDBC_RCA_PASSWORD_ALIAS = "shpurdp.db.password"
...
# The user which will bootstrap embedded postgres database setup by creating the default schema and shpurdp user.
...
# LDAP security
LDAP_MGR_PASSWORD_ALIAS = "shpurdp.ldap.manager.password"
LDAP_MGR_PASSWORD_PROPERTY = "shpurdp.ldap.connectivity.bind_password"

# SSL truststore
SSL_TRUSTSTORE_PASSWORD_ALIAS = "shpurdp.ssl.trustStore.password"
...
# Kerberos
CHECK_SHPURDP_KRB_JAAS_CONFIGURATION_PROPERTY = "kerberos.check.jaas.configuration"
...
class JavaHomeType(Enum):
  ...
  SHPURDP = SHPURDP_JAVA_HOME_PROPERTY
...
SETUP_OR_UPGRADE_MSG = (
  '- If this is a new setup, then run the "shpurdp-server setup" command to create the user\n'
  '- If this is an upgrade of an existing setup, run the "shpurdp-server upgrade" command.\n'
  "Refer to the Shpurdp documentation for more information on setup and upgrade."
)

GPL_LICENSE_PROMPT_TEXT = """GPL License for LZO: https://www.gnu.org/licenses/old-licenses/gpl-2.0.en.html
Enable Shpurdp Server to download and install GPL Licensed LZO packages [y/n] (n)? """

DEFAULT_DB_NAME = "shpurdp"
...
# if these properties are available 'shpurdp-server setup -s' is not triggered again.
...
def get_default_views_dir():
  return ShpurdpPath.get("/var/lib/shpurdp-server/resources/views")


def get_conf_dir():
  try:
    conf_dir = os.environ[SHPURDP_CONF_VAR]
    return conf_dir
  except KeyError:
    default_conf_dir = ShpurdpPath.get("/etc/shpurdp-server/conf")
    print_info_msg(f"{SHPURDP_CONF_VAR} is not set, using default {default_conf_dir}")
    return default_conf_dir


def find_properties_file():
  conf_file = search_file(SHPURDP_PROPERTIES_FILE, get_conf_dir())
  if conf_file is None:
    err = "File %s not found in search path $%s: %s" % (
      SHPURDP_PROPERTIES_FILE,
      SHPURDP_CONF_VAR,
      get_conf_dir(),
    )
    print_error_msg(err)
...
# Load shpurdp properties and return dict with values
def get_shpurdp_properties():
  conf_file = find_properties_file()

  # Try to read shpurdp properties file. It is OK to fail.
  # Return -1 on failure.
  properties = None
  try:
...
class ServerConfigDefaults(object):
  def __init__(self):
    ...
    self.SERVER_OUT_FILE = os.path.join(self.OUT_DIR, "shpurdp-server.out")
    self.SERVER_LOG_FILE = os.path.join(self.OUT_DIR, "shpurdp-server.log")
    self.DB_CHECK_LOG = os.path.join(self.OUT_DIR, "shpurdp-server-check-database.log")
    ...
    if properties == -1:
      self.PID_DIR = ShpurdpPath.get(
        os.sep + os.path.join("var", "run", "shpurdp-server")
      )
      self.BOOTSTRAP_DIR = ShpurdpPath.get(
        os.sep + os.path.join("var", "run", "shpurdp-server", "bootstrap")
      )
      self.RECOMMENDATIONS_DIR = ShpurdpPath.get(
        os.sep + os.path.join("var", "run", "shpurdp-server", "stack-recommendations")
      )
    else:
    ...
    self.SHPURDP_PROPERTIES_BACKUP_FILE = ""
    self.SHPURDP_ENV_BACKUP_FILE = ""
    self.SHPURDP_KRB_JAAS_LOGIN_BACKUP_FILE = ""
    ...
    # {0} in user/group will be replaced by customized shpurdp-server username
    ...
    for directory in directories:
      if not os.path.isdir(directory):
        try:
          ...
          # permission denied here is expected when shpurdp runs as non-root
          print_error_msg(f"Could not create {directory}. Reason: {str(ex)}")

      if not os.path.isdir(directory) or not os.access(directory, os.W_OK):
        raise FatalException(
          -1,
          f"Unable to access {directory} directory. Confirm the directory is created and is writable by Shpurdp Server user account '{getpass.getuser()}'",
        )
...
class ServerConfigDefaultsWindows(ServerConfigDefaults):
  def __init__(self):
    ...
    self.SHPURDP_PROPERTIES_BACKUP_FILE = "shpurdp.properties.backup"
    self.SHPURDP_KRB_JAAS_LOGIN_BACKUP_FILE = ""  # ToDo: should be adjusted later
    # ownership/permissions mapping
    # path - permissions - user - group - recursive
    # Rules are executed in the same order as they are listed
    # {0} in user/group will be replaced by customized shpurdp-server username
    ...
    self.NR_ADJUST_OWNERSHIP_LIST = [
      ...
      # Also, /etc/shpurdp-server/conf/password.dat
    ...
    # Standard messages
    self.MESSAGE_SERVER_RUNNING_AS_ROOT = (
      "Shpurdp Server running with 'root' privileges."
    )
    self.MESSAGE_WARN_SETUP_NOT_ROOT = "Shpurdp-server setup is run with root-level privileges, passwordless sudo access for some commands commands may be required"
    self.MESSAGE_ERROR_RESET_NOT_ROOT = (
      "Shpurdp-server reset must be run with administrator-level privileges"
    )
    self.MESSAGE_ERROR_UPGRADE_NOT_ROOT = (
      "Shpurdp-server upgrade must be run with administrator-level privileges"
    )
...
class ServerConfigDefaultsLinux(ServerConfigDefaults):
  def __init__(self):
    ...
    # JDK
    self.JDK_INSTALL_DIR = ShpurdpPath.get("/usr/jdk64")
    self.JDK_SEARCH_PATTERN = "jdk*"
    self.JAVA_EXE_SUBPATH = "bin/java"

    # Configuration defaults
    self.DEFAULT_CONF_DIR = ShpurdpPath.get("/etc/shpurdp-server/conf")
    self.DEFAULT_LIBS_DIR = ShpurdpPath.get("/usr/lib/shpurdp-server")
    self.DEFAULT_VLIBS_DIR = ShpurdpPath.get("/var/lib/shpurdp-server")

    self.SHPURDP_PROPERTIES_BACKUP_FILE = "shpurdp.properties.rpmsave"
    self.SHPURDP_ENV_BACKUP_FILE = "shpurdp-env.sh.rpmsave"
    self.SHPURDP_KRB_JAAS_LOGIN_BACKUP_FILE = "krb5JAASLogin.conf.rpmsave"
    # ownership/permissions mapping
    # path - permissions - user - group - recursive
    # Rules are executed in the same order as they are listed
    # {0} in user/group will be replaced by customized shpurdp-server username
    self.NR_ADJUST_OWNERSHIP_LIST = [
      ...
      (ShpurdpPath.get("/var/lib/shpurdp-server/shpurdp-env.sh"), "700", "{0}", False),
      (ShpurdpPath.get("/var/lib/shpurdp-server/shpurdp-sudo.sh"), "700", "{0}", False),
      (ShpurdpPath.get("/var/lib/shpurdp-server/keys/*"), "600", "{0}", True),
      (ShpurdpPath.get("/var/lib/shpurdp-server/keys/"), "700", "{0}", False),
      (ShpurdpPath.get("/var/lib/shpurdp-server/keys/db/"), "700", "{0}", False),
      (ShpurdpPath.get("/var/lib/shpurdp-server/keys/db/newcerts/"), "700", "{0}", False),
      (ShpurdpPath.get("/var/lib/shpurdp-server/keys/.ssh"), "700", "{0}", False),
      (
        ShpurdpPath.get("/var/lib/shpurdp-server/resources/common-services/"),
        "755",
        "{0}",
        True,
      ),
      (ShpurdpPath.get("/var/lib/shpurdp-server/resources/stacks/"), "755", "{0}", True),
      (
        ShpurdpPath.get("/var/lib/shpurdp-server/resources/extensions/"),
        "755",
        "{0}",
        True,
      ),
      (
        ShpurdpPath.get("/var/lib/shpurdp-server/resources/dashboards/"),
        "755",
        "{0}",
        True,
      ),
      (ShpurdpPath.get("/var/lib/shpurdp-server/resources/mpacks/"), "755", "{0}", True),
      (
        ShpurdpPath.get("/var/lib/shpurdp-server/resources/custom_actions/"),
        "755",
        "{0}",
        True,
      ),
      (
        ShpurdpPath.get("/var/lib/shpurdp-server/resources/host_scripts/"),
        "755",
        "{0}",
        True,
      ),
      (ShpurdpPath.get("/var/lib/shpurdp-server/resources/views/*"), "644", "{0}", True),
      (ShpurdpPath.get("/var/lib/shpurdp-server/resources/views/"), "755", "{0}", False),
      (
        ShpurdpPath.get("/var/lib/shpurdp-server/resources/views/work/"),
        "755",
        "{0}",
        True,
      ),
      (ShpurdpPath.get("/etc/shpurdp-server/conf/*"), "644", "{0}", True),
      (ShpurdpPath.get("/etc/shpurdp-server/conf/"), "755", "{0}", False),
      (ShpurdpPath.get("/etc/shpurdp-server/conf/password.dat"), "640", "{0}", False),
      (ShpurdpPath.get("/var/lib/shpurdp-server/keys/pass.txt"), "600", "{0}", False),
      (
        ShpurdpPath.get("/etc/shpurdp-server/conf/ldap-password.dat"),
        "640",
        "{0}",
        False,
      ),
      ...
      (ShpurdpPath.get("/var/lib/shpurdp-server/resources/data/"), "644", "{0}", False),
      (ShpurdpPath.get("/var/lib/shpurdp-server/resources/data/"), "755", "{0}", False),
      (ShpurdpPath.get("/var/lib/shpurdp-server/data/tmp/*"), "644", "{0}", True),
      (ShpurdpPath.get("/var/lib/shpurdp-server/data/tmp/"), "755", "{0}", False),
      (ShpurdpPath.get("/var/lib/shpurdp-server/data/cache/"), "600", "{0}", True),
      (ShpurdpPath.get("/var/lib/shpurdp-server/data/cache/"), "700", "{0}", False),
      (
        ShpurdpPath.get(
          "/var/lib/shpurdp-server/resources/common-services/STORM/0.9.1/package/files/wordCount.jar"
        ),
        "644",
        "{0}",
        False,
      ...
      (
        ShpurdpPath.get(
          "/var/lib/shpurdp-server/resources/stacks/HDP/2.1.GlusterFS/services/STORM/package/files/wordCount.jar"
        ),
        "644",
        "{0}",
        False,
      ...
      (
        ShpurdpPath.get(
          "/var/lib/shpurdp-server/resources/stack-hooks/before-START/files/fast-hdfs-resource.jar"
        ),
        "644",
        "{0}",
        False,
      ...
      (
        ShpurdpPath.get(
          "/var/lib/shpurdp-server/resources/stacks/HDP/2.1/services/SMARTSENSE/package/files/view/smartsense-shpurdp-view-1.4.0.0.60.jar"
        ),
        "644",
        "{0}",
        False,
      ...
      (
        ShpurdpPath.get(
          "/var/lib/shpurdp-server/resources/stacks/HDP/3.0/hooks/before-START/files/fast-hdfs-resource.jar"
        ),
        "644",
        "{0}",
        False,
    ...
    self.NR_CHANGE_OWNERSHIP_LIST = [
      (ShpurdpPath.get("/var/lib/shpurdp-server"), "{0}", True),
      (ShpurdpPath.get("/usr/lib/shpurdp-server"), "{0}", True),
      (self.OUT_DIR, "{0}", True),
      (self.PID_DIR, "{0}", True),
      (ShpurdpPath.get("/etc/shpurdp-server"), "{0}", True),
    ]
    self.NR_USERADD_CMD = (
      'useradd -M --comment "{1}" '
      "--shell %s "
    ...
    self.SERVER_RESOURCES_DIR = ShpurdpPath.get("/var/lib/shpurdp-server/resources")
    self.STACK_LOCATION_DEFAULT = ShpurdpPath.get(
      "/var/lib/shpurdp-server/resources/stacks"
    )
    self.EXTENSION_LOCATION_DEFAULT = ShpurdpPath.get(
      "/var/lib/shpurdp-server/resources/extensions"
    )
    self.COMMON_SERVICES_LOCATION_DEFAULT = ShpurdpPath.get(
      "/var/lib/shpurdp-server/resources/common-services"
    )
    self.MPACKS_STAGING_LOCATION_DEFAULT = ShpurdpPath.get(
      "/var/lib/shpurdp-server/resources/mpacks"
    )
    self.SERVER_TMP_DIR_DEFAULT = ShpurdpPath.get("/var/lib/shpurdp-server/data/tmp")

    self.DEFAULT_VIEWS_DIR = ShpurdpPath.get("/var/lib/shpurdp-server/resources/views")
    ...
    # Standard messages
    self.MESSAGE_SERVER_RUNNING_AS_ROOT = (
      "Shpurdp Server running with administrator privileges."
    )
    self.MESSAGE_WARN_SETUP_NOT_ROOT = "Shpurdp-server setup is run with root-level privileges, passwordless sudo access for some commands commands may be required"
    self.MESSAGE_ERROR_RESET_NOT_ROOT = (
      "Shpurdp-server reset should be run with root-level privileges"
    )
    self.MESSAGE_ERROR_UPGRADE_NOT_ROOT = (
      "Shpurdp-server upgrade must be run with root-level privileges"
    )
    self.MESSAGE_CHECK_FIREWALL = "Checking firewall status..."
...
SECURITY_KEY_ENV_VAR_NAME = "SHPURDP_SECURITY_MASTER_KEY"
...
SECURITY_PROVIDER_GET_CMD = (
  "{0} -cp {1} "
  + "org.apache.shpurdp.server.security.encryption"
...
SECURITY_SENSITIVE_DATA_ENCRYPTON_CMD = (
  "{0} -cp {1} "
  + "org.apache.shpurdp.server.security.encryption.SensitiveDataEncryption"
  + " {2} "
  + "> "
  + configDefaults.SERVER_OUT_FILE
  + " 2>&1"
...
def read_shpurdp_user():
  """
  Reads shpurdp user from properties file
  """
  properties = get_shpurdp_properties()
  if properties != -1:
...
def get_admin_views_dir(properties):
  ...
  if views_dir is None or views_dir == "":
    views_dirs = glob.glob(
      ShpurdpPath.get("/var/lib/shpurdp-server/resources/views/work/ADMIN_VIEW*")
    )
  else:
...
def get_views_jars(properties):
  ...
  if views_dir is None or views_dir == "":
    views_jars = glob.glob(
      ShpurdpPath.get("/var/lib/shpurdp-server/resources/views/*.jar")
    )
  else:
...
def get_credential_store_location(properties):
  ...
  if store_loc is None or store_loc == "":
    store_loc = ShpurdpPath.get("/var/lib/shpurdp-server/keys/credentials.jceks")
  else:
...
def get_shpurdp_server_ui_port(properties):
  shpurdp_server_ui_port = CLIENT_API_PORT
  ...
  if api_ssl and str(api_ssl).lower() == "true":
    shpurdp_server_ui_port = DEFAULT_SSL_API_PORT
    ssl_api_port = properties.get_property(SSL_API_PORT)
    if ssl_api_port:
      shpurdp_server_ui_port = ssl_api_port
  return shpurdp_server_ui_port
...
def backup_file_in_temp(filePath):
  if filePath is not None:
    ...
    back_up_file_count = len(glob.glob1(tmpDir, SHPURDP_PROPERTIES_FILE + "*"))
    try:
      shutil.copyfile(
        filePath,
        tmpDir + os.sep + SHPURDP_PROPERTIES_FILE + "." + str(back_up_file_count + 1),
      )
    except Exception as e:
      print_error_msg(
...
def get_shpurdp_version(properties):
  """
  :param properties: Shpurdp properties
  :return: Return a string of the shpurdp version. When comparing versions, please use "compare_versions" function.
  """
  version = None
  try:
    ...
    print_error_msg("Error getting shpurdp version")
  return version
...
def check_database_name_property(upgrade=False):
  """
  :param upgrade: If Shpurdp is being upgraded.
  :return:
  """
  properties = get_shpurdp_properties()
  if properties == -1:
  ...
  version = get_shpurdp_version(properties)
  if upgrade and (
    properties[JDBC_DATABASE_PROPERTY] not in ServerDatabases.databases()
    or JDBC_RCA_SCHEMA_PROPERTY in properties
  ):
    # This code exists for historic reasons in which property names changed from Shpurdp 1.6.1 to 1.7.0
    persistence_type = properties[PERSISTENCE_TYPE_PROPERTY]
    if persistence_type == "remote":
      db_name = properties[
        JDBC_RCA_SCHEMA_PROPERTY
      ]  # this was a property in Shpurdp 1.6.1, but not after 1.7.0
      if db_name:
      ...
      # Shpurdp 1.6.1, had "server.jdbc.database" as the DB name, and the
...
def update_database_name_property(upgrade=False):
  try:
    ...
    if properties == -1:
      ...
      raise FatalException(-1, err)
    print_warning_msg(
      f"{JDBC_DATABASE_NAME_PROPERTY} property isn't set in {SHPURDP_PROPERTIES_FILE} . Setting it to default value - {configDefaults.DEFAULT_DB_NAME}"
    )
    ...
    try:
      ...
      err = f'Could not write shpurdp config file "{conf_file}": {e}'
      raise FatalException(-1, err)
...
def read_passwd_for_alias(alias, masterKey="", options=None):
  if alias:
    ...
    serverClassPath = shpurdp_server.serverClassPath.ServerClassPath(
      get_shpurdp_properties(), None
    )
...
def store_password_file(password, filename):
  ...
  if shpurdp_user:  # at the first install shpurdp_user can be None. Which is fine since later on password.dat is chowned with the correct ownership.
    set_file_permissions(passFilePath, "660", shpurdp_user, False)

  # Windows paths need double backslashes, otherwise the Shpurdp server deserializer will think the single \ are escape markers
...
def get_web_server_startup_timeout(properties):
  ...
  error code even though Shpurdp is actually starting up.
  :param properties:
...
def update_krb_jaas_login_properties():
  ...
  prev_conf_file = search_file(
    configDefaults.SHPURDP_KRB_JAAS_LOGIN_BACKUP_FILE, get_conf_dir()
  )
  conf_file = search_file(SHPURDP_KRB_JAAS_LOGIN_FILE, get_conf_dir())
  ...
  try:
    ...
    print_warning_msg(f"Original file {SHPURDP_KRB_JAAS_LOGIN_FILE} kept")
  except OSError as e:
...
def update_shpurdp_env():
  prev_env_file = search_file(
    configDefaults.SHPURDP_ENV_BACKUP_FILE, configDefaults.DEFAULT_VLIBS_DIR
  )
  env_file = search_file(SHPURDP_ENV_FILE, configDefaults.DEFAULT_VLIBS_DIR)
  ...
  if (not prev_env_file) or (prev_env_file is None):
    print(
      ...
      % (configDefaults.SHPURDP_ENV_BACKUP_FILE, SHPURDP_ENV_FILE)
    )
    return 0
  ...
  try:
    if env_file is not None:
      ...
      print(f"INFO: Original file {SHPURDP_ENV_FILE} kept")
  except OSError as e:
...
def update_shpurdp_properties():
  prev_conf_file = search_file(
    configDefaults.SHPURDP_PROPERTIES_BACKUP_FILE, get_conf_dir()
  )
  conf_file = search_file(SHPURDP_PROPERTIES_FILE, get_conf_dir())
  ...
  if (not prev_conf_file) or (prev_conf_file is None):
    print_warning_msg(
      f"Can not find {configDefaults.SHPURDP_PROPERTIES_BACKUP_FILE} file from previous version, skipping import of settings"
    )
    return 0

  # shpurdp.properties file does not exists
  if conf_file is None:
    print_error_msg(f"Can't find {SHPURDP_PROPERTIES_FILE} file")
    return -1
  ...
  try:
    ...
    for prop_key, prop_value in old_properties.getPropertyDict().items():
      prop_value = prop_value.replace(
        "/usr/lib/python2.6/site-packages", "/usr/lib/shpurdp-server/lib"
      )
      if "agent.fqdn.service.url" == prop_key:
        # what is agent.fqdn property in shpurdp.props?
...
def write_property(key, value):
  ...
  try:
    ...
    print_error_msg(f'Could not read shpurdp config file "{conf_file}": {e}')
    return -1
  properties.process_pair(key, value)
  try:
    with open(conf_file, "w") as hfW:
    ...
    print_error_msg(f'Could not write shpurdp config file "{conf_file}": {e}')
    return -1
  return 0
...
def update_debug_mode():
  ...
  # The command-line settings supersede the ones in shpurdp.properties
  if not debug_mode & 1:
    properties = get_shpurdp_properties()
    if properties == -1:
      print_error_msg("Error getting shpurdp properties")
...
def get_JAVA_HOME(java_home_type=JavaHomeType.SHPURDP):
  properties = get_shpurdp_properties()
  if properties == -1:
    print_error_msg("Error getting shpurdp properties")
    return None
...
#
def find_jdk(java_home_type=JavaHomeType.SHPURDP):
...
def get_java_exe_path(java_home_type=JavaHomeType.SHPURDP):
