...
from shpurdp_commons.shpurdp_service import ShpurdpService
from shpurdp_commons.logging_utils import set_silent, set_verbose, print_info_msg
from shpurdp_commons.os_utils import remove_file
from shpurdp_commons.os_windows import SvcStatusCallback
from shpurdp_server.serverConfiguration import (
  ...
  PID_NAME,
  SERVER_OUT_FILE_KEY,
  SUSPEND_START_MODE_KEY,
  VERBOSE_OUTPUT_KEY,
)


class ShpurdpServerService(ShpurdpService):
  ShpurdpService._svc_name_ = "Shpurdp Server"
  ShpurdpService._svc_display_name_ = "Shpurdp Server"
  ShpurdpService._svc_description_ = "Shpurdp Server"
...
def ctrlHandler(ctrlType):
  ShpurdpServerService.DefCtrlCHandler()
  return True
...
def svcsetup(register_service, username=None, password=None):
  ...
  classPath = scriptFile + "." + ShpurdpServerService.__name__
  ...
  if register_service or ShpurdpServerService.QueryStatus() == "not installed":
    return ShpurdpServerService.Install(
      classPath=classPath, username=username, password=password
    )
  else:
    return 0
