...
if OSCheck.is_windows_family():
  from shpurdp_commons.os_utils import run_os_command, run_in_shell
else:
  from resource_management.core.shell import quote_bash_args

SHPURDP_PASSPHRASE_VAR_NAME = "SHPURDP_PASSPHRASE"
...
POLL_INTERVAL_SEC = 1
DEBUG = False
DEFAULT_AGENT_TEMP_FOLDER = "/var/lib/shpurdp-agent/tmp"
DEFAULT_AGENT_DATA_FOLDER = "/var/lib/shpurdp-agent/data"
DEFAULT_AGENT_LIB_FOLDER = "/var/lib/shpurdp-agent"
PYTHON_ENV = "env PYTHONPATH=$PYTHONPATH:" + DEFAULT_AGENT_TEMP_FOLDER
SERVER_SHPURDP_SUDO = (
  os.getenv("ROOT", "/").rstrip("/") + "/var/lib/shpurdp-server/shpurdp-sudo.sh"
)
CREATE_PYTHON_WRAP_SCRIPT = (
  os.getenv("ROOT", "/").rstrip("/") + "/var/lib/shpurdp-server/create-python-wrap.sh"
)
REMOTE_CREATE_PYTHON_WRAP_SCRIPT = os.path.join(
  DEFAULT_AGENT_TEMP_FOLDER, "create-python-wrap.sh"
)
SHPURDP_SUDO = os.path.join(DEFAULT_AGENT_TEMP_FOLDER, "shpurdp-sudo.sh")
...
class Bootstrap(threading.Thread):
  ...
  def __init__(self, host, shared_state):
    ...
    if OSCheck.is_ubuntu_family():
      self.SHPURDP_REPO_FILENAME = self.SHPURDP_REPO_FILENAME + ".list"
    else:
      self.SHPURDP_REPO_FILENAME = self.SHPURDP_REPO_FILENAME + ".repo"
  ...
  def getShpurdpVersion(self):
    shpurdp_version = self.shared_state.shpurdp_version
    if shpurdp_version is None or shpurdp_version == "null":
      return ""
    else:
...
class BootstrapWindows(Bootstrap):
  ...
  def getTempFolder(self):
    ...
    return os.path.join(
      installationDrive, os.sep, "var", "temp", "bootstrap", self.getShpurdpVersion()
    )
  ...
  def getRunSetupCommand(self, expected_hostname):
    ...
    passphrase = os.environ[SHPURDP_PASSPHRASE_VAR_NAME]
    user_run_as = self.shared_state.user_run_as
    server = self.shared_state.shpurdp_server
    version = self.getShpurdpVersion()
    return " ".join(
      [
        "python3",
        setupFile,
...
class BootstrapDefault(Bootstrap):
  shpurdp_commons = "/usr/lib/shpurdp-server/lib/shpurdp_commons"
  ...
  def getRepoFile(self):
    """Shpurdp repo file for Shpurdp."""
    return os.path.join(self.getRepoDir(), self.SHPURDP_REPO_FILENAME)
  ...
  def copyShpurdpSudo(self):
    # Copying the os check script file
    fileToCopy = SERVER_SHPURDP_SUDO
    target = self.TEMP_FOLDER
    params = self.shared_state
    self.host_log.write("==========================\n")
    self.host_log.write("Copying shpurdp sudo script...")
    scp = SCP(
      params.user,
      params.sshPort,
      params.sshkey_file,
  ...
  def copyCommonFunctions(self):
    # Copying the os check script file
    fileToCopy = self.shpurdp_commons
  ...
  def getMoveRepoFileWithPasswordCommand(self, targetDir):
    return (
      f"{SHPURDP_SUDO} -S mv "
      + str(self.getRemoteName(self.SHPURDP_REPO_FILENAME))
      + " "
      + os.path.join(str(targetDir), self.SHPURDP_REPO_FILENAME)
      + " < "
  ...
  def getAptUpdateCommand(self):
    return (
      ...
      % (SHPURDP_SUDO, "sources.list.d", self.SHPURDP_REPO_FILENAME)
    )

  def getRepoFileChmodCommand(self):
    return f"{SHPURDP_SUDO} chmod 644 {self.getRepoFile()}"
  ...
  def copyNeededFiles(self):
    ...
    target = self.getRemoteName(self.SHPURDP_REPO_FILENAME)
    ...
    if os.path.exists(fileToCopy):
      ...
      # Change permissions on shpurdp.repo
      self.host_log.write("==========================\n")
      self.host_log.write("Changing permissions for shpurdp.repo...")
      ...
      self.host_log.write(f"Shpurdp repo file not found: {self.getRepoFile()}")
      retcode = -1
      pass
  ...
  def getRunSetupWithPasswordCommand(self, expected_hostname):
    ...
    user_run_as = self.shared_state.user_run_as
    version = self.getShpurdpVersion()
    port = self.getShpurdpPort()
    passwordFile = self.getPasswordFile()
    return (
      f"{SHPURDP_SUDO} -S python3 "
      + str(setupFile)
      + " "
  ...
  def runCreatePythonWrapScript(self):
    ...
    command = "chmod a+x {script} && {sudo} {script}".format(
      sudo=SHPURDP_SUDO, script=REMOTE_CREATE_PYTHON_WRAP_SCRIPT
    )
...
class SharedState:
  def __init__(
    ...
    self.shpurdp_server = shpurdp_server
    self.cluster_os_type = cluster_os_type
    self.shpurdp_version = shpurdp_version
...
def main(argv=None):
  ...
  if len(onlyargs) < 3:
    sys.stderr.write(
      ...
      " <shpurdp-server name> <cluster os type> <shpurdp version> <shpurdp port> <user_run_as> <passwordFile>\n"
    )
    sys.exit(2)
    pass
  ...
  shpurdpServer = onlyargs[6]
  cluster_os_type = onlyargs[7]
  shpurdpVersion = onlyargs[8]
