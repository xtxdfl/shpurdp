...
class Configurator:
  ...
  RESULT_FILE_PATH = "shpurdp-server-state.xml"
  configurationType = 0
  ...
  def getStackInformation(self):
    ...
    self.configurations.stack.comment = (
      stack.getElementsByTagName("comment")[0].childNodes[0].data
    )
    ...
    for service in services:
      ...
      serviceStructure.version = (
        service.getElementsByTagName("version")[0].childNodes[0].data
      )
      serviceStructure.comment = (
        service.getElementsByTagName("comment")[0].childNodes[0].data
      )
      serviceStructure.user = service.getElementsByTagName("user")[0].childNodes[0].data
      serviceStructure.enabled = (
        service.getElementsByTagName("enabled")[0].childNodes[0].data
      )
    ...
    for repositiry in repositories:
      self.configurations.stack.repository.comment = (
        repositiry.getElementsByTagName("comment")[0].childNodes[0].data
      )
  ...
  def getConfiguration(self):
    ...
    namenodeJvmOpts = (
      env.getElementsByTagName("namenode-jvm-opts")[0].childNodes[0].data
    )
    ...
    fsDefaultName = (
      coreSite[0].getElementsByTagName("fs-default-name")[0].childNodes[0].data
    )
    coreSiteResultStructure.fsDefaultName = fsDefaultName

    hadoopSecurityAuthentication = (
      coreSite[0]
      .getElementsByTagName("hadoop-security-authentication")[0]
      .childNodes[0]
      .data
    )
    ...
    for property in properties:
      ...
      if name == "fs.default.name":
        self.configurations.configuration.coreSite.fsDefaultName = (
          property.getElementsByTagName("value")[0].childNodes[0].data
        )
      if name == "hadoop.security.authentication":
        self.configurations.configuration.coreSite.hadoopSecurityAuthentication = (
          property.getElementsByTagName("value")[0].childNodes[0].data
        )
    ...
    if self.configurationType != 0:
      for service in self.servicesPath.keys():
        path = input(
          ...
          + service
          + '(if there is no such service type "no") :'
        )
        if len(path) > 0 and not path == "no":
          ...
          raise ValueError(
            "Path to the configuration file can't be empty."
          )  # Catch it layter and start input mode automatically
