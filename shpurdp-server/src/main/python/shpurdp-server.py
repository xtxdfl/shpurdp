...
from shpurdp_commons.os_utils import remove_file
from optparse import OptionValueError

from shpurdp_server.BackupRestore import main as BackupRestore_main
from shpurdp_server.checkDatabase import check_database
from shpurdp_server.dbCleanup import db_purge
from shpurdp_server.dbConfiguration import DATABASE_NAMES, LINUX_DBMS_KEYS_LIST
from shpurdp_server.enableStack import enable_stack_version
from shpurdp_server.hostUpdate import update_host_names
from shpurdp_server.kerberos_setup import setup_kerberos
from shpurdp_server.serverConfiguration import (
  configDefaults,
  get_shpurdp_properties,
  PID_NAME,
)
from shpurdp_server.serverSetup import reset, setup, setup_jce_policy, setup_jdbc
from shpurdp_server.serverUpgrade import upgrade, set_current
from shpurdp_server.serverUtils import (
  is_server_runing,
  refresh_stack_hash,
  wait_for_server_to_stop,
)
from shpurdp_server.setupActions import (
  BACKUP_ACTION,
  LDAP_SETUP_ACTION,
  LDAP_SYNC_ACTION,
  PSTART_ACTION,
...
from shpurdp_server.setupHttps import setup_https, setup_truststore
...
from shpurdp_server.setupSso import setup_sso
from shpurdp_server.setupTrustedProxy import setup_trusted_proxy
from shpurdp_server.userInput import get_validated_string_input
from shpurdp_server_main import server_process_main
...
#
# Starts the Shpurdp Server as a standalone process.
...
def start(options):
  from shpurdp_windows_service import ShpurdpServerService, ctrlHandler

  status, pid = is_server_runing()
  if status:
    err = "Shpurdp Server is already running."
    raise FatalException(1, err)

  ShpurdpServerService.set_ctrl_c_handler(ctrlHandler)
...
#
# Starts the Shpurdp Server.
# Ensures only one instance of the process is running.
#     If this is the second instance of the process, the function fails.
#
@OsFamilyFuncImpl(OsFamilyImpl.DEFAULT)
def start(args):
  logger.info("Starting shpurdp-server.")
  status, pid = is_server_runing()
  if status:
    err = "Shpurdp Server is already running."
    raise FatalException(1, err)
  ...
  logger.info("Started shpurdp-server.")


#
# Starts the Shpurdp Server as a service.
# Start the server as a Windows service. If the Shpurdp server is
...
def svcstart():
  ...
  ShpurdpServerService.Start()
  pass


#
# Stops the Shpurdp Server service.
#
@OsFamilyFuncImpl(OSConst.WINSRV_FAMILY)
def stop():
  from shpurdp_windows_service import ShpurdpServerService

  ShpurdpServerService.Stop()


#
# Stops the Shpurdp Server.
#
@OsFamilyFuncImpl(OsFamilyImpl.DEFAULT)
def stop(args):
  logger.info("Stopping shpurdp-server.")
  if args != None:
  ...
  if status:
    try:
      ...
      err = "Unable to stop Shpurdp Server - " + str(e)
      print_info_msg(err)
      raise FatalException(1, err)
    ...
    if not wait_for_server_to_stop(SERVER_STOP_TIMEOUT):
      err = "Shpurdp-server failed to stop gracefully. Sending SIGKILL to it"
      print(err)
      logger.error(err)
    ...
    print("Shpurdp Server stopped")
    logger.info("Shpurdp Server stopped")
  else:
    print("Shpurdp Server is not running")
    logger.info("Shpurdp Server is not running")


#
# Restarts the Shpurdp Server.
#
@OsFamilyFuncImpl(OsFamilyImpl.DEFAULT)
def restart(args):
  logger.info("Restarting shpurdp-server.")
  stop(args)
  start(args)
...
def status(args):
  ...
  print("Shpurdp Server is " + statusStr)
...
@OsFamilyFuncImpl(OsFamilyImpl.DEFAULT)
def status(args):
  logger.info("Get status of shpurdp-server.")
  args.exit_message = None
  status, pid = is_server_runing()
  pid_file_path = os.path.join(configDefaults.PID_DIR, PID_NAME)
  if status:
    args.exit_code = 0
    print("Shpurdp Server running")
    print("Found Shpurdp Server PID: " + str(pid) + " at: " + pid_file_path)
  else:
    if os.path.exists(pid_file_path):
      print("Shpurdp Server not running. Stale PID File at: " + pid_file_path)
    else:
      print("Shpurdp Server not running.")
    args.exit_code = 3
...
def create_setup_security_actions(args):
  action_list = [
    [
      "setup-https",
      "Enable HTTPS for Shpurdp server.",
      UserActionRestart(setup_https, args),
    ],
    [
      "encrypt-passwords",
      "Encrypt passwords managed by Shpurdp.",
      UserAction(setup_sensitive_data_encryption, args),
    ],
    [
      "setup-kerberos-jaas",
      "Setup Shpurdp kerberos JAAS configuration.",
      UserAction(setup_shpurdp_krb5_jaas, args),
    ],
    [
      "setup-truststore",
      "Setup truststore.",
...
def init_action_parser(action, parser):
  parser.add_option(
    ...
    dest="svc_user",
    default=None,
    help="User account under which the Shpurdp Server service will run",
  )
  parser.add_option(
    "-x",
    ...
    dest="svc_password",
    default=None,
    help="Password for the Shpurdp Server service user account",
  )
  ...
  parser.add_option(
    ...
    default="resources" + os.sep + "Shpurdp-DDL-SQLServer-CREATE.sql",
  ...
  parser.add_option(
    ...
    default="resources" + os.sep + "Shpurdp-DDL-SQLServer-DROP.sql",
  ...
  parser.add_option(
    ...
    dest="debug",
    default=False,
    help="Start shpurdp-server in debug mode",
  )
  parser.add_option(
    "-y",
    "--suspend-start",
    ...
    dest="suspend_start",
    default=False,
    help="Freeze shpurdp-server Java process at startup in debug mode",
  )
...
def init_setup_parser_options(parser):
  ...
  other_group.add_option(
    "--shpurdp-java-home",
    dest="shpurdp_java_home",
    help="Use specified java_home for shpurdp.  Must be valid on Shpurdp server hosts",
  )
  other_group.add_option(
    "--stack-java-home",
  ...
  # the --master-key option is needed in the event passwords in the shpurdp.properties file are encrypted
  other_group.add_option(
    "--master-key",
    default=None,
...
def init_ldap_sync_parser_options(parser):
  ...
  parser.add_option(
    ...
    action="store_true",
    default=False,
    help="LDAP sync existing option.  Synchronize existing Shpurdp users and groups only.",
...
def init_ldap_setup_parser_options(parser):
  ...
  parser.add_option(
    "--shpurdp-admin-username",
    default=None,
    help="Shpurdp administrator username for accessing Shpurdp's REST API",
    dest="shpurdp_admin_username",
  )
  parser.add_option(
    "--shpurdp-admin-password",
    default=None,
    help="Shpurdp administrator password for accessing Shpurdp's REST API",
    dest="shpurdp_admin_password",
  )
  parser.add_option(
    "--truststore-type",
    default=None,
  ...
  parser.add_option(
    "--ldap-enabled-shpurdp",
    default=None,
    help="Indicates whether to enable/disable LDAP authentication for Shpurdp, itself",
    dest="ldap_enabled_shpurdp",
  )
  parser.add_option(
    "--ldap-manage-services",
    default=None,
    help="Indicates whether Shpurdp should manage the LDAP configurations for specified services",
...
def init_setup_sso_options(parser):
  ...
  parser.add_option(
    "--sso-enabled-shpurdp",
    default=None,
    help="Indicates whether to enable/disable SSO authentication for Shpurdp, itself",
    dest="sso_enabled_shpurdp",
  )
  parser.add_option(
    "--sso-manage-services",
    default=None,
    help="Indicates whether Shpurdp should manage the SSO configurations for specified services",
...
def init_setup_security_parser_options(parser):
  ...
  https_group.add_option(
    "--api-ssl",
    default=None,
    help="Enable SSL for Shpurdp API [true/false]",
    dest="api_ssl",
  )
  https_group.add_option(
  ...
  setup_kerberos_jaas_group.add_option(
    "--jaas-principal",
    default=None,
    help="Kerberos principal for shpurdp server",
    dest="jaas_principal",
  )
  setup_kerberos_jaas_group.add_option(
    "--jaas-keytab",
...
def are_cmd_line_db_args_valid(options):
  if (
    ...
    # and options.database_name is not None \         # shpurdp by default is ok
...
def init_action_parser(action, parser):
  ...
  parser.add_option(
    ...
    dest="silent",
    default=False,
    help="Silently accepts default prompt values. For db-cleanup command, silent mode will stop shpurdp server.",
  )
  try:
...
def init_logging():
  ...
  python_log_name = "shpurdp-server-command.log"
...
def main(options, args, parser):
  ...
  options.exit_message = f"Shpurdp Server '{action}' completed successfully."
  ...
  try:
    if action in _action_option_dependence_map:
      ...
      for opt_str, opt_dest in required:
        if hasattr(options, opt_dest) and getattr(options, opt_dest) is None:
          ...
          print(
            "Run shpurdp-server.py --help to see detailed description of each option"
          )
    ...
    if options.warnings:
      for warning in options.warnings:
        ...
        pass
      options.exit_message = f"Shpurdp Server '{action}' completed with warnings."
      pass
  ...
  if options.verbose:
    ...
    try:
      ...
      print_error_msg(
        ...
        + "\nFor more info run shpurdp-server with -v or --verbose option"
      )
      sys.exit(1)
...
def enable_stack(options, args):
  ...
  if retcode == 0:
    status, pid = is_server_runing()
    if status:
      print("restarting shpurdp server")
      stop(options)
      start(options)
