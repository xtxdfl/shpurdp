...
from shpurdp_commons.os_utils import is_root
...
from shpurdp_server.serverUtils import refresh_stack_hash
from shpurdp_server.setupHttps import get_fqdn
...
# server commands
shpurdp_provider_module_option = ""
shpurdp_provider_module = os.environ.get("SHPURDP_PROVIDER_MODULE")
if shpurdp_provider_module is not None:
  shpurdp_provider_module_option = (
    "-Dprovider.module.class=" + shpurdp_provider_module + " "
  )

jvm_args = os.getenv("SHPURDP_JVM_ARGS", "-Xms512m -Xmx2048m")
ENV_FOREGROUND_KEY = "SHPURDP_SERVER_RUN_IN_FOREGROUND"
...
SERVER_START_CMD = (
  ...
  "-cp {3} "
  "org.apache.shpurdp.server.controller.ShpurdpServer "
  "> {4} 2>&1 || echo $? > {5}"
)
SERVER_START_CMD_DEBUG = (
  ...
  "server=y,suspend={6} "
  "-cp {3} " + "org.apache.shpurdp.server.controller.ShpurdpServer "
  "> {4} 2>&1 || echo $? > {5}"
)
...
SERVER_SEARCH_PATTERN = "org.apache.shpurdp.server.controller.ShpurdpServer"

EXITCODE_NAME = "shpurdp-server.exitcode"

CHECK_DATABASE_SKIPPED_PROPERTY = "check_database_skipped"

SHPURDP_SERVER_DIE_MSG = (
  "Shpurdp Server java process died with exitcode {0}. Check {1} for more information."
)
SHPURDP_SERVER_NOT_STARTED_MSG = (
  "Shpurdp Server java process hasn't been started or can't be determined."
)
SHPURDP_SERVER_STOPPED = (
  "Shpurdp Server java process has stopped. Please check the logs for more information."
)
SHPURDP_SERVER_UI_TIMEOUT = (
  "Server not yet listening on http port {0} after {1} seconds. Exiting."
)
SHPURDP_SERVER_STARTED_SUCCESS_MSG = "Shpurdp Server has started successfully"

# linux open-file limit
ULIMIT_OPEN_FILES_KEY = "ulimit.open.files"
ULIMIT_OPEN_FILES_DEFAULT = 65536

SHPURDP_ENV_FILE = ShpurdpPath.get("/var/lib/shpurdp-server/shpurdp-env.sh")
...
def generate_child_process_param_list(
  shpurdp_user, java_exe, class_path, debug_start, suspend_mode
):
  conf_dir = class_path
  ...
  command = command_base.format(
    java_exe, shpurdp_provider_module_option, jvm_args, conf_dir, suspend_mode
  )
  return command
...
def generate_child_process_param_list(
  ...
  from shpurdp_commons.os_linux import ULIMIT_CMD
  ...
  if is_root() and shpurdp_user != "root":
    # To inherit exported environment variables (especially SHPURDP_PASSPHRASE),
    ...
    cmd = "{ulimit_cmd} ; {su} {shpurdp_user} -s {sh_shell} -c '. {shpurdp_env_file} && {command}'".format(
      ulimit_cmd=ulimit_cmd,
      su=locate_file("su", "/bin"),
      shpurdp_user=shpurdp_user,
      sh_shell=locate_file("sh", "/bin"),
      command=command,
      shpurdp_env_file=SHPURDP_ENV_FILE,
    )
  else:
...
def wait_for_server_start(pidFile, scmStatus):
  ...
  exception = None
  if pid:
    shpurdp_server_ui_port = get_shpurdp_server_ui_port(properties)
    web_server_startup_timeout = get_web_server_startup_timeout(properties)
    waitStart = time.time()
    if not wait_for_ui_start(
      int(shpurdp_server_ui_port), pid, web_server_startup_timeout
    ):
      ...
      if waitTime < web_server_startup_timeout:
        exception = FatalException(-1, SHPURDP_SERVER_STOPPED)
      # UI didn't come up on time
      else:
        exception = FatalException(
          1,
          SHPURDP_SERVER_UI_TIMEOUT.format(
            shpurdp_server_ui_port, web_server_startup_timeout
          ),
        )
  elif get_live_pids_count(pids) <= 0:
    exitcode = check_exitcode(os.path.join(configDefaults.PID_DIR, EXITCODE_NAME))
    exception = FatalException(
      -1, SHPURDP_SERVER_DIE_MSG.format(exitcode, configDefaults.SERVER_OUT_FILE)
    )
  else:
    exception = FatalException(-1, SHPURDP_SERVER_NOT_STARTED_MSG)

  if os.path.isfile(configDefaults.SERVER_OUT_FILE):
    if "DB_CHECK_ERROR" in open(configDefaults.SERVER_OUT_FILE).read():
      print(
        '\nDB configs consistency check failed. Run "shpurdp-server start --skip-database-check" to skip. '
...
def server_process_main(options, scmStatus=None):
  ...
  logger.info("Shpurdp server properties config:")
  ...
  if not is_active_instance:
    print_warning_msg(
      "This instance of shpurdp server is not designated as active. Cannot start shpurdp server."
    )
  ...
  print_info_msg("Shpurdp Server is not running...")

  jdk_path = find_jdk()
  if jdk_path is None:
    err = (
      'No JDK found, please run the "shpurdp-server setup" '
  ...
  if options.skip_database_check:
    ...
    print(
      "Shpurdp Server is starting with the database consistency check skipped. Do not make any changes to your cluster "
    ...
    print("Shpurdp database consistency check started...")
