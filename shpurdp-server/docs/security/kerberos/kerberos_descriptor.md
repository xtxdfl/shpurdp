...
The Kerberos Descriptor is a JSON-formatted text file containing information needed by Shpurdp to enable
...
Shpurdp smoke test user, which is used by all services to perform service check operations. Service-
...
If `cluster-env/smokuser` was `"shpurdp-qa"` and realm was `"EXAMPLE.COM"`, the above block would
...
previous versions of Shpurdp, it may be a reference identifier to some other identity in the
Kerberos Descriptor. This feature is deprecated and may not be available in future versions of Shpurdp.
...
indicates to Shpurdp how to handle the principal and relevant keytab file reguarding the user interface
...
```
{
  ...
  "identities": [
    ...
    },
    {
      ...
      // (typically shpurdp-qa) and users in the cluster’s default
