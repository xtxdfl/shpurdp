...
Enabling Kerberos on the cluster may be done using the _Enable Kerberos Wizard_ within the Shpurdp UI
or using the REST API.

<a name="the-enable-kerberos-wizard"></a>
### The Enable Kerberos Wizard

The _Enable Kerberos Wizard_, in the Shpurdp UI, provides an easy to use wizard interface that walks
through the process of enabling Kerberos.

<a name="the-rest-api"></a>
### The REST API

It is possible to enable Kerberos using Shpurdp's REST API using the following API calls:
...
- Change the authentication credentials as needed
  ...
  - The examples below use
    - username: admin
    - password: admin
- Change the Shpurdp server host name and port as needed
...
```
curl -H "X-Requested-By:shpurdp" -u admin:admin -i -X POST http://SHPURDP_SERVER:8080/api/v1/clusters/CLUSTER_NAME/services/KERBEROS
```

#### Add the KERBEROS_CLIENT component to the KERBEROS service

```
curl -H "X-Requested-By:shpurdp" -u admin:admin -i -X POST http://SHPURDP_SERVER:8080/api/v1/clusters/CLUSTER_NAME/services/KERBEROS/components/KERBEROS_CLIENT
```

####  Create and set KERBEROS service configurations

```
curl -H "X-Requested-By:shpurdp" -u admin:admin -i -X PUT -d @./payload http://SHPURDP_SERVER:8080/api/v1/clusters/CLUSTER_NAME
```
...
```
curl -H "X-Requested-By:shpurdp" -u admin:admin -i -X POST -d '{"host_components" : [{"HostRoles" : {"component_name":"KERBEROS_CLIENT"}}]}' http://SHPURDP_SERVER:8080/api/v1/clusters/CLUSTER_NAME/hosts?Hosts/host_name=HOST_NAME
```

#### Install the KERBEROS service and components

```
curl -H "X-Requested-By:shpurdp" -u admin:admin -i -X PUT -d '{"ServiceInfo": {"state" : "INSTALLED"}}' http://SHPURDP_SERVER:8080/api/v1/clusters/CLUSTER_NAME/services/KERBEROS
```

#### Stop all services

```
curl -H "X-Requested-By:shpurdp" -u admin:admin -i -X PUT -d  '{"RequestInfo":{"context":"Stop Service"},"Body":{"ServiceInfo":{"state":"INSTALLED"}}}' http://SHPURDP_SERVER:8080/api/v1/clusters/CLUSTER_NAME/services
```

#### Get the default Kerberos Descriptor

```
curl -H "X-Requested-By:shpurdp" -u admin:admin -i -X GET http://SHPURDP_SERVER:8080/api/v1/stacks/STACK_NAME/versions/STACK_VERSION/artifacts/kerberos_descriptor
```

#### Get the customized Kerberos Descriptor (if previously set)

```
curl -H "X-Requested-By:shpurdp" -u admin:admin -i -X GET http://SHPURDP_SERVER:8080/api/v1/clusters/CLUSTER_NAME/artifacts/kerberos_descriptor
```

#### Set the Kerberos Descriptor

```
curl -H "X-Requested-By:shpurdp" -u admin:admin -i -X POST -d @./payload http://SHPURDP_SERVER:8080/api/v1/clusters/CLUSTER_NAME/artifacts/kerberos_descriptor
```
...
```
curl -H "X-Requested-By:shpurdp" -u admin:admin -i -X POST -d @./payload http://SHPURDP_SERVER:8080/api/v1/clusters/CLUSTER_NAME/credentials/kdc.admin.credential
```
...
if Shpurdp's credential store has been previously setup.
...
```
curl -H "X-Requested-By:shpurdp" -u admin:admin -i -X PUT -d '{"ServiceInfo": {"state" : "STARTED"}}' http://SHPURDP_SERVER:8080/api/v1/clusters/CLUSTER_NAME/services
```
...
When enabling Kerberos using an Active Directory, Shpurdp must use an internal mechanism to build 
the keytab files. This is because keytab files cannot be requested remotely from an Active Directory. 
In order to create keytab files, Shpurdp needs to know the password for each relevant Kerberos 
identity.  Therefore, Shpurdp sets or updates the identity's password as needed.

The password for each Shpurdp-managed account in an Active Directory is randomly generated and
