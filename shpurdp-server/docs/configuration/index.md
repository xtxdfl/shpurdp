...
| --- | --- | --- |
| active.instance | Indicates whether the current shpurdp server instance is active or not. |`true` | 
| addservice.hostgroup.strategy | Fully qualified class name of the strategy used to form host groups for add service request layout recommendation. |`org.apache.shpurdp.server.topology.addservice.GroupByComponentsStrategy` | 
| agent.api.acceptor.count | Count of acceptors to configure for the jetty connector used for Shpurdp agent. | | 
| agent.api.gzip.compression.enabled | Determiens whether communication with the Shpurdp Agents should have the JSON payloads compressed with GZIP. |`true` | 
| agent.auto.cache.update | Determines whether the agents will automatically attempt to download updates to stack resources from the Shpurdp Server. |`true` | 
| agent.check.mounts.timeout | The timeout, used by the `timeout` command in linux, when checking mounts for free capacity. |`0` | 
| agent.check.remote.mounts | Determines whether the Shpurdp Agents will use the `df` or `df -l` command when checking disk mounts for capacity issues. Auto-mounted remote directories can cause long delays. |`false` | 
...
| agent.ssl | Determines whether SSL is used to communicate between Shpurdp Server and Shpurdp Agents. |`true` | 
...
| agent.stack.retry.tries | The number of times an Shpurdp Agent should retry package installation when it fails due to a repository error. <br/><br/> This property is related to `agent.stack.retry.on_repo_unavailability`. |`5` | 
...
| agent.threadpool.size.max | The size of the Jetty connection pool used for handling incoming Shpurdp Agent requests. |`25` | 
...
| alerts.shpurdp.snmp.dispatcher.udp.port | The UDP port to use when binding the Shpurdp SNMP dispatcher on Shpurdp Server startup. If no port is specified, then a random port will be used. | | 
...
| client.api.ssl.keystore_name | The name of the keystore used when the Shpurdp Server REST API is protected by SSL. |`https.keystore.p12` | 
...
| client.api.ssl.port | The port that client connections will use with the REST API when using SSL. The Shpurdp Web client runs on this port if SSL is enabled. |`8443` | 
| client.api.ssl.truststore_name | The name of the truststore used when the Shpurdp Server REST API is protected by SSL. |`https.keystore.p12` | 
...
| client.security | The type of authentication mechanism used by Shpurdp.<br/><br/>The following are examples of valid values:<ul><li>`local`<li>`ldap`<li>`pam`</ul> | | 
...
| common.services.path | The location on the Shpurdp Server where common service resources exist. Stack services share the common service files.<br/><br/>The following are examples of valid values:<ul><li>`/var/lib/shpurdp-server/resources/common-services`</ul> | | 
| custom.action.definitions | The location on the Shpurdp Server where custom actions are defined. |`/var/lib/shpurdp-server/resources/custom_action_definitions` | 
...
| extensions.path | The location on the Shpurdp Server where stack extensions exist.<br/><br/>The following are examples of valid values:<ul><li>`/var/lib/shpurdp-server/resources/extensions`</ul> | | 
...
| java.home | The location of the JDK on the Shpurdp Agent hosts. If stack.java.home exists, that is only used by Shpurdp Server (or you can find that as shpurdp_java_home in the commandParams on the agent side)<br/><br/>The following are examples of valid values:<ul><li>`/usr/jdk64/jdk1.8.0_112`</ul> | | 
| jce.name | The name of the JCE policy ZIP file. If stack.jce.name exists, that is only used by Shpurdp Server (or you can find that as shpurdp_jce_name in the commandParams on the agent side)<br/><br/>The following are examples of valid values:<ul><li>`UnlimitedJCEPolicyJDK8.zip`</ul> | | 
| jdk.name | The name of the JDK installation binary. If stack.jdk.name exists, that is only used by Shpurdp Server (or you can find that as shpurdp_jdk_name in the commandParams on the agent side)<br/><br/>The following are examples of valid values:<ul><li>`jdk-8u112-linux-x64.tar.gz`</ul> | | 
...
| kerberos.check.jaas.configuration | Determines whether Kerberos-enabled Shpurdp deployments should use JAAS to validate login credentials. |`false` | 
| kerberos.keytab.cache.dir | The location on the Shpurdp Server where Kerberos keytabs are cached. |`/var/lib/shpurdp-server/data/cache` | 
...
| logsearch.metadata.cache.expire.timeout | The time, in hours, that the Shpurdp Server will hold Log File metadata in its internal cache before making a request to the LogSearch Portal to get the latest metadata. |`24` | 
| logsearch.portal.connect.timeout | The time, in milliseconds, that the Shpurdp Server will wait while attempting to connect to the LogSearch Portal service. |`5000` | 
| logsearch.portal.external.address | Address of an external LogSearch Portal service. (managed outside of Shpurdp) Using Shpurdp Credential store is required for this feature (credential: 'logsearch.admin.credential') | | 
| logsearch.portal.read.timeout | The time, in milliseconds, that the Shpurdp Server will wait while attempting to read a response from the LogSearch Portal service. |`5000` | 
| messaging.threadpool.size | Thread pool size for spring messaging |`10` | 
| metadata.path | The location on the Shpurdp Server where the stack resources exist.<br/><br/>The following are examples of valid values:<ul><li>`/var/lib/shpurdp-server/resources/stacks`</ul> | | 
...
| metrics.retrieval-service.request.ttl.enabled | Enables throttling requests to the same endpoint within a fixed amount of time. This property will prevent Shpurdp from making new metric requests to update the cache for URLs which have been recently retrieved.<br/><br/> This property is related to `metrics.retrieval-service.request.ttl`. |`true` | 
| mpacks-v2.staging.path | The Shpurdp Management Pack version-2 staging directory on the Shpurdp Server.<br/><br/>The following are examples of valid values:<ul><li>`/var/lib/shpurdp-server/resources/mpacks-v2`</ul> | | 
| mpacks.staging.path | The Shpurdp Management Pack staging directory on the Shpurdp Server.<br/><br/>The following are examples of valid values:<ul><li>`/var/lib/shpurdp-server/resources/mpacks`</ul> | | 
| notification.dispatch.alert.script.directory | The directory for scripts which are used by the alert notification dispatcher. |`/var/lib/shpurdp-server/resources/scripts` | 
| packages.pre.installed | Determines whether Shpurdp Agent instances have already have the necessary stack software installed |`false` | 
| pam.configuration | The PAM configuration file. | | 
| property.mask.file | The path of the file which lists the properties that should be masked from the api that returns shpurdp.properties | | 
| proxy.allowed.hostports | A comma-separated whitelist of host and port values which Shpurdp Server can use to determine if a proxy value is valid. |`*:*` | 
| recommendations.artifacts.lifetime | The amount of time that Recommendation API data is kept on the Shpurdp Server file system. This is specified using a `hdwmy` syntax for pairing the value with a time unit (hours, days, weeks, months, years)<br/><br/>The following are examples of valid values:<ul><li>`8h`<li>`2w`<li>`1m`</ul> |`1w` | 
...
| recommendations.dir | The directory on the Shpurdp Server file system used for storing Recommendation API artifacts. |`/var/run/shpurdp-server/stack-recommendations` | 
...
| recovery.lifetime_max_count | The maximum number of recovery attempts of a failed component during the lifetime of an Shpurdp Agent instance. This is reset when the Shpurdp Agent is restarted. | | 
...
| resources.dir | The location on the Shpurdp Server where all resources exist, including common services, stacks, and scripts. |`/var/lib/shpurdp-server/resources/` | 
| rolling.upgrade.skip.packages.prefixes | A comma-separated list of packages which will be skipped during a stack upgrade. | | 
| security.agent.hostname.validate | Determines whether the Shpurdp Agent host names should be validated against a regular expression to ensure that they are well-formed.<br><br>WARNING: By setting this value to false, host names will not be validated, allowing a possible security vulnerability as described in CVE-2014-3582. See https://cwiki.apache.org/confluence/display/SHPURDP/Shpurdp+Vulnerabilities for more information. |`true` | 
| security.master.key.location | The location on the Shpurdp Server of the master key file. This is the key to the master keystore. | | 
| security.master.keystore.location | The location on the Shpurdp Server of the master keystore file. | | 
...
| server.requestlogs.namepattern | The pattern of request log file name |`shpurdp-access-yyyy_mm_dd.log` | 
| server.requestlogs.path | The location on the Shpurdp Server where request logs can be created. | | 
...
| server.startup.web.timeout | The time, in seconds, that the shpurdp-server Python script will wait for Jetty to startup before returning an error code. |`50` | 
| server.task.timeout | The time, in seconds, before a server-side operation is terminated. |`1200` | 
| server.timeline.metrics.cache.catchup.interval | The time, in milliseconds, that Shpurdp Metrics intervals should use when extending the boundaries of the original request.<br/><br/> This property is related to `server.timeline.metrics.cache.disabled`. |`300000` | 
| server.timeline.metrics.cache.connect.timeout.millis | The time, in milliseconds, to wait while attempting to connect to Shpurdp Metrics.<br/><br/> This property is related to `server.timeline.metrics.cache.disabled`. |`5000` | 
| server.timeline.metrics.cache.disabled | Determines whether Shpurdp Metric data is cached. |`false` | 
| server.timeline.metrics.cache.entry.idle.seconds | The time, in seconds, that Shpurdp Metric data can remain in the cache without being accessed.<br/><br/> This property is related to `server.timeline.metrics.cache.disabled`. |`1800` | 
| server.timeline.metrics.cache.entry.ttl.seconds | The time, in seconds, that Shpurdp Metric timeline data is cached by Shpurdp Server.<br/><br/> This property is related to `server.timeline.metrics.cache.disabled`. |`3600` | 
| server.timeline.metrics.cache.heap.percent | The amount of heap on the Shpurdp Server dedicated to the caching values from Shpurdp Metrics. Measured as part of the total heap of Shpurdp Server.<br/><br/> This property is related to `server.timeline.metrics.cache.disabled`. |`15%` | 
| server.timeline.metrics.cache.interval.read.timeout.millis | The time, in milliseconds, that requests to update stale metric data will wait while reading from Shpurdp Metrics. This allows for greater control by allowing stale values to be returned instead of waiting for Shpurdp Metrics to always populate responses with the latest data.<br/><br/> This property is related to `server.timeline.metrics.cache.disabled`. |`10000` | 
| server.timeline.metrics.cache.read.timeout.millis | The time, in milliseconds, that initial requests to populate metric data will wait while reading from Shpurdp Metrics.<br/><br/> This property is related to `server.timeline.metrics.cache.disabled`. |`10000` | 
| server.timeline.metrics.cache.use.custom.sizing.engine | Determines if a custom engine should be used to increase performance of calculating the current size of the cache for Shpurdp Metric data.<br/><br/> This property is related to `server.timeline.metrics.cache.disabled`. |`true` | 
| server.timeline.metrics.https.enabled | Determines whether to use to SSL to connect to Shpurdp Metrics when retrieving metric data. |`false` | 
| server.tmp.dir | The location on the Shpurdp Server where temporary artifacts can be created. |`/var/lib/shpurdp-server/tmp` | 
| server.version.file | The full path to the file which contains the Shpurdp Server version. This is used to ensure that there is not a version mismatch between Shpurdp Agents and Shpurdp Server.<br/><br/>The following are examples of valid values:<ul><li>`/var/lib/shpurdp-server/resources/version`</ul> | | 
...
| views.http.cache-control | The value that will be used to set the `Cache-Control` HTTP response header for Shpurdp View requests. |`no-store` | 
