...
from shpurdp_jinja2.runtime import Undefined, concat
from shpurdp_jinja2.exceptions import TemplateAssertionError, TemplateSyntaxError
from shpurdp_jinja2.utils import contextfunction, import_string, Markup, next
...
class Extension(object, metaclass=ExtensionRegistry):
  ...
  def filter_stream(self, stream):
    """It's passed a :class:`~shpurdp_jinja2.lexer.TokenStream` that can be used
    to filter tokens returned.  This method has to return an iterable of
    :class:`~shpurdp_jinja2.lexer.Token`\s, but it doesn't have to return a
    :class:`~shpurdp_jinja2.lexer.TokenStream`.
  ...
  def call_method(
    ...
    :meth:`attr` + :class:`shpurdp_jinja2.nodes.Call`.
    """
    if args is None:
      args = []
    if kwargs is None:
