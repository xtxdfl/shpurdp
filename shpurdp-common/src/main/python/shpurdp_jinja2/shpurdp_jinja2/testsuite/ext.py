...
from shpurdp_jinja2.testsuite import JinjaTestCase, filesystem_loader

from shpurdp_jinja2 import Environment, DictLoader, contextfunction, nodes
from shpurdp_jinja2.exceptions import TemplateAssertionError
from shpurdp_jinja2.ext import Extension
from shpurdp_jinja2.lexer import Token, count_newlines
from shpurdp_jinja2.utils import next
...
i18n_env = Environment(
  loader=DictLoader(i18n_templates), extensions=["shpurdp_jinja2.ext.i18n"]
)
i18n_env.globals.update({"_": gettext, "gettext": gettext, "ngettext": ngettext})

newstyle_i18n_env = Environment(
  loader=DictLoader(newstyle_i18n_templates), extensions=["shpurdp_jinja2.ext.i18n"]
)
...
class ExtensionsTestCase(JinjaTestCase):
  def test_extend_late(self):
    env = Environment()
    env.add_extension("shpurdp_jinja2.ext.autoescape")
    t = env.from_string('{% autoescape true %}{{ "<test>" }}{% endautoescape %}')
    assert t.render() == "&lt;test&gt;"

  def test_loop_controls(self):
    env = Environment(extensions=["shpurdp_jinja2.ext.loopcontrols"])
  ...
  def test_do(self):
    env = Environment(extensions=["shpurdp_jinja2.ext.do"])
    tmpl = env.from_string("""
            {%- set items = [] %}
            {%- for char in "foo" %}
  ...
  def test_with(self):
    env = Environment(extensions=["shpurdp_jinja2.ext.with_"])
    tmpl = env.from_string("""\
        {% with a=42, b=23 -%}
            {{ a }} = {{ b }}
        {% endwith -%}
...
class InternationalizationTestCase(JinjaTestCase):
  ...
  def test_extract(self):
    from shpurdp_jinja2.ext import babel_extract
...
class NewstyleInternationalizationTestCase(JinjaTestCase):
  ...
  def test_autoescape_support(self):
    env = Environment(
      extensions=["shpurdp_jinja2.ext.autoescape", "shpurdp_jinja2.ext.i18n"]
    )
...
class AutoEscapeTestCase(JinjaTestCase):
  def test_scoped_setting(self):
    env = Environment(extensions=["shpurdp_jinja2.ext.autoescape"], autoescape=True)
    tmpl = env.from_string("""
            {{ "<HelloWorld>" }}
            {% autoescape false %}
                {{ "<HelloWorld>" }}
    ...
    env = Environment(extensions=["shpurdp_jinja2.ext.autoescape"], autoescape=False)
    tmpl = env.from_string("""
            {{ "<HelloWorld>" }}
            {% autoescape true %}
                {{ "<HelloWorld>" }}
  ...
  def test_scoping(self):
    env = Environment(extensions=["shpurdp_jinja2.ext.autoescape"])
    tmpl = env.from_string(
