...
class GeneratorHelper(object):
  ...
  def copy_shpurdp_properties(self):
    source_shpurdp_properties = join(
      os.path.abspath(join(self.resources_folder, "..", "..", "..", "..")),
      "shpurdp-server",
      "conf",
      "unix",
    ...
    target_shpurdp_properties = join(
      self.output_folder, "conf", "unix", "shpurdp.properties"
    )
    target_dirname = os.path.dirname(target_shpurdp_properties)

    if not os.path.exists(target_dirname):
      os.makedirs(target_dirname)
    propertyMap = {}
    if "shpurdpProperties" in self.config_data:
      propertyMap = self.config_data.shpurdpProperties

    with open(source_shpurdp_properties, "r") as in_file:
      with open(target_shpurdp_properties, "w") as out_file:
        replaced_properties = []
        for line in in_file:
...
def main(argv):
  ...
  gen_helper.copy_shpurdp_properties()
