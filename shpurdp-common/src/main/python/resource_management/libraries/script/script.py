...
from shpurdp_commons.constants import UPGRADE_TYPE_NON_ROLLING
from shpurdp_commons.constants import UPGRADE_TYPE_ROLLING
from shpurdp_commons.constants import UPGRADE_TYPE_HOST_ORDERED
from shpurdp_commons.network import reconfigure_urllib2_opener
from shpurdp_commons.inet_utils import resolve_address, ensure_ssl_using_protocol
...
<JSON_CONFIG> path to command json file. Ex: /var/lib/shpurdp-agent/data/command-2.json
<BASEDIR> path to service metadata dir. Ex: /var/lib/shpurdp-agent/cache/common-services/HDFS/2.1.0.2.0/package
...
<TMP_DIR> temporary directory for executable scripts. Ex: /var/lib/shpurdp-agent/tmp
...
OUT_FILES_MASK = "*.out"
AGENT_TASKS_LOG_FILE = "/var/log/shpurdp-agent/agent_tasks.log"
...
class Script(object):
  ...
  def get_component_from_role(role_directory_map, default_role):
    """
    Gets the <stack-root>/current/<component> component given an Shpurdp role,
  ...
  def install_packages(self, env):
    ...
    if "host_sys_prepped" in config["shpurdpLevelParams"]:
      # do not install anything on sys-prepped host
      if config["shpurdpLevelParams"]["host_sys_prepped"] is True:
    ...
    try:
      ...
      agent_stack_retry_on_unavailability = bool(
        config["shpurdpLevelParams"]["agent_stack_retry_on_unavailability"]
      )
      agent_stack_retry_count = int(
        config["shpurdpLevelParams"]["agent_stack_retry_count"]
      )
      if isinstance(package_list_str, str) and len(package_list_str) > 0:
        ...
        for package in package_list:
          if self.check_package_condition(package):
            ...
            # HACK: On Windows, only install shpurdp-metrics packages using Choco Package Installer
            # TODO: Update this once choco packages for hadoop are created. This is because, service metainfo.xml support
            # <osFamily>any<osFamily> which would cause installation failure on Windows.
            if OSCheck.is_windows_family():
              if "shpurdp-metrics" in name:
                Package(name)
            else:
              Package(
                name,
    ...
    if OSCheck.is_windows_family():
      ...
      install_windows_msi(
        config["shpurdpLevelParams"]["jdk_location"],
