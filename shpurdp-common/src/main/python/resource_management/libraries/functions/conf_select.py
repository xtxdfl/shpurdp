    ...
    try:
      ...
      if os.path.islink(old_conf):
        ...
        if package in [
          ...
          prevent Shpurdp from by copying those unmanaged contents into /etc/atlas/$version/0
          """
...
def _seed_new_configuration_directories(package, created_directories):
  ...
  by Shpurdp will be available after performing a stack upgrade. Although old configurations
  ...
  # this allows files which are not tracked by Shpurdp to be available after an upgrade
