...
logger = logging.getLogger("shpurdp_alerts")

return_values_map = {
  "true": [RESULT_CODE_OK, OK_MESSAGE],
  "false": [RESULT_CODE_CRITICAL, FAIL_MESSAGE],
...
def simulate_perf_cluster_alert_behaviour(alert_behaviour_properties, configurations):
  ...
  if alert_behaviour_type == "percentage":
    ...
    alert_success_percentage_key = alert_behaviour_properties[
    ...
    if alert_success_percentage:
      ...
      raise Fail(
        "Percentage behaviour was set but alert.success.percentage was not set!"
      )
  elif alert_behaviour_type == "timeout":
    alert_timeout_return_value = None
    alert_timeout_secs = None
    alert_timeout_return_value_key = alert_behaviour_properties[
    ...
    if alert_timeout_return_value_key in configurations:
      alert_timeout_return_value = configurations[
        alert_timeout_return_value_key
      ].lower()
    ...
    if alert_timeout_return_value and alert_timeout_secs:
      logger.info(f"Sleeping for {alert_timeout_secs} seconds")
      print(f"Sleeping for {alert_timeout_secs} seconds")
      time.sleep(int(alert_timeout_secs))
      return (
        return_values_map[alert_timeout_return_value][0],
        [return_values_map[alert_timeout_return_value][1]],
      )
    else:
      raise Fail(
        "Timeout behaviour was set but alert.timeout.return.value/alert.timeout.secs were not set!"
      )
  elif alert_behaviour_type == "flip":
    alert_flip_interval_mins = None
    alert_flip_interval_mins_key = alert_behaviour_properties[
    ...
    if alert_flip_interval_mins:
      ...
      return (
        return_values_map[str(return_value).lower()][0],
        [return_values_map[str(return_value).lower()][1]],
      )
    else:
