...
def get_metric_collectors_from_properties_file(sink_name):
  ...
  properties_filepath = os.path.join(
    hadoop_conf_dir, DEFAULT_METRICS2_PROPERTIES_FILE_NAME
  )

  if not os.path.exists(properties_filepath):
    raise Exception(
      f"Properties file doesn't exist : {properties_filepath}. Can't define metric collector hosts"
    )
  ...
  if property_key in props:
    ...
    raise Exception(
      f"Properties file doesn't contain {property_key}. Can't define metric collector hosts"
    )
...
def create_ams_client(alert_id, ams_app_id, configurations, parameters):
  if (
    ...
    if not _valid_collector_webapp_address(collector_webapp_address):
      raise Exception(
        f'{METRICS_COLLECTOR_WEBAPP_ADDRESS_KEY} value should be set as "fqdn_hostname:port", but set to {configurations[METRICS_COLLECTOR_WEBAPP_ADDRESS_KEY]}'
      )

    ams_collector_hosts = default("/clusterHostInfo/metrics_collector_hosts", [])
    if not ams_collector_hosts:
      raise Exception("Shpurdp metrics is not available: ams_collector_hosts is None")
...
class AmsClient:
  ...
  def load_metric(self, ams_metric, host_filter):
    ...
    for ams_collector_host in self.ams_collector_hosts:
      try:
        metric_dict, http_code = self._load_metric(
          ams_collector_host, ams_metric, host_filter
        )
        ...
        if logger.isEnabledFor(logging.DEBUG):
          logger.exception(
            f"[Alert][{self.alert_id}] Unable to retrieve metrics from AMS ({ams_collector_host}:{self.ams_collector_port}): {str(exception)}"
          )

    if not http_code:
      raise Exception("Shpurdp metrics is not available: no response")
    if http_code not in [200, 299]:
      raise Exception(
        "Shpurdp metrics is not available: http status code = " + str(http_code)
      )
    if not metric_dict:
      raise Exception("Shpurdp metrics is not available: no metrics were found.")
  ...
  def _load_metric(self, ams_collector_host, ams_metric, host_filter):
    ...
    _ssl_version = ShpurdpConfig.get_resolved_config().get_force_https_protocol_value()
    ...
    try:
      ...
      if logger.isEnabledFor(logging.DEBUG):
        logger.exception(
          f"[Alert][{self.alert_id}] Unable to retrieve metrics from AMS: {str(exception)}"
        )
      ...
      if conn is not None:
        try:
          ...
          logger.debug(
            f"[Alert][{self.alert_id}] Unable to close URL connection to {url}"
          )
    ...
    try:
      ...
      if logger.isEnabledFor(logging.DEBUG):
        logger.exception(
          f"[Alert][{self.alert_id}] Convert response to json failed or json doesn't contain needed data: {str(exception)}"
        )
