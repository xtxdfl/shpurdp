...
class AptManagerProperties(GenericManagerProperties):
  ...
  configuration_dump_cmd = [SHPURDP_SUDO_BINARY, "apt-config", "dump"]
...
class AptManager(GenericManager):
  ...
  def get_available_packages_in_repos(self, repos):
    ...
    if repos.feat.scoped:
      ...
      # search the repos on-system anyway.  the url specified in shpurdp must match the one
    ...
    if not name:
      ...
      if context.use_repos:
        ...
        if use_repos:
          ...
          apt_sources_list_tmp_dir = tempfile.mkdtemp(suffix="-shpurdp-apt-sources-d")
          Logger.info(
