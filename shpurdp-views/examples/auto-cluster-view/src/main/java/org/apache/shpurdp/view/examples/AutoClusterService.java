 ...
 */
package org.apache.shpurdp.view.examples;
...
public class AutoClusterService {
  ...
  public Response getValue(@Context HttpHeaders headers, @Context UriInfo ui) {
    ...
    try {
        ...
        if (c != null) {
            ShpurdpStreamProvider stream = context.getShpurdpStreamProvider();

            // use the cluster name and ShpurdpStreamProvider to access Shpurdp REST API
            ...
            // use the view instance configuration to access Shpurdp REST API
            String baseUrl = context.getProperties().get("shpurdp.server.url");
            String username = context.getProperties().get("shpurdp.server.username");
            String password = context.getProperties().get("shpurdp.server.password");
