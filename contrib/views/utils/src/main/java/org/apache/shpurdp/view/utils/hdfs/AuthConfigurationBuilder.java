...
import org.apache.shpurdp.view.utils.shpurdp.NoClusterAssociatedException;
...
public class AuthConfigurationBuilder {
  ...
  private void parseProperties() throws HdfsApiException {
    ...
    if (Strings.isNullOrEmpty(auth)) {
      if (context.getCluster() != null) {
        auth = getConfigurationFromShpurdp();
      } else {
        auth = "auth=SIMPLE";
  ...
  /**
   * Determine configuration from Shpurdp.
   */
  private String getConfigurationFromShpurdp() throws NoClusterAssociatedException {
    ...
    String proxyUser = context.getCluster().getConfigurationValue("cluster-env","shpurdp_principal_name");
