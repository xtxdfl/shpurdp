...
import org.apache.shpurdp.view.capacityscheduler.utils.ServiceFormattedException;
import org.apache.shpurdp.view.utils.shpurdp.ShpurdpApi;
...
/**
 * Configuration service for accessing Shpurdp REST API for capacity scheduler config.
 *
 */
public class ConfigurationService {

  private static final Logger LOG = LoggerFactory.getLogger(ConfigurationService.class);
  private final ShpurdpApi shpurdpApi;
  ...
  public ConfigurationService(ViewContext context) {
    ...
    this.shpurdpApi = new ShpurdpApi(context);
    this.shpurdpApi.setRequestedBy("view-capacity-scheduler");
  }
  ...
  private static final String SHPURDP_OR_CLUSTER_ADMIN_PRIVILEGE_URL = "/api/v1/users/%s?privileges/PrivilegeInfo/permission_name=SHPURDP.ADMINISTRATOR|" +
  ...
  public Response readLatestConfiguration() {
    ...
    try {
      ...
      JSONObject configurations = getConfigurationFromShpurdp(versionTag);
  ...
  public Response readConfigurationByTag(@PathParam("tag") String tag) {
    ...
    Response response = null;
    try {
      JSONObject configurations = getConfigurationFromShpurdp(tag);
      response = Response.ok(configurations).build();
    } catch (WebApplicationException ex) {
  ...
  private boolean isOperator() {
    ...
    String url = String.format(SHPURDP_OR_CLUSTER_ADMIN_PRIVILEGE_URL, context.getLoggedinUser(), context.getCluster().getName());

    try {
      String response = shpurdpApi.readFromShpurdp(url, "GET", null, null);
  ...
  private JSONObject readFromCluster(String url) throws ShpurdpHttpException {
    String response = shpurdpApi.requestClusterAPI(url);
    if (response == null || response.isEmpty()) {
      return null;
    }
  ...
  private JSONObject getConfigurationFromShpurdp(String versionTag) throws ShpurdpHttpException {
   ...
   */
  private String getVersionTag() throws ShpurdpHttpException {
   ...
   */
  private String getClusterName() throws ShpurdpHttpException {
    JSONObject json = getDesiredConfigs();
    JSONObject clusters = (JSONObject) json.get("Clusters");
   ...
   */
  private JSONObject getDesiredConfigs() throws ShpurdpHttpException {
  ...
  public Response writeConfiguration(String requestBody) {
    ...
    try {
      ...
      String responseString = shpurdpApi.requestClusterAPI("", "PUT",requestBody, headers);
  ...
  public Response writeAndRefreshConfiguration(JSONObject request) {
    ...
    try {
      ...
      Map<String, String> headers = new HashMap<String, String>();
      headers.put("Content-Type", "application/x-www-form-urlencoded");
      shpurdpApi.requestClusterAPI("requests/", "POST", data.toJSONString(), headers);
  ...
  private String getRMUrl() {
    return shpurdpApi.getServices().getRMUrl();
  }
