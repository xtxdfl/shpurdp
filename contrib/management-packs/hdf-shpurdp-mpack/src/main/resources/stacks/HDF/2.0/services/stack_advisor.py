...
class HDF20StackAdvisor(DefaultStackAdvisor):
  ...
  def recommendNIFIConfigurations(self, configurations, clusterData, services, hosts):
    ...
    if "ranger-env" in services["configurations"] and "ranger-nifi-plugin-properties" in services["configurations"] and \
      ...
      if rangerEnvNiFiPluginProperty == 'Yes' and \
                      ...
                      "nifi.node.ssl.isenabled" in services["configurations"]["nifi-shpurdp-ssl-config"]["properties"]:
        nifiShpurdpSSLConfig = 'SSL' if services["configurations"]["nifi-shpurdp-ssl-config"]["properties"]["nifi.node.ssl.isenabled"] == 'true' else 'NONE'
        putNiFiRangerPluginProperty("nifi.authentication",nifiShpurdpSSLConfig)
  ...
  def recommendRangerConfigurations(self, configurations, clusterData, services, hosts):
    ...
    # Recommend Shpurdp Infra Solr properties
    ...
    if 'SHPURDP_INFRA' in servicesList and zookeeper_host_port and is_solr_cloud_enabled and not is_external_solr_cloud_enabled:
  ...
  def getServiceConfigurationValidators(self):
    return {
      ...
      "NIFI": {"ranger-nifi-plugin-properties": self.validateNiFiRangerPluginConfigurations,
               "nifi-shpurdp-ssl-config": self.validateNiFiSslProperties }
    }
  ...
  def recommendLogsearchConfigurations(self, configurations, clusterData, services, hosts):
    ...
    infraSolrHosts = self.getComponentHostNames(services, "SHPURDP_INFRA", "INFRA_SOLR")
  ...
  def validateConfigurationsForSite(self, configurations, recommendedDefaults, services, hosts, siteName, method):
    if siteName == 'nifi-shpurdp-ssl-config':
  ...
  def validateNiFiSslProperties(self, properties, recommendedDefaults, configurations, services, hosts):
    ...
    return self.toConfigurationValidationProblems(validationItems, "nifi-shpurdp-ssl-config")
