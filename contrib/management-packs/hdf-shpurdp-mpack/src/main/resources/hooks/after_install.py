...
import sys
import os
from shpurdp_server.serverConfiguration import get_shpurdp_properties, get_resources_location
from resource_management.core import sudo

def main():
  properties = get_shpurdp_properties()
  if properties == -1:
    print >> sys.stderr, "Error getting shpurdp properties"
    return -1
  ...
  for file in os.listdir(views_dir):
    ...
    if os.path.isfile(path):
      if "shpurdp-admin" in path or "storm-view" in path:
