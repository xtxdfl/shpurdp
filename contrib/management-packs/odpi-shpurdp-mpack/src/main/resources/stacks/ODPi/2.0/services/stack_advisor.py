...
class ODPi20StackAdvisor(DefaultStackAdvisor):
  ...
  def getServiceConfigurationRecommenderDict(self):
    return {
      ...
      "SHPURDP_METRICS": self.recommendAmsConfigurations,
  ...
  def recommendYARNConfigurations(self, configurations, clusterData, services, hosts):
    ...
    if "TEZ" in servicesList:
        shpurdp_user = self.getShpurdpUser(services)
        shpurdpHostName = socket.getfqdn()
        putYarnProperty("yarn.timeline-service.http-authentication.proxyuser.{0}.hosts".format(shpurdp_user), shpurdpHostName)
        putYarnProperty("yarn.timeline-service.http-authentication.proxyuser.{0}.groups".format(shpurdp_user), "*")
        old_shpurdp_user = self.getOldShpurdpUser(services)
        if old_shpurdp_user is not None:
            putYarnPropertyAttribute("yarn.timeline-service.http-authentication.proxyuser.{0}.hosts".format(old_shpurdp_user), 'delete', 'true')
            putYarnPropertyAttribute("yarn.timeline-service.http-authentication.proxyuser.{0}.groups".format(old_shpurdp_user), 'delete', 'true')
  ...
  def getShpurdpUser(self, services):
    shpurdp_user = services['shpurdp-server-properties']['shpurdp-server.user']
    if "cluster-env" in services["configurations"] \
          and "shpurdp_principal_name" in services["configurations"]["cluster-env"]["properties"] \
      ...
      shpurdp_user = services["configurations"]["cluster-env"]["properties"]["shpurdp_principal_name"]
      shpurdp_user = shpurdp_user.split('@')[0]
    return shpurdp_user

  def getOldShpurdpUser(self, services):
    shpurdp_user = None
    if "cluster-env" in services["configurations"]:
      if "security_enabled" in services["configurations"]["cluster-env"]["properties"] \
              and services["configurations"]["cluster-env"]["properties"]["security_enabled"].lower() == "true":
         shpurdp_user = services['shpurdp-server-properties']['shpurdp-server.user']
      elif "shpurdp_principal_name" in services["configurations"]["cluster-env"]["properties"]:
         shpurdp_user = services["configurations"]["cluster-env"]["properties"]["shpurdp_principal_name"]
         shpurdp_user = shpurdp_user.split('@')[0]
    return shpurdp_user

  def recommendShpurdpProxyUsersForHDFS(self, services, servicesList, putCoreSiteProperty, putCoreSitePropertyAttribute):
      if "HDFS" in servicesList:
          shpurdp_user = self.getShpurdpUser(services)
          shpurdpHostName = socket.getfqdn()
          putCoreSiteProperty("hadoop.proxyuser.{0}.hosts".format(shpurdp_user), shpurdpHostName)
          putCoreSiteProperty("hadoop.proxyuser.{0}.groups".format(shpurdp_user), "*")
          old_shpurdp_user = self.getOldShpurdpUser(services)
          if old_shpurdp_user is not None:
            putCoreSitePropertyAttribute("hadoop.proxyuser.{0}.hosts".format(old_shpurdp_user), 'delete', 'true')
            putCoreSitePropertyAttribute("hadoop.proxyuser.{0}.groups".format(old_shpurdp_user), 'delete', 'true')
  ...
  def recommendHadoopProxyUsers (self, configurations, services, hosts):
    ...
    self.recommendShpurdpProxyUsersForHDFS(services, servicesList, putCoreSiteProperty, putCoreSitePropertyAttribute)
  ...
  def recommendRangerConfigurations(self, configurations, clusterData, services, hosts):
    ...
    if rangerServiceVersion == '0.4.0':
      # Recommend ldap settings based on shpurdp.properties configuration
      # If 'shpurdp.ldap.isConfigured' == true
      # For Ranger version 0.4.0
      if 'shpurdp-server-properties' in services and \
      'shpurdp.ldap.isConfigured' in services['shpurdp-server-properties'] and \
        services['shpurdp-server-properties']['shpurdp.ldap.isConfigured'].lower() == "true":
        putUserSyncProperty = self.putProperty(configurations, "usersync-properties", services)
        serverProperties = services['shpurdp-server-properties']
  ...
  def recommendStormConfigurations(self, configurations, clusterData, services, hosts):
    ...
    if 'SHPURDP_METRICS' in servicesList:
  ...
  def recommendAmsConfigurations(self, configurations, clusterData, services, hosts):
    ...
    amsCollectorHosts = self.getComponentHostNames(services, "SHPURDP_METRICS", "METRICS_COLLECTOR")
    ...
    if operatingMode == "embedded":
      if isLocalRootDir:
        ...
        putAmsHbaseSiteProperty("hbase.rootdir", "file:///var/lib/shpurdp-metrics-collector/hbase")
    ...
    metricsDir = os.path.join(scriptDir, '../../../../common-services/SHPURDP_METRICS/0.1.0/package')
  ...
  def getServiceConfigurationValidators(self):
    return {
      ...
      "SHPURDP_METRICS": {"ams-hbase-site": self.validateAmsHbaseSiteConfigurations,
  ...
  def validateStormConfigurations(self, properties, recommendedDefaults, configurations, services, hosts):
    ...
    servicesList = [service["StackServices"]["service_name"] for service in services["services"]]
    # Storm AMS integration
    if 'SHPURDP_METRICS' in servicesList and "metrics.reporter.register" in properties and \
      ...
      validationItems.append({"config-name": 'metrics.reporter.register',
                              "item": self.getWarnItem(
                                "Should be set to org.apache.hadoop.metrics2.sink.storm.StormTimelineMetricsReporter to report the metrics to Shpurdp Metrics service.")})
  ...
  def validatorEnoughDiskSpace(self, properties, propertyName, hostInfo, reqiuredDiskSpace):
    ...
    if mountPoints[mountPoint] < reqiuredDiskSpace:
      msg = "Shpurdp Metrics disk space requirements not met. \n" \
