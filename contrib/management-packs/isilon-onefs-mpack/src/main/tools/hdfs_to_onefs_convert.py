...
class Header:
  @classmethod
  def csrf(clazz):
    return clazz('X-Requested-By', 'shpurdp')
...
class Conversion:
  ...
  def check_prerequisites(self):
    ...
    if not self.cluster.installed_stack().has_service('ONEFS'):
      ...
      return False
    sys.stdout.write('Please, confirm you have made backup of the Shpurdp db [y/n] (n)? ')
    if raw_input() != 'y':
      return False
    return True
...
class CommandLine:
  def __init__(self):
    ...
    self.parser.add_option("-o", '--host', dest='host', help='Shpurdp server host', default='localhost')
    self.parser.add_option("-p", '--port', dest='port', help='Shpurdp server port', default='8080')
...
if __name__ == '__main__':
  ...
  print '  * Shpurdp must be upgraded to >=v2.7.1'
  print '  * Stack must be upgraded to >=HDP-3.0'
  print '  * Is highly recommended to backup shpurdp database before you proceed.'
