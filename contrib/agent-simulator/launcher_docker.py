...
# configure the VM to set Docker, Weave network, run Shpurdp-agent inside the Docker
# argv 1: the external IP of this VM
# argv 2: the Weave IP of the Shpurdp-server
# argv 3: the external IP of the Shpurdp-server
...
if __name__ == "__main__":
    if len(sys.argv) < 5:
        print "configure the VM to set Docker, Weave network, run Shpurdp-agent inside the Docker"
        print "Args: <the external IP of this VM>"
        print "     <the Weave IP of the Shpurdp-server>"
        print "     <the external IP of the Shpurdp-server>"
    ...
    # which is not necessary, connecting to shpurdp-server and service server is enough.
    # for vm in cluster.shpurdp_agent_vm_list:
