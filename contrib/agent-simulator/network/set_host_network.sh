...
# for example, an Shpurdp-agent within a VM (not in a Docker container)
...
# $6 <External IP address of Shpurdp server>

if [ $# -lt 6 ]; then
    echo "usage: ./set_shpurdp_server_network.sh <Weave internal IP> <Weave DNS IP> <Mask> <Weave host name> <Weave domain name> <Shpurdp server IP>"
    echo "example: ./set_shpurdp_server_network.sh 192.168.254.1 192.168.254.2 16 yourname-group-c-service-server-1 yourname-group-c-service-server-1.weave.local 104.196.91.170"
    exit 1
fi
...
# launch weave
weave launch $shpurdp_server_ip
