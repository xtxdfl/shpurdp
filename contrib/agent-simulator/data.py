...
class Data:
    ...
    def print_cluster_summary_list(self):
        ...
        :return: a list of tuple. The elements of the tuple are:
                 ...
                 service_server_num, shpurdp_server_num, create_time
        """
        print "(cluster_name, state, agent_number, service_server_num, shpurdp_server_num, create_time)"
        ...
        for cluster in json_data["clusters"]:
            ...
            agent_number = 0
            for agent_vm in cluster["shpurdp_agent_vm_list"]:
                agent_number += len(agent_vm["docker_list"])
            service_server_num = len(cluster["service_server_vm_list"])
            shpurdp_server_num = len(cluster["shpurdp_server_vm"])
            print cluster_name, state, agent_number, service_server_num, shpurdp_server_num, create_time
