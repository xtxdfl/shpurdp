...
# This script will install and start the shpurdp-server on a CentOS 7 machine
...
yum install -y -q shpurdp-server
yum install -y -q java-1.8.0-openjdk.x86_64
yum install -y -q java-1.8.0-openjdk-devel.x86_64
# shpurdp-server setup -s
# shpurdp-server setup -j <path to JDK on all hosts>
shpurdp-server setup -j /usr/lib/jvm/java-openjdk << _EOF_
y
n
n
_EOF_
