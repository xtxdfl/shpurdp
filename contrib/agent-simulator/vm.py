...
class VM:
    ...
    def _set_weave_network(self, vm_external_ip_list, weave_dns_ip):
        ...
        :return: None
        """
        # add other VMs and the shpurdp-server to set up connections
    ...
    def _launch_containers(self, docker_image, server_weave_ip):
        ...
        configure and launch Shpurdp-gent inside Docker.
        :param docker_image: the name of the Docker image
        :param server_weave_ip: Weave internal IP of Shpurdp-server
        :return: None
        """
        for docker in self.docker_list:
    ...
    def run_shpurdp_server(self):
        """
        set up Weave network, run Shpurdp-server in this VM
        :return: None
        """
        # set up network, run script inside the network directory
        os.chdir("network")
        subprocess.call(["./set_shpurdp_server_network.sh", self.weave_internal_ip,
                         self.weave_dns_ip, self.weave_ip_mask])
        os.chdir("..")

        # install shpurdp server and start service
        subprocess.call(["./server/shpurdp_server_install.sh"])

        # start service
        subprocess.call(["./server/shpurdp_server_start.sh"])

    def run_service_server(self, shpurdp_server_weave_ip, shpurdp_server_external_ip):
        """
        set up Weave network, run Shpurdp-agent in this VM
        :param shpurdp_server_weave_ip: the Weave IP of Shpurdp-server
        :param shpurdp_server_external_ip: the external IP of Shpurdp-server
        :return: None
        """
        # set up network, run script inside the network directory
        os.chdir("network")
        subprocess.call(["./set_host_network.sh", self.weave_internal_ip,
                         ...
                         self.weave_domain_name, shpurdp_server_external_ip])
        os.chdir("..")

        # install shpurdp agent and start service
        subprocess.call(["./docker_image/shpurdp_agent_install.sh"])
        replace_conf(shpurdp_server_weave_ip)

        # start service
        subprocess.call(["./docker_image/shpurdp_agent_start.sh"])
    ...
    def run_docker(self, server_weave_ip, vm_ip_list):
        """
        run all Docker containers with Shpurdp-agent inside
        :param server_weave_ip: Weave internal IP of Shpurdp-server
        :param vm_ip_list: external IP list of all other VMs to be connected
                each docker vm connect to each other and service VM (and shpurdp-server)
        :return: None
        """
    ...
    @staticmethod
    def get_shpurdp_agent_vm_name(cluster_name):
        return "{0}-agent-vm".format(cluster_name)

    @staticmethod
    def get_shpurdp_server_vm_name(cluster_name):
        return "{0}-shpurdp-server".format(cluster_name)
