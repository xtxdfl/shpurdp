...
class Cluster:
    ...
    it has the methods to request cluster, generate information and run all Shpurdp-agent and Shpurdp-server
    """

    # The constants represent the state of the cluster
    # A newly requested cluster is in FREE state
    STATE_FREE = "FREE"
    # A cluster with running Shpurdp-server and Shpurdp-agents is in RUNNING state
    ...
    def __init__(self):
        ...
        self.state = ""
        self.create_time = ""
        # The list should only has one or zero VM, which holds the Shpurdp-server
        self.shpurdp_server_vm = []
        # The list of VMs, with Shpurdp-agents directly inside (not in Docker)
        self.service_server_vm_list = []
        # The list of VMs, each will hold multiple Docker containers with Shpurdp-agent inside
        self.shpurdp_agent_vm_list = []
    ...
    def print_description(self):
        ...
        print "Shpurdp Server: "
        shpurdp_server_vm = self.get_shpurdp_server_vm()
        if shpurdp_server_vm is None:
            print "None"
        else:
            print shpurdp_server_vm.domain_name, " ", shpurdp_server_vm.external_ip, " ",\
                shpurdp_server_vm.weave_internal_ip
        print

        print "Service Server with Shpurdp Agent directly installed: "
        ...
        print "Shpurdp Agent in Docker Container: "
        int_list = []
        for vm in self.shpurdp_agent_vm_list:
    ...
    def get_shpurdp_server_vm(self):
        """
        get the VM instance which hold the Shpurdp-server
        :return: the VM instance hold the Shpurdp-server, or None if no Shpurdp-server in this cluster
        """
        for vm in self.shpurdp_server_vm:
            return vm
        return None

    def get_service_server_vm(self, vm_ip):
        """
        get the VM instance which directly hold the Shpurdp-agent
        :param vm_ip: the external IP of the target VM
        :return:
        """
        for vm in self.service_server_vm_list:
    ...
    def to_json(self):
        ...
        cluster["shpurdp_server_vm"] = []
        for vm in self.shpurdp_server_vm:
            cluster["shpurdp_server_vm"].append(vm.to_json())

        cluster["service_server_vm_list"] = []
        for vm in self.service_server_vm_list:
            cluster["service_server_vm_list"].append(vm.to_json())

        cluster["shpurdp_agent_vm_list"] = []
        for vm in self.shpurdp_agent_vm_list:
            cluster["shpurdp_agent_vm_list"].append(vm.to_json())
    ...
    def load_from_json(cluster_name):
        ...
        for vm_json in json_data["shpurdp_server_vm"]:
            shpurdp_server_vm.append(VM.load_from_json(vm_json))

        for vm_json in json_data["service_server_vm_list"]:
            service_server_vm_list.append(VM.load_from_json(vm_json))

        for vm_json in json_data["shpurdp_agent_vm_list"]:
            shpurdp_agent_vm_list.append(VM.load_from_json(vm_json))

        cluster = Cluster()
        cluster.cluster_name = cluster_name
        cluster.state = json_data["state"]
        cluster.create_time = json_data["create_time"]
        cluster.shpurdp_server_vm = shpurdp_server_vm
        cluster.service_server_vm_list = service_server_vm_list
        cluster.shpurdp_agent_vm_list = shpurdp_agent_vm_list
        return cluster
    ...
    def request_shpurdp_server_vm(self, name):
        """
        request a VM for holding Shpurdp-server
        :param name: the name prefix of all requesting VMs
        :return: A list of tuple, each tuple has domain name and IP of a VM
        """
        # only 1 shpurdp server
        vm_num = 1
        gce_vm_type = Config.ATTRIBUTES["shpurdp_server_vm_type"]
        gce_vm_os = Config.ATTRIBUTES["shpurdp_server_vm_os"]

        gce_extra_cmd = ""
        if "shpurdp_server_vm_extra" in Config.ATTRIBUTES:
            gce_extra_cmd = Config.ATTRIBUTES["shpurdp_server_vm_extra"]
    ...
    def reqeust_service_server_vm(self, vm_num, name):
        """
        Request VMs to directly hold Shpurdp-agent (not inside Docker)
    ...
    def request_agent_vm(self, vm_num, name):
        """
        Request VMs to hold Docker containers, each with Shpurdp-agent inside
        :param vm_num: the number of VM to request
        :param name: the name prefix of all requesting VMs
        :return: A list of tuple, each tuple has domain name and IP of a VM
        """
        gce_vm_type = Config.ATTRIBUTES["shpurdp_agent_vm_type"]
        gce_vm_os = Config.ATTRIBUTES["shpurdp_agent_vm_os"]
        gce_extra_disk = ""
        if "shpurdp_agent_vm_extra_disk" in Config.ATTRIBUTES:
            gce_extra_disk = Config.ATTRIBUTES["shpurdp_agent_vm_extra_disk"]

        fqdn_ip_pairs = self.request_vm(name, vm_num, gce_vm_type, gce_vm_os, gce_extra_disk)
        return fqdn_ip_pairs

    def request_gce_cluster(self, shpurdp_agent_vm_num, docker_num,
                            service_server_num, with_shpurdp_server, cluster_name):
        """
        Request a cluster from GCE
        :param shpurdp_agent_vm_num: number of VMs to hold Docker containers
        :param docker_num: number of Docker containers inside each VM
        :param service_server_num: number of VMs which has Shpurdp-agent directly installed (not in Docker)
        :param with_shpurdp_server: True or False, whether to request a VM to hold Shpurdp-server
        :param cluster_name: the name of the cluster
        :return: None
        """
        shpurdp_server_fqdn_ip_pairs = []
        if with_shpurdp_server is True:
            shpurdp_server_fqdn_ip_pairs = self.request_shpurdp_server_vm(VM.get_shpurdp_server_vm_name(cluster_name))
        service_server_fqdn_ip_pairs = self.reqeust_service_server_vm(service_server_num,
                                                                      VM.get_service_server_vm_name(cluster_name))
        shpurdp_agent_fqdn_ip_pairs = self.request_agent_vm(shpurdp_agent_vm_num,
                                                           VM.get_shpurdp_agent_vm_name(cluster_name))

        # prepare all attributes of the cluster, write to a file
        self.generate_cluster_info(cluster_name, shpurdp_server_fqdn_ip_pairs, service_server_fqdn_ip_pairs,
                                   shpurdp_agent_fqdn_ip_pairs, docker_num)

    def generate_cluster_info(self, cluster_name, shpurdp_server_fqdn_ip_pairs, service_server_fqdn_ip_pairs,
                              shpurdp_agent_fqdn_ip_pairs, docker_num):
        """
        generate VM and docker info for this cluster
        set up parameter of the class instance as this info
        :param cluster_name: the name of the cluster
        :param shpurdp_server_fqdn_ip_pairs: the domain name and IP pairs for Shpurdp-server
        :param service_server_fqdn_ip_pairs: the domain name and IP pairs for VMs with Shpurdp-agent installed
        :param shpurdp_agent_fqdn_ip_pairs: the domain name and IP pairs for VM with Docker containers
        ...
        for vm_domain_name, vm_ip in shpurdp_server_fqdn_ip_pairs:
            ...
            self.shpurdp_server_vm.append(vm)
        ...
        vm_index = 0
        for vm_domain_name, vm_ip in shpurdp_agent_fqdn_ip_pairs:
            current_ip = self._increase_ip(current_ip, 1)
            weave_dns_ip = current_ip
            vm = VM(vm_ip, vm_domain_name, weave_dns_ip, weave_ip_mask)
            ...
            vm_index += 1
            self.shpurdp_agent_vm_list.append(vm)
    ...
    def run_cluster(self, server_weave_ip, server_external_ip):
        """
        Run all Shpurdp-agents and Shpurdp-server in the cluster in parallel
        Wait until all processes finish
        :param server_weave_ip: the Weave IP of Shpurdp-server
        :param server_external_ip: the external IP of Shpurdp-server
        :return: None
        """
        process_list = {}
        process_list.update(self.run_shpurdp_server_asyn())
    ...
    def run_shpurdp_server_asyn(self):
        """
        Run configuration for Shpurdp-server in this cluster
        Set up Shpurdp-server and Weave network
        ...
        for vm in self.shpurdp_server_vm:
            ...
            vm_ssh_login = "{0}@{1}".format(Config.ATTRIBUTES["vm_user"], vm_external_ip)
            vm_ssh_cd_cmd = "cd {0}".format(Config.ATTRIBUTES["vm_code_directory"])
            vm_ssh_python_cmd = "python launcher_shpurdp_server.py {0}".format(self.cluster_name)
    ...
    def run_service_server_asyn(self, server_weave_ip, server_external_ip):
        """
        Run configuration, set up Shpurdp-agent in this VM, and the Weave network
        :param server_weave_ip: the Weave IP of the Shpurdp-server
        :param server_external_ip: the external IP of the Shpurdp-server
        The method is NON-BLOCK
        :return: a map of tuple, the key of the map is the host name of the VM,
                the tuple has 3 elements: the file handler of the output of the VM,
                the file path of the output of the VM,
    ...
    def run_docker_on_cluster_asyn(self, server_weave_ip, server_external_ip):
        ...
        run all containers, each with Shpurdp-agent inside
        :param server_weave_ip: the Weave IP of the Shpurdp-server
        :param server_external_ip: the external IP of the Shpurdp-server
        The method is NON-BLOCK
        :return: a map of tuple, the key of the map is the host name of the VM,
