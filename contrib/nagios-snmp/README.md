...
Shpurdp Server integration with SNMP traps
=========================================
Shpurdp uses nagios to manage alerts in the cluster.  When a node goes down or a service state changes in the cluster, nagios
will handle those events and is monitored using Shpurdp-web.  This document describes how to integrate alerts with remote
SNMP management station by sending SNMP traps.  By enabling SNMP traps, Shpurdp & Hadoop cluster alerts can be monitored
using remote management station (like OpenNMS, HP OpenView, etc.,).

    This will work with Shpurdp Server 1.7.0 and below.  In Shpurdp 2.0.0, this feature will be
    replaced by alert framework.

Prerequisites:
--------------
1. Nagios server should be running in one of the hadoop cluster node. (Need not to be same node as Shpurdp server).
...
3. In the node where shpurdp-server is running, edit file /var/lib/shpurdp-server/resources/stacks/HDP/2.0.6/services/NAGIOS/package/templates/nagios.cfg.j2
and add below lines just before the {{nagios\_host\_cfg}}
    ...
    pushed to the nagios server when shpurdp-server restarted.

4. To enable SNMP trap, edit file /var/lib/shpurdp-server/resources/stacks/HDP/2.0.6/services/NAGIOS/package/templates/contacts.cfg.j2
in the shpurdp-server and add **snmp-management-station** to the contract group **admins**
...
8. Restart shpurdp-server
	> shpurdp-server restart

9. Launch shpurdp-web (or GUI) in the browser and login.  Select Nagios server and restart the service.
...
3. Open the shpurdp-web in the browser and login.  Try to stop some services from shpurdp-web and check the snmptraps
