...
Param ($MonitoringShpurdpUri, $Username, $Password)

function Main {
    $response = InvokeRestAPI (JoinUri "$MonitoringShpurdpUri" 'components/NAMENODE?fields=metrics/dfs') $Username $Password
    ...
    $response = InvokeRestAPI (JoinUri "$MonitoringShpurdpUri" "components/DATANODE?fields=metrics/dfs/datanode[$startTimeStamp]") $Username $Password
