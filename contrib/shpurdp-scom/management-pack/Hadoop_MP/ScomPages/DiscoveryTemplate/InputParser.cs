...
namespace Shpurdp.SCOM.ScomPages.DiscoveryTemplate {
    public class InputParser : Component, IInputConfigurationParser {
        private const string ClassNamePrefix = "Shpurdp.SCOM.ShpurdpSeed.";
        private const string SeedDiscoveryNamePrefix = "Shpurdp.SCOM.Discovery.ShpurdpSeed.";
        private const string ClustersDiscoveryNamePrefix = "Shpurdp.SCOM.Discovery.Clusters.";
        private const string AccountOverrideNamePrefix = "Shpurdp.SCOM.Profile.Shpurdp.";
        private static Regex ShpurdpUriNodeRegex = new Regex("<ShpurdpUri>([^<]*)</ShpurdpUri>", RegexOptions.CultureInvariant | RegexOptions.Compiled);
        ...
        private void GetDiscoveryConfig(ITemplateContext templateContext, TemplateInputConfig templateConfig) {
            foreach (var discovery in SDKHelper.GetFolderItems<ManagementPackDiscovery>(this, templateContext.OutputFolder)) {
                ...
                if (discovery.Name.StartsWith(ClustersDiscoveryNamePrefix, StringComparison.OrdinalIgnoreCase)) {
                    ...
                    if (ShpurdpUriNodeRegex.IsMatch(discoveryConfig))
                        templateConfig.ShpurdpUri = ShpurdpUriNodeRegex.Match(discoveryConfig).Groups[1].Value;
                }
            }
        }
