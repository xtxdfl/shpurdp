...
# CREATE DATABASE `shpurdpmetrics` /*!40100 DEFAULT CHARACTER SET utf8 */;
#
...
CREATE DEFINER=`shpurdp`@`%` PROCEDURE `uspInsertMetricValue`(recordID bigint, metricName nvarchar(256), metricValue nvarchar(512))
proc_label:BEGIN
...
CREATE DEFINER=`shpurdp`@`%` PROCEDURE `uspGetMetricRecord`(
...
CREATE DEFINER=`shpurdp`@`%` FUNCTION `ufnIsIPAddress`(inputString varchar(1024)) RETURNS tinyint(4)
BEGIN
...
CREATE DEFINER=`shpurdp`@`%` PROCEDURE `ufGetMetrics`(startTimeStamp bigint,
		 endTimeStamp bigint,
