...
copy shpurdp-scom-server\target\shpurdp-scom*.zip %SCOM_DIR% || exit /b 1
copy shpurdp-scom-server\target\shpurdp-scom*.jar %SCOM_DIR% || exit /b 1
...
powershell.exe -NoProfile -InputFormat none -ExecutionPolicy unrestricted -command "%cd%\msi\build\zip.ps1" "%FINAL_ZIPS%" "%ARTIFACT_DIR%\shpurdp-scom-%SCOM_VERSION%.zip" || exit /b 1
