...
function modify_xml($name)
{
	...
	if (Test-Path $xml_file)
	{
		...
		for ($i = 0; $i -lt $arguments.length; $i++){
			...
			{
				$txt = $arguments[$i]
				$arguments[$i] = "$txt;$destination\$shpurdp_metrics;$destination\sqljdbc4.jar"
				$flag = $false
			}
		}
...
function Main( $scriptDir )
{
    ...
    Write-Log "Reading Shpurdp layout from $ENV:AMB_LAYOUT"
    if ( -not (Test-Path $ENV:AMB_LAYOUT))
    {
        throw "No Shpurdp Properties file found, make sure the file $ENV:AMB_LAYOUT exists and contains the current cluster layout"
    }
	...
	Write-Log "Copying Shpurdp-Scom.Jar"
	$jar = Join-Path $Package_trim "resources\shpurdp-scom-*.jar"
	Copy-Item -Force $jar $destination
	$shpurdp_scom = Getname $jar "short"
	Write-Log "Copying Shpurdp Metrics Jar"
	$jar = Join-Path $Package_trim "resources\metrics-sink-*.jar"
	Copy-Item -Force $jar $destination
	$shpurdp_metrics = Getname $jar "short"
	...
	Copy-Item -Force $jar $destination
   	Write-log "Pushing Shpurdp-SCOM and SQL Server JDBC to each node"
	$current = @()
    $failed=@()
 	$failed_file = join-path $env:TMP "shpurdp_failed.txt"
	$db_file = join-path $env:TMP "db_shpurdp_failed.txt"
    ...
    if (-not (Test-Path ENV:HDP_VERSION))
    {
        ...
        else
        {
            ...
            if ($out -eq $null)
    		{
    			Write-Log "Cannot detect HDP version. Please add correct HDP version into shpurdp.properties file"
    		}
			else 
			{
				$ENV:HDP_VERSION = $out
	...
	foreach ($server in $hosts)
	{
		if (($current -notcontains $server) -and ($server -ne $null))
		{
			...
			$out = Invoke-Command -ComputerName $server -ScriptBlock {
				param( $server,$SQL_SERVER_NAME,$SQL_SERVER_PORT,$SQL_SERVER_LOGIN,$SQL_SERVER_PASSWORD,$destination,$shpurdp_metrics,$START_SERVICES,$RECREATE_DB,$hdpversion )
				$log = Join-Path $destination "shpurdp_install.log"
				...
				{	
					...
					$log_new = Join-Path $ENV:TMP "shpurdp_install.log"
					Copy-Item $log $log_new
					Remove-Item $Destination -force -Recurse
				}
			} -ArgumentList ($server,$SQL_SERVER_NAME,$SQL_SERVER_PORT,$SQL_SERVER_LOGIN,$SQL_SERVER_PASSWORD,$destination,$shpurdp_metrics,$START_SERVICES,$RECREATE_DB,$hdpversion)
    ...
    Write-Log "Unpacking shpurdp-server-conf"
    $package = Join-Path $Package_trim "resources\shpurdp-scom-*-conf.zip"
    $shpurdp_conf = GetName $package "short"
    $shpurdp_conf = $shpurdp_conf -replace ".zip"
    ...
    Write-Log "Unpacking shpurdp-server-lib"
    $package = Join-Path $Package_trim "resources\shpurdp-scom-*-lib.zip"
    $shpurdp_lib = GetName $package "short"
    $shpurdp_lib = $shpurdp_lib -replace ".zip"
    $package = GetName $package "full"
    $command = "$winpkg $Package unzip $destination"
    Invoke-Pschk $command
    Write-Log "Modifiying shpurdp.properties"
    $props = Join-Path $destination "$shpurdp_conf\conf\shpurdp.properties"
    ...
    {
        ReplaceShpurdpServiceXML $props "%version%" $ENV:HDP_VERSION
    }
    Write-Log "Copying cluster.properties to shpurdp config"
    $clp = $ENV:HDP_LAYOUT
    $destination_conf = Join-Path $destination "$shpurdp_conf\conf\clusterproperties.txt"
    Copy-Item -Force $clp $destination_conf
    Write-Log "Copying shpurdp properties file"
    Copy-Item $ENV:AMB_LAYOUT "$env:AMB_DATA_DIR\shpurdpproperties.txt"
    Write-Log "Creating service"
    $classpath = "$env:AMB_DATA_DIR\$shpurdp_conf\conf\;$env:AMB_DATA_DIR\sqljdbc4.jar;$env:AMB_DATA_DIR\$shpurdp_scom;$env:AMB_DATA_DIR\$shpurdp_lib\lib\*;$env:HADOOP_HOME\etc\hadoop"
    $arguments = "-server -XX:NewRatio=3 -XX:+UseConcMarkSweepGC -XX:-UseGCOverheadLimit -XX:CMSInitiatingOccupancyFraction=60  -cp $classpath org.apache.shpurdp.scom.ShpurdpServer"
    $servicename = "shpurdpscom"
    ...
    CreateAndConfigureShpurdpService $servicename "$Package_trim\resources" $servicedir
    Copy-Item -Path "$Package_trim\resources\servicehost.xml" -Destination $serviceconfig -Force -ErrorAction Stop
    ReplaceShpurdpServiceXML $serviceconfig "%service%" $servicename
    ReplaceShpurdpServiceXML $serviceconfig "%JAVA_HOME%" $ENV:JAVA_HOME
    ReplaceShpurdpServiceXML $serviceconfig "%arguments%" $arguments
    Write-Log "Creating shortcut to start Shpurdp"
    $objShell = New-Object -ComObject ("WScript.Shell")
    $objShortCut = $objShell.CreateShortcut($env:USERPROFILE + "\Desktop" + "\Start Shpurdp SCOM Server.lnk")
    $objShortCut.TargetPath = "sc.exe"
    $objShortCut.Arguments = "start $servicename"
    $objShortCut.Description = "Start Shpurdp"
    $objShortCut.IconLocation = "cmd.exe"
    $objShortCut.Save()
    Write-Log "Creating shortcut to stop Shpurdp"
    $objShortCut = $objShell.CreateShortcut($env:USERPROFILE + "\Desktop" + "\Stop Shpurdp SCOM Server.lnk")
    $objShortCut.TargetPath = "sc.exe"
    $objShortCut.Arguments = "stop $servicename"
    $objShortCut.Description = "Stop Shpurdp"
    $objShortCut.IconLocation = "cmd.exe"
    $objShortCut.Save()
    CreateUrl "http://$ENV:COMPUTERNAME':8080/api/v1/clusters" "Browse Shpurdp API.url"
    CreateUrl "http://$ENV:COMPUTERNAME':8080/api/v1/clusters/shpurdp/services/HDFS/components/NAMENODE" "Browse Shpurdp Metrics.url"
