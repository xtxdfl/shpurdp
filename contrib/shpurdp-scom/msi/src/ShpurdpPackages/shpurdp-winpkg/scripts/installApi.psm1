...
function CreateAndConfigureShpurdpService(
    [String]
    [Parameter( Position=0, Mandatory=$true )]
    $service,
    [String]
...
)
{
    if ( -not ( Get-Service "$service" -ErrorAction SilentlyContinue ) )
    {
        ...
        $s = New-Service -Name "$service" -BinaryPathName "$serviceBinDir\$service.exe" -DisplayName "Apache Shpurdp $service"
        if ( $s -eq $null )
        {
            throw "CreateAndConfigureShpurdpService: Service `"$service`" creation failed"
        }
    ...
    else
    {
        ...
        StopAndDeleteShpurdpService $service
        CreateAndConfigureShpurdpService $service $ambResourcesDir $serviceBinDir
    }
}

### Stops and deletes the Shpurdp service.
function StopAndDeleteShpurdpService(
    [String]
    [Parameter( Position=0, Mandatory=$true )]
    $service
)
...
function ReplaceShpurdpServiceXML(
    [String]
    [Parameter( Position=0, Mandatory=$true )]
    $file,
    [String]
...
Export-ModuleMember -Function CreateAndConfigureShpurdpService
Export-ModuleMember -Function StopAndDeleteShpurdpService
Export-ModuleMember -Function ReplaceShpurdpServiceXML
