...
function Main( $scriptDir )
{
    ...
    Write-Log "Metrics sink is $shpurdp_metrics" 
	$current = @()
	...
	foreach ($server in $hosts)
	{
		if (($current -notcontains $server) -and ($server -ne $null))
		{
			...
			$out = Invoke-Command -ComputerName $server -ScriptBlock {
				param( $server,$SQL_SERVER_NAME,$destination,$shpurdp_metrics)
				$log = Join-Path $env:tmp "shpurdp_uninstall.log"
				...
				if ($hdp_home -ne $null)
				{
					Out-File -FilePath $log -InputObject "Starting Shpurdp removal" -Append -Encoding "UTF8"
					Out-File -FilePath $log -InputObject "Cleaning up Shpurdp folder" -Append -Encoding "UTF8"
					...
					foreach ($name in $names)
					{
                        ...
                        Out-File -FilePath $log -InputObject "Removing ;$destination\$shpurdp_metrics;$destination\sqljdbc4.jar" -Append -Encoding "UTF8"
						(Get-Content $xml_file)|ForEach-Object {
						$_.Replace(";$destination\$shpurdp_metrics;$destination\sqljdbc4.jar","")
...
#					invoke-cmd $cmd 
			...
			} -ArgumentList ($server,$SQL_SERVER_NAME,$destination,$shpurdp_metrics)
	...
	Write-Log "Removing Shpurdp folder"
	Remove-Item $destination -force -Recurse
	Write-Log "Removing shortcut"
    $shortcuts = @("$env:USERPROFILE\Desktop\Stop Shpurdp SCOM Server.lnk","$env:USERPROFILE\Desktop\Start Shpurdp SCOM Server.lnk","$env:USERPROFILE\Desktop\Browse Shpurdp API.url","$env:USERPROFILE\Desktop\Browse Shpurdp Metrics.url")
    foreach ($shortcut in $shortcuts)
    {
    	Remove-Item $shortcut -Force
    }
    Write-Log "Removing service"
    StopAndDeleteShpurdpService "shpurdpscom"
