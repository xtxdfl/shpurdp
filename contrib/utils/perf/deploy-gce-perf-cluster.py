...
cluster_prefix = "perf"
shpurdp_repo_file_url = "http://s3.amazonaws.com/dev.hortonworks.com/shpurdp/centos7/2.x/updates/2.7.1.0/shpurdpbn.repo"
...
def main():
  parser = argparse.ArgumentParser(
    description='This script brings up a cluster with shpurdp installed, configured and started',
...
def deploy_cluster(args):
  ...
  # installing/starting shpurdp-server and shpurdp-agents on each host
  ...
  print "Creating server.sh script (which will be executed on server to install/configure/start shpurdp-server)..."
  create_server_script(server_host_name)

  print "Creating agent.sh script (which will be executed on agent hosts to install/configure/start shpurdp-agent..."
  ...
  for (hostname, ip) in sorted_agents:
    ...
    cmd_generate_multiplier_conf = "mkdir -p /etc/shpurdp-agent/conf/ ; printf \"start={0}\\nnum={1}\\nprefix={2}\" > /etc/shpurdp-agent/conf/agent-multiplier.conf".format(start_num, num_agents_on_this_host, args.agent_prefix)
...
def create_server_script(server_host_name):
  ...
  server.sh script will install, configure and start shpurdp-server and shpurdp-agent on host
  :param server_host_name: Server host name
  """

  # shpurdp-server setup <options> may not work property, so doing several calls like
  # echo "arg=value" >> .../shpurdp.properties

  contents = "#!/bin/bash\n" + \
  "yum install wget -y\n" + \
  "wget -O /etc/yum.repos.d/shpurdp.repo {0}\n".format(shpurdp_repo_file_url) + \
  "yum clean all; yum install git shpurdp-server -y\n" + \
  "mkdir /home ; cd /home ; git clone https://github.com/apache/shpurdp.git ; cd shpurdp ; git checkout branch-2.5\n" + \
  "cp -r /home/shpurdp/shpurdp-server/src/main/resources/stacks/PERF /var/lib/shpurdp-server/resources/stacks/PERF\n" + \
  "cp -r /home/shpurdp/shpurdp-server/src/main/resources/stacks/PERF /var/lib/shpurdp-agent/cache/stacks/PERF\n" + \
  "sed -i -f /home/shpurdp/shpurdp-server/src/main/resources/stacks/PERF/install_packages.sed /var/lib/shpurdp-server/resources/custom_actions/scripts/install_packages.py\n" + \
  "sed -i -f /home/shpurdp/shpurdp-server/src/main/resources/stacks/PERF/install_packages.sed /var/lib/shpurdp-agent/cache/custom_actions/scripts/install_packages.py\n" + \
  "\n" + \
  "\n" + \
  ...
  "mysql -uroot -e \"CREATE DATABASE shpurdp;\"\n" + \
  "mysql -uroot -e \"SOURCE /var/lib/shpurdp-server/resources/Shpurdp-DDL-MySQL-CREATE.sql;\" shpurdp\n" + \
  "mysql -uroot -e \"CREATE USER 'shpurdp'@'%' IDENTIFIED BY 'bigdata';\"\n" + \
  "mysql -uroot -e \"GRANT ALL PRIVILEGES ON *.* TO 'shpurdp'@'%%';\"\n" + \
  "mysql -uroot -e \"CREATE USER 'shpurdp'@'localhost' IDENTIFIED BY 'bigdata';\"\n" + \
  "mysql -uroot -e \"GRANT ALL PRIVILEGES ON *.* TO 'shpurdp'@'localhost';\"\n" + \
  "mysql -uroot -e \"CREATE USER 'shpurdp'@'{0}' IDENTIFIED BY 'bigdata';\"\n".format(server_host_name) + \
  "mysql -uroot -e \"GRANT ALL PRIVILEGES ON *.* TO 'shpurdp'@'{0}';\"\n".format(server_host_name) + \
  "mysql -uroot -e \"FLUSH PRIVILEGES;\"\n" + \
  "\n" + \
  "\n" + \
  "shpurdp-server setup -s\n" + \
  "shpurdp-server setup --database mysql --jdbc-db=mysql --jdbc-driver=/usr/share/java/mysql-connector-java.jar --databasehost=localhost --databaseport=3306 --databasename=shpurdp --databaseusername=shpurdp --databasepassword=bigdata\n" + \
  "sed -i -e 's/=postgres/=mysql/g' /etc/shpurdp-server/conf/shpurdp.properties\n" + \
  "sed -i -e 's/server.persistence.type=local/server.persistence.type=remote/g' /etc/shpurdp-server/conf/shpurdp.properties\n" + \
  "sed -i -e 's/local.database.user=postgres//g' /etc/shpurdp-server/conf/shpurdp.properties\n" + \
  "sed -i -e 's/server.jdbc.postgres.schema=shpurdp//g' /etc/shpurdp-server/conf/shpurdp.properties\n" + \
  "sed -i -e 's/agent.threadpool.size.max=25/agent.threadpool.size.max=100/g' /etc/shpurdp-server/conf/shpurdp.properties\n" + \
  "sed -i -e 's/client.threadpool.size.max=25/client.threadpool.size.max=65/g' /etc/shpurdp-server/conf/shpurdp.properties\n" + \
  "sed -i -e 's/false/true/g' /var/lib/shpurdp-server/resources/stacks/PERF/1.0/metainfo.xml\n" + \
  "sed -i -e 's/false/true/g' /var/lib/shpurdp-server/resources/stacks/PERF/2.0/metainfo.xml\n" + \
  "sed -i -e 's/-Xmx2048m/-Xmx16384m/g' /var/lib/shpurdp-server/shpurdp-env.sh\n" + \
  "\n" + \
  "echo 'server.jdbc.driver=com.mysql.jdbc.Driver' >> /etc/shpurdp-server/conf/shpurdp.properties\n" + \
  "echo 'server.jdbc.rca.url=jdbc:mysql://{0}:3306/shpurdp' >> /etc/shpurdp-server/conf/shpurdp.properties\n".format(server_host_name) + \
  "echo 'server.jdbc.rca.driver=com.mysql.jdbc.Driver' >> /etc/shpurdp-server/conf/shpurdp.properties\n" + \
  "echo 'server.jdbc.url=jdbc:mysql://{0}:3306/shpurdp' >> /etc/shpurdp-server/conf/shpurdp.properties\n".format(server_host_name) + \
  "echo 'server.jdbc.port=3306' >> /etc/shpurdp-server/conf/shpurdp.properties\n" + \
  "echo 'server.jdbc.hostname=localhost' >> /etc/shpurdp-server/conf/shpurdp.properties\n" + \
  "echo 'server.jdbc.driver.path=/usr/share/java/mysql-connector-java.jar' >> /etc/shpurdp-server/conf/shpurdp.properties\n" + \
  "echo 'alerts.cache.enabled=true' >> /etc/shpurdp-server/conf/shpurdp.properties\n" + \
  "echo 'alerts.cache.size=100000' >> /etc/shpurdp-server/conf/shpurdp.properties\n" + \
  "echo 'alerts.execution.scheduler.maxThreads=4' >> /etc/shpurdp-server/conf/shpurdp.properties\n" + \
  "echo 'security.temporary.keystore.retention.minutes=180' >> /etc/shpurdp-server/conf/shpurdp.properties\n" + \
  "echo 'stack.hooks.folder=stacks/PERF/1.0/hooks' >> /etc/shpurdp-server/conf/shpurdp.properties\n" + \
  "\n" + \
  "shpurdp-server start --skip-database-check\n" + \
  "exit 0"
...
def create_agent_script(server_host_name):
  ...
  agent.sh script will install, configure and start shpurdp-agent on host
  :param server_host_name: Server host name
  """

  # TODO, instead of cloning Shpurdp repo on each VM, do it on the server once and distribute to all of the agents.
  contents = "#!/bin/bash\n" + \
  "yum install wget -y\n" + \
  "wget -O /etc/yum.repos.d/shpurdp.repo {0}\n".format(shpurdp_repo_file_url) + \
  "yum clean all; yum install krb5-workstation git shpurdp-agent -y\n" + \
  "mkdir /home ; cd /home; git clone https://github.com/apache/shpurdp.git ; cd shpurdp ; git checkout branch-2.5\n" + \
  "cp -r /home/shpurdp/shpurdp-server/src/main/resources/stacks/PERF /var/lib/shpurdp-agent/cache/stacks/PERF\n" + \
  "sed -i -f /var/lib/shpurdp-agent/cache/stacks/PERF/PythonExecutor.sed /usr/lib/shpurdp-agent/lib/shpurdp_agent/PythonExecutor.py\n" + \
  "sed -i -f /var/lib/shpurdp-agent/cache/stacks/PERF/check_host.sed /var/lib/shpurdp-agent/cache/custom_actions/scripts/check_host.py\n" + \
  "sed -i -e 's/hostname=localhost/hostname={0}/g' /etc/shpurdp-agent/conf/shpurdp-agent.ini\n".format(server_host_name) + \
  "sed -i -e 's/agent]/agent]\\nhostname_script={0}\\npublic_hostname_script={1}\\n/1' /etc/shpurdp-agent/conf/shpurdp-agent.ini\n".format(hostname_script, public_hostname_script) + \
  "python /home/shpurdp/shpurdp-agent/conf/unix/agent-multiplier.py start\n" + \
  "exit 0"
