...
DEFAULT_SHPURDP_SERVER_PROPERTIES='/etc/shpurdp-server/conf/shpurdp.properties'
...
def init_parser_options(parser):
  parser.add_option('-p', '--port',
                    ...
                    help="Shpurdp Server port corrsponding to the network protocol. Default port is {0} for an HTTP connection".format(DEFAULT_HTTP_PORT))
  parser.add_option('-u', '--user',
                    dest="user", default=DEFAULT_ADMIN_USER,
                    help="Shpurdp admin user. Default user name is {0}".format(DEFAULT_ADMIN_USER))
  parser.add_option('-a', '--password',
                    dest="password",
                    help="Shpurdp admin user password.")
...
def validate_options(options):
  ...
  if not options.port:
    errors.append("No Shpurdp server port provided.")
  if not options.user:
      errors.append("No Shpurdp admin user name provided.")
  if not options.password:
    errors.append("No Shpurdp admin user passsword provided.")
  if not options.log:
...
"""
Retrieve property value from Shpurdp Server properties file.
"""
def get_shpurdp_server_property(key):
  try:
    with open(DEFAULT_SHPURDP_SERVER_PROPERTIES, 'r') as property_file:
...
def get_server_protocol():
  sslActive = get_shpurdp_server_property('api.ssl')
  if sslActive == "true":
    return "https"
  else:
    return "http"
...
"""
Submit REST API to Shpurdp Server
"""
def execute_curl_command(url, headers=[], request_type=DEFAULT_HTTP_REQUEST_TYPE, request_body=None, user=DEFAULT_ADMIN_USER, password=None):
  ...
  @param user: User for Shpurdp REST API authentication
  @param password: Password for the user used to authenticate the Shpurdp REST API call
  """
  curl_cmd_array = ["curl", "-v", "-u", "{0}:{1}".format(user,password), "-k", "-H", "X-Requested-By: shpurdp"]
...
"""
Determine if Shpurdp Server responded with an error message for the REST API call
"""
...
"""
Determine if Shpurdp Server has accepted the REST API call
"""
...
def polling_request(url, user=None, password=None, timer=DEFAULT_POLLING_TIMER_REQUEST):
  """
  @param url: Request URL returned by the Shpurdp Server
  @param user: User for Shpurdp REST API authentication
  @param password: Password for the user used to authenticate the Shpurdp REST API call
...
def summarize_tasks_by_hosts(request_url, user, password):
  """
  @param request_url: Request URL returned by the Shpurdp Server
  @param user: User for Shpurdp REST API authentication
  @param password: Password for the user used to authenticate the Shpurdp REST API call
  """
...
"""
Check if the script can log in Shpurdp Server REST API via user and password provided
"""
...
"""
Obtain a list of Shpurdp Agents registered to the host via a REST API call
"""
def get_shpurdp_agent_nodes(server_url, user, password):
  """
  @param server_url: Basic server url to connect and log in
  @param user: User for Shpurdp REST API authentication
  @param password: Password for the user used to authenticate the Shpurdp REST API call
  """
  url = "{0}/api/v1/services/SHPURDP/components/SHPURDP_AGENT".format(server_url)
  hosts = set()
  out, err, ec = execute_curl_command(url, user=user, password=password)
...
def run_check(options, url, label_check, data):
  ...
  @param url: Shpurdp Server URL
  @param label_check: Text to display for the check result section
  @param data: Data to be submitted to the Shpurdp Server via a REST API call
  """
  ...
  if CODE_SUCCESS != ec or not out:
    ...
    logger.debug(ec)
    logger.debug(err)
    print_check_result(label_check, ['Failed to connect to Shpurdp server'], ec)
    return ec
  else:
    is_erroneous_response, http_ec, http_err = is_erroneous_response_by_server(out)
    if is_erroneous_response:
...
def run_java_home_checks(options, agents, server_url):
  ...
  step(label_check)
  url = '{0}/api/v1/requests'.format(server_url)
  java_home = get_shpurdp_server_property('java.home')
  logger.info('Shpurdp server java home: {0}'.format(java_home))
  ...
  for key in task_results_by_host:
    ...
    if structured_out:
      ...
      if 'firewallRunning' in last_agent_env:
        ...
        if firewall_running:
          results_to_print.append({'key':key, 'status':STATUS_WARNING, 'warning':['Firewall is running on the host', 'Please configure the firewall to allow communications on the ports documented in the Configuring Ports section of the Shpurdp documentation.']})
        else:
          results_to_print.append({'key':key, 'status':STATUS_PASSED})
      else:
...
def run_agent_checks(options, agents, server_url):
  logger.info('')
  logger.info('Prepare for Shpurdp Agent host check')
  label_check = 'Shpurdp Agent host check'
  url = '{0}/api/v1/requests'.format(server_url)
  ...
  if os.path.isfile(blueprint_file):
    ...
    if valid_json_file:
      ...
      if CODE_ERROR == ec:
        ...
        logger.debug('HTTP response from the Shpurdp server: {0}'.format(http_response_code))
...
def run(options):
  ...
  server_url = get_server_url(options.port)
  label_check = 'Shpurdp server reachable by user credentials'
  step(label_check)
  out, ec = server_reachable_by_credentials_with_retry(server_url, options.user, options.password)
  if CODE_SUCCESS == ec:
    print_check_result(label_check, ['Shpurdp server reachable via {0}'.format(server_url)], ec)
  elif CODE_ERROR == ec:
    ...
    return ec
  agents = get_shpurdp_agent_nodes(server_url, options.user, options.password)
  logger.info('Total number of agents {0}'.format(len(agents)))
  if not agents:
    logger.error('No Shpurdp Agent registered to the Shpurdp Server. Install Shpurdp Agent first.')
    return CODE_ERROR
